...
interface AIExplanation {
  ...
  narrative: string;
  confidence: number;
  factors: { name: string; value: string; impact: 'positive' | 'negative' | 'neutral'; explanation: string }[];
...
function estimateBoundary(
  ...
  const filtered = pivots.filter(p => 
    type === 'support' ? p.type === 'low' : p.type === 'high'
  ).slice(-10);
...
export function PCTTScreen({
  ...
  const { structure, pivots, signal } = useMemo(() => {
    ...
    if (er >= 0.5) {
      regime = lastPrice > data[data.length - 20]?.close ? 'trend_up' : 'trend_down';
    } else if (er < 0.3) {
      regime = 'range';
    }
    ...
    const structure: StructureObject = {
      ...
      distanceToResistance: atr > 0 ? (resistancePrice - lastPrice) / atr : Infinity,
    };
  ...
  const aiExplanation: AIExplanation = useMemo(() => {
    ...
    if (structure.support) {
      factors.push({
        ...
        impact: structure.support.qScore >= 0.65 ? 'positive' : structure.support.qScore >= 0.5 ? 'neutral' : 'negative',
        explanation: structure.support.qScore >= 0.65 
    ...
    if (structure.resistance) {
      factors.push({
        ...
        impact: structure.resistance.qScore >= 0.65 ? 'positive' : structure.resistance.qScore >= 0.5 ? 'neutral' : 'negative',
        explanation: structure.resistance.qScore >= 0.65
    ...
    factors.push({
      ...
      impact: structure.regime === 'transition' ? 'negative' : structure.regime === 'range' ? 'neutral' : 'positive',
      explanation: structure.regime === 'trend_up' 
    ...
    factors.push({
      ...
      impact: structure.efficiencyRatio >= 0.5 ? 'positive' : structure.efficiencyRatio >= 0.3 ? 'neutral' : 'negative',
    ...
    const regimeScore = structure.regime.includes('trend') ? 80 : structure.regime === 'range' ? 60 : 30;
    const efficiencyScore = structure.efficiencyRatio * 100;
    const confidence = Math.round(structureScore * 0.4 + regimeScore * 0.3 + efficiencyScore * 0.3);
    
    // Identify risks
    const risks: AIExplanation['risks'] = [];
    if (structure.regime === 'transition') {
      risks.push({ level: 'high', description: 'Market in transition - direction unclear' });
    }
    if (structure.efficiencyRatio < 0.3) {
      risks.push({ level: 'medium', description: 'Low trend efficiency - choppy conditions' });
    }
    if (supportQ < 0.5 && resistanceQ < 0.5) {
      risks.push({ level: 'high', description: 'Weak structure quality on both levels' });
    }
    ...
    const eventDesc = structure.event === 'idle' 
      ? 'Waiting for setup'
      : structure.event.replace('_', ' ');
    
    return {
      summary: `${structure.regime.replace('_', ' ')} market, ${eventDesc}. Confidence: ${confidence}%`,
      narrative: `The AI has detected a ${structure.regime.replace('_', ' ')} market regime with ${(structure.efficiencyRatio * 100).toFixed(0)}% trend efficiency. ` +
  ...
  const xScale = useCallback((index: number) => {
    return CHART_PADDING.left + (index / visibleData.length) * chartInnerWidth + candleWidth / 2;
  }, [visibleData.length, chartInnerWidth, candleWidth]);

  const yScale = useCallback((price: number) => {
    return CHART_PADDING.top + (1 - (price - priceMin) / priceRange) * chartInnerHeight;
  ...
  const handleSignalAction = (action: 'buy' | 'sell') => {
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          text: 'Confirm', 
          onPress: () => action === 'buy' ? onBuy?.(signal) : onSell?.(signal) 
        },
      ]
    );
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.regimeBadge}>
          <View style={[styles.regimeDot, { backgroundColor: getRegimeColor(structure.regime) }]} />
      ...
      <View style={styles.chartContainer}>
        <Svg width={chartWidth} height={CHART_HEIGHT}>
          ...
          {structure.support && (
            <Line
              x1={CHART_PADDING.left}
              y1={yScale(structure.support.intercept + structure.support.slope * (data.length - 60))}
              x2={chartWidth - CHART_PADDING.right}
              y2={yScale(structure.support.intercept + structure.support.slope * (data.length - 1))}
          ...
          {structure.resistance && (
            <Line
              x1={CHART_PADDING.left}
              y1={yScale(structure.resistance.intercept + structure.resistance.slope * (data.length - 60))}
              x2={chartWidth - CHART_PADDING.right}
              y2={yScale(structure.resistance.intercept + structure.resistance.slope * (data.length - 1))}
          ...
          {/* Pivot Markers */}
          {showPivots && pivots.slice(-20).map((pivot, i) => {
            ...
            return (
              <Circle
                ...
                r={4}
                fill={pivot.type === 'high' ? COLORS.pivotHigh : COLORS.pivotLow}
              />
            );
          })}
      ...
      <View style={styles.structureCard}>
        ...
        <View style={styles.structureRow}>
          <View style={styles.structureItem}>
            ...
            <Text style={[
              ...
              { color: structure.support ? getQScoreColor(structure.support.qScore) : COLORS.textSecondary }
            ]}>
              {structure.support ? `${(structure.support.qScore * 100).toFixed(0)}%` : 'N/A'}
            </Text>
          </View>
          <View style={styles.structureItem}>
            ...
            <Text style={[
              styles.structureValue,
              { color: structure.resistance ? getQScoreColor(structure.resistance.qScore) : COLORS.textSecondary }
            ]}>
              {structure.resistance ? `${(structure.resistance.qScore * 100).toFixed(0)}%` : 'N/A'}
            </Text>
          </View>
          <View style={styles.structureItem}>
        ...
        <View style={styles.eventRow}>
          ...
          <View style={[
            styles.eventBadge,
            { backgroundColor: structure.event.includes('entry') ? COLORS.green + '30' : COLORS.cardBorder }
          ]}>
            <Text style={[
              styles.eventText,
              { color: structure.event.includes('entry') ? COLORS.green : COLORS.text }
            ]}>
        ...
        <View style={styles.aiCard}>
          <View style={styles.aiHeader}>
            ...
            <View style={[
              ...
              { backgroundColor: aiExplanation.confidence >= 70 ? COLORS.green + '30' : 
                               aiExplanation.confidence >= 50 ? COLORS.qMed + '30' : COLORS.red + '30' }
            ]}>
              <Text style={[
                styles.confidenceText,
                { color: aiExplanation.confidence >= 70 ? COLORS.green : 
                        aiExplanation.confidence >= 50 ? COLORS.qMed : COLORS.red }
              ]}>
          ...
          {aiExplanation.factors.map((factor, idx) => (
            <TouchableOpacity 
              ...
              onPress={() => setExpandedFactor(expandedFactor === factor.name ? null : factor.name)}
            >
              <View style={styles.factorHeader}>
                <View style={[
                  styles.impactDot,
                  { backgroundColor: factor.impact === 'positive' ? COLORS.green : 
                                    factor.impact === 'negative' ? COLORS.red : COLORS.textSecondary }
                ]} />
                <Text style={styles.factorName}>{factor.name}</Text>
                <Text style={[
                  styles.factorValue,
                  { color: factor.impact === 'positive' ? COLORS.green : 
                          factor.impact === 'negative' ? COLORS.red : COLORS.text }
                ]}>
                  {factor.value}
                </Text>
              </View>
            ...
            <>
              ...
              {aiExplanation.risks.map((risk, idx) => (
                <View key={idx} style={styles.riskRow}>
                  <View style={[
                    styles.riskBadge,
                    { backgroundColor: risk.level === 'high' ? COLORS.red + '30' : COLORS.qMed + '30' }
                  ]}>
                    <Text style={[
                      styles.riskLevel,
                      { color: risk.level === 'high' ? COLORS.red : COLORS.qMed }
                    ]}>
      ...
      <View style={styles.toggleContainer}>
        ...
        >
          <Text style={[styles.toggleText, showExplanation && styles.toggleTextActive]}>
            🤖 AI
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text style={[styles.toggleText, showPivots && styles.toggleTextActive]}>
            Pivots
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text style={[styles.toggleText, showSignals && styles.toggleTextActive]}>
            Show Signals
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.legend}>
        <View style={styles.legendItem}>
          <View style={[styles.legendLine, { backgroundColor: COLORS.support }]} />
          <Text style={styles.legendText}>Support</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendLine, { backgroundColor: COLORS.resistance }]} />
          <Text style={styles.legendText}>Resistance</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: COLORS.pivotHigh }]} />
          <Text style={styles.legendText}>Pivot High</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: COLORS.pivotLow }]} />
