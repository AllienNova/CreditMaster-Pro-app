...
import Svg, { Path, G, Line, Text as SvgText, Defs, LinearGradient, Stop } from 'react-native-svg';
...
export function AreaChart({
  ...
  const getX = (index: number) => padding.left + (index / (maxDataLength - 1 || 1)) * chartWidth;
  ...
  return (
    <View style={[styles.container, { width, height }]} accessibilityLabel="Area chart showing data trends">
      <Svg width={width} height={height}>
        <Defs>
          {allSeries.map((s, idx) => (
            <LinearGradient key={`gradient-${idx}`} id={`areaGradient-${idx}`} x1="0" y1="0" x2="0" y2="1">
              <Stop offset="0%" stopColor={s.color} stopOpacity={gradientOpacity} />
        ...
        {allSeries.map((s, idx) => (
          <G key={`series-${idx}`}>
            ...
            />
            {/* Dots */}
            {showDots && s.data.map((point, index) => (
              <G key={`dot-${idx}-${index}`}>
                <Path
        ...
        {/* X-axis labels */}
        {showLabels && allSeries[0]?.data.map((point, index) => {
          const showEvery = Math.ceil(maxDataLength / 6);
          if (index % showEvery !== 0 && index !== maxDataLength - 1) return null;
        ...
        {/* Value labels */}
        {showValues && allSeries[0]?.data.map((point, index) => (
          <SvgText
            key={`value-${index}`}
            x={getX(index)}
