...
describe('Credit Score API', () => {
  ...
  describe('getScores', () => {
    it('should fetch all credit scores', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ success: true, data: mockScores });
  ...
  describe('getScoreByBureau', () => {
    it('should fetch score for specific bureau', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ success: true, data: mockScore });
  ...
  describe('simulateImpact', () => {
    it('should simulate score impact', async () => {
      ...
      (api.post as jest.Mock).mockResolvedValueOnce({ success: true, data: mockResult });

      const result = await creditScoreApi.simulateImpact({ payOffDebt: 5000 });

      expect(api.post).toHaveBeenCalledWith('/credit/simulate', { payOffDebt: 5000 });
  ...
  describe('refreshScores', () => {
    it('should request score refresh', async () => {
      (api.post as jest.Mock).mockResolvedValueOnce({ success: true, data: { message: 'Refresh initiated' } });
...
describe('Credit Monitoring API', () => {
  ...
  describe('getStatus', () => {
    it('should fetch monitoring status', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ success: true, data: mockStatus });
  ...
  describe('getAlerts', () => {
    it('should fetch alerts with filters', async () => {
      ...
      await creditMonitoringApi.getAlerts({ page: 1, limit: 10, unreadOnly: true });

      expect(api.get).toHaveBeenCalledWith('/credit/monitoring/alerts?page=1&limit=10&unread=true');
    });
  });
  ...
  describe('acknowledgeAlert', () => {
    it('should acknowledge single alert', async () => {
      ...
      expect(api.patch).toHaveBeenCalledWith('/credit/monitoring/alerts/alert-123/acknowledge');
    });
  });

  describe('acknowledgeAllAlerts', () => {
    it('should acknowledge all alerts', async () => {
      (api.post as jest.Mock).mockResolvedValueOnce({ success: true, data: { acknowledged: 5 } });

      await creditMonitoringApi.acknowledgeAllAlerts();

      expect(api.post).toHaveBeenCalledWith('/credit/monitoring/alerts/acknowledge-all');
    });
  });
});
...
describe('Credit Report API', () => {
  describe('getReports', () => {
    it('should fetch all reports', async () => {
      (api.get as jest.Mock).mockResolvedValueOnce({ success: true, data: { reports: [] } });
  ...
  describe('getReport', () => {
    it('should fetch single report', async () => {
      (api.get as jest.Mock).mockResolvedValueOnce({ success: true, data: { id: 'report-1' } });
  ...
  describe('analyzeReport', () => {
    it('should request AI analysis', async () => {
      (api.post as jest.Mock).mockResolvedValueOnce({ success: true, data: { summary: 'Analysis' } });
