...
try {
  ...
  setNotificationHandlerCalledAtLoad = Notifications.setNotificationHandler.mock !== undefined;
} catch {
  // ignore
}
...
describe('PushNotificationService', () => {
  ...
  describe('initialize', () => {
    ...
    it('should return false on unexpected error', async () => {
      Notifications.getPermissionsAsync.mockRejectedValue(new Error('Critical error'));
  ...
  describe('scheduleLocalNotification', () => {
    it('should schedule a notification with title and body', async () => {
      const id = await pushNotificationService.scheduleLocalNotification('Test Title', 'Test Body');
    ...
    it('should include data in the notification', async () => {
      ...
      await pushNotificationService.scheduleLocalNotification('Alert', 'Update', data);
    ...
    it('should use custom trigger when provided', async () => {
      ...
      await pushNotificationService.scheduleLocalNotification('Reminder', 'Body', undefined, trigger);
  ...
  describe('cleanup', () => {
    it('should remove notification subscriptions after initialize', async () => {
      ...
      Notifications.addNotificationReceivedListener.mockReturnValue(mockRemoveReceived);
      Notifications.addNotificationResponseReceivedListener.mockReturnValue(mockRemoveResponse);

      await pushNotificationService.initialize();
      pushNotificationService.cleanup();

      expect(Notifications.removeNotificationSubscription).toHaveBeenCalledWith(mockRemoveReceived);
      expect(Notifications.removeNotificationSubscription).toHaveBeenCalledWith(mockRemoveResponse);
    });
  });
