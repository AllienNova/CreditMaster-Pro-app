...
export const getRecommendations = async (
  ...
  return apiClient.get(`${BASE_PATH}/recommendations${query ? `?${query}` : ''}`);
};
...
export const createGoal = async (params: CreateGoalParams): Promise<FinancialGoalPlan> => {
  return apiClient.post(`${BASE_PATH}/goals`, params);
};

export const getGoal = async (goalId: string): Promise<{ goal: FinancialGoalPlan; adjustments: any[] }> => {
...
export const getBudgetOptimization = async (
  ...
  if (params?.includeScenarios !== undefined) queryParams.set('includeScenarios', String(params.includeScenarios));
  if (params?.targetSavingsRate) queryParams.set('targetSavingsRate', String(params.targetSavingsRate));
...
export const getDebtStrategy = async (
  ...
  const queryParams = new URLSearchParams();
  if (params?.extraMonthlyPayment) queryParams.set('extraPayment', String(params.extraMonthlyPayment));
  if (params?.includeRefinancing !== undefined) queryParams.set('includeRefinancing', String(params.includeRefinancing));
  if (params?.targetPayoffDate) queryParams.set('targetPayoffDate', params.targetPayoffDate);
