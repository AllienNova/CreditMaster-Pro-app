...
import { useFinancialStore, selectNetWorth, selectTotalBalance, selectBudgetProgress, selectGoalProgress } from '../financialStore';
...
describe('Financial Store', () => {
  ...
  describe('Transactions', () => {
    it('should fetch transactions successfully', async () => {
      ...
      expect(useFinancialStore.getState().transactions).toEqual(mockTransactions);
    ...
    it('should update transaction category', async () => {
      ...
      let result;
      await act(async () => {
        result = await useFinancialStore.getState().updateTransactionCategory('1', 'Dining');
      });

      expect(result).toBe(true);
      expect(useFinancialStore.getState().transactions[0].category).toBe('Dining');
    });
  });
  ...
  describe('Debt', () => {
    ...
    it('should calculate payoff', async () => {
      debtApi.calculatePayoff.mockResolvedValueOnce({
        success: true,
        data: {
          timeline: [{ month: '2024-01', totalPaid: 500, remainingDebt: 24500 }],
