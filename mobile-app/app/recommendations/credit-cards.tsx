...
const CREDIT_CARDS: CreditCard[] = [
  {
    ...
    rewards: ["1.5% on everything", "5% on travel via Chase", "3% on dining"],
    approvalOdds: 85,
  ...
  },
  {
    ...
    rewards: ["2% on everything (1% + 1%)", "No categories to track"],
    approvalOdds: 78,
    creditNeeded: "Good (670+)",
    featured: false,
  },
  ...
  },
  {
    ...
    annualFee: 95,
    apr: "19.24% - 29.99%",
    signupBonus: "$350 after $3,000 spend",
    rewards: ["6% at US supermarkets", "6% on streaming", "3% on transit"],
    approvalOdds: 70,
    creditNeeded: "Good (670+)",
    featured: true,
  },
...
export default function CreditCardsScreen() {
  ...
  const filteredCards =
    selectedType === "all"
      ? CREDIT_CARDS
      : CREDIT_CARDS.filter((c) => c.type === selectedType);
  const sortedCards = [...filteredCards].sort((a, b) =>
    sortBy === "approval" ? b.approvalOdds - a.approvalOdds : 0,
  );
      ...
      >
        ...
        <Card style={styles.matchCard}>
          ...
          <Text style={styles.matchText}>
            Based on your credit score of 720, we found {CREDIT_CARDS.length}{" "}
        ...
        <View style={styles.sortRow}>
          ...
          <TouchableOpacity
            style={[
              styles.sortButton,
              sortBy === "approval" && styles.sortButtonActive,
            ]}
            onPress={() => setSortBy("approval")}
          >
            <Text
              style={[
                styles.sortButtonText,
                sortBy === "approval" && styles.sortButtonTextActive,
              ]}
            >
              Approval Odds
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.sortButton,
              sortBy === "rewards" && styles.sortButtonActive,
            ]}
          ...
          >
            <Text
              style={[
                styles.sortButtonText,
                sortBy === "rewards" && styles.sortButtonTextActive,
              ]}
            >
              Best Rewards
            </Text>
        ...
        {sortedCards.map((card) => (
          <Card
            key={card.id}
            style={[styles.cardItem, card.featured && styles.cardItemFeatured]}
          >
            {card.featured && (
            ...
            <View style={styles.cardHeader}>
              <View style={styles.cardInfo}>
                ...
                <Text style={styles.cardIssuer}>
                  {card.issuer} • {getTypeLabel(card.type)}
                </Text>
              </View>
              <View
                style={[
                  ...
                  {
                    backgroundColor: `${getApprovalColor(card.approvalOdds)}15`,
                  },
                ]}
              >
                <Text
                  style={[
                    styles.approvalText,
                    { color: getApprovalColor(card.approvalOdds) },
                  ]}
                >
                  {card.approvalOdds}%
                </Text>
            ...
            <View style={styles.cardDetails}>
              ...
              <View style={styles.detailItem}>
                <Text style={styles.detailLabel}>Credit Needed</Text>
                <Text style={styles.detailValue}>
                  {card.creditNeeded.split(" ")[0]}
                </Text>
              </View>
            </View>
