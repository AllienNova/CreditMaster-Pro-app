...
const LOCAL_TEMPLATES: DisputeTemplate[] = [
  {
    ...
    bestPractices: ['Send within 30 days of discovery', 'Include copy of credit report'],
    whenToUse: ['Unknown company made inquiry', 'Never applied for credit with this lender'],
    whenNotToUse: ['You authorized the inquiry but forgot', 'Pre-qualification inquiry'],
  },
  {
    id: 'obsolete_debt',
    ...
    bestPractices: ['Calculate exact date of first delinquency', 'Cite FCRA Section 605'],
    whenToUse: ['Debt is older than 7 years', 'Account should have aged off'],
    whenNotToUse: ['Debt is within reporting period', 'Recent re-aging occurred'],
  },
  {
    ...
    bestPractices: ['Include complete identity verification', 'List all incorrect accounts'],
    whenToUse: ['Someone else\'s accounts on your report', 'Similar name/SSN confusion'],
    whenNotToUse: ['Accounts are yours but disputed', 'Joint account confusion'],
  },
  {
    ...
    placeholders: ['YOUR_NAME', 'COLLECTION_AGENCY', 'PAYMENT_DATE', 'AMOUNT_PAID'],
    bestPractices: ['Include proof of payment', 'Reference original settlement terms'],
    whenToUse: ['Collection paid but still shows open', 'Balance shows incorrect amount'],
  ...
  },
  {
    ...
    tone: 'assertive',
    letterText: '',
    requiredDocuments: ['Bank statements', 'Payment confirmations', 'Credit report'],
    placeholders: ['YOUR_NAME', 'CREDITOR_NAME', 'ACCOUNT_NUMBER', 'INCORRECT_MONTH'],
    bestPractices: ['Provide bank statements showing on-time payments'],
    whenToUse: ['Payment was made on time but reported late', 'Wrong month marked late'],
  ...
  },
  {
    ...
    placeholders: ['YOUR_NAME', 'MEDICAL_PROVIDER', 'COLLECTION_AGENCY', 'AMOUNT'],
    bestPractices: ['Cite new FCRA medical debt rules', 'Verify amount is under $500'],
    whenToUse: ['Medical debt under $500', 'Debt went to collections after April 2023'],
    whenNotToUse: ['Medical debt over $500', 'Debt reported before rule change'],
  },
  {
    ...
    tone: 'formal',
    letterText: '',
    requiredDocuments: ['Rehabilitation completion letter', 'Servicer confirmation'],
    placeholders: ['YOUR_NAME', 'LOAN_SERVICER', 'REHABILITATION_DATE'],
    bestPractices: ['Include official rehabilitation certificate', 'Request complete removal'],
    whenToUse: ['Completed 9 qualifying payments', 'Have rehabilitation completion letter'],
    whenNotToUse: ['Still in rehabilitation program', 'Consolidation not rehabilitation'],
  },
  {
    ...
    placeholders: ['YOUR_NAME', 'CASE_NUMBER', 'DISCHARGE_DATE', 'CREDITOR_NAME'],
    bestPractices: ['Include certified copy of discharge', 'Reference specific account'],
    whenToUse: ['Debt was included in bankruptcy', 'Account shows balance after discharge'],
...
export default function TemplatesScreen() {
  ...
  const [templates, setTemplates] = useState<DisputeTemplate[]>(LOCAL_TEMPLATES);
    ...
    <View style={styles.container}>
        ...
        {CATEGORIES.map(cat => (
          ...
          >
            <Ionicons 
              ...
              size={16} 
              color={selectedCategory === cat.id ? '#FFFFFF' : lightTheme.colors.textSecondary} 
            />
      ...
      {loading ? (
        ...
        <ScrollView style={styles.templatesList} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultsText}>{filteredTemplates.length} templates found</Text>
          ...
          {filteredTemplates.map(template => (
            ...
            >
              <View style={styles.templateHeader}>
                ...
                <View style={[styles.successBadge, { backgroundColor: getSuccessColor(template.successRate) + '20' }]}>
                  <Text style={[styles.successText, { color: getSuccessColor(template.successRate) }]}>
                    {template.successRate}% success
                  </Text>
                </View>
              </View>
              
              <Text style={styles.templateScenario} numberOfLines={2}>{template.scenario}</Text>
              
              <View style={styles.templateMeta}>
                <View style={[styles.toneBadge, { backgroundColor: getToneColor(template.tone) + '20' }]}>
                  <Text style={[styles.toneText, { color: getToneColor(template.tone) }]}>
                    {template.tone}
                  </Text>
                </View>
                <Text style={styles.docsRequired}>
                  <Ionicons name="document-attach-outline" size={14} color={lightTheme.colors.textSecondary} />
              ...
              <View style={styles.templateFooter}>
                <Ionicons name="chevron-forward" size={20} color={lightTheme.colors.primary} />
              </View>
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: 'row', alignItems: 'center', margin: 16, padding: 12, backgroundColor: lightTheme.colors.surface, borderRadius: 12, gap: 8 },
  searchInput: { flex: 1, fontSize: 16, color: lightTheme.colors.text },
  categoriesContainer: { maxHeight: 50, paddingHorizontal: 16 },
  categoryChip: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8, backgroundColor: lightTheme.colors.surface, borderRadius: 20, marginRight: 8, gap: 6 },
  ...
  resultsText: { fontSize: 14, color: lightTheme.colors.textSecondary, marginBottom: 12 },
  templateCard: { backgroundColor: lightTheme.colors.surface, borderRadius: 16, padding: 16, marginBottom: 12 },
  templateHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 },
  templateName: { fontSize: 17, fontWeight: '600', color: lightTheme.colors.text, flex: 1, marginRight: 8 },
  successBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12 },
  successText: { fontSize: 12, fontWeight: '600' },
  templateScenario: { fontSize: 14, color: lightTheme.colors.textSecondary, marginBottom: 12, lineHeight: 20 },
  templateMeta: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 },
  ...
  whenToUse: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingTop: 12, borderTopWidth: 1, borderTopColor: lightTheme.colors.border },
  whenToUseLabel: { fontSize: 13, color: lightTheme.colors.textSecondary, fontWeight: '500' },
  whenToUseText: { fontSize: 13, color: lightTheme.colors.text, flex: 1 },
  templateFooter: { position: 'absolute', right: 16, top: '50%', marginTop: -10 },
});
