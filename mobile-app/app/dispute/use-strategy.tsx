...
export default function UseStrategyScreen() {
  ...
  const { strategyId, strategyName } = useLocalSearchParams<{ strategyId: string; strategyName: string }>();
  const [strategy, setStrategy] = useState<DisputeStrategy | null>(null);
  const [variables, setVariables] = useState<Record<string, string>>({});
  const [loading, setLoading] = useState(true);
  const [generating, setGenerating] = useState(false);
  const [result, setResult] = useState<{ letter: string; nextSteps: string[] } | null>(null);
  ...
  const getRequiredVariables = (id: string): string[] => {
    switch (id) {
      case 'escalation_tactics':
        return ['DISPUTE_DETAILS', 'PREVIOUS_RESPONSES', 'ESCALATION_LEVEL', 'BUREAU_NAME'];
      case 'mov_challenge':
        return ['DISPUTE_DETAILS', 'BUREAU_NAME', 'CREDITOR_NAME', 'DISPUTE_DATE'];
      case 'furnisher_direct':
        return ['DISPUTE_DETAILS', 'CREDITOR_NAME', 'ACCOUNT_NUMBER'];
      case 'debt_validation':
        return ['COLLECTOR_NAME', 'DEBT_AMOUNT', 'CONTACT_DATE', 'ACCOUNT_NUMBER'];
  ...
  const fetchStrategy = async () => {
    ...
    if (data?.strategy) {
      ...
      requiredVars.forEach(v => { initial[v] = ''; });
  ...
  const handleGenerateLetter = async () => {
    ...
    setGenerating(true);
    const { data, error } = await disputesAPI.generateFromStrategy(strategyId || '', variables);
  ...
  const handleSave = () => {
    Alert.alert('Strategy Applied', 'Your dispute has been created with this strategy!', [
      ...
      <View style={styles.container}>
        ...
        <ScrollView style={styles.resultContent}>
          ...
          {strategy?.steps && (
            <View style={styles.stepsProgress}>
              ...
              {strategy.steps.map((step, i) => (
                <TouchableOpacity 
                  key={i} 
                  style={[styles.progressStep, currentStep >= i && styles.progressStepActive]}
                  onPress={() => setCurrentStep(i)}
                >
                  <View style={[styles.progressDot, currentStep >= i && styles.progressDotActive]}>
                    {currentStep > i ? (
                  ...
                  <View style={styles.progressInfo}>
                    <Text style={[styles.progressStepTitle, currentStep >= i && styles.progressStepTitleActive]}>
                      {step.title}
                    </Text>
                    <Text style={styles.progressStepDesc}>{step.description}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={styles.letterSection}>
            <Text style={styles.sectionTitle}>📄 Generated Letter for Step {currentStep + 1}</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>{strategyName || 'Apply Strategy'}</Text>
        <View style={{ width: 24 }} />
      </View>
            ...
            <View style={styles.strategyInfo}>
              <View style={styles.metaRow}>
                <View style={[styles.badge, { backgroundColor: '#16A34A20' }]}>
                  <Text style={[styles.badgeText, { color: '#16A34A' }]}>{strategy.successRate}% success</Text>
                </View>
            ...
            <View style={styles.section}>
              ...
              {requiredVariables.map((varName) => (
                <View key={varName} style={styles.inputGroup}>
                  <Text style={styles.inputLabel}>{VARIABLE_LABELS[varName] || varName}</Text>
                  <TextInput
                    style={[styles.input, varName === 'DISPUTE_DETAILS' || varName === 'PREVIOUS_RESPONSES' ? styles.textArea : null]}
                    ...
                    multiline={varName === 'DISPUTE_DETAILS' || varName === 'PREVIOUS_RESPONSES'}
                    numberOfLines={varName === 'DISPUTE_DETAILS' || varName === 'PREVIOUS_RESPONSES' ? 4 : 1}
                  />
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  strategyInfo: { backgroundColor: lightTheme.colors.surface, borderRadius: 12, padding: 16, marginBottom: 16 },
  ...
  strategyDesc: { fontSize: 14, color: lightTheme.colors.textSecondary, lineHeight: 20 },
  ...
  legalItem: { fontSize: 14, color: lightTheme.colors.textSecondary, marginBottom: 6 },
  ...
  actionButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: lightTheme.colors.primary, padding: 16, borderRadius: 12, gap: 8 },
  actionButtonText: { color: '#FFFFFF', fontSize: 16, fontWeight: '600' },
  buttonDisabled: { opacity: 0.6 },
  resultContent: { flex: 1, padding: 16 },
  stepsProgress: { backgroundColor: lightTheme.colors.surface, borderRadius: 12, padding: 16, marginBottom: 16 },
  progressTitle: { fontSize: 16, fontWeight: '600', color: lightTheme.colors.text, marginBottom: 16 },
  progressStep: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 16, opacity: 0.6 },
  progressStepActive: { opacity: 1 },
  progressDot: { width: 28, height: 28, borderRadius: 14, backgroundColor: lightTheme.colors.border, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  progressDotActive: { backgroundColor: lightTheme.colors.primary },
  progressDotText: { color: lightTheme.colors.textSecondary, fontSize: 12, fontWeight: '600' },
  progressInfo: { flex: 1 },
  progressStepTitle: { fontSize: 15, fontWeight: '500', color: lightTheme.colors.textSecondary, marginBottom: 4 },
  progressStepTitleActive: { color: lightTheme.colors.text },
  progressStepDesc: { fontSize: 13, color: lightTheme.colors.textSecondary },
  letterSection: { backgroundColor: lightTheme.colors.surface, borderRadius: 12, padding: 16, marginBottom: 16 },
  letterBox: { backgroundColor: lightTheme.colors.background, borderRadius: 8, padding: 12 },
  letterText: { fontSize: 13, color: lightTheme.colors.text, lineHeight: 20, fontFamily: 'monospace' },
  nextStepsSection: { backgroundColor: lightTheme.colors.surface, borderRadius: 12, padding: 16 },
  nextStepItem: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12 },
  nextStepNumber: { width: 24, height: 24, borderRadius: 12, backgroundColor: lightTheme.colors.primary + '20', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  nextStepNumberText: { color: lightTheme.colors.primary, fontSize: 12, fontWeight: '600' },
  nextStepText: { flex: 1, fontSize: 14, color: lightTheme.colors.text, lineHeight: 20 },
});
