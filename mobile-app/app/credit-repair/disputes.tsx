...
interface Dispute { id: string; item: string; bureau: string; status: 'pending' | 'in_progress' | 'resolved' | 'rejected'; date: string; }

const DISPUTES: Dispute[] = [
  { id: '1', item: 'Late Payment - Chase', bureau: 'Experian', status: 'in_progress', date: '2024-12-01' },
  { id: '2', item: 'Collection Account', bureau: 'Equifax', status: 'pending', date: '2024-11-28' },
  { id: '3', item: 'Hard Inquiry', bureau: 'TransUnion', status: 'resolved', date: '2024-11-15' },
  { id: '4', item: 'Balance Error', bureau: 'Experian', status: 'rejected', date: '2024-11-10' },
];
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <Card style={styles.statCard}><Text style={styles.statValue}>{DISPUTES.length}</Text><Text style={styles.statLabel}>Total</Text></Card>
          <Card style={[styles.statCard, { backgroundColor: `${theme.colors.primary}10` }]}><Text style={[styles.statValue, { color: theme.colors.primary }]}>{DISPUTES.filter(d => d.status === 'in_progress').length}</Text><Text style={styles.statLabel}>Active</Text></Card>
          <Card style={[styles.statCard, { backgroundColor: `${theme.colors.success}10` }]}><Text style={[styles.statValue, { color: theme.colors.success }]}>{DISPUTES.filter(d => d.status === 'resolved').length}</Text><Text style={styles.statLabel}>Resolved</Text></Card>
        </View>
            ...
            <Card key={dispute.id} style={styles.disputeCard}>
              <View style={styles.disputeHeader}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: `${getStatusColor(dispute.status)}15` }]}>
                  <Text style={[styles.statusText, { color: getStatusColor(dispute.status) }]}>{dispute.status.replace('_', ' ')}</Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  disputeItem: { fontSize: 15, fontWeight: '600', color: theme.colors.text, flex: 1 },
  ...
  newDisputeButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: theme.colors.primary, marginHorizontal: theme.spacing.lg, marginVertical: theme.spacing.lg, paddingVertical: 14, borderRadius: 12 },
  newDisputeText: { fontSize: 16, fontWeight: '600', color: '#fff', marginLeft: 8 },
});
