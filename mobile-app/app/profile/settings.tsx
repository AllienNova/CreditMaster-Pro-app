    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
        <View style={[
          ...
          item.id === 'delete_account' && { backgroundColor: withOpacity(colors.error, 0.1) },
        ]}>
          <Ionicons name={item.icon as any} size={22} color={item.id === 'delete_account' ? colors.error : colors.primary} />
        </View>
        <View style={{ marginLeft: 12, flex: 1 }}>
          ...
          {item.description && <Text style={{ fontSize: fontSize.xs, color: colors.textSecondary, marginTop: 2 }}>{item.description}</Text>}
        </View>
      </View>
      {item.type === 'toggle' ? (
        <Switch
          ...
          trackColor={{ false: colors.border, true: withOpacity(colors.primary, 0.38) }}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{
        ...
        <Text style={{ fontSize: fontSize.lg, fontWeight: fontWeight.semibold, color: colors.text }}>Settings</Text>
        <View style={{ width: 28 }} />
      </View>
      ...
      <ScrollView style={{ flex: 1 }}>
        ...
        <View style={{ alignItems: 'center', padding: spacing.lg, marginTop: spacing.lg }}>
          <Text style={{ fontSize: fontSize.sm, color: colors.textSecondary }}>Fynvita v1.0.0</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: spacing.sm }}>
            <TouchableOpacity><Text style={{ fontSize: fontSize.sm, color: colors.primary }}>Terms of Service</Text></TouchableOpacity>
            <Text style={{ marginHorizontal: spacing.sm, color: colors.textSecondary }}>•</Text>
            <TouchableOpacity><Text style={{ fontSize: fontSize.sm, color: colors.primary }}>Privacy Policy</Text></TouchableOpacity>
          </View>
        </View>
      </ScrollView>
    </View>
