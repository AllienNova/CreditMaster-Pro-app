...
export default function DebtStrategyScreen() {
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          refreshing={debtLoading}
          onRefresh={() => fetchDebtStrategy({ includeRefinancing: true })}
        />
      }
    >
      {/* Summary Card */}
        ...
        <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.summaryValue}>
              {data.averageInterestRate.toFixed(1)}%
            </Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryLabel}>Monthly</Text>
            <Text style={styles.summaryValue}>
              ${data.monthlyPayments.toFixed(0)}
            </Text>
          </View>
        </View>
        {data.debtToIncomeRatio > 40 && (
          <View style={styles.warningBanner}>
            ...
            <Text style={styles.warningText}>
              High debt-to-income ratio: {data.debtToIncomeRatio.toFixed(0)}%
            </Text>
          </View>
        )}
      </View>
          ...
          {data.strategies.map((strategy) => (
            ...
            >
              <Ionicons
                ...
                size={24}
                color={
                  selectedStrategy === strategy.strategy ? "#fff" : colors.text
                }
              />
              <Text
                style={[
                  styles.strategyTabText,
                  selectedStrategy === strategy.strategy && { color: "#fff" },
                ]}
              >
                {strategy.name}
              </Text>
      ...
      {activeStrategy && (
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          ...
          <View style={styles.metricsGrid}>
            ...
            <View style={styles.metricItem}>
              <Text style={[styles.metricValue, { color: colors.text }]}>
                ${activeStrategy.totalInterestPaid.toLocaleString()}
              </Text>
              <Text
                style={[styles.metricLabel, { color: colors.textSecondary }]}
              >
          ...
          <View style={styles.payoffDate}>
            ...
            <Text style={[styles.payoffDateText, { color: colors.text }]}>
              Debt-free by:{" "}
              {new Date(activeStrategy.payoffDate).toLocaleDateString("en-US", {
                month: "long",
                year: "numeric",
              })}
            </Text>
          ...
          <View style={styles.prosConsContainer}>
            <View style={styles.prosSection}>
              <Text style={[styles.prosConsTitle, { color: "#22c55e" }]}>
                Advantages
              </Text>
              {activeStrategy.advantages.map((adv, idx) => (
                <View key={idx} style={styles.prosConsItem}>
                  <Ionicons name="checkmark-circle" size={16} color="#22c55e" />
                  <Text style={[styles.prosConsText, { color: colors.text }]}>
                    {adv}
                  </Text>
                </View>
              ))}
            ...
            <View style={styles.consSection}>
              <Text style={[styles.prosConsTitle, { color: "#ef4444" }]}>
                Considerations
              </Text>
      ...
      {data.refinancingOpportunities &&
        data.refinancingOpportunities.length > 0 && (
          <View style={[styles.card, { backgroundColor: colors.card }]}>
            <Text style={[styles.cardTitle, { color: colors.text }]}>
              Refinancing Opportunities
            </Text>
            {data.refinancingOpportunities.map((opp) => (
              <View key={opp.debtId} style={styles.refiCard}>
                <Text style={[styles.refiName, { color: colors.text }]}>
                  {opp.debtName}
                </Text>
                <View style={styles.refiRates}>
                  <Text style={[styles.refiRate, { color: "#ef4444" }]}>
                    {opp.currentRate}%
                  </Text>
                  <Ionicons
                    name="arrow-forward"
                    ...
                    color={colors.textSecondary}
                  />
                  <Text style={[styles.refiRate, { color: "#22c55e" }]}>
                    {opp.potentialRate}%
                  </Text>
                </View>
                <Text style={[styles.refiSavings, { color: "#22c55e" }]}>
                  Save ${opp.monthlySavings.toFixed(0)}/mo ($
                  {opp.totalSavings.toLocaleString()} total)
                </Text>
              </View>
            ))}
          </View>
