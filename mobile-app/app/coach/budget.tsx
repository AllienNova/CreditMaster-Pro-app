...
export default function BudgetOptimizerScreen() {
  ...
  const { budgetOptimization, budgetLoading, fetchBudgetOptimization } =
    useCoachStore();
    ...
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          ...
          onRefresh={() => fetchBudgetOptimization({ includeTemplates: true })}
        />
      }
    >
      {/* Summary Card */}
      <View style={[styles.summaryCard, { backgroundColor: colors.primary }]}>
        ...
        <Text style={styles.summaryAmount}>
          ${data?.potentialMonthlySavings?.toFixed(0) || 0}
        </Text>
        <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.summaryValue}>
              ${data?.totalBudgeted?.toLocaleString() || 0}
            </Text>
          </View>
          <View style={styles.summaryItem}>
        ...
        {data?.optimizations?.map((opt: BudgetOptimization) => (
          ...
          >
            <View style={styles.optHeader}>
              ...
              >
                <Ionicons
                  name={getTypeIcon(opt.type) as any}
                  size={20}
                  color={colors.primary}
                />
              </View>
              <View style={styles.optContent}>
                <Text style={[styles.optCategory, { color: colors.text }]}>
                  {opt.category}
                </Text>
                <Text style={[styles.optType, { color: colors.textSecondary }]}>
                  {opt.type}
                </Text>
              </View>
              <View style={styles.optSavings}>
                <Text style={styles.optSavingsAmount}>
                  +${opt.potentialSavings.toFixed(0)}
                </Text>
                <View
                  style={[
                    styles.difficultyBadge,
                    { backgroundColor: getDifficultyColor(opt.difficulty) },
                  ]}
                >
            ...
            <View style={styles.optAmounts}>
              <Text
                style={[styles.optAmountText, { color: colors.textSecondary }]}
              >
                Current: ${opt.currentAmount.toFixed(0)} → Suggested: $
                {opt.suggestedAmount.toFixed(0)}
              </Text>
            </View>

            <Text style={[styles.optReason, { color: colors.text }]}>
              {opt.reason}
            </Text>
          ...
          <View key={cat.category} style={styles.budgetItem}>
            <View style={styles.budgetHeader}>
              ...
              <Text style={[styles.budgetAmount, { color: colors.text }]}>
                ${cat.spent.toFixed(0)} / ${cat.budgeted.toFixed(0)}
              </Text>
            </View>
              ...
              <View
                style={[
                  styles.budgetFill,
                  {
                    ...
                    backgroundColor:
                      cat.percentUsed > 100 ? "#ef4444" : colors.primary,
                  },
                ]}
              />
            </View>
