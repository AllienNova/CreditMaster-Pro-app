...
export default function GoalsScreen() {
  ...
  const { goals, goalsLoading, fetchGoals, createGoal, selectGoal } =
    useCoachStore();
  ...
  const renderGoalCard = (goal: FinancialGoalPlan) => {
    const typeInfo =
      GOAL_TYPES.find((t) => t.type === goal.type) || GOAL_TYPES[9];
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={goalsLoading} onRefresh={fetchGoals} />
        }
      >
              ...
              {GOAL_TYPES.map((t) => (
                ...
                >
                  <Ionicons
                    name={t.icon as any}
                    size={20}
                    color={newGoal.type === t.type ? "#fff" : colors.text}
                  />
                  <Text
                    style={[
                      styles.typeLabel,
                      newGoal.type === t.type && { color: "#fff" },
                    ]}
                  >
                    {t.label}
                  </Text>
