...
const MOCK_DATA = {
  userGrowth: [
    { date: 'Jan', count: 1200 }, { date: 'Feb', count: 1450 }, { date: 'Mar', count: 1680 },
    { date: 'Apr', count: 1920 }, { date: 'May', count: 2150 }, { date: 'Jun', count: 2480 },
  ],
  revenueByMonth: [
    { month: 'Jan', revenue: 45000 }, { month: 'Feb', revenue: 52000 }, { month: 'Mar', revenue: 58000 },
    { month: 'Apr', revenue: 65000 }, { month: 'May', revenue: 72000 }, { month: 'Jun', revenue: 85000 },
  ],
  disputesByStatus: [
    { status: 'Pending', count: 156 }, { status: 'In Progress', count: 89 },
    { status: 'Resolved', count: 423 }, { status: 'Rejected', count: 45 },
  ],
  topFeatures: [
    { feature: 'Disputes', usage: 15420 }, { feature: 'Score Tracking', usage: 12800 },
    { feature: 'AI Analysis', usage: 9650 }, { feature: 'Reports', usage: 7230 },
  ],
};
...
export default function AdminAnalyticsScreen() {
  ...
  const maxRevenue = Math.max(...MOCK_DATA.revenueByMonth.map(d => d.revenue));
  const maxDisputes = Math.max(...MOCK_DATA.disputesByStatus.map(d => d.count));
          ...
          <View style={styles.chartContainer}>
            {MOCK_DATA.userGrowth.map((item, i) => (
              <View key={i} style={styles.barColumn}>
                <View style={[styles.bar, styles.blueBar, { height: (item.count / maxUsers) * 80 }]} />
          ...
          <View style={styles.chartContainer}>
            {MOCK_DATA.revenueByMonth.map((item, i) => (
              <View key={i} style={styles.barColumn}>
                ...
                <View style={[styles.bar, styles.greenBar, { height: (item.revenue / maxRevenue) * 80 }]} />
          ...
          {MOCK_DATA.disputesByStatus.map((item, i) => (
            <View key={i} style={styles.statusRow}>
              ...
              <View style={styles.statusBarBg}>
                <View style={[styles.statusBarFill, { width: `${(item.count / maxDisputes) * 100}%` }]} />
              </View>
          ...
          <View style={styles.featuresGrid}>
            {MOCK_DATA.topFeatures.map((item, i) => (
              <View key={i} style={styles.featureItem}>
                <Text style={styles.featureValue}>{item.usage.toLocaleString()}</Text>
...
const styles = StyleSheet.create({
  ...
  barValue: { fontSize: 9, fontWeight: '600', color: theme.colors.text, marginBottom: 4 },
  statusRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  statusLabel: { width: 80, fontSize: 12, color: theme.colors.textSecondary },
  statusBarBg: { flex: 1, height: 10, backgroundColor: theme.colors.border, borderRadius: 5, marginHorizontal: 8 },
  statusBarFill: { height: 10, backgroundColor: theme.colors.secondary, borderRadius: 5 },
  statusCount: { width: 40, fontSize: 12, fontWeight: '600', color: theme.colors.text, textAlign: 'right' },
  featuresCard: { marginHorizontal: theme.spacing.lg, marginBottom: theme.spacing.xl, padding: theme.spacing.lg },
  featuresGrid: { flexDirection: 'row', flexWrap: 'wrap' },
  featureItem: { width: '50%', padding: theme.spacing.sm, alignItems: 'center' },
  featureValue: { fontSize: 20, fontWeight: '700', color: theme.colors.text },
  featureLabel: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 4 },
});
