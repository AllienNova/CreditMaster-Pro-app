...
const SCORE_HISTORY: ScoreDataPoint[] = [
  { month: 'Jul', score: 680 }, { month: 'Aug', score: 695 }, { month: 'Sep', score: 702 },
  { month: 'Oct', score: 715 }, { month: 'Nov', score: 728 }, { month: 'Dec', score: 742 },
];

const SCORE_FACTORS: ScoreFactor[] = [
  { name: 'Payment History', impact: 'positive', weight: 35, description: '100% on-time payments' },
  { name: 'Credit Utilization', impact: 'positive', weight: 30, description: '18% utilization rate' },
  { name: 'Credit Age', impact: 'neutral', weight: 15, description: '4.5 years average age' },
  { name: 'Credit Mix', impact: 'positive', weight: 10, description: 'Good variety of accounts' },
  { name: 'New Credit', impact: 'negative', weight: 10, description: '2 recent inquiries' },
];
...
export default function CreditScoreAnalyticsScreen() {
  ...
  const scoreChange = SCORE_HISTORY[SCORE_HISTORY.length - 1].score - SCORE_HISTORY[0].score;
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.scoreCard}>
          <View style={styles.scoreHeader}>
            ...
            <View style={[styles.changeBadge, { backgroundColor: scoreChange >= 0 ? '#22C55E15' : '#EF444415' }]}>
              <Ionicons name={scoreChange >= 0 ? 'arrow-up' : 'arrow-down'} size={14} color={scoreChange >= 0 ? '#22C55E' : '#EF4444'} />
              <Text style={[styles.changeText, { color: scoreChange >= 0 ? '#22C55E' : '#EF4444' }]}>+{scoreChange} pts</Text>
            </View>
          </View>
          <View style={styles.scoreRange}>
            <Text style={styles.rangeLabel}>Excellent (740-850)</Text>
            <View style={styles.rangeBar}>
              <View style={[styles.rangeFill, { width: `${((742 - 300) / 550) * 100}%` }]} />
            </View>
          </View>
        </Card>
          ...
          <View style={styles.chart}>
            {SCORE_HISTORY.map((point, index) => (
              <View key={index} style={styles.chartBar}>
                <View style={[styles.bar, { height: `${((point.score - minScore + 20) / (maxScore - minScore + 40)) * 100}%` }]}>
        ...
        {SCORE_FACTORS.map((factor, index) => (
          <Card key={index} style={styles.factorCard}>
            <View style={styles.factorHeader}>
              <View style={[styles.factorIcon, { backgroundColor: `${getImpactColor(factor.impact)}15` }]}>
                <Ionicons name={factor.impact === 'positive' ? 'checkmark-circle' : factor.impact === 'negative' ? 'close-circle' : 'remove-circle'} size={20} color={getImpactColor(factor.impact)} />
              </View>
            ...
            <View style={styles.factorBar}>
              <View style={[styles.factorFill, { width: `${factor.weight}%`, backgroundColor: getImpactColor(factor.impact) }]} />
            </View>
          </Card>
        ))}
        ...
        <Card style={styles.predictionCard}>
          ...
          <Text style={styles.predictionNote}>Based on current trends and planned actions</Text>
        </Card>
...
const styles = StyleSheet.create({
  ...
  changeBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  changeText: { fontSize: 13, fontWeight: '600', marginLeft: 4 },
  scoreRange: { marginTop: theme.spacing.md },
  rangeLabel: { fontSize: 12, color: theme.colors.textSecondary, marginBottom: 6 },
  rangeBar: { height: 8, backgroundColor: theme.colors.border, borderRadius: 4 },
  rangeFill: { height: '100%', backgroundColor: theme.colors.primary, borderRadius: 4 },
  ...
  chartTitle: { fontSize: 15, fontWeight: '600', color: theme.colors.text, marginBottom: theme.spacing.md },
  chart: { flexDirection: 'row', height: 150, alignItems: 'flex-end' },
  chartBar: { flex: 1, alignItems: 'center' },
  bar: { width: '70%', backgroundColor: theme.colors.primary, borderRadius: 4, justifyContent: 'flex-start', alignItems: 'center', paddingTop: 4 },
  ...
  factorIcon: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  factorInfo: { flex: 1 },
  factorName: { fontSize: 14, fontWeight: '600', color: theme.colors.text },
  factorDescription: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
  factorWeight: { fontSize: 14, fontWeight: '600', color: theme.colors.textSecondary },
  factorBar: { height: 4, backgroundColor: theme.colors.border, borderRadius: 2, marginTop: theme.spacing.sm },
  ...
  predictionValue: { fontSize: 24, fontWeight: '700', color: theme.colors.text, marginTop: 4 },
  predictionChange: { fontSize: 12, fontWeight: '500', color: '#22C55E', marginTop: 2 },
  predictionDivider: { width: 1, backgroundColor: theme.colors.border },
  predictionNote: { fontSize: 11, color: theme.colors.textSecondary, textAlign: 'center', marginTop: theme.spacing.md },
});
