...
const INCOME_SOURCES: IncomeSource[] = [
  { id: '1', name: 'Primary Job', amount: 4800, frequency: 'monthly', type: 'salary', taxWithheld: 960 },
  { id: '2', name: 'Freelance Work', amount: 800, frequency: 'monthly', type: 'freelance', taxWithheld: 0 },
  { id: '3', name: 'Dividend Income', amount: 150, frequency: 'monthly', type: 'investment', taxWithheld: 22 },
  { id: '4', name: 'Rental Property', amount: 1200, frequency: 'monthly', type: 'rental', taxWithheld: 0 },
];
...
const getSourceIcon = (type: IncomeSource['type']): keyof typeof Ionicons.glyphMap => {
  const icons: Record<IncomeSource['type'], keyof typeof Ionicons.glyphMap> = {
    salary: 'briefcase', freelance: 'laptop', investment: 'trending-up', rental: 'home', other: 'cash'
  };
  return icons[type];
};
...
const getSourceColor = (type: IncomeSource['type']): string => {
  const colors: Record<IncomeSource['type'], string> = {
    salary: '#22C55E', freelance: '#3B82F6', investment: '#8B5CF6', rental: '#F59E0B', other: '#6B7280'
  };
  return colors[type];
};
...
const getFrequencyLabel = (freq: IncomeSource['frequency']): string => {
  const labels: Record<IncomeSource['frequency'], string> = {
    weekly: '/week', biweekly: '/2 weeks', monthly: '/month', annual: '/year'
  };
  return labels[freq];
};
...
export default function IncomeScreen() {
  ...
  const totalMonthlyGross = INCOME_SOURCES.reduce((sum, s) => sum + s.amount, 0);
  const totalMonthlyTax = INCOME_SOURCES.reduce((sum, s) => sum + s.taxWithheld, 0);
  const totalMonthlyNet = totalMonthlyGross - totalMonthlyTax;
  const effectiveTaxRate = ((totalMonthlyTax / totalMonthlyGross) * 100).toFixed(1);
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.summaryCard}>
          <View style={styles.summaryRow}>
            <View style={styles.summaryItem}>
              ...
              <Text style={styles.summaryValue}>${totalMonthlyGross.toLocaleString()}</Text>
            </View>
            <View style={styles.summaryDivider} />
            <View style={styles.summaryItem}>
              <Text style={styles.summaryLabel}>Taxes</Text>
              <Text style={[styles.summaryValue, { color: '#EF4444' }]}>-${totalMonthlyTax.toLocaleString()}</Text>
            </View>
            <View style={styles.summaryDivider} />
            <View style={styles.summaryItem}>
              <Text style={styles.summaryLabel}>Net</Text>
              <Text style={[styles.summaryValue, { color: '#22C55E' }]}>${totalMonthlyNet.toLocaleString()}</Text>
            </View>
          </View>
        ...
        <Card style={styles.chartCard}>
            ...
            {INCOME_HISTORY.map((month) => (
              <View key={month.month} style={styles.chartColumn}>
                <View style={styles.barGroup}>
                  <View style={[styles.grossBar, { height: (month.gross / maxIncome) * 70 }]} />
                  <View style={[styles.netBar, { height: (month.net / maxIncome) * 70 }]} />
                </View>
          ...
          <View style={styles.chartLegend}>
            <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: theme.colors.primary }]} /><Text style={styles.legendText}>Gross</Text></View>
            <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: '#22C55E' }]} /><Text style={styles.legendText}>Net</Text></View>
          </View>
        </Card>
        ...
        {INCOME_SOURCES.map((source) => {
          ...
          const percent = ((source.amount / totalMonthlyGross) * 100).toFixed(0);
          return (
            <Card key={source.id} style={styles.sourceCard}>
              <View style={styles.sourceRow}>
                <View style={[styles.sourceIcon, { backgroundColor: `${color}15` }]}>
                  <Ionicons name={getSourceIcon(source.type)} size={20} color={color} />
                </View>
                <View style={styles.sourceInfo}>
                  <Text style={styles.sourceName}>{source.name}</Text>
                  <Text style={styles.sourceType}>{source.type} • {percent}% of income</Text>
                </View>
                <View style={styles.sourceValues}>
                  <Text style={styles.sourceAmount}>${source.amount.toLocaleString()}{getFrequencyLabel(source.frequency)}</Text>
                  {source.taxWithheld > 0 && <Text style={styles.sourceTax}>-${source.taxWithheld} tax</Text>}
                </View>
              </View>
            </Card>
          );
        ...
        <Card style={styles.taxCard}>
          ...
          <View style={styles.taxRow}>
            ...
            <Text style={styles.taxValue}>${(totalMonthlyGross * 12).toLocaleString()}</Text>
          </View>
          <View style={styles.taxRow}>
            <Text style={styles.taxLabel}>Estimated Federal Tax</Text>
            <Text style={styles.taxValue}>${(totalMonthlyTax * 12).toLocaleString()}</Text>
          </View>
          <View style={[styles.taxRow, { borderTopWidth: 1, borderTopColor: theme.colors.border, paddingTop: 8 }]}>
            <Text style={[styles.taxLabel, { fontWeight: '600' }]}>Estimated Take-Home</Text>
            <Text style={[styles.taxValue, { color: '#22C55E', fontWeight: '600' }]}>${(totalMonthlyNet * 12).toLocaleString()}</Text>
          </View>
        </Card>
...
const styles = StyleSheet.create({
  ...
  summaryValue: { fontSize: 18, fontWeight: '700', color: theme.colors.text, marginTop: 2 },
  taxRateRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: theme.spacing.md, paddingTop: theme.spacing.md, borderTopWidth: 1, borderTopColor: theme.colors.border },
  ...
  periodChip: { flex: 1, paddingVertical: 8, alignItems: 'center', backgroundColor: theme.colors.surface, marginHorizontal: 2, borderRadius: 8 },
  periodChipActive: { backgroundColor: theme.colors.primary },
  periodText: { fontSize: 12, color: theme.colors.textSecondary, fontWeight: '500', textTransform: 'capitalize' },
  ...
  grossBar: { width: 12, backgroundColor: theme.colors.primary, borderRadius: 2, marginRight: 2 },
  ...
  sourceIcon: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  sourceInfo: { flex: 1 },
  sourceName: { fontSize: 14, fontWeight: '500', color: theme.colors.text },
  sourceType: { fontSize: 11, color: theme.colors.textSecondary, marginTop: 2, textTransform: 'capitalize' },
  ...
  taxHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: theme.spacing.md },
  taxTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginLeft: 8 },
