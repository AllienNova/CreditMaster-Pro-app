...
const MOCK_BILLS: Bill[] = [
  { id: '1', name: 'Rent', amount: 1850, dueDate: 'Dec 1', daysUntil: -5, category: 'Housing', autoPay: false, status: 'paid', icon: 'home' },
  { id: '2', name: 'Chase Sapphire', amount: 2340, dueDate: 'Dec 15', daysUntil: 9, category: 'Credit Card', autoPay: true, status: 'upcoming', icon: 'card' },
  { id: '3', name: 'Electric Bill', amount: 145, dueDate: 'Dec 10', daysUntil: 4, category: 'Utilities', autoPay: false, status: 'due_soon', icon: 'flash' },
  { id: '4', name: 'Internet', amount: 79, dueDate: 'Dec 12', daysUntil: 6, category: 'Utilities', autoPay: true, status: 'upcoming', icon: 'wifi' },
  { id: '5', name: 'Car Insurance', amount: 185, dueDate: 'Dec 20', daysUntil: 14, category: 'Insurance', autoPay: true, status: 'upcoming', icon: 'car' },
  { id: '6', name: 'Netflix', amount: 15.99, dueDate: 'Dec 8', daysUntil: 2, category: 'Subscriptions', autoPay: true, status: 'due_soon', icon: 'tv' },
  { id: '7', name: 'Gym Membership', amount: 49, dueDate: 'Dec 1', daysUntil: -5, category: 'Health', autoPay: true, status: 'paid', icon: 'fitness' },
];

const CALENDAR_DAYS = Array.from({ length: 14 }, (_, i) => {
  const date = new Date();
  date.setDate(date.getDate() + i);
  return { day: date.getDate(), weekday: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][date.getDay()], hasBill: [8, 10, 12, 15, 20].includes(date.getDate()) };
});
...
export default function BillsScreen() {
  ...
  const filteredBills = selectedFilter === 'all' ? bills : bills.filter(b => b.status === selectedFilter);
      ...
      >
        ...
        <Card style={styles.summaryCard}>
          <View style={styles.summaryRow}>
            ...
            <View style={styles.summaryItem}>
              <Text style={[styles.summaryValue, { color: '#F59E0B' }]}>{dueSoon}</Text>
              <Text style={styles.summaryLabel}>Due Soon</Text>
            </View>
            <View style={styles.summaryDivider} />
            <View style={styles.summaryItem}>
              <Text style={[styles.summaryValue, { color: '#22C55E' }]}>{bills.filter(b => b.autoPay).length}</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.calendarScroll}>
          {CALENDAR_DAYS.map((day, idx) => (
            <View key={idx} style={[styles.calendarDay, idx === 0 && styles.calendarDayToday]}>
              <Text style={[styles.calendarWeekday, idx === 0 && styles.calendarTextToday]}>{day.weekday}</Text>
              <Text style={[styles.calendarDate, idx === 0 && styles.calendarTextToday]}>{day.day}</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
          {filters.map((filter) => (
            <TouchableOpacity key={filter} style={[styles.filterChip, selectedFilter === filter && styles.filterChipActive]} onPress={() => setSelectedFilter(filter)}>
              <Text style={[styles.filterChipText, selectedFilter === filter && styles.filterChipTextActive]}>{filter === 'due_soon' ? 'Due Soon' : filter.charAt(0).toUpperCase() + filter.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {filteredBills.map((bill) => (
          <Card key={bill.id} style={styles.billCard}>
            <View style={styles.billRow}>
              <View style={[styles.billIcon, { backgroundColor: bill.status === 'paid' ? '#DCFCE7' : bill.status === 'due_soon' ? '#FEF3C7' : '#F3F4F6' }]}>
                <Ionicons name={bill.icon as keyof typeof Ionicons.glyphMap} size={20} color={bill.status === 'paid' ? '#22C55E' : bill.status === 'due_soon' ? '#F59E0B' : theme.colors.textSecondary} />
              </View>
              <View style={styles.billInfo}>
                <View style={styles.billNameRow}>
                  <Text style={styles.billName}>{bill.name}</Text>
                  {bill.autoPay && <View style={styles.autoPayBadge}><Ionicons name="sync" size={10} color="#3B82F6" /><Text style={styles.autoPayText}>Auto</Text></View>}
                </View>
                <Text style={styles.billMeta}>{bill.category} • {bill.dueDate}</Text>
              </View>
              <View style={styles.billAmountContainer}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: `${getStatusColor(bill.status)}15` }]}>
                  <Text style={[styles.statusText, { color: getStatusColor(bill.status) }]}>{getStatusLabel(bill.status)}</Text>
                </View>
              </View>
            </View>
          </Card>
...
const styles = StyleSheet.create({
  ...
  calendarScroll: { marginBottom: theme.spacing.lg, marginHorizontal: -theme.spacing.lg, paddingHorizontal: theme.spacing.lg },
  calendarDay: { width: 48, height: 64, alignItems: 'center', justifyContent: 'center', marginRight: 8, backgroundColor: theme.colors.surface, borderRadius: 12 },
  calendarDayToday: { backgroundColor: theme.colors.primary },
  calendarWeekday: { fontSize: 10, color: theme.colors.textSecondary },
  calendarDate: { fontSize: 18, fontWeight: '600', color: theme.colors.text, marginTop: 2 },
  calendarTextToday: { color: '#fff' },
  calendarDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#EF4444', marginTop: 4 },
  ...
  autoPayBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#DBEAFE', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 8, marginLeft: 8 },
  autoPayText: { fontSize: 9, color: '#3B82F6', fontWeight: '600', marginLeft: 2 },
