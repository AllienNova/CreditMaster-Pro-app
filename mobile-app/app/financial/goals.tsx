...
const MOCK_GOALS: Goal[] = [
  { id: '1', name: 'Emergency Fund', icon: 'shield-checkmark', color: '#22C55E', targetAmount: 15000, currentAmount: 8500, targetDate: 'Jun 2025', monthlyContribution: 500, milestones: [{ percent: 25, reached: true }, { percent: 50, reached: true }, { percent: 75, reached: false }, { percent: 100, reached: false }], category: 'savings' },
  { id: '2', name: 'Vacation Fund', icon: 'airplane', color: '#3B82F6', targetAmount: 5000, currentAmount: 2200, targetDate: 'Aug 2025', monthlyContribution: 300, milestones: [{ percent: 25, reached: true }, { percent: 50, reached: false }, { percent: 75, reached: false }, { percent: 100, reached: false }], category: 'savings' },
  { id: '3', name: 'Pay Off Credit Card', icon: 'card', color: '#EF4444', targetAmount: 4500, currentAmount: 3200, targetDate: 'Mar 2025', monthlyContribution: 400, milestones: [{ percent: 25, reached: true }, { percent: 50, reached: true }, { percent: 75, reached: false }, { percent: 100, reached: false }], category: 'debt' },
  { id: '4', name: 'New Car Down Payment', icon: 'car', color: '#8B5CF6', targetAmount: 8000, currentAmount: 1500, targetDate: 'Dec 2025', monthlyContribution: 250, milestones: [{ percent: 25, reached: false }, { percent: 50, reached: false }, { percent: 75, reached: false }, { percent: 100, reached: false }], category: 'purchase' },
];
      ...
      if (storeGoals.length > 0) {
        const transformedGoals = storeGoals.map(g => {
          ...
          return {
            ...
            category: (g.type === 'debt_payoff' ? 'debt' : g.type === 'emergency_fund' ? 'savings' : g.type) as Goal['category'],
          };
        });
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => router.push('/financial/create-goal')}>
        ...
        <Card style={styles.overviewCard}>
          <View style={styles.overviewHeader}>
            <View>
              ...
              <Text style={styles.overviewValue}>${totalSaved.toLocaleString()} <Text style={styles.overviewTarget}>/ ${totalTarget.toLocaleString()}</Text></Text>
            </View>
          ...
          <View style={styles.overviewStats}>
            ...
            <View style={styles.overviewStat}>
              ...
              <Text style={styles.statValue}>{goals.filter(g => g.currentAmount >= g.targetAmount).length}</Text>
              <Text style={styles.statLabel}>Completed</Text>
            </View>
            <View style={styles.overviewStat}>
              <Ionicons name="trending-up" size={16} color="#3B82F6" />
              <Text style={styles.statValue}>${goals.reduce((sum, g) => sum + g.monthlyContribution, 0)}</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
          {CATEGORIES.map((cat) => (
            <TouchableOpacity key={cat} style={[styles.filterChip, selectedCategory === cat && styles.filterChipActive]} onPress={() => setSelectedCategory(cat)}>
              <Text style={[styles.filterChipText, selectedCategory === cat && styles.filterChipTextActive]}>{cat.charAt(0).toUpperCase() + cat.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {filteredGoals.map((goal) => {
          ...
          return (
            <TouchableOpacity key={goal.id} onPress={() => router.push(`/financial/goal-detail?id=${goal.id}`)}>
              <Card style={styles.goalCard}>
                <View style={styles.goalHeader}>
                  ...
                  <View style={styles.goalAmounts}>
                    ...
                    <Text style={styles.goalTargetAmount}>/ ${goal.targetAmount.toLocaleString()}</Text>
                  </View>
                </View>
                <View style={styles.goalProgressContainer}>
                  <View style={[styles.goalProgress, { width: `${progress}%`, backgroundColor: goal.color }]} />
                </View>
                <View style={styles.milestonesRow}>
                  {goal.milestones.map((m, idx) => (
                    <View key={idx} style={styles.milestone}>
                      <View style={[styles.milestoneIcon, m.reached && { backgroundColor: goal.color }]}>
                        {m.reached && <Ionicons name="checkmark" size={10} color="#fff" />}
                      </View>
                ...
                <View style={styles.goalFooter}>
                  <View style={styles.contributionBadge}>
                    ...
                    <Text style={styles.contributionText}>${goal.monthlyContribution}/mo</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  overviewValue: { fontSize: 24, fontWeight: '700', color: theme.colors.text, marginTop: 2 },
  overviewTarget: { fontSize: 14, fontWeight: '400', color: theme.colors.textSecondary },
  progressCircle: { width: 56, height: 56, borderRadius: 28, backgroundColor: theme.colors.primary, justifyContent: 'center', alignItems: 'center' },
  ...
  statValue: { fontSize: 18, fontWeight: '600', color: theme.colors.text, marginTop: 4 },
  ...
  goalProgressContainer: { height: 6, backgroundColor: theme.colors.border, borderRadius: 3, marginTop: theme.spacing.md },
  goalProgress: { height: '100%', borderRadius: 3 },
  milestonesRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: theme.spacing.md, paddingHorizontal: theme.spacing.sm },
  milestone: { alignItems: 'center' },
  milestoneIcon: { width: 20, height: 20, borderRadius: 10, backgroundColor: theme.colors.border, justifyContent: 'center', alignItems: 'center' },
  milestoneText: { fontSize: 9, color: theme.colors.textSecondary, marginTop: 2 },
  goalFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: theme.spacing.md, paddingTop: theme.spacing.md, borderTopWidth: 1, borderTopColor: theme.colors.border },
  contributionBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: `${theme.colors.primary}10`, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  contributionText: { fontSize: 12, color: theme.colors.primary, fontWeight: '500', marginLeft: 4 },
