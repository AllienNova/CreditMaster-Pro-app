...
interface SpendingCategory { name: string; amount: number; budget: number; icon: string; color: string; }
interface SpendingTrend { month: string; amount: number; }
...
const MOCK_CATEGORIES: SpendingCategory[] = [
  { name: 'Housing', amount: 1800, budget: 1800, icon: 'home', color: '#3B82F6' },
  { name: 'Food & Dining', amount: 650, budget: 600, icon: 'restaurant', color: '#F59E0B' },
  { name: 'Transportation', amount: 420, budget: 500, icon: 'car', color: '#10B981' },
  { name: 'Shopping', amount: 380, budget: 300, icon: 'bag', color: '#EC4899' },
  { name: 'Entertainment', amount: 180, budget: 200, icon: 'game-controller', color: '#8B5CF6' },
  { name: 'Utilities', amount: 220, budget: 250, icon: 'flash', color: '#F97316' },
];
...
export default function SpendingScreen() {
    ...
    try {
      ...
      if (response.success && response.data) {
        const catData = response.data.byCategory.map(cat => ({
          name: cat.category,
          amount: cat.amount,
          budget: budgets.find(b => b.category === cat.category)?.limit || cat.amount * 1.2,
  ...
  const avgMonthly = trends.length > 0 ? trends.reduce((sum, t) => sum + t.amount, 0) / trends.length : 0;
  ...
  const generateInsights = () => {
    ...
    categories.forEach(cat => {
      ...
      if (percent > 100) {
        insights.push({ type: 'warning', text: `${cat.name} exceeded budget by $${(cat.amount - cat.budget).toFixed(0)}` });
      } else if (percent < 80) {
        insights.push({ type: 'success', text: `${cat.name} is ${(100 - percent).toFixed(0)}% under budget` });
      }
    });
      ...
      >
        ...
        <View style={styles.statsRow}>
          ...
          <Card style={styles.statCard}>
            ...
            <Text style={[styles.statValue, { color: totalBudget - totalSpent >= 0 ? '#22C55E' : '#EF4444' }]}>
        ...
        <View style={styles.chartToggle}>
          ...
          >
            <Ionicons name="pie-chart" size={18} color={chartView === 'pie' ? '#fff' : theme.colors.textSecondary} />
            <Text style={[styles.toggleText, chartView === 'pie' && styles.toggleTextActive]}>Pie</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.toggleButton, chartView === 'bar' && styles.toggleButtonActive]}
            onPress={() => setChartView('bar')}
          >
            <Ionicons name="bar-chart" size={18} color={chartView === 'bar' ? '#fff' : theme.colors.textSecondary} />
            <Text style={[styles.toggleText, chartView === 'bar' && styles.toggleTextActive]}>Bar</Text>
          </TouchableOpacity>
        </View>
                ...
                <View style={styles.categoryHeader}>
                  ...
                  <View style={styles.categoryInfo}>
                    ...
                    <View style={styles.progressBarBg}>
                      <View style={[styles.progressBarFill, { width: `${Math.min(progress, 100)}%`, backgroundColor: isOver ? '#EF4444' : category.color }]} />
                    </View>
                  </View>
                  <View style={styles.categoryAmounts}>
                    <Text style={[styles.spentAmount, isOver && { color: '#EF4444' }]}>${category.amount}</Text>
                    <Text style={styles.budgetAmount}>/ ${category.budget}</Text>
                  </View>
                </View>
              </Card>
            );
        ...
        <Card style={styles.insightsCard}>
          ...
          {insights.length > 0 ? (
            ...
            <Text style={styles.insightText}>✅ All categories are within budget!</Text>
          )}
          <TouchableOpacity style={styles.viewAllButton} onPress={() => router.push('/insights')}>
...
const styles = StyleSheet.create({
  ...
  chartToggle: { flexDirection: 'row', justifyContent: 'center', marginTop: theme.spacing.lg, gap: 8 },
  ...
  chartCard: { marginHorizontal: theme.spacing.lg, marginTop: theme.spacing.md, padding: theme.spacing.lg, alignItems: 'center' },
  ...
  categoryName: { fontSize: 14, fontWeight: '500', color: theme.colors.text, marginBottom: 6 },
  progressBarBg: { height: 6, backgroundColor: theme.colors.border, borderRadius: 3 },
  ...
  trendCard: { marginHorizontal: theme.spacing.lg, marginTop: theme.spacing.lg, padding: theme.spacing.lg },
  avgRow: { flexDirection: 'row', justifyContent: 'center', marginTop: theme.spacing.md },
  avgLabel: { fontSize: 12, color: theme.colors.textSecondary },
  avgValue: { fontSize: 12, fontWeight: '600', color: theme.colors.text, marginLeft: 4 },
  insightsCard: { marginHorizontal: theme.spacing.lg, marginVertical: theme.spacing.lg, padding: theme.spacing.lg },
  insightsHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: theme.spacing.md },
  insightText: { fontSize: 13, color: theme.colors.text, lineHeight: 24 },
  viewAllButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: theme.spacing.md, paddingTop: theme.spacing.md, borderTopWidth: 1, borderTopColor: theme.colors.border },
