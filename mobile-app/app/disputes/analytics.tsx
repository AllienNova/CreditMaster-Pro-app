...
const BUREAU_STATS: BureauStats[] = [
  { name: 'Experian', total: 15, successful: 12, pending: 2, avgDays: 28, color: '#3B82F6' },
  { name: 'Equifax', total: 12, successful: 9, pending: 1, avgDays: 32, color: '#22C55E' },
  { name: 'TransUnion', total: 10, successful: 7, pending: 1, avgDays: 25, color: '#F59E0B' },
];
...
export default function DisputeAnalyticsScreen() {
  ...
  const totalSuccessful = BUREAU_STATS.reduce((sum, b) => sum + b.successful, 0);
  const overallSuccessRate = Math.round((totalSuccessful / totalDisputes) * 100);
  const avgResolutionDays = Math.round(BUREAU_STATS.reduce((sum, b) => sum + b.avgDays, 0) / BUREAU_STATS.length);
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.overviewCard}>
          <View style={styles.overviewRow}>
            ...
            <View style={styles.overviewItem}>
              <Text style={[styles.overviewValue, { color: '#22C55E' }]}>{overallSuccessRate}%</Text>
              <Text style={styles.overviewLabel}>Success Rate</Text>
            </View>
            <View style={styles.overviewDivider} />
            <View style={styles.overviewItem}>
              <Text style={[styles.overviewValue, { color: theme.colors.primary }]}>{avgResolutionDays}</Text>
          ...
          <Card key={bureau.name} style={styles.bureauCard}>
            ...
            <View style={styles.bureauStats}>
              <View style={styles.bureauStat}>
                <Text style={styles.bureauStatValue}>{Math.round((bureau.successful / bureau.total) * 100)}%</Text>
              ...
              <View style={styles.bureauStat}>
                <Text style={[styles.bureauStatValue, { color: '#F59E0B' }]}>{bureau.pending}</Text>
            ...
            <View style={styles.progressContainer}>
              <View style={[styles.progressBar, { width: `${(bureau.successful / bureau.total) * 100}%`, backgroundColor: bureau.color }]} />
            </View>
          </Card>
        ))}
        ...
        <Card style={styles.typesCard}>
          {DISPUTE_TYPES.map((item, idx) => (
            <View key={item.type} style={[styles.typeRow, idx < DISPUTE_TYPES.length - 1 && styles.typeRowBorder]}>
              ...
              <View style={styles.typeSuccess}>
                <Text style={[styles.typeSuccessRate, { color: item.success / item.count >= 0.75 ? '#22C55E' : item.success / item.count >= 0.5 ? '#F59E0B' : '#EF4444' }]}>{Math.round((item.success / item.count) * 100)}%</Text>
        ...
        <Card style={styles.trendCard}>
          <View style={styles.trendLegend}>
            <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: theme.colors.primary }]} /><Text style={styles.legendText}>Submitted</Text></View>
            ...
            {MONTHLY_DATA.map((data, idx) => (
              <View key={data.month} style={styles.chartColumn}>
                <View style={styles.barsContainer}>
                  <View style={[styles.bar, { height: data.submitted * 10, backgroundColor: theme.colors.primary }]} />
                  <View style={[styles.bar, { height: data.resolved * 10, backgroundColor: '#22C55E' }]} />
                </View>
...
const styles = StyleSheet.create({
  ...
  overviewLabel: { fontSize: 11, color: theme.colors.textSecondary, marginTop: 2 },
  ...
  bureauName: { fontSize: 16, fontWeight: '600', color: theme.colors.text, flex: 1 },
  ...
  bureauStatValue: { fontSize: 16, fontWeight: '600', color: theme.colors.text },
  bureauStatLabel: { fontSize: 10, color: theme.colors.textSecondary, marginTop: 2 },
  progressContainer: { height: 4, backgroundColor: theme.colors.border, borderRadius: 2, marginTop: theme.spacing.md },
  progressBar: { height: '100%', borderRadius: 2 },
  typesCard: { marginBottom: theme.spacing.lg },
  typeRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: theme.spacing.sm },
  typeRowBorder: { borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  typeInfo: { flex: 1 },
  ...
  trendLegend: { flexDirection: 'row', justifyContent: 'center', marginBottom: theme.spacing.md },
  ...
  chartContainer: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'flex-end', height: 100 },
