...
const TIMELINE: TimelineEvent[] = [
  {
    ...
    description: "Your dispute was submitted to Experian",
    status: "complete",
  },
  {
    id: "2",
    date: "Nov 17, 2024",
    title: "Received by Bureau",
    description: "Experian acknowledged receipt of your dispute",
    status: "complete",
  },
  {
    id: "3",
    ...
    description: "Bureau began investigating with creditor",
    status: "current",
  },
  {
    id: "4",
    ...
    description: "Bureau must respond within 30 days",
    status: "pending",
  },
];
        ...
        <Card style={styles.statusCard}>
          <View style={styles.statusHeader}>
            <View
              style={[
                styles.statusBadge,
                { backgroundColor: `${getStatusColor(DISPUTE_DATA.status)}15` },
              ]}
            >
              <View
                style={[
                  styles.statusDot,
                  { backgroundColor: getStatusColor(DISPUTE_DATA.status) },
                ]}
              />
              <Text
                style={[
                  styles.statusText,
                  { color: getStatusColor(DISPUTE_DATA.status) },
                ]}
              >
                {DISPUTE_DATA.status.charAt(0).toUpperCase() +
                  DISPUTE_DATA.status.slice(1)}
              </Text>
            </View>
            <Text style={styles.daysRemaining}>
              {DISPUTE_DATA.daysRemaining} days remaining
            </Text>
          </View>
          ...
          <View style={styles.detailsGrid}>
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailValue}>{DISPUTE_DATA.type}</Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>Bureau</Text>
              <Text style={styles.detailValue}>{DISPUTE_DATA.bureau}</Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>Method</Text>
              <Text style={styles.detailValue}>
                {DISPUTE_DATA.disputeMethod}
              </Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>Confirmation</Text>
              <Text style={styles.detailValue}>
                {DISPUTE_DATA.confirmationNumber}
              </Text>
            </View>
          </View>
        </Card>
            ...
            <View key={event.id} style={styles.timelineItem}>
              <View style={styles.timelineLeft}>
                <View
                  style={[
                    styles.timelineDot,
                    { backgroundColor: getTimelineColor(event.status) },
                  ]}
                >
