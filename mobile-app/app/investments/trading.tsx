...
export default function TradingScreen() {
  ...
  const handleBuy = useCallback((price: number) => {
    Alert.alert(
      'Buy Order',
      `Buy ${symbol} at $${price.toFixed(2)}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        { 
          text: 'Confirm', 
          onPress: () => {
            Alert.alert('Order Placed', `Buy order for ${symbol} submitted at $${price.toFixed(2)}`);
          }
        },
      ]
    );
  ...
  const handleSell = useCallback((price: number) => {
    Alert.alert(
      'Sell Order',
      `Sell ${symbol} at $${price.toFixed(2)}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        { 
          text: 'Confirm', 
          onPress: () => {
            Alert.alert('Order Placed', `Sell order for ${symbol} submitted at $${price.toFixed(2)}`);
          }
        },
      ]
    );
