...
import { DonutChart, BarChart, LineChart } from '../../../src/components/charts';
...
const INSIDER_TRADES: InsiderTrade[] = [
  { name: 'Tim Cook', title: 'CEO', type: 'sell', shares: 50000, value: '$9.2M', date: 'Jan 2' },
  { name: 'Luca Maestri', title: 'CFO', type: 'sell', shares: 25000, value: '$4.5M', date: 'Dec 28' },
  { name: 'Jeff Williams', title: 'COO', type: 'buy', shares: 10000, value: '$1.8M', date: 'Dec 15' },
];
...
export default function SentimentAnalysisScreen() {
  ...
  const getRatingColor = (rating: string) => {
    if (rating.toLowerCase().includes('buy') || rating.toLowerCase().includes('overweight')) {
      return theme.colors.success;
    }
    if (rating.toLowerCase().includes('sell') || rating.toLowerCase().includes('underweight')) {
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.scoreCard}>
          <View style={styles.scoreHeader}>
            ...
            <View style={[styles.changeBadge, { backgroundColor: theme.colors.success + '20' }]}>
          ...
          <View style={styles.sentimentBar}>
            <View style={[styles.sentimentSegment, { flex: 58, backgroundColor: theme.colors.success }]} />
            <View style={[styles.sentimentSegment, { flex: 28, backgroundColor: theme.colors.warning }]} />
            <View style={[styles.sentimentSegment, { flex: 14, backgroundColor: theme.colors.error }]} />
          </View>
              ...
              {SOCIAL_SENTIMENT.map((item, idx) => (
                <View key={idx} style={styles.socialRow}>
                  <View style={styles.socialPlatform}>
                    <Ionicons
                      name={item.platform === 'Twitter/X' ? 'logo-twitter' :
                            item.platform === 'Reddit' ? 'logo-reddit' :
                            item.platform === 'StockTwits' ? 'chatbubbles' : 'newspaper'}
                      size={20}
                      color={theme.colors.primary}
                    />
                  ...
                  <View style={styles.socialScore}>
                    <Text style={[styles.scoreNum, {
                      color: item.sentiment >= 70 ? theme.colors.success :
                             item.sentiment >= 50 ? theme.colors.warning : theme.colors.error
                    }]}>
                      {item.sentiment}
                    </Text>
                    <Ionicons
                      ...
                      size={14}
                      color={item.trend === 'up' ? theme.colors.success :
                             item.trend === 'down' ? theme.colors.error : theme.colors.textSecondary}
                    />
                  </View>
                </View>
              ))}
          ...
          <Card style={styles.newsCard}>
            ...
            {NEWS_ITEMS.map((news, idx) => (
              <TouchableOpacity key={idx} style={styles.newsItem}>
                ...
                <View style={[styles.sentimentBadge, { backgroundColor: getSentimentColor(news.sentiment) + '20' }]}>
                  <Ionicons
                    name={getSentimentIcon(news.sentiment) as keyof typeof Ionicons.glyphMap}
                    size={16}
          ...
          <>
            ...
            <Card style={styles.consensusCard}>
              ...
              <View style={styles.consensusBar}>
                <View style={[styles.consensusSegment, { flex: ANALYST_CONSENSUS.buy, backgroundColor: theme.colors.success }]}>
                  <Text style={styles.consensusText}>{ANALYST_CONSENSUS.buy} Buy</Text>
                </View>
                <View style={[styles.consensusSegment, { flex: ANALYST_CONSENSUS.hold, backgroundColor: theme.colors.warning }]}>
                  <Text style={styles.consensusText}>{ANALYST_CONSENSUS.hold}</Text>
                </View>
                <View style={[styles.consensusSegment, { flex: ANALYST_CONSENSUS.sell, backgroundColor: theme.colors.error }]}>
                  <Text style={styles.consensusText}>{ANALYST_CONSENSUS.sell}</Text>
                </View>
              </View>
              <View style={styles.targetGrid}>
                <View style={styles.targetItem}>
                  <Text style={styles.targetValue}>${ANALYST_CONSENSUS.avgTarget}</Text>
                  <Text style={styles.targetLabel}>Avg Target</Text>
                </View>
                <View style={styles.targetItem}>
                  <Text style={[styles.targetValue, { color: theme.colors.success }]}>${ANALYST_CONSENSUS.highTarget}</Text>
                  <Text style={styles.targetLabel}>High</Text>
                </View>
                <View style={styles.targetItem}>
                  <Text style={[styles.targetValue, { color: theme.colors.error }]}>${ANALYST_CONSENSUS.lowTarget}</Text>
            ...
            <Card style={styles.ratingsCard}>
              ...
              {ANALYST_RATINGS.map((rating, idx) => (
                <View key={idx} style={styles.ratingRow}>
                  ...
                  <View style={[styles.ratingBadge, { backgroundColor: getRatingColor(rating.rating) + '20' }]}>
        ...
        {activeTab === 'insiders' && (
          <>
            ...
            <Card style={styles.institutionalCard}>
              <Text style={styles.sectionTitle}>Institutional Activity (Q4)</Text>
              <View style={styles.instGrid}>
                <View style={styles.instItem}>
                  <Ionicons name="arrow-up-circle" size={24} color={theme.colors.success} />
                  <Text style={styles.instValue}>{INSTITUTIONAL_ACTIVITY.increased}</Text>
                  <Text style={styles.instLabel}>Increased</Text>
                </View>
                <View style={styles.instItem}>
                  <Ionicons name="arrow-down-circle" size={24} color={theme.colors.error} />
                  <Text style={styles.instValue}>{INSTITUTIONAL_ACTIVITY.decreased}</Text>
                  <Text style={styles.instLabel}>Decreased</Text>
                </View>
                <View style={styles.instItem}>
                  <Ionicons name="add-circle" size={24} color={theme.colors.primary} />
                  <Text style={styles.instValue}>{INSTITUTIONAL_ACTIVITY.newPositions}</Text>
                  <Text style={styles.instLabel}>New</Text>
                </View>
                <View style={styles.instItem}>
                  <Ionicons name="remove-circle" size={24} color={theme.colors.warning} />
                  <Text style={styles.instValue}>{INSTITUTIONAL_ACTIVITY.soldOut}</Text>
                  <Text style={styles.instLabel}>Sold Out</Text>
                </View>
              </View>
              <View style={styles.netChange}>
                <Text style={styles.netChangeLabel}>Net Institutional Flow:</Text>
                <Text style={[styles.netChangeValue, { color: theme.colors.success }]}>
                ...
                <View key={idx} style={styles.tradeRow}>
                  ...
                  <View style={styles.tradeDetails}>
                    <View style={[styles.tradeBadge, {
                      backgroundColor: trade.type === 'buy' ? theme.colors.success + '20' : theme.colors.error + '20'
                    }]}>
...
const styles = StyleSheet.create({
  ...
  socialRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  socialPlatform: { flexDirection: 'row', alignItems: 'center', gap: 10, width: 120 },
  ...
  newsItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  newsContent: { flex: 1 },
  newsTitle: { fontSize: 14, fontWeight: '500', color: theme.colors.text, lineHeight: 20 },
  ...
  consensusBar: { flexDirection: 'row', height: 36, borderRadius: 8, overflow: 'hidden', marginBottom: 16 },
  ...
  targetLabel: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 4 },

  ratingsCard: { marginBottom: theme.spacing.md },
  ratingRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  ratingInfo: { flex: 1 },
  ...
  ratingBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, marginRight: 12 },
  ratingText: { fontSize: 12, fontWeight: '600' },
  ratingTarget: { fontSize: 14, fontWeight: '600', color: theme.colors.text, width: 60, textAlign: 'right' },

  institutionalCard: { marginBottom: theme.spacing.md },
  instGrid: { flexDirection: 'row', justifyContent: 'space-around', marginBottom: 16 },
  instItem: { alignItems: 'center' },
  instValue: { fontSize: 20, fontWeight: '700', color: theme.colors.text, marginTop: 4 },
  instLabel: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
  netChange: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingTop: 16, borderTopWidth: 1, borderTopColor: theme.colors.border, gap: 8 },
  ...
  tradeRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  tradeInfo: { flex: 1 },
  ...
  tradeDate: { fontSize: 12, color: theme.colors.textSecondary, width: 50, textAlign: 'right' },
});
