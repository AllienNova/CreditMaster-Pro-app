...
const SCORE_RANGES = [
  ...
  },
  {
    ...
    label: "Good",
    color: "#EAB308",
    description: "Near or slightly above average",
  },
  {
    min: 740,
    max: 799,
...
export default function ScoreDetailScreen() {
  ...
  const bureauBreakdown = useMemo(() => {
    return scores.map((score) => {
      const bureauFactors = factors.filter(
        (f) =>
              ...
              <View style={styles.bureauBreakdownHeader}>
                <View style={styles.bureauInfo}>
                  ...
                  <Text style={styles.bureauDate}>
                    {bureau.lastUpdated
                      ? new Date(bureau.lastUpdated).toLocaleDateString(
                          "en-US",
