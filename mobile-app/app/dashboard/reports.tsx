...
const REPORT_TYPES = [
  { type: 'analysis', name: 'Credit Analysis', desc: 'Comprehensive credit report analysis', icon: 'analytics' },
  { type: 'disputes', name: 'Dispute Summary', desc: 'All disputes and outcomes', icon: 'document-text' },
  { type: 'score', name: 'Score Report', desc: 'Detailed score breakdown', icon: 'trending-up' },
  { type: 'plan', name: 'Improvement Plan', desc: 'AI-generated roadmap', icon: 'map' },
];
...
export default function ReportsScreen() {
  ...
  const handleGenerate = (type: string, name: string) => {
    ...
    setTimeout(() => {
      ...
      setGenerating(null);
      Alert.alert('Report Generated', `Your ${name} has been generated successfully.`);
    }, 2000);
  };
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'disputes': return 'document-text';
      case 'score': return 'trending-up';
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'analysis': return theme.colors.primary;
      case 'disputes': return theme.colors.warning;
      case 'score': return theme.colors.success;
        ...
        <Card style={styles.generateCard}>
          ...
          <View style={styles.reportTypesGrid}>
            {REPORT_TYPES.map((rt) => (
              <TouchableOpacity
                key={rt.type}
                style={[styles.reportTypeCard, generating === rt.type && styles.reportTypeGenerating]}
                onPress={() => handleGenerate(rt.type, rt.name)}
                disabled={generating !== null}
              >
                <View style={[styles.typeIconContainer, { backgroundColor: getTypeColor(rt.type) + '20' }]}>
                  <Ionicons
          ...
          {reports.map((report) => (
            <View key={report.id} style={styles.reportItem}>
              <View style={[styles.reportIcon, { backgroundColor: getTypeColor(report.type) + '20' }]}>
                <Ionicons
                  name={getTypeIcon(report.type) as keyof typeof Ionicons.glyphMap}
                  size={20}
...
const styles = StyleSheet.create({
  ...
  typeIconContainer: { width: 44, height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  typeName: { fontSize: 14, fontWeight: '600', color: theme.colors.text, marginBottom: 4 },
  typeDesc: { fontSize: 12, color: theme.colors.textSecondary, lineHeight: 16 },
  generatingBadge: { marginTop: 8, backgroundColor: theme.colors.primary + '20', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, alignSelf: 'flex-start' },
  generatingText: { fontSize: 11, fontWeight: '600', color: theme.colors.primary },

  previousCard: {},
  reportItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  reportIcon: { width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  reportInfo: { flex: 1 },
  reportName: { fontSize: 14, fontWeight: '500', color: theme.colors.text, marginBottom: 2 },
