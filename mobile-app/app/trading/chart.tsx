...
const formatChange = (current: number, previous: number): { value: string; percent: string; isPositive: boolean } => {
...
const generateMockData = (symbol: string, timeframe: TimeFrame): PriceData[] => {
  const data: PriceData[] = [];
  const now = new Date();
  let basePrice = 150 + Math.random() * 100;

  const points = timeframe === '1m' ? 60 : timeframe === '5m' ? 60 : timeframe === '15m' ? 48 : timeframe === '1h' ? 24 : timeframe === '4h' ? 30 : 60;
    ...
    <View style={styles.priceHeader}>
      <View style={styles.priceMainContainer}>
        ...
        <View style={styles.changeContainer}>
          <Text style={[styles.changeText, { color: change.isPositive ? '#10B981' : '#EF4444' }]}>
            {change.value}
          </Text>
          <Text style={[styles.changeText, { color: change.isPositive ? '#10B981' : '#EF4444' }]}>
            ({change.percent})
...
function ChartPlaceholder({
  ...
  const chartData = data.map((d) => ({
    label: new Date(d.time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
    value: d.close,
  }));
    ...
    <View style={styles.indicatorPanel}>
      ...
      <View style={styles.indicatorGrid}>
        <View style={styles.indicatorItem}>
          ...
          <Text style={[styles.indicatorValue, { color: '#F59E0B' }]}>54.32</Text>
          <Text style={styles.indicatorStatus}>Neutral</Text>
        </View>
        <View style={styles.indicatorItem}>
          <Text style={styles.indicatorLabel}>MACD</Text>
          <Text style={[styles.indicatorValue, { color: '#10B981' }]}>+0.45</Text>
        ...
        <View style={styles.indicatorItem}>
          ...
          <Text style={[styles.indicatorValue, { color: '#10B981' }]}>1.5x Avg</Text>
...
export default function TradingChartScreen() {
  ...
  const low = Math.min(...chartData.filter((d) => d.low > 0).map((d) => d.low), currentPrice);
