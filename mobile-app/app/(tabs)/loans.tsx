...
export default function LoansScreen() {
  ...
  const fetchLoans = async () => {
    setLoans([
      { id: '1', user_id: '1', servicer: 'Nelnet', loan_type: 'federal', original_balance: 45000, current_balance: 38500, interest_rate: 4.5, monthly_payment: 450, status: 'current' },
      { id: '2', user_id: '1', servicer: 'Great Lakes', loan_type: 'federal', original_balance: 25000, current_balance: 22000, interest_rate: 5.0, monthly_payment: 280, status: 'current' },
      { id: '3', user_id: '1', servicer: 'Sallie Mae', loan_type: 'private', original_balance: 15000, current_balance: 12500, interest_rate: 7.5, monthly_payment: 200, status: 'current' },
    ]);
  };
  ...
  const avgRate = loans.length ? loans.reduce((sum, l) => sum + l.interest_rate, 0) / loans.length : 0;
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={theme.colors.primary} />}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addButton} onPress={() => router.push('/loans/add' as never)}>
      ...
      <View style={styles.summaryCard}>
        ...
        <View style={styles.summaryRow}>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryItemValue}>{formatCurrency(totalMonthly)}</Text>
      ...
      <View style={styles.actionsRow}>
        {[
          { icon: 'calculator', label: 'Calculator', route: '/loans/calculator' },
          { icon: 'school', label: 'Programs', route: '/loans/programs' },
          { icon: 'trending-down', label: 'Refinance', route: '/loans/refinance' },
        ].map((action, i) => (
          <TouchableOpacity key={i} style={styles.actionCard} onPress={() => router.push(action.route as never)}>
      ...
      <TouchableOpacity style={styles.programBanner} onPress={() => router.push('/loans/programs' as never)}>
        ...
        <View style={styles.programContent}>
          ...
          <Text style={styles.programDesc}>Check your eligibility for PSLF, IDR, and more</Text>
        </View>
      ...
      {loans.map(loan => (
        <TouchableOpacity key={loan.id} style={styles.loanCard} onPress={() => router.push(`/loans/${loan.id}` as never)}>
          <View style={styles.loanHeader}>
            <View>
              <Text style={styles.loanServicer}>{loan.servicer}</Text>
              <View style={[styles.typeBadge, { backgroundColor: loan.loan_type === 'federal' ? '#DBEAFE' : '#FEF3C7' }]}>
                <Text style={[styles.typeText, { color: loan.loan_type === 'federal' ? '#2563EB' : '#D97706' }]}>
                  {loan.loan_type.charAt(0).toUpperCase() + loan.loan_type.slice(1)}
                </Text>
              </View>
            </View>
            <View style={styles.loanBalance}>
              <Text style={styles.balanceValue}>{formatCurrency(loan.current_balance)}</Text>
          ...
          <View style={styles.loanDetails}>
            ...
            <View style={styles.detailItem}>
              ...
              <Text style={[styles.detailValue, { color: loan.status === 'current' ? theme.colors.success : theme.colors.error }]}>
          ...
          <View style={styles.progressContainer}>
            <View style={styles.progressBar}>
              <View style={[styles.progressFill, { width: `${((loan.original_balance - loan.current_balance) / loan.original_balance) * 100}%` }]} />
            </View>
            <Text style={styles.progressText}>{Math.round(((loan.original_balance - loan.current_balance) / loan.original_balance) * 100)}% paid off</Text>
          </View>
        </TouchableOpacity>
      ))}
...
const styles = StyleSheet.create({
  ...
  summaryCard: { backgroundColor: theme.colors.primary, marginHorizontal: theme.spacing.md, borderRadius: theme.borderRadius.lg, padding: theme.spacing.lg, marginBottom: theme.spacing.md },
  summaryLabel: { fontSize: 14, color: 'rgba(255,255,255,0.8)' },
  summaryValue: { fontSize: 36, fontWeight: '700', color: '#FFFFFF', marginVertical: 8 },
  ...
  actionsRow: { flexDirection: 'row', paddingHorizontal: theme.spacing.md, marginBottom: theme.spacing.md },
  actionCard: { flex: 1, backgroundColor: theme.colors.surface, marginHorizontal: 4, padding: theme.spacing.md, borderRadius: theme.borderRadius.md, alignItems: 'center' },
  actionLabel: { fontSize: 12, color: theme.colors.text, marginTop: 6 },
  programBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#DCFCE7', marginHorizontal: theme.spacing.md, borderRadius: theme.borderRadius.lg, padding: theme.spacing.md, marginBottom: theme.spacing.md },
  programIcon: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center', marginRight: theme.spacing.md },
  ...
  loanCard: { backgroundColor: theme.colors.surface, marginHorizontal: theme.spacing.md, borderRadius: theme.borderRadius.lg, padding: theme.spacing.md, marginBottom: theme.spacing.md },
  loanHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: theme.spacing.md },
  loanServicer: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginBottom: 4 },
  ...
  progressBar: { flex: 1, height: 6, backgroundColor: theme.colors.border, borderRadius: 3, marginRight: 8, overflow: 'hidden' },
