...
export default function ReportsScreen() {
  ...
  const fetchDocuments = async () => {
    setDocuments([
      ...
      },
      {
        id: "3",
        user_id: "1",
        name: "TransUnion_Report_Sep2024.pdf",
        type: "credit_report",
        file_url: "",
        file_size: 2123456,
        status: "analyzed",
        uploaded_at: "2024-09-15T10:00:00Z",
        analysis_result: {
          bureau: "TransUnion",
          score: 682,
          accounts_count: 13,
  ...
  const getBureauColor = (bureau?: string) => {
    ...
    return (
      colors.bureaus[lower as keyof typeof colors.bureaus] ||
    ...
    >
      <View style={styles.docIcon}>
        <Ionicons
          name="document-text"
          size={32}
          color={getBureauColor(item.analysis_result?.bureau)}
        />
      </View>
          ...
          <View style={styles.analysisRow}>
            <View
              style={[
                styles.scoreBadge,
                {
                  backgroundColor: `${getBureauColor(item.analysis_result.bureau)}20`,
                },
              ]}
            >
              <Text
            ...
            <Text style={styles.itemsText}>
              {item.analysis_result.disputable_items} disputable items
            </Text>
          </View>
        )}
      </View>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push("/reports/upload" as never)}
        >
        ...
        <View style={styles.bureauRow}>
          {["Experian", "Equifax", "TransUnion"].map((bureau, i) => {
            const doc = documents.find(
              (d) => d.analysis_result?.bureau === bureau,
            );
            return (
              <View key={i} style={styles.bureauItem}>
                <Text
                  style={[
                    styles.bureauName,
                    {
                      color:
                        colors.bureaus[
                          bureau.toLowerCase() as keyof typeof colors.bureaus
                        ],
                    },
                  ]}
                >
                ...
                <Text style={styles.bureauScore}>
                  {doc?.analysis_result?.score || "---"}
                </Text>
      ...
      >
        ...
        <View style={styles.uploadPromptText}>
          ...
          <Text style={styles.uploadPromptDesc}>
            Get AI-powered analysis of your credit report
          </Text>
        </View>
      </TouchableOpacity>
