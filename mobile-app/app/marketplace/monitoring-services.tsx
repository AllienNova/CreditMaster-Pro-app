      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.infoCard}>
          ...
          <View style={styles.infoContent}>
            ...
            <Text style={styles.infoText}>Stay informed about changes to your credit report and catch identity theft early.</Text>
          </View>
        </Card>

        {/* Services List */}
        {SERVICES.map((service) => (
          <Card key={service.id} style={[styles.serviceCard, service.recommended && styles.recommendedCard]}>
            {service.recommended && (
            ...
            <View style={styles.serviceHeader}>
              ...
              <View style={styles.bureauBadge}>
                <Text style={styles.bureauText}>{service.bureaus} Bureau{service.bureaus > 1 ? 's' : ''}</Text>
              </View>
            </View>
            ...
            <View style={styles.serviceFooter}>
              ...
              <TouchableOpacity style={[styles.learnButton, service.recommended && styles.learnButtonPrimary]} onPress={() => handleLearnMore(service)}>
                <Text style={[styles.learnButtonText, service.recommended && styles.learnButtonTextPrimary]}>{service.recommended ? 'Get Started' : 'Learn More'}</Text>
              </TouchableOpacity>
            </View>
          </Card>
        ))}
        ...
        <Card style={styles.comparisonCard}>
          ...
          {[
            ...
            <View key={idx} style={styles.comparisonRow}>
              <Text style={styles.comparisonLabel}>{feature}</Text>
              <Ionicons name={cpfi === true ? 'checkmark-circle' : 'close-circle'} size={18} color={cpfi === true ? '#22C55E' : '#EF4444'} />
              <Text style={styles.comparisonOther}>{others === true ? '✓' : others === false ? '✗' : others}</Text>
            </View>
          ))}
        </Card>
...
const styles = StyleSheet.create({
  ...
  recommendedBadge: { position: 'absolute', top: -10, right: 12, backgroundColor: theme.colors.primary, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  ...
  bureauBadge: { backgroundColor: `${theme.colors.primary}15`, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  ...
  serviceFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: theme.spacing.sm, borderTopWidth: 1, borderTopColor: theme.colors.border },
  ratingRow: { flexDirection: 'row', alignItems: 'center' },
  ratingText: { fontSize: 14, fontWeight: '600', color: theme.colors.text, marginLeft: 4 },
  learnButton: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: theme.colors.primary },
  ...
  comparisonCard: {},
  comparisonRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  comparisonLabel: { flex: 1, fontSize: 13, color: theme.colors.text },
  comparisonValue: { width: 60, fontSize: 12, fontWeight: '600', color: theme.colors.text, textAlign: 'center' },
  comparisonOther: { width: 60, fontSize: 12, color: theme.colors.textSecondary, textAlign: 'center' },
});
