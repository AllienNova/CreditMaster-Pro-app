...
const CALCULATOR_OPTIONS: CalculatorOption[] = [
  { id: 'loan', name: 'Loan Payment', icon: 'cash', description: 'Calculate monthly loan payments' },
  { id: 'debt', name: 'Debt Payoff', icon: 'trending-down', description: 'Plan your debt payoff timeline' },
  { id: 'utilization', name: 'Utilization', icon: 'pie-chart', description: 'Optimize credit utilization' },
  { id: 'mortgage', name: 'Mortgage', icon: 'home', description: 'Estimate mortgage payments' },
  { id: 'savings', name: 'Savings Goal', icon: 'wallet', description: 'Plan your savings timeline' },
];
  ...
  return (
    <View style={styles.calculatorContent}>
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Loan Amount: ${principal.toLocaleString()}</Text>
        <Slider
          style={styles.slider}
          minimumValue={1000}
          maximumValue={100000}
      ...
      <View style={styles.resultsGrid}>
        <View style={styles.resultItem}>
          ...
          <Text style={[styles.resultValue, { color: theme.colors.primary }]}>${payment.toFixed(2)}</Text>
        </View>
        <View style={styles.resultItem}>
          <Text style={styles.resultLabel}>Total Interest</Text>
          <Text style={[styles.resultValue, { color: '#EF4444' }]}>${totalInterest.toFixed(2)}</Text>
        </View>
        <View style={styles.resultItem}>
...
function DebtPayoffCalculator() {
  ...
  return (
    <View style={styles.calculatorContent}>
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Current Balance: ${balance.toLocaleString()}</Text>
        <Slider
          style={styles.slider}
          minimumValue={500}
          maximumValue={50000}
      ...
      <View style={styles.resultsRow}>
        <View style={[styles.resultItem, { flex: 1 }]}>
          ...
          <Text style={[styles.resultValue, { color: '#22C55E' }]}>{years}y {remainingMonths}m</Text>
        </View>
        <View style={[styles.resultItem, { flex: 1 }]}>
          <Text style={styles.resultLabel}>Total Interest</Text>
          <Text style={[styles.resultValue, { color: '#EF4444' }]}>${totalInterest.toFixed(2)}</Text>
...
function UtilizationCalculator() {
  ...
  return (
    <View style={styles.calculatorContent}>
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Credit Limit: ${limit.toLocaleString()}</Text>
        <Slider
          style={styles.slider}
          minimumValue={500}
          maximumValue={50000}
      ...
      <View style={styles.utilizationDisplay}>
        <View style={styles.utilizationHeader}>
          ...
          <Text style={[styles.utilizationValue, { color: getUtilizationColor() }]}>
        ...
        <View style={styles.utilizationBar}>
          <View
            style={[
              ...
              { width: `${Math.min(utilization, 100)}%`, backgroundColor: getUtilizationColor() }
            ]}
          />
        </View>
      ...
      <View style={styles.utilizationTips}>
        <View style={styles.tipRow}>
          ...
          <Text style={styles.tipRowText}>For 30% utilization: Keep balance under ${optimal.toFixed(0)}</Text>
        </View>
        <View style={styles.tipRow}>
          <Ionicons name="star" size={16} color="#F59E0B" />
          <Text style={styles.tipRowText}>For 10% utilization: Keep balance under ${excellent.toFixed(0)}</Text>
        </View>
      </View>
    </View>
  );
...
function MortgageCalculator() {
  ...
  return (
    <View style={styles.calculatorContent}>
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Home Price: ${homePrice.toLocaleString()}</Text>
        <Slider
          style={styles.slider}
          minimumValue={100000}
      ...
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Down Payment: {downPayment}% (${downPaymentAmount.toLocaleString()})</Text>
        <Slider
          style={styles.slider}
          minimumValue={3}
          maximumValue={40}
        ...
        <View style={styles.termButtons}>
          {[15, 20, 30].map((t) => (
            <TouchableOpacity
              ...
              onPress={() => setTerm(t)}
            >
              <Text style={[styles.termButtonText, term === t && styles.termButtonTextActive]}>{t}yr</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      <View style={styles.resultsGrid}>
        ...
        <View style={styles.resultItem}>
          <Text style={styles.resultLabel}>Total Interest</Text>
          <Text style={[styles.resultValue, { color: '#EF4444' }]}>${totalInterest.toLocaleString()}</Text>
        </View>
      </View>
    </View>
  );
...
function SavingsCalculator() {
  ...
  return (
    <View style={styles.calculatorContent}>
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Savings Goal: ${goal.toLocaleString()}</Text>
        <Slider
          style={styles.slider}
          minimumValue={1000}
          maximumValue={100000}
      ...
      <View style={styles.inputGroup}>
        <Text style={styles.inputLabel}>Annual Interest Rate: {rate.toFixed(1)}%</Text>
        <Slider
          style={styles.slider}
          minimumValue={0}
          maximumValue={10}
      ...
      <View style={styles.resultsGrid}>
        <View style={styles.resultItem}>
          ...
          <Text style={[styles.resultValue, { color: theme.colors.primary }]}>{years}y {remainingMonths}m</Text>
        </View>
        <View style={styles.resultItem}>
          <Text style={styles.resultLabel}>Total Contributed</Text>
          <Text style={styles.resultValue}>${totalContributed.toLocaleString()}</Text>
        </View>
        <View style={styles.resultItem}>
          <Text style={styles.resultLabel}>Interest Earned</Text>
          <Text style={[styles.resultValue, { color: '#22C55E' }]}>${interestEarned.toFixed(2)}</Text>
        </View>
      </View>
    </View>
  );
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        >
          {CALCULATOR_OPTIONS.map((calc) => (
            <TouchableOpacity
              key={calc.id}
              style={[styles.selectorItem, activeCalculator === calc.id && styles.selectorItemActive]}
              onPress={() => setActiveCalculator(calc.id)}
            >
              <View style={[styles.selectorIcon, activeCalculator === calc.id && styles.selectorIconActive]}>
                <Ionicons
                  name={calc.icon}
                  size={20}
                  color={activeCalculator === calc.id ? '#fff' : theme.colors.primary}
                />
              </View>
              <Text style={[styles.selectorText, activeCalculator === calc.id && styles.selectorTextActive]}>
                {calc.name}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <Card style={styles.calculatorCard}>
          <View style={styles.calculatorHeader}>
            <Ionicons
              name={CALCULATOR_OPTIONS.find(c => c.id === activeCalculator)?.icon || 'calculator'}
              size={24}
              color={theme.colors.primary}
            />
            <View style={styles.calculatorTitleContainer}>
              <Text style={styles.calculatorTitle}>
                {CALCULATOR_OPTIONS.find(c => c.id === activeCalculator)?.name} Calculator
              </Text>
              <Text style={styles.calculatorDescription}>
                {CALCULATOR_OPTIONS.find(c => c.id === activeCalculator)?.description}
              </Text>
            </View>
          </View>
          {renderCalculator()}
        ...
        <Card style={styles.tipCard}>
          ...
          <View style={styles.tipContent}>
            ...
            <Text style={styles.tipText}>
              Keep your credit utilization below 30% for a good score, and below 10% for an excellent score.
            </Text>
          </View>
        </Card>
...
const styles = StyleSheet.create({
  ...
  selectorItem: { alignItems: 'center', marginRight: theme.spacing.md, width: 80 },
  selectorItemActive: {},
  selectorIcon: {
    width: 48,
    height: 48,
  ...
  selectorText: { fontSize: 11, fontWeight: '500', color: theme.colors.textSecondary, textAlign: 'center' },
  selectorTextActive: { color: theme.colors.primary, fontWeight: '600' },

  // Calculator Card
  calculatorCard: { marginBottom: theme.spacing.lg },
  calculatorHeader: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: theme.spacing.md, paddingBottom: theme.spacing.md, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  calculatorTitleContainer: { marginLeft: 12, flex: 1 },
  calculatorTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text },
  calculatorDescription: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
  calculatorContent: {},

  // Input Groups
  inputGroup: { marginBottom: theme.spacing.md },
  inputLabel: { fontSize: 13, fontWeight: '500', color: theme.colors.text, marginBottom: 8 },
  slider: { width: '100%', height: 40 },

  // Results
  resultsGrid: { flexDirection: 'row', justifyContent: 'space-between', backgroundColor: `${theme.colors.primary}08`, borderRadius: 12, padding: theme.spacing.md, marginTop: theme.spacing.sm },
  resultsRow: { flexDirection: 'row', justifyContent: 'space-between', backgroundColor: `${theme.colors.primary}08`, borderRadius: 12, padding: theme.spacing.md, marginTop: theme.spacing.sm },
  resultItem: { alignItems: 'center' },
  resultLabel: { fontSize: 11, color: theme.colors.textSecondary, marginBottom: 4 },
  ...
  utilizationHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  utilizationLabel: { fontSize: 13, fontWeight: '500', color: theme.colors.text },
  utilizationValue: { fontSize: 14, fontWeight: '700' },
  utilizationBar: { height: 12, backgroundColor: theme.colors.border, borderRadius: 6, overflow: 'hidden' },
  utilizationFill: { height: '100%', borderRadius: 6 },
  utilizationMarkers: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 },
  markerText: { fontSize: 10, color: theme.colors.textSecondary },
  utilizationTips: { marginTop: theme.spacing.md, backgroundColor: `${theme.colors.primary}08`, borderRadius: 8, padding: theme.spacing.sm },
  ...
  termButton: { flex: 1, paddingVertical: 10, alignItems: 'center', backgroundColor: theme.colors.surface, borderRadius: 8, marginRight: 8 },
  termButtonActive: { backgroundColor: theme.colors.primary },
  termButtonText: { fontSize: 13, fontWeight: '600', color: theme.colors.textSecondary },
  ...
  // Quick Links
  quickLinksGrid: { flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: -4, marginBottom: theme.spacing.lg },
  quickLinkCard: {
    width: '48%',
  ...
  quickLinkText: { fontSize: 13, fontWeight: '500', color: theme.colors.text, marginLeft: 10 },

  // Tip Card
  tipCard: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#FEF3C7' },
