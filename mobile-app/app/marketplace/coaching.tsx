...
const COACHES: Coach[] = [
  { id: '1', name: 'David Miller', title: 'Certified Credit Counselor', specialty: 'Credit Repair', rating: 4.9, sessions: 450, price: '$75/hr', available: true },
  { id: '2', name: 'Lisa Johnson', title: 'Financial Coach', specialty: 'Debt Management', rating: 4.8, sessions: 320, price: '$65/hr', available: true },
  { id: '3', name: 'Robert Kim', title: 'Credit Expert', specialty: 'Score Optimization', rating: 4.9, sessions: 580, price: '$85/hr', available: false },
];

const PACKAGES: Package[] = [
  { id: '1', name: 'Starter', sessions: 1, price: '$75', features: ['1 coaching session', 'Credit report review', 'Action plan'], popular: false },
  { id: '2', name: 'Growth', sessions: 4, price: '$250', features: ['4 coaching sessions', 'Weekly check-ins', 'Dispute assistance', 'Priority support'], popular: true },
  { id: '3', name: 'Transformation', sessions: 8, price: '$450', features: ['8 coaching sessions', 'Bi-weekly calls', 'Full dispute management', 'Score guarantee'], popular: false },
];
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.heroCard}>
          ...
          <Text style={styles.heroText}>Work 1-on-1 with certified credit experts to achieve your financial goals faster.</Text>
        </Card>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.packagesScroll}>
          {PACKAGES.map((pkg) => (
            <TouchableOpacity key={pkg.id} style={[styles.packageCard, pkg.popular && styles.popularPackage]} onPress={() => handleSelectPackage(pkg)}>
              {pkg.popular && <View style={styles.popularBadge}><Text style={styles.popularText}>Most Popular</Text></View>}
              <Text style={styles.packageName}>{pkg.name}</Text>
              <Text style={styles.packagePrice}>{pkg.price}</Text>
              <Text style={styles.packageSessions}>{pkg.sessions} session{pkg.sessions > 1 ? 's' : ''}</Text>
              ...
              <TouchableOpacity style={[styles.selectButton, pkg.popular && styles.selectButtonPrimary]}>
                <Text style={[styles.selectButtonText, pkg.popular && styles.selectButtonTextPrimary]}>Select</Text>
              </TouchableOpacity>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          <Card key={coach.id} style={styles.coachCard}>
            <View style={styles.coachHeader}>
              <View style={styles.avatarCircle}>
                <Text style={styles.avatarText}>{coach.name.split(' ').map(n => n[0]).join('')}</Text>
              </View>
              ...
              <View>
                ...
                <View style={[styles.availabilityBadge, { backgroundColor: coach.available ? '#22C55E15' : '#EF444415' }]}>
                  <Text style={[styles.availabilityText, { color: coach.available ? '#22C55E' : '#EF4444' }]}>{coach.available ? 'Available' : 'Booked'}</Text>
                </View>
              </View>
            </View>
            <View style={styles.specialtyRow}>
              <Ionicons name="ribbon" size={14} color={theme.colors.primary} />
              <Text style={styles.specialtyText}>Specialty: {coach.specialty}</Text>
            </View>
            <TouchableOpacity style={[styles.bookButton, !coach.available && styles.bookButtonDisabled]} onPress={() => handleBookCoach(coach)} disabled={!coach.available}>
              <Text style={styles.bookButtonText}>{coach.available ? 'Book Session' : 'Join Waitlist'}</Text>
            </TouchableOpacity>
          </Card>
        ))}
...
const styles = StyleSheet.create({
  ...
  packageCard: { width: 200, backgroundColor: theme.colors.surface, borderRadius: 12, padding: theme.spacing.md, marginRight: 12 },
  popularPackage: { borderWidth: 2, borderColor: theme.colors.primary },
  popularBadge: { position: 'absolute', top: -10, right: 12, backgroundColor: theme.colors.primary, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 },
  popularText: { fontSize: 10, fontWeight: '600', color: '#fff' },
  packageName: { fontSize: 16, fontWeight: '600', color: theme.colors.text },
  packagePrice: { fontSize: 24, fontWeight: '700', color: theme.colors.primary, marginTop: 4 },
  ...
  featureText: { fontSize: 11, color: theme.colors.textSecondary, marginLeft: 6 },
  selectButton: { marginTop: theme.spacing.md, paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: theme.colors.primary, alignItems: 'center' },
  ...
  coachSessions: { fontSize: 12, color: theme.colors.textSecondary, marginLeft: 4 },
  coachPrice: { fontSize: 16, fontWeight: '700', color: theme.colors.primary, textAlign: 'right' },
