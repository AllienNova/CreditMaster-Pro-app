...
const RECENT_POSTS: RecentPost[] = [
  { id: '0', title: 'Welcome! Read the community guidelines', author: 'Admin', category: 'Announcements', replies: 12, views: 2450, timeAgo: 'Pinned', pinned: true },
  ...
  { id: '4', title: 'Goodwill letter success with Chase!', author: 'DebtFreeJourney', category: 'Success Stories', replies: 38, views: 890, timeAgo: '8h ago' },
  { id: '5', title: 'Understanding FCRA timelines', author: 'CreditExpert', category: 'Education', replies: 15, views: 456, timeAgo: '12h ago' },
];
...
const SUCCESS_STORIES: SuccessStory[] = [
  ...
  { id: '4', author: 'CreditRebuild', scoreBefore: 540, scoreAfter: 710, timeframe: '10 months', summary: 'Settled collections and built positive payment history', likes: 142 },
];
...
export default function CommunityScreen() {
  ...
  const categories = ['all', 'Announcements', 'Success Stories', 'Dispute Strategies', 'Credit Score Help', 'Education'];
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.tabsRow}>
          <TouchableOpacity style={[styles.tab, activeTab === 'discussions' && styles.tabActive]} onPress={() => setActiveTab('discussions')}>
            <Ionicons name="chatbubbles" size={16} color={activeTab === 'discussions' ? theme.colors.primary : theme.colors.textSecondary} style={{ marginRight: 6 }} />
            <Text style={[styles.tabText, activeTab === 'discussions' && styles.tabTextActive]}>Discussions</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.tab, activeTab === 'stories' && styles.tabActive]} onPress={() => setActiveTab('stories')}>
            <Ionicons name="trophy" size={16} color={activeTab === 'stories' ? theme.colors.primary : theme.colors.textSecondary} style={{ marginRight: 6 }} />
            <Text style={[styles.tabText, activeTab === 'stories' && styles.tabTextActive]}>Success Stories</Text>
          </TouchableOpacity>
        </View>
          ...
          <>
            ...
            {filteredPosts.map((post) => (
              <TouchableOpacity key={post.id}>
                <Card style={[styles.postCard, post.pinned && styles.pinnedCard]}>
                  <View style={styles.postHeader}>
                    <View style={[styles.categoryBadge, getCategoryBadgeStyle(post.category)]}>
                      <Text style={[styles.categoryBadgeText, getCategoryTextStyle(post.category)]}>{post.category}</Text>
                    </View>
                    {post.pinned && (
                  ...
                  <Card style={styles.storyCard}>
                    <View style={styles.storyHeader}>
                      <View style={styles.avatarCircle}>
                        <Text style={styles.avatarText}>{story.author.charAt(0)}</Text>
                      </View>
                      <View style={styles.storyAuthorInfo}>
                        <Text style={styles.storyAuthor}>{story.author}</Text>
                        <Text style={styles.storyTimeframe}>{story.timeframe} journey</Text>
                      </View>
                    ...
                    <View style={styles.scoreComparison}>
                      <View style={styles.scoreItem}>
                        ...
                        <Text style={[styles.scoreValue, { color: '#EF4444' }]}>{story.scoreBefore}</Text>
                      </View>
                      ...
                      <View style={styles.scoreItem}>
                        ...
                        <Text style={[styles.scoreValue, { color: '#22C55E' }]}>{story.scoreAfter}</Text>
                      </View>
                    </View>
                    ...
                    <View style={styles.storyFooter}>
                      <View style={styles.likesRow}>
                        ...
                        <Text style={styles.likesText}>{story.likes} found this inspiring</Text>
                      </View>
        ...
        <TouchableOpacity style={styles.newPostButton}>
          ...
          <Text style={styles.newPostText}>{activeTab === 'discussions' ? 'Start a Discussion' : 'Share Your Story'}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, flexDirection: 'row', paddingVertical: 12, alignItems: 'center', justifyContent: 'center', borderBottomWidth: 2, borderBottomColor: 'transparent' },
  ...
  filterText: { fontSize: 12, fontWeight: '500', color: theme.colors.textSecondary },
  ...
  postStats: { flexDirection: 'row', alignItems: 'center', paddingTop: theme.spacing.sm, borderTopWidth: 1, borderTopColor: theme.colors.border },
  ...
  storyHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: theme.spacing.md },
  avatarCircle: { width: 44, height: 44, borderRadius: 22, backgroundColor: theme.colors.primary, justifyContent: 'center', alignItems: 'center' },
  ...
  storyTimeframe: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
  improvementBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#D1FAE5', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  improvementText: { fontSize: 13, fontWeight: '700', color: '#22C55E', marginLeft: 4 },

  scoreComparison: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: theme.spacing.md, backgroundColor: '#fff', borderRadius: 12, marginBottom: theme.spacing.md },
  scoreItem: { alignItems: 'center', flex: 1 },
  scoreLabel: { fontSize: 11, color: theme.colors.textSecondary, marginBottom: 4 },
  scoreValue: { fontSize: 28, fontWeight: '700' },
  scoreArrow: { paddingHorizontal: theme.spacing.md },

  storySummary: { fontSize: 13, color: theme.colors.text, lineHeight: 20, marginBottom: theme.spacing.md },
  storyFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: theme.spacing.sm, borderTopWidth: 1, borderTopColor: '#BBF7D0' },
  likesRow: { flexDirection: 'row', alignItems: 'center' },
  likesText: { fontSize: 12, color: theme.colors.textSecondary, marginLeft: 6 },
  readMoreButton: { paddingHorizontal: 14, paddingVertical: 6, backgroundColor: '#22C55E', borderRadius: 16 },
