...
export default function StudentLoansPortfolio() {
  ...
  const analyzePortfolio = useStudentLoanStore(
    (state) => state.analyzePortfolio,
  );
  ...
  const totalDebt =
    stats?.totalDebt || loans.reduce((sum, l) => sum + l.currentBalance, 0);
  const monthlyPayment =
    stats?.totalMonthlyPayment ||
    loans.reduce((sum, l) => sum + l.monthlyPayment, 0);
  const avgRate =
    stats?.weightedInterestRate ||
    (loans.length > 0
          ...
          {loans.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>
                Add your student loans to track them and get AI-powered
