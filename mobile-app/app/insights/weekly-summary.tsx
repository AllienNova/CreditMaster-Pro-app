...
const MOCK_SUMMARY: WeeklySummary = {
  ...
  spending: {
    ...
    trend: 'down',
    topCategories: [
      { category: 'Groceries', amount: 342.15, percentOfTotal: 27.5, icon: 'cart' },
      { category: 'Dining', amount: 256.8, percentOfTotal: 20.6, icon: 'restaurant' },
      { category: 'Transportation', amount: 189.45, percentOfTotal: 15.2, icon: 'car' },
      { category: 'Shopping', amount: 167.9, percentOfTotal: 13.5, icon: 'bag' },
      { category: 'Entertainment', amount: 98.5, percentOfTotal: 7.9, icon: 'film' },
    ],
  },
  budget: {
    totalBudget: 4000,
  ...
  insights: [
    {
      ...
      description: 'Your credit score went up 8 points this week. Great progress!',
      ...
      >
              ...
              <View style={styles.scoreRow}>
                ...
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        summary.healthScoreChange >= 0 ? 'rgba(255,255,255,0.3)' : '#FEE2E2',
                    },
                  ]}
                >
                  <Ionicons
                    name={summary.healthScoreChange >= 0 ? 'arrow-up' : 'arrow-down'}
                    size={12}
        ...
        <View style={styles.statsGrid}>
          <Card style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{formatCurrency(summary.spending.totalSpent)}</Text>
            <Text style={styles.statLabel}>Spent This Week</Text>
            <View style={styles.statTrend}>
              <Ionicons
                name={summary.spending.trend === 'down' ? 'trending-down' : 'trending-up'}
                size={14}
                color={summary.spending.trend === 'down' ? '#22C55E' : '#EF4444'}
              />
              <Text
                style={[
          ...
          <Card style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{summary.budget.percentUsed.toFixed(0)}%</Text>
            <Text style={styles.statLabel}>Budget Used</Text>
            <Text style={styles.statSubtext}>{summary.budget.daysRemaining} days left</Text>
          </Card>
          ...
          <Card style={styles.statCard}>
            ...
            <View style={styles.statTrend}>
              <Ionicons
                name={summary.credit.scoreChange >= 0 ? 'arrow-up' : 'arrow-down'}
                size={14}
          ...
          <Card style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{formatCurrency(summary.investments.portfolioValue)}</Text>
            <Text style={styles.statLabel}>Portfolio</Text>
            <View style={styles.statTrend}>
              <Ionicons
                name={summary.investments.weeklyChangePercent >= 0 ? 'arrow-up' : 'arrow-down'}
                size={14}
                color={summary.investments.weeklyChangePercent >= 0 ? '#22C55E' : '#EF4444'}
              />
              <Text
                style={[
                  styles.statTrendText,
                  {
                    color:
                      summary.investments.weeklyChangePercent >= 0 ? '#22C55E' : '#EF4444',
                  },
                ]}
              >
        ...
        {summary.insights.map((insight) => (
          <TouchableOpacity
            key={insight.id}
            onPress={() => insight.actionRoute && router.push(insight.actionRoute as never)}
          >
            <Card
              style={[
                styles.insightCard,
            ...
            <View key={cat.category} style={styles.categoryRow}>
              ...
              <View style={styles.categoryContent}>
                ...
                <View style={styles.progressBar}>
                  <View
                    style={[styles.progressFill, { width: `${cat.percentOfTotal}%` }]}
                  />
                </View>
              </View>
            </View>
        ...
        <View style={styles.actionsRow}>
          ...
          >
            ...
            <Text style={styles.actionValue}>{summary.bills.upcomingCount}</Text>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  statSubtext: { fontSize: 11, color: theme.colors.textSecondary, marginTop: 4 },
  ...
  insightDescription: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
  ...
  viewAllText: { fontSize: 14, color: theme.colors.primary, fontWeight: '500', marginRight: 4 },
  ...
  actionLabel: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 8 },
  actionValue: { fontSize: 18, fontWeight: '700', color: theme.colors.text, marginTop: 4 },
});
