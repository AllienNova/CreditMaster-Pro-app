...
const MOCK_ALERTS: Alert[] = [
  {
    ...
    message: 'Unusual transaction pattern detected on your credit card ending in 4532',
  ...
  },
  {
    ...
    data: { category: 'Shopping', currentSpending: 650, average: 350, percentIncrease: 85 },
    status: 'read',
  ...
  },
  {
    ...
    actionRoute: '/financial/bills',
    actionLabel: 'Review',
    data: { potentialSavings: 35, subscriptions: ['Netflix', 'Hulu', 'Disney+'] },
    status: 'pending',
    createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000),
  },
];

const ALERT_TYPE_CONFIG: Record<AlertType, { icon: keyof typeof Ionicons.glyphMap; color: string }> = {
...
export default function SmartAlertsScreen() {
  ...
  const handleMarkAllRead = () => {
    setAlerts((prev) =>
      prev.map((a) => (a.status === 'pending' ? { ...a, status: 'read' as AlertStatus } : a))
    );
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => router.push('/settings/notifications' as never)}>
        ...
        {criticalCount > 0 && (
          <View style={styles.criticalBanner}>
            ...
            <View style={styles.criticalContent}>
              ...
              <Text style={styles.criticalSubtitle}>Requires immediate attention</Text>
            </View>
          </View>
        )}
                ...
                <View style={styles.alertHeader}>
                  ...
                  <View style={styles.alertContent}>
                    ...
                    <View style={styles.alertMeta}>
                      ...
                      >
                        <Text
                          style={[styles.priorityText, { color: getPriorityColor(alert.priority) }]}
                        >
                          {alert.priority}
                        </Text>
                      </View>
                      <Text style={styles.timeAgo}>{formatTimeAgo(alert.createdAt)}</Text>
                      {alert.status === 'pending' && <View style={styles.unreadDot} />}
                    </View>
...
const styles = StyleSheet.create({
  ...
  markAllText: { fontSize: 13, color: theme.colors.primary, marginLeft: 4, fontWeight: '500' },
  emptyState: {
    alignItems: 'center',
  ...
  alertCardPending: { borderLeftWidth: 3, borderLeftColor: theme.colors.primary },
  ...
  alertTitle: { fontSize: 15, fontWeight: '600', color: theme.colors.text, flex: 1, marginRight: 8 },
  alertMeta: { flexDirection: 'row', alignItems: 'center', marginTop: 4, marginBottom: 8 },
  ...
  alertMessage: { fontSize: 13, color: theme.colors.textSecondary, lineHeight: 19 },
  alertActions: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: theme.spacing.md,
  ...
  actionButtonText: { fontSize: 14, fontWeight: '600', color: '#fff', marginRight: 4 },
