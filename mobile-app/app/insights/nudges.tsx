...
const typeConfig: Record<NudgeType, { icon: keyof typeof Ionicons.glyphMap; color: string; label: string }> = {
...
export default function NudgesScreen() {
  const { nudges, activeNudge, respondToNudge, fetchNudges, isLoading } = useNudges();
      ...
      >
        ...
        <Card style={styles.statsCard}>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={styles.statItem}>
              <Text style={[styles.statValue, { color: '#22C55E' }]}>{stats.acceptedCount}</Text>
              <Text style={styles.statLabel}>Accepted</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, { color: '#3B82F6' }]}>{stats.acceptanceRate}%</Text>
              <Text style={styles.statLabel}>Rate</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, { color: '#8B5CF6' }]}>${stats.monthlySavings}</Text>
            ...
            {nudges.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySubtitle}>
                  No active recommendations right now. Check back later for personalized tips.
                </Text>
              </View>
            ) : (
              nudges.map((nudge) => {
                ...
                return (
                  <Card key={nudge.id} style={styles.nudgeCard}>
                    <View style={styles.nudgeHeader}>
                      <View style={[styles.nudgeIcon, { backgroundColor: `${config.color}15` }]}>
                      ...
                      {nudge.actionLabel && (
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            ...
                            nudge.actionRoute && router.push(nudge.actionRoute as never);
                          }}
                        >
                    ...
                    <View style={styles.historyContent}>
                      <View style={styles.historyTitleRow}>
                        ...
                        <View style={[styles.responseBadge, { backgroundColor: badge.bg }]}>
                          <Text style={[styles.responseText, { color: badge.text }]}>
                            {badge.label}
                          </Text>
                        </View>
                      </View>
                      ...
                      <Text style={styles.historyTime}>{formatTimeAgo(item.respondedAt)}</Text>
                    </View>
                  </View>
                </Card>
              );
        ...
        {activeTab === 'settings' && (
          <View>
            ...
            <Card style={styles.settingsCard}>
              ...
              <TouchableOpacity style={[styles.frequencyOption, { borderBottomWidth: 0 }]}>
...
const styles = StyleSheet.create({
  ...
  impactText: { fontSize: 13, fontWeight: '600', color: '#D97706', marginLeft: 4 },
  ...
  emptyTitle: { fontSize: 20, fontWeight: '600', color: theme.colors.text, marginTop: theme.spacing.md },
  emptySubtitle: {
    fontSize: 14,
  ...
  nudgeTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginBottom: 4 },
  nudgeMessage: { fontSize: 14, color: theme.colors.textSecondary, lineHeight: 20 },
  nudgeActions: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  historyTitle: { fontSize: 14, fontWeight: '600', color: theme.colors.text, flex: 1 },
  responseBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 },
  responseText: { fontSize: 10, fontWeight: '600' },
  historyMessage: { fontSize: 13, color: theme.colors.textSecondary, marginBottom: 6 },
  actionTakenRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 },
  actionTakenText: { fontSize: 12, color: '#22C55E', marginLeft: 4, fontWeight: '500' },
  ...
  settingRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 8 },
  settingRowBorder: { borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  settingIcon: {
    width: 40,
    height: 40,
    borderRadius: 20,
  ...
  frequencyNumber: { fontSize: 15, color: theme.colors.textSecondary, marginRight: 4 },
});
