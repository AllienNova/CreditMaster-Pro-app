...
const MOCK_CARDS: CreditCard[] = [
  { id: '1', name: 'Chase Freedom', balance: 1500, limit: 5000, utilization: 30 },
  { id: '2', name: 'Capital One Quicksilver', balance: 800, limit: 3000, utilization: 27 },
  { id: '3', name: 'Discover It', balance: 200, limit: 2000, utilization: 10 },
  { id: '4', name: 'Citi Double Cash', balance: 2500, limit: 8000, utilization: 31 },
];
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.overallCard}>
          <View style={styles.utilizationCircle}>
            <Text style={[styles.utilizationPercent, { color: getUtilizationColor(overallUtilization) }]}>{overallUtilization}%</Text>
            <Text style={styles.utilizationLabel}>{getUtilizationLabel(overallUtilization)}</Text>
          </View>
          <View style={styles.utilizationBar}>
            <View style={[styles.utilizationFill, { width: `${Math.min(overallUtilization, 100)}%`, backgroundColor: getUtilizationColor(overallUtilization) }]} />
        ...
        {amountToPayOff > 0 && (
          <Card style={styles.recommendationCard}>
            ...
            <View style={styles.recommendationContent}>
              <Text style={styles.recommendationTitle}>Pay off ${amountToPayOff.toLocaleString()}</Text>
              <Text style={styles.recommendationText}>To reach the ideal 10% utilization, pay down your balances by ${amountToPayOff.toLocaleString()}</Text>
            </View>
          </Card>
        )}
          ...
          <Card key={card.id} style={styles.cardItem}>
            <View style={styles.cardHeader}>
              ...
              <View style={styles.cardInfo}>
                ...
                <Text style={styles.cardBalance}>${card.balance.toLocaleString()} / ${card.limit.toLocaleString()}</Text>
              </View>
              <View style={[styles.utilizationBadge, { backgroundColor: `${getUtilizationColor(card.utilization)}20` }]}>
                <Text style={[styles.utilizationBadgeText, { color: getUtilizationColor(card.utilization) }]}>{card.utilization}%</Text>
              </View>
            </View>
            <View style={styles.cardBar}>
              <View style={[styles.cardBarFill, { width: `${card.utilization}%`, backgroundColor: getUtilizationColor(card.utilization) }]} />
            </View>
          </Card>
        ))}
        ...
        <Card style={styles.tipsCard}>
          ...
          <View style={styles.tipItem}>
            ...
            <Text style={styles.tipText}>Pay balances before statement closes</Text>
          </View>
          <View style={styles.tipItem}>
            <Ionicons name="checkmark-circle" size={18} color="#22C55E" />
          ...
          <View style={styles.tipItem}>
            <Ionicons name="checkmark-circle" size={18} color="#22C55E" />
            <Text style={styles.tipText}>Spread spending across multiple cards</Text>
          </View>
          <View style={styles.tipItem}>
            <Ionicons name="checkmark-circle" size={18} color="#22C55E" />
            <Text style={styles.tipText}>Keep old cards open (even if unused)</Text>
          </View>
        </Card>
...
const styles = StyleSheet.create({
  ...
  utilizationBar: { width: '100%', height: 12, backgroundColor: theme.colors.border, borderRadius: 6, position: 'relative' },
  utilizationFill: { height: '100%', borderRadius: 6 },
  idealMarker: { position: 'absolute', left: '10%', top: -4, width: 2, height: 20, backgroundColor: '#22C55E' },
  utilizationLegend: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', marginTop: 8 },
  ...
  recommendationCard: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: theme.spacing.lg, backgroundColor: '#FEF3C720' },
  recommendationIcon: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#FEF3C7', justifyContent: 'center', alignItems: 'center', marginRight: theme.spacing.md },
  recommendationContent: { flex: 1 },
  recommendationTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text },
  recommendationText: { fontSize: 13, color: theme.colors.textSecondary, marginTop: 4 },
  ...
  cardIcon: { width: 36, height: 36, borderRadius: 18, backgroundColor: `${theme.colors.primary}15`, justifyContent: 'center', alignItems: 'center', marginRight: theme.spacing.sm },
  cardInfo: { flex: 1 },
  cardName: { fontSize: 15, fontWeight: '600', color: theme.colors.text },
  cardBalance: { fontSize: 13, color: theme.colors.textSecondary },
  utilizationBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
