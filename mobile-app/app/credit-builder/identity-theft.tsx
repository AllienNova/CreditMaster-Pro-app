...
export default function IdentityTheftRecoveryScreen() {
  const [recoverySteps, setRecoverySteps] = useState<RecoveryStep[]>(INITIAL_RECOVERY_STEPS);
  ...
  const addFraudulentAccount = () => {
    ...
    setNewAccount({ creditor: '', accountNumber: '', amountOwed: '', type: 'credit_card' });
  ...
  const renderStepsByCategory = (category: RecoveryStep['category'], title: string, icon: string) => {
    ...
    return (
      <View style={styles.categorySection}>
        <View style={styles.categoryHeader}>
          ...
          <Text style={[styles.categoryPercentage, { color }]}>{progress.percentage}%</Text>
        </View>
        {steps.map((step) => (
          <TouchableOpacity
            key={step.id}
            ...
            activeOpacity={0.7}
          >
            <Card style={[styles.stepCard, step.completed && styles.stepCardCompleted]}>
        ...
        <Card style={styles.emergencyCard}>
          <View style={styles.emergencyHeader}>
            ...
            <View style={styles.emergencyContent}>
              ...
              <Text style={styles.emergencySubtitle}>The first 24-48 hours are critical</Text>
            </View>
          </View>
          ...
          <Card style={styles.progressCard}>
            ...
            <View style={styles.progressRow}>
              ...
              <View style={styles.progressDetails}>
                ...
                <View style={styles.progressBarContainer}>
                  <View
                    style={[styles.progressBarFill, { width: `${overallProgress.percentage}%` }]}
                  />
                </View>
              </View>
            </View>
            ...
            <View style={styles.categoryProgressGrid}>
              <View style={styles.categoryProgressItem}>
                ...
                <Text style={styles.categoryProgressValue}>{immediateProgress.percentage}%</Text>
              </View>
              <View style={styles.categoryProgressItem}>
                <View style={[styles.categoryDot, { backgroundColor: '#F59E0B' }]} />
                <Text style={styles.categoryProgressLabel}>Short-term</Text>
                <Text style={styles.categoryProgressValue}>{shortTermProgress.percentage}%</Text>
              </View>
              <View style={styles.categoryProgressItem}>
                <View style={[styles.categoryDot, { backgroundColor: '#FBBF24' }]} />
                <Text style={styles.categoryProgressLabel}>Long-term</Text>
                <Text style={styles.categoryProgressValue}>{longTermProgress.percentage}%</Text>
              </View>
              <View style={styles.categoryProgressItem}>
                <View style={[styles.categoryDot, { backgroundColor: '#3B82F6' }]} />
                <Text style={styles.categoryProgressLabel}>Ongoing</Text>
                <Text style={styles.categoryProgressValue}>{ongoingProgress.percentage}%</Text>
              </View>
            </View>
          </Card>
        )}
        ...
        {!isRecoveryMode ? (
          <>
            <Card style={styles.infoCard}>
              ...
              <Text style={styles.infoTitle}>Identity Theft Recovery Center</Text>
              <Text style={styles.infoText}>
                If you've been a victim of identity theft, we'll guide you through the recovery
                process step by step with a comprehensive action plan.
              </Text>
              <TouchableOpacity style={styles.startButton} onPress={startRecovery}>
                <Text style={styles.startButtonText}>Start Recovery Process</Text>
            ...
            {IMPORTANT_CONTACTS.slice(0, 4).map((contact, idx) => (
              <TouchableOpacity key={idx} onPress={() => Linking.openURL(`tel:${contact.phone}`)}>
                <Card style={styles.contactCard}>
                  <View style={styles.contactRow}>
                    ...
                    <View style={styles.contactInfo}>
                      <Text style={styles.contactName}>{contact.organization}</Text>
          ...
          <>
            ...
            {renderStepsByCategory('immediate', 'Immediate Actions (24 Hours)', 'alert-circle')}
            {renderStepsByCategory('short_term', 'Short-term Actions (1 Week)', 'flash')}
            {renderStepsByCategory('long_term', 'Long-term Actions (1 Month)', 'calendar')}
            ...
            <View style={styles.accountsSection}>
              ...
              {fraudulentAccounts.length === 0 ? (
                <Card style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptyText}>No fraudulent accounts tracked yet</Text>
            ...
            {IMPORTANT_CONTACTS.map((contact, idx) => (
              <Card key={idx} style={styles.contactCardFull}>
                <Text style={styles.contactNameFull}>{contact.organization}</Text>
            ...
            <ScrollView style={styles.modalScroll}>
              ...
              <View style={styles.modalMeta}>
                <View style={styles.modalMetaItem}>
                  ...
                  <Text style={styles.modalMetaValue}>{selectedStep?.estimatedTime}</Text>
                </View>
                <View style={styles.modalMetaItem}>
                  ...
                  <Text
                    style={[
                      ...
                      { color: getPriorityColor(selectedStep?.priority || 'low') },
                    ]}
                  >
              ...
              {selectedStep?.substeps && selectedStep.substeps.length > 0 && (
                <View style={styles.substepsSection}>
                  <Text style={styles.substepsTitle}>Step-by-Step Instructions</Text>
              ...
              <TouchableOpacity
                style={[styles.markCompleteButton, selectedStep?.completed && styles.markIncompleteButton]}
                onPress={() => {
                  if (selectedStep) {
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setNewAccount((prev) => ({ ...prev, creditor: text }))}
            ...
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setNewAccount((prev) => ({ ...prev, amountOwed: text }))}
                placeholder="0.00"
                placeholderTextColor={theme.colors.textSecondary}
                keyboardType="numeric"
              />
            </View>
            <TouchableOpacity style={styles.addAccountButton} onPress={addFraudulentAccount}>
...
const styles = StyleSheet.create({
  ...
  progressTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginBottom: 12 },
  ...
  progressPercentage: { fontSize: 20, fontWeight: '700', color: theme.colors.primary },
  progressDetails: { flex: 1 },
  progressSteps: { fontSize: 14, color: theme.colors.textSecondary, marginBottom: 8 },
  ...
  categoryProgressValue: { fontSize: 14, fontWeight: '600', color: theme.colors.text },
  infoCard: { alignItems: 'center', marginBottom: theme.spacing.lg, paddingVertical: 24 },
  infoTitle: {
    fontSize: 20,
    fontWeight: '700',
  ...
  stepCardCompleted: { backgroundColor: '#22C55E10', borderColor: '#22C55E', borderWidth: 1 },
  ...
  stepTitleCompleted: { textDecorationLine: 'line-through', color: theme.colors.textSecondary },
  ...
  priorityBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, marginLeft: 8 },
  ...
  contactNameFull: { fontSize: 15, fontWeight: '600', color: theme.colors.text },
  contactPurpose: { fontSize: 13, color: theme.colors.textSecondary, marginTop: 2 },
  contactActions: {
    flexDirection: 'row',
    marginTop: theme.spacing.sm,
    gap: 16,
  ...
  contactActionText: { fontSize: 13, color: theme.colors.primary, marginLeft: 6 },
  contactHours: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 8 },
  ...
  emptySubtext: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 4 },
  accountCard: { marginBottom: theme.spacing.sm },
  accountRow: { flexDirection: 'row', alignItems: 'center' },
  accountInfo: { flex: 1 },
  accountCreditor: { fontSize: 15, fontWeight: '600', color: theme.colors.text },
  ...
  modalDescription: { fontSize: 15, color: theme.colors.textSecondary, lineHeight: 22 },
  modalMeta: {
    flexDirection: 'row',
  ...
  modalMetaValue: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginTop: 4 },
  substepsSection: { marginTop: theme.spacing.lg },
  substepsTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginBottom: 12 },
  substepRow: { flexDirection: 'row', marginBottom: 8 },
  substepNumber: { fontSize: 14, fontWeight: '600', color: theme.colors.textSecondary, width: 24 },
  substepText: { fontSize: 14, color: theme.colors.text, flex: 1 },
  resourcesSection: { marginTop: theme.spacing.lg },
  resourcesTitle: { fontSize: 16, fontWeight: '600', color: theme.colors.text, marginBottom: 12 },
  ...
  inputLabel: { fontSize: 14, color: theme.colors.textSecondary, marginBottom: 6 },
  input: {
