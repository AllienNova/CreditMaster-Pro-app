...
const RECOMMENDED_ACTIONS = [
  { icon: 'trending-down', title: 'Lower utilization to 10%', impact: '+15-25 pts', priority: 'high' },
  { icon: 'time', title: 'Pay all bills on time', impact: '+5-10 pts/mo', priority: 'high' },
  { icon: 'close-circle', title: 'Remove collections account', impact: '+30-50 pts', priority: 'medium' },
  { icon: 'add-circle', title: 'Become authorized user', impact: '+10-20 pts', priority: 'medium' },
];
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.scoreCard}>
          <View style={styles.scoreRow}>
            ...
            <ProgressRing progress={(currentScore - 300) / 550} size={80} strokeWidth={8} color={getScoreColor(currentScore)} />
          </View>
        </Card>
        ...
        {goals.map((goal) => {
          const progress = ((goal.currentScore - 300) / (goal.targetScore - 300)) * 100;
          ...
          return (
            <Card key={goal.id} style={styles.goalCard}>
              <View style={styles.goalHeader}>
                ...
                <View style={[styles.statusBadge, goal.status === 'on_track' ? styles.onTrack : styles.atRisk]}>
                  <Text style={styles.statusText}>{goal.status === 'on_track' ? 'On Track' : 'At Risk'}</Text>
                </View>
              </View>
              ...
              <View style={styles.goalStats}>
                <View style={styles.goalStat}>
                  ...
                  <Text style={styles.goalStatText}>{pointsNeeded} pts needed</Text>
                </View>
                <View style={styles.goalStat}>
                  <Ionicons name="calendar" size={16} color={theme.colors.textSecondary} />
                  <Text style={styles.goalStatText}>{daysRemaining} days left</Text>
                </View>
              </View>
                ...
                {goal.milestones.map((milestone, idx) => (
                  <View key={idx} style={styles.milestone}>
                    <Ionicons
                      ...
                      size={16}
                      color={milestone.achieved ? theme.colors.success : theme.colors.border}
                    />
                    <Text style={[styles.milestoneText, milestone.achieved && styles.milestoneAchieved]}>
          ...
          <TouchableOpacity key={idx} activeOpacity={0.7}>
            <Card style={styles.actionCard}>
              <View style={styles.actionRow}>
                <View style={[styles.actionIcon, action.priority === 'high' ? styles.highPriority : styles.mediumPriority]}>
                  <Ionicons
...
const styles = StyleSheet.create({
  ...
  trendStats: { flexDirection: 'row', marginTop: theme.spacing.md, paddingTop: theme.spacing.md, borderTopWidth: 1, borderTopColor: theme.colors.border },
  ...
  progressContainer: { flexDirection: 'row', alignItems: 'center', marginTop: theme.spacing.md },
  progressBar: { flex: 1, height: 8, backgroundColor: theme.colors.border, borderRadius: 4, overflow: 'hidden' },
  progressFill: { height: '100%', backgroundColor: theme.colors.primary, borderRadius: 4 },
  progressText: { marginLeft: 12, fontSize: 14, fontWeight: '600', color: theme.colors.text },
  goalStats: { flexDirection: 'row', marginTop: theme.spacing.md, gap: 16 },
  goalStat: { flexDirection: 'row', alignItems: 'center' },
  goalStatText: { marginLeft: 6, fontSize: 13, color: theme.colors.textSecondary },
  milestones: { marginTop: theme.spacing.md, paddingTop: theme.spacing.md, borderTopWidth: 1, borderTopColor: theme.colors.border },
  milestone: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 },
  milestoneText: { marginLeft: 8, fontSize: 13, color: theme.colors.textSecondary },
  milestoneAchieved: { color: theme.colors.success, textDecorationLine: 'line-through' },
  ...
  addGoalButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16, borderWidth: 2, borderColor: theme.colors.primary, borderStyle: 'dashed', borderRadius: 12, marginTop: theme.spacing.md },
  addGoalText: { marginLeft: 8, fontSize: 16, fontWeight: '600', color: theme.colors.primary },
});
