...
test.describe("Student Loan Agent Page", () => {
  ...
  test("should have loan calculator section", async ({ page }) => {
    ...
    const inputs = page.locator(
      'input[type="number"], input[inputmode="numeric"], [class*="calculator"]',
    );
  ...
  test("should display program options", async ({ page }) => {
    ...
    const programs = page.locator(
      "text=/IDR|PAYE|SAVE|IBR|PSLF|Income.*Driven/i",
    );
...
test.describe("Student Loan Calculations", () => {
  test("should calculate loan payments", async ({ page }) => {
    ...
    const loanInput = page.locator(
      'input[name*="loan"], input[name*="balance"], input[placeholder*="loan"]',
    );
    ...
    if ((await loanInput.count()) > 0) {
      ...
      const rateInput = page.locator(
        'input[name*="rate"], input[name*="interest"]',
      );
      ...
      const calculateBtn = page.getByRole("button", {
        name: /calculate|estimate|show/i,
      });
