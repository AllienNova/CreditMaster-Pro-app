  ...
  await page.route("**/api/chat/financial/sessions**", async (route) => {
    ...
    if (pathname.endsWith("/messages")) {
      const match = pathname.match(
        /\/api\/chat\/financial\/sessions\/([^/]+)\/messages/,
      );
    ...
    if (sessionMatch) {
      ...
      if (method === "GET") {
        return route.fulfill({
          ...
          contentType: "application/json",
          body: JSON.stringify({
            session: { id: sessionId, userId: "e2e-user" },
          }),
        });
      }
      if (method === "DELETE") {
...
test.describe("Chat Flows E2E Tests", () => {
  ...
  test("Chat Session Management", async ({ page }) => {
    ...
    await targetSession
      .locator('[data-testid="delete-session-button"]')
      .click();
    await targetSession
      .locator('[data-testid="delete-session-button"]')
      .click();
  ...
  test("Intent Detection and Suggested Actions", async ({ page }) => {
    ...
    await page.fill(
      ...
      "I want to analyze my investments",
    );
    await page.click('[data-testid="send-button"]');
    ...
    const firstAction = page
      .locator('[data-testid="suggested-action"]')
      .first();
  ...
  test("Mobile Chat Interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="mobile-chat-header"]'),
    ).toBeVisible();
  });
  ...
  test("Character Limit Validation", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="char-counter-error"]'),
    ).toBeVisible();
