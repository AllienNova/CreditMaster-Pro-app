...
test.describe("Asset Allocation Demo", () => {
  ...
  test.skip("should load the demo page successfully", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Asset Allocation Analyzer/i }),
    ).toBeVisible();
  ...
  test.skip("should display risk tolerance selector with all 5 levels", async ({
    page,
  }) => {
  ...
  test.skip("should analyze portfolio and display results", async ({
    page,
  }) => {
    // Skip - requires authentication
  ...
  test.skip("should display rebalancing recommendations", async ({ page }) => {
    ...
    const rebalanceButton = page.getByRole("button", {
      name: /Execute Rebalancing/i,
    });
  ...
  test("should require authentication to access demo page", async ({
    page,
  }) => {
