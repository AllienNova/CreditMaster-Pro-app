...
|-------------|-----------|--------|-------|
| Disputes — general (`api/disputes/`) | `disputes/dispute-service-db.ts` (real `disputes` table) | **WORKING** (verified closed — CRD-3) | All 5 routes (route, [id], [id]/send, stats, generate) re-wired to the DB service; every query `.eq("user_id", userId)`-scoped; cross-user IDOR tests. The in-memory `dispute-service.ts` is no longer imported by any production route. |
...
| Documents (`api/documents/`) | `documents/document-service-db.ts` (real `documents` + `document_share_links` tables) | **WORKING** (verified closed — CRD-4) | GET/PATCH/DELETE/upload/share all re-wired to the DB service; share links persisted via the `20260517000004_document_shares.sql` migration; `confirmUpload` user-scoped; cross-user IDOR tests. The in-memory `document-service.ts` is no longer imported by any production route. |
...
| Marketplace tradelines (`api/marketplace/tradelines/`) | `tradeline-service` → `supabase.from("tradelines")` | **DEGRADED — no auth guard (tracked, out of Credit-vertical scope)** | Real DB table, but `GET /tradelines` and `GET /tradelines/[id]` use bare `export async function GET()` — no auth wrapper (an AUTH-03 gap). Explicitly out of the Credit vertical's scope per the plan; tracked as a separate follow-up task (#46) for a dedicated fix + negative-auth test. Not a Credit-vertical regression. |
