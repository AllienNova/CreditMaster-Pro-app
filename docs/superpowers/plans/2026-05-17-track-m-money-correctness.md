...
- [ ] **Step 5: Fix** — `revenue-tracker.ts` writes through to `revenue_events` and reads from it; delete the in-memory array. **Unit-change lockstep:** the in-memory tracker currently holds `commissionAmount` in dollars (test fixture uses `commissionAmount: 50` meaning $50); the table column is `commission_amount_cents`. Convert dollars→cents on write (`Math.round(commissionAmount * 100)`) and cents→dollars on read so `getReport`/`getTopProducts` keep returning the dollar values their callers and the existing `revenue-tracker.test.ts` aggregation assertions expect. Existing dollar-total assertions must still pass; touch a test assertion ONLY if its unit genuinely changed, and document why in the commit (Test Integrity Rule).
...
- [ ] **Step 3: Create the atomic RPC migration** — `increment_referral_use(p_code text, p_user_id uuid)`: a single atomic statement (`UPDATE referral_codes SET uses_count = uses_count + 1 WHERE code = p_code AND is_active AND uses_count < max_uses AND user_id <> p_user_id AND (expires_at IS NULL OR expires_at > now()) RETURNING ...`), returning a typed result the caller maps to `applied` / `cap_reached` / `invalid` / `self_referral`. The `user_id <> p_user_id` clause enforces the self-referral guard IN the RPC (defense in depth). Follow the `d64e8d5` template: `REVOKE EXECUTE … FROM PUBLIC; GRANT EXECUTE … TO service_role`.
- [ ] **Step 4: Write the failing tests** — self-referral (a user's own code) → rejected; a concurrency test — N parallel `applyReferralCode` on a code with `max_uses = 1` → exactly ONE succeeds, `uses_count` ends at 1 not N.
- [ ] **Step 5: Run — expect FAIL.**
- [ ] **Step 6: Fix** — `applyReferralCode` calls `increment_referral_use` instead of read-modify-write; add an explicit fast-fail self-referral guard at the service layer too; map the RPC's typed result to the service response.
...
- [ ] **Step 3: Run — expect FAIL.**
- [ ] **Step 4: Fix** — the webhook ignores any inbound `commission`; it maps `eventType` → `ConversionType`, calls `calculateCommission(partnerId, conversionType, data.amount)`, and persists THAT. (The webhook signature verification from WBH-05 stays — a valid signature proves origin, not that the body's numbers are right.)
- [ ] **Step 5: Run — expect PASS.** Full suite 0 failures; `npm run type-check` 0 errors.
