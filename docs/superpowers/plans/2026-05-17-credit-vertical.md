...
- **Discovery (verified at HEAD — corrects the roadmap's "verify-pass only" billing):**
  - **The dispute domain has FOUR modules across TWO route trees — a genuine architecture tangle CRD-1 must fully map and CRD-3 must resolve (with a human-decision valve):**
    1. `src/lib/disputes/dispute-service.ts` — **in-memory `Map`** (`:87`), wired to `api/disputes/route.ts:3`, `disputes/[id]/route.ts:10`, `disputes/[id]/send/route.ts:8`, `disputes/stats/route.ts:8`. The de-mock target.
    2. `src/lib/disputes/dispute-service-db.ts` — DB-backed (`DisputeServiceDB`), **wired to no route** (possibly dead code).
    3. `src/lib/credit-repair/dispute-service.ts` — DB-backed (`getSupabase()`), an AI-dispute-letter helper; imported via `@/lib/credit-repair` by `api/credit-repair/disputes/route.ts:20`.
    4. `src/lib/credit-repair/db/disputes-db-service.ts` — a full DB CRUD service, imported as `db` by `api/credit-repair/disputes/route.ts:22`. So `api/credit-repair/disputes/**` already runs on real persistence.
    `api/disputes/**` (broken, in-memory) and `api/credit-repair/disputes/**` (already DB) are two separate route trees on two separate persistence layers. Whether they are the same feature (→ consolidate onto one store) or two distinct dispute surfaces (→ de-mock `api/disputes` independently, accept two stores) is an **architecture decision** — CRD-3 makes it from CRD-1's map, with a STOP-and-surface-to-human valve if it is not mechanically clear.
...
- [ ] **Step 1: Enumerate.** All ~57 `src/app/api/{credit*,credits,disputes,documents,tax/documents,marketplace/tradelines}/**/route.ts` routes (path + methods + auth guard); all services under `src/lib/{credit,credit-builder,credits,disputes,documents}/` AND **`src/lib/credit-repair/db/`** (the 5-ish DB CRUD services there — disputes, goodwill, credit-cards, credit-reports, negotiations — plus the `db` aggregator); the two document services; credit/dispute/document pages and components. **Produce a definitive dispute-architecture map**: the 4 dispute modules (§Pre-state), every route that imports each, and which `disputes`-type table(s) each writes — this map is the input to CRD-3's canonical-store decision.
...
- [ ] **Step 1: Pick the canonical dispute store — from CRD-1's map.** Using CRD-1's dispute-architecture map, decide which DB service `api/disputes/**` should persist to. Decision rule:
  - If `api/disputes/**` and `api/credit-repair/disputes/**` are the **same feature** (same `disputes` table shape, same user-facing concept) → consolidate: de-mock `api/disputes/**` onto `credit-repair/db/disputes-db-service.ts` (the most complete, already-wired DB service), and treat `disputes/dispute-service-db.ts` as dead code to delete alongside the in-memory `dispute-service.ts`.
  - If they are **genuinely distinct surfaces** (different table shapes / different concepts — e.g. general bureau disputes vs credit-repair-workflow disputes) → de-mock `api/disputes/**` onto whichever DB service matches its table (`disputes/dispute-service-db.ts` if it targets the `disputes` table CRD-1 mapped), document the two-surface decision in the commit body, and leave `api/credit-repair/disputes/**` untouched.
  - **If the call is not mechanically clear from CRD-1's map — STOP and surface the decision to the human.** Do not guess a consolidation that merges two schemas.
  State the chosen canonical store and the rationale before proceeding to Step 2.
...
- [ ] **Step 1: Method- AND type-parity audit.** List every `documentService.*` method the three `api/documents` routes call (incl. share-link creation — `document-service.ts:379` `createShareLink`). Check `document-service-db.ts` has each; note gaps. **Type-parity:** `document-service-db.ts`'s `DocumentType` union differs from the in-memory `document-service.ts`'s — re-wiring will narrow the accepted document types and break callers passing a now-unsupported type. Diff the two `DocumentType` unions; if the DB union is narrower, widen it to cover every type the in-memory service (and its callers/routes) accept, so the de-mock is behavior-preserving. Share links are in-memory in the current service — the DB version needs a persisted share-link path (a `document_shares` table, or share metadata on the `documents` row — verify what exists in `types.ts`/migrations; if neither, add a migration for share-link persistence).
