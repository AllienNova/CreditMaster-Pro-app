...
- [ ] **Step 4: Fix** —
  ...
  - Migrate all ~20 engines: replace `new AIMLService()` + `.chat(...)` with the `ModelRouter` execution method from CMP-4. Each migration preserves the engine's existing behaviour — same messages, same intent; `ModelRouter` now selects the model.
  - Set the rule severity to **error** ONLY once every violation is migrated (zero violations remain). If a residual legitimate violation exists, make it `warn` and explain.
...
- [ ] **Step 1: Survey.** Read the EXISTING `pii-protection.ts` — its `detectPII` / `anonymizePII` API. Read `financial-chat-engine.ts:205` (`INTENT_DETECTION_PROMPT.replace("{{MESSAGE}}", message)`, called `~:213`) and `:242` (`RESPONSE_GENERATION_PROMPT.replace("{{INTENT}}", ...)`, called `~:248`) — note the `intent` object at `:242` is derived from user text and is attacker-influenced. Read `chat-engine.ts` — it pushes the user message as a separate `role:"user"` message (no injection hole there); its only defect is missing PII redaction.
...
- [ ] **Step 4: Fix** —
  ...
  - `financial-chat-engine.ts`: restructure BOTH `.replace()`-template calls (`:205` and `:242`) into proper `system` + `user` message pairs — the template text becomes the system message, the user-derived value (`message`, `intent`) becomes a separate sanitised user message. Neither raw value is spliced into the system string.
