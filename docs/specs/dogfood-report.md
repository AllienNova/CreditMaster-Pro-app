...
| | Result |
|---|---|
| Web page sweep | **197 static routes**, signed in, real Chromium |
...
| Mobile build + boot | bundled **2,119 modules**, ran on iPhone 17 Pro |
...
- **76 of 197 web routes are UNMEASURED, not passing.** The final sweep's dev
  server died partway (`ERR_CONNECTION_REFUSED` for everything after
  `/help/faq`) — resource pressure from Next + Metro + a booted simulator on one
  machine. Needs a re-run on a quiet machine.
- **7 dynamic web routes** were never exercised; they need real record ids.
- **Authenticated mobile screens were never reached.** The app builds, boots and
  renders its login screen correctly, but signing in on the simulator could not
  be automated: `idb` is not installed, `simctl` has no tap primitive, and
  AppleScript clicks did not register as taps — focus stayed in one field, so
  both strings landed in the password box. Mobile beyond the login screen is
  therefore **unverified**, not working-or-broken.
