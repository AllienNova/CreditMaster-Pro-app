...
| **G-019** | P2 | Process conflict | **The changed-code coverage gate and the DO-NOT-WIRE verdicts contradict each other.** `npm run test:coverage:changed` requires ≥85% on every changed line of the branch. Four files still fail: `accountability-partners-service` (0%), `commitment-device-service` (0%), `points-rewards-service` (33%), `email-preferences-service` (69%). Two of those are DO-NOT-WIRE for **fabricated money movement** — writing tests for code we have decided not to wire is waste, and worse, it would encode the fabricated behaviour as expected. | `npm run test:coverage:changed`; `orphan-module-review.md` verdicts | **Owner decision, three options:** (a) revert the restored-but-DO-NOT-WIRE modules out of this branch so they are not "changed lines" — cleanest, and consistent with them not shipping; (b) land them behind an explicit quarantine directory the gate excludes **by policy, with the exclusion reviewed**, not as a dodge; (c) accept the gate failure for this branch with the exception recorded here. Do NOT resolve it by lowering the threshold or writing tests that assert fabricated behaviour. |
