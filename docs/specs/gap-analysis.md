...
| **G-019** | P2 | Process conflict | **The coverage gate and the DO-NOT-WIRE verdicts conflict — but over 2 modules, not 4.** Measured with `git diff --numstat origin/main`, the two sets barely overlap. Of the four fabrication P0s, **three have NO diff against `origin/main`** (`auto-save-rules-service`, `ContributionSchedulerService`, `AutoRebalanceScheduler`) — they already sit on `main` untouched by this branch, so they are not "changed lines" and the gate never sees them. Only `commitment-device-service` (29+/10-) is both a fabrication module and changed. The other three gate failures are unrelated to fabrication: `accountability-partners-service` (54+/6-, **0%** — those lines are the IDOR ownership-check restore from `b1e993a`, i.e. untested *security* code), `points-rewards-service` (5+/0-, 33%), `email-preferences-service` (32+/5-, 69% — one of the 8 anon-client modules, slated for conversion). | `git diff --numstat origin/main` per file; `npm run test:coverage:changed` | Split the decision, because these are not one problem. **(a)** `accountability-partners-service` — write the tests. 54 changed lines at 0% covering an IDOR fix is a straightforward gap, not a philosophical conflict, whatever the module's wiring verdict. **(b)** `email-preferences-service`, `points-rewards-service` — small gaps, close them when the module is converted/wired. **(c)** `commitment-device-service` — the only true conflict, and an **owner decision**: a test asserting `executeConsequence()` records a charity donation with no payment rail would encode the fabrication as a specification. Do NOT lower the threshold or exclude files. |
