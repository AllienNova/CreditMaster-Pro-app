...
## SF-30 — a tab file shadows a whole directory, and 15 routes lose their deep link

**Severity: HIGH (user-facing; the same mechanism serves push notifications and
universal links).** Found while investigating why 30 routes could not be
measured by the device sweep.

Six route paths are claimed by TWO files each:

| path | tab file | directory file |
|---|---|---|
| `/` | `app/(tabs)/index.tsx` | `app/index.tsx` |
| `/credit` | `app/(tabs)/credit.tsx` | `app/credit/index.tsx` |
| `/financial` | `app/(tabs)/financial.tsx` | `app/financial/index.tsx` |
| `/investments` | `app/(tabs)/investments.tsx` | `app/investments/index.tsx` |
| `/reports` | `app/(tabs)/reports.tsx` | `app/reports/index.tsx` |
| `/student-loans` | `app/(tabs)/student-loans.tsx` | `app/student-loans/index.tsx` |

An earlier pass recorded these as "one file of each pair is dead code that can
never render". **The damage is larger than that.** The tab file does not merely
win its own path — it shadows everything nested under the directory:

```
exp://…/--/financial/income   ->  renders the FINANCES TAB
                                  ("Net Worth $0 / Assets / Liabilities")
                                  not app/financial/income.tsx
```

Verified from a clean terminate-and-relaunch, so it is not a sequencing
artefact. **15 routes are affected**: the thirteen under `app/financial/`
(debt, goal-detail, goals, holdings, income, investments, net-worth, overview,
reports, savings, spending, stock-analysis, transactions), plus `/reports/1`
and `/student-loans`.

`app/(tabs)/financial.tsx` is a full screen, not a `<Redirect>`, so this is not
a one-line fix — it is a navigation-architecture decision about which of the
two owns the path and where the other's content lives.

### Why this is not just a testing problem

A deep link, a push-notification tap and a universal link all resolve through
the same expo-router path table. If `exp://…/--/financial/income` lands on the
Finances tab, then a notification saying "your paycheque landed" that targets
`/financial/income` lands there too, and the user has to find the screen
themselves. Worth checking every notification payload for these paths before
deciding priority.

### The other 15 of the 30 are not this

Two further causes were separated by testing each route from a clean relaunch:

- **Sequencing.** `/analytics` renders correctly in 8 elements from a clean
  launch. Its sweep failures were an artefact of what preceded it.
- **An ambiguous error screen.** `/credit-builder/age` renders exactly
  `HTTP 500` + `Try Again` in four nodes — its own error state from a real 500
  — which is character-identical to Expo Go's bundle-load failure. The sweep's
  marker cannot tell them apart and no longer pretends to (see rev 56).
...
| 2026-08-18 (rev 52) | Fixed the most actionable fabrication in the audit: `src/app/identity/dark-web/page.tsx`. It told the user **"MegaCorp suffered a breach exposing 12M accounts. Your email and hashed password were included in the dump found on a dark web marketplace"** — a named source, a date, a severity badge, and `Affected: user@example.com`, the placeholder that made the card read as the caller's own breach. Above it sat three summary tiles: "Breaches Found", "Exposed Data Points" and "High / Critical", each computed FROM the invented list (`BREACH_ENTRIES.length`, a sum over `EXPOSED_CATEGORIES`, a severity filter) — fabrications one step removed, the same shape as the retirement figures on `/tax/optimizer` and the "Disputable" tile on `/marketplace/analysis`. **There is no dark-web monitoring in this codebase at all** — no provider client, no route, no table. Searching for a breach source finds only `src/lib/compliance/gdpr-ccpa.ts`, which is about notifying regulators of a breach of OURS, not checking whether the caller's credentials appear in someone else's dump. The page now says monitoring is not active and points at Have I Been Pwned, which is free and independent. Removed with it: the `BreachEntry` interface, both constants and a `severityClasses()` palette for severities nobody measured. **Left alone deliberately, and flagged instead:** the page's own SEO description still reads "Scan the dark web for your exposed personal information including emails, passwords, and financial data." That is a marketing claim about a product capability rather than a per-user fabrication, and rewriting product copy is not mine to do — but it now contradicts the page under it. Verified: tsc 0 on the changed page, `audit:screen-data` PASSES with the web baseline 90 → 89 entries (56 catalogue / 33 fabrication). |
| 2026-08-18 (rev 53) | Fixed the WEB twin of the connected-accounts screen repaired on mobile earlier today. `src/app/settings/connected-accounts/page.tsx` listed Chase Bank, Bank of America and three credit bureaus with statuses and "2 hours ago" sync times, identical for every caller, with no request of any kind — and both controls were theatre: **Disconnect filtered the local array** and **Reconnect set the row to "pending" while a `setTimeout` flipped it back**. A user checking whether their bank was still linked was shown a yes; a user cutting a bank off was shown a disconnection that never happened. Now: banks from `GET /api/financial/connections` (`plaidService.listConnections` → `bank_connections` with their `financial_accounts`, where `status` is `active`/`needs_attention` derived from Plaid's webhooks and `lastSynced` is the real `financial_accounts.last_synced`), bureaus from `GET /api/credit-bureau/connect` (`getBureauConnectionStatuses`, with a real `last_pull_date`), and removal through `DELETE /api/financial/connections/[connectionId]`, which revokes at Plaid. **The row is dropped only when the server says it is gone** — the old handler removed it locally whatever happened, which is precisely how a disconnection that never occurred looked successful. The confirmation now states how many accounts go with the connection, because a user disconnecting "their checking account" also loses the savings account behind the same Item. **There is deliberately no Reconnect button:** re-authenticating a Plaid Item needs Link's update mode (a `link_token` minted from the existing `access_token`), which this app does not build, so a needs-attention connection says so rather than offering a control that cannot work. A failed read and "nothing linked" render differently. Verified: tsc 0, `audit:screen-data` PASSES, web baseline 89 → 88 (56 catalogue / 32 fabrication). |
| 2026-08-18 (rev 54) | Re-ran the 232-route sweep against the seen-set guard. It reports **227 ok / 5 FAIL, and the honest figure is 197 verified**. The duplicate-signature check — the same one that invalidated the previous run — still finds **26 routes reading the home screen and 6 reading the Finances screen**. Two of those are legitimate (`/` IS home, `/financial` IS Finances), leaving **30 routes that rendered a screen other than their own**. The seen-set guard did fire on them: it relaunched, re-navigated and re-read, and got the same wrong screen back. **That means this is not masking.** A stuck alert or a leftover error screen clears on relaunch; this does not. The deep link genuinely lands on the group's initial route for these 30, and the guard's founding assumption — that a relaunch distinguishes a masked reading from a real one — does not hold when the NAVIGATION itself is what is unreliable. The 30 cluster tightly: `/financial/*` (13 of them), `/dashboard/*`, `/dispute/*`, `/credit-builder/*`, `/insights/spending`, `/loans/calculator`, `/documents`, `/handoff`. **This is worth more than a testing note.** If `exp://…/--/financial/income` lands on the dashboard, then so does a push notification or a universal link pointing at that path — the same mechanism serves both. Whether that is an expo-router group-routing fact, a `(tabs)` nesting artefact, or a timing race is not established: `/financial/income` has read 7 and 12 elements in standalone probes and the home screen in two full sweeps, so it is not a hard failure. Recorded as unverified rather than counted, and the fix is to assert the ARRIVAL (a screen-specific marker) rather than infer it from the reading being different — inference is what has now failed three times in a row. **Verified findings that survived both runs**: `/financial-intelligence/{bill-negotiator,goals-manager,smart-budget,spending-insights}` each raise a modal `Alert.alert` at mount (task #92), and one route is unmeasured behind an Expo Go client error. |
| 2026-08-18 (rev 55) | Fixed the four screens that greeted the user with a modal alert on load — `financial-intelligence/{spending-insights,bill-negotiator,goals-manager,smart-budget}`, the only real app failures in both full sweeps. Each caught its fetch error and called `Alert.alert("Error", "Failed to load X. Please try again.")` **at mount**. A native alert is a separate window: it covers the whole screen, offers only "OK" with no retry, and stays until dismissed — and it is what masked every route measured after `spending-insights` in the first sweep, turning a 57-route run into "0 FAIL" over 31 screens nobody had looked at. Each now sets a `loadFailed` flag and renders an inline state with a working Try again, matching the pattern used on `financial/income`, `financial/transactions` and `budgeting/index`. The action alerts elsewhere in those files are untouched: an alert after something the user did is fine, an alert for a page failing to open is not. **A bug in my own first pass, caught before commit:** only `spending-insights` cleared the flag when a fetch began, so on the other three a retry that legitimately returned ZERO rows would still have shown "we could not load" — the exact failure/empty conflation this session has been removing. All four now clear it at the start of every fetch. **The device verification failed the first time and I nearly accepted it.** The four-route sweep reported 0 FAIL / 4 ok, and every one of those readings was the same wrong screen — "Strategies / Strategy Detail / Strategy not found" — left over from an earlier probe. The seen-set guard did not catch four identical signatures because its relaunch reproduced the same failed navigation, which is the mode recorded in rev 54. Re-verified properly with a full terminate-and-relaunch before each route: spending-insights renders "We could not load your spending insights." + Try again, and bill-negotiator, goals-manager and smart-budget each render their own titled screen with the same shape and no alert. Verified: tsc 0, lint 0 errors, jest 1,643 pass, all seven mobile gates PASSED. |
| 2026-08-18 (rev 56) | Added SF-30 and corrected the harness marker a second time. **SF-30:** six route paths are claimed by two files each — `app/(tabs)/X.tsx` and `app/X/index.tsx` — and an earlier pass recorded that as "one file of each pair is dead code". The damage is larger: the tab file shadows everything NESTED under the directory, so `exp://…/--/financial/income` renders the Finances TAB rather than `app/financial/income.tsx`. Verified from a clean terminate-and-relaunch, so it is not a sequencing artefact. **15 of the 30 unverified routes are this** — the thirteen under `app/financial/`, plus `/reports/1` and `/student-loans`. It is not a testing problem: a deep link, a push-notification tap and a universal link all resolve through the same path table, so a notification targeting `/financial/income` lands on the Finances tab too. `app/(tabs)/financial.tsx` is a full screen rather than a `<Redirect>`, so the repair is a navigation-architecture decision, not a one-liner. **The marker correction:** rev 45 fixed the Expo-client marker by adding an element-count discriminator, on the reasoning that Expo Go's error screen is small and an app error state is large. That was also wrong. `/credit-builder/age` renders exactly `HTTP 500` + `Try Again` in FOUR nodes — its own error state, from a real 500 — which is character-identical to Expo Go's four-node `HTTP 404` + `Try Again`; `/documents` is five. The app and the client draw the same screen, and no property of the reading separates them. The marker no longer claims to know which: it reports the reading as **AMBIGUOUS**, still surfaced as a problem so it can never be a silent pass, but no longer asserting a cause. Deciding needs arrival assertion, which this script cannot yet do — the third time inference has been wrong where a positive check was needed. |
| 2026-08-18 (rev 57) | **SF-30 urgency established: it reaches users through push notifications today.** The cheap check recommended in rev 56 came back positive. `src/lib/notifications/notification-scheduler.ts` schedules pushes with `url: "/financial/bills"` (:127), `"/financial/transactions"` (:139) and `"/financial/goals"` (:156), and `web-push-service.ts:596` sends `/financial/bills` as well. All three are real screens — `mobile-app/app/financial/{bills,transactions,goals}.tsx` all exist — and all three sit under the path that `app/(tabs)/financial.tsx` shadows. **So a mobile user who taps a "bill due" or "new transaction" notification lands on the Finances tab, not the screen the notification named.** The web side is unaffected: `src/app/financial/{bills,transactions,goals}/page.tsx` are ordinary Next routes with no colliding tab file, so the same payload resolves correctly in a browser. That makes this a mobile-only navigation defect with a live delivery path, not a latent one, and it raises SF-30 from "found by a test harness" to "the product sends people to the wrong screen". Recorded rather than fixed: which of the two files owns `/financial` is a navigation-architecture decision (task #96), and 28 in-app `router.push` calls to `/financial/*` would move with it. Whether those in-app pushes are shadowed the same way as external deep links is NOT established — they resolve through the same path table, but I did not reproduce an in-app navigation failure and will not claim one. |
| 2026-08-18 (rev 58) | **Sharpens SF-30, and narrows a claim I overstated.** Rev 56 said 15 routes "lose their deep link", on the strength of `/financial/income` rendering the Finances tab from a clean terminate-and-relaunch. Building the arrival assertion produced the opposite reading: in a warm four-route sweep the same link rendered the Income screen itself ("Income / We could not load your income. / Try again"). Re-tested cold twice more: **both cold starts gave the Finances tab, and the warm navigation gave the right screen.** So the defect is COLD-START-SPECIFIC — the shadowing decides the route when the app is launched into it, and a warm navigation resolves correctly. "15 routes lose their deep link" was too strong; the accurate statement is that a cold-start deep link to a shadowed nested path lands on the tab. **This makes the notification impact sharper, not weaker.** Tapping a push notification when the app is not running IS a cold start, which is the exact case that fails — so the three live payloads (`/financial/bills`, `/financial/transactions`, `/financial/goals`) land on the Finances tab precisely when the user is most likely to tap them. It also means the 15 are not permanently unverifiable by the sweep, which runs warm: `/financial/income` confirmed arrival in this run. **Built the arrival assertion (task #95).** The sweep now reads each screen's own title out of its source — `<ScreenHeader title="…">` or a `styles.title`/`headerTitle` Text, 164 of 238 screens expose one — and requires that title to be present as a WHOLE accessibility node after navigating. Substring matching was tried first and immediately produced a false confirm: the Finances tab lists a row labelled "Income", so a one-word title matched the wrong screen — the same false confidence the check exists to remove. Screens with no extractable title are reported as **arrival UNCHECKED** rather than counted as confirmed, so "ok" no longer conflates "the right screen rendered" with "something rendered and nothing looked broken". The first run flagged `financial-intelligence/spending-insights` as a wrong screen, correctly: its error state renders the message and a Try again but NOT the screen title, so a user who lands there in that state cannot see where they are. |
| 2026-08-18 (rev 59) | Fixed `src/app/admin/audit/page.tsx`, the second-worst fabrication in the web triage. `auditLogs` was eight invented entries — `user.login` by admin@fynvitapro.com from 192.168.1.1, "Deleted user: test@example.com", "Created new API key for integration" — with plausible timestamps and risk ratings, shown to any admin with no request made. **An audit log is a compliance artefact, and a fabricated one is worse than an empty one:** it answers "did anyone touch this account?" with a confident no that was never checked. Now reads `GET /api/admin/audit` (`withRole("admin")`), mapping to the real columns: `action`, `profiles.email`, `ip_address`, `message`, `severity`, `created_at` rendered in UTC. **`message` and `severity` come from the later of the two migrations that build this table and may be null on older rows, so neither gets a default — a missing severity renders no badge rather than "low", because "low risk" is a claim about the event.** The four summary tiles were counting the invented array, and one of them, "Total Events (24h)", claimed a 24-hour window over data with no window at all; total now comes from the route's own `total` and the other three are computed over the loaded page and say so ("Logins in the latest 50"), because counting logins or unique users across ALL events needs a server aggregate this route does not provide — and a page-scoped number under an all-time label would be the same mislabel in a new place. Four states are now distinct where there was one: unreadable, empty, filtered-to-nothing, and loading. Verified: tsc 0, `audit:screen-data` PASSES, web baseline 88 → 87 (56 catalogue / 31 fabrication). |
| 2026-08-18 (rev 60) | Fixed `src/app/admin/logs/page.tsx`, and it is the clearest case yet of a screen contradicting its own backend. The page rendered invented system output — "Failed to connect to Equifax API: Connection timeout" seen 12 times, "Rate limit approaching for Experian API (85%)" — with sources, levels and timestamps, to any admin, with no request made. **`GET /api/admin/logs` was already honest:** it returns `{ logs: [], total: 0, dataAvailable: false, message: "System logs are not yet available. A system_logs table and writer are needed to populate this view." }`. Somebody had already done the hard part — admitting the capability does not exist — and the screen went on rendering a fiction over the top of it. The page now calls that route and prints what it says, and will render real rows unchanged when `dataAvailable` turns true. Four states are distinct where there was one: not built yet, unreadable, nothing logged, and filtered-to-nothing — an empty table reads as the third when the truth is the first. The three summary tiles were labelled "Errors (24h)", "Warnings (24h)" and "Total Logs (24h)" while counting a constant with no time window at all; they now say "in this view", the same mislabel corrected on the audit page in rev 59. The occurrence `count` column is typed OPTIONAL rather than assumed: it came from the invented rows, and the real row shape is not defined until the table exists. Verified: tsc 0, `audit:screen-data` PASSES, web baseline 87 → 86 (56 catalogue / 30 fabrication). |
| 2026-08-18 (rev 61) | Fixed `src/app/admin/disputes/page.tsx`. Two constants: `disputes` invented dispute records with real-looking user emails, and `stats` invented four platform metrics — "24,567 Total Disputes", "**78% Success Rate**", "32 days Avg Resolution Time", "1,234 Pending Review" — each carrying a period-over-period change ("+18%", "+3%", "-5 days") computed from nothing. `GET /api/admin/disputes` returns `{ disputes, total }` and no statistics, so Total Disputes is now the route's own `total` and Pending is counted over the loaded rows and labelled "Pending in this view". **Success Rate and Avg Resolution Time are removed rather than estimated:** no dispute-outcome history is recorded anywhere, which is the same gap SF-09 records for the per-template `successRate` shown to users — an invented 78% on an operator dashboard is how a number like that becomes a quoted fact. Every `change` figure is gone as well: a delta needs a previous period and nothing stores one. `priority` renders "—" when absent rather than defaulting to a badge, for the same reason `severity` does on the audit page. Four states are distinct: unreadable, none filed, filtered-to-nothing, loading. Verified: tsc 0, `audit:screen-data` PASSES, web baseline 86 → 85 (56 catalogue / 29 fabrication). **Web constant-data fabrications are now 29, down from 34 at the start of the triage; the three admin surfaces fixed in revs 59-61 (audit, logs, disputes) were the ones whose invented content was operator- or compliance-facing.** |
| 2026-08-18 (rev 62) | **First sweep whose "ok" means the screen's own title was on screen, and it corrects every mobile figure reported today — downward.** 232 routes: **114 ARRIVAL CONFIRMED**, 71 arrival-unchecked (the screen exposes no extractable title), **47 flagged WRONG SCREEN**, 0 other problems. The previously reported 197 and 201 were inference — "this reading is not a crash, not near-empty, not identical to another route's" — and positive assertion does not support them. Of the 47: **34 read the HOME screen** (67 elements, "Good morning… 731 Good +14 Experian 738"), and they are NOT confined to the six colliding paths of SF-30 — `/credit-builder/age`, `/credit-builder/utilization`, `/credit-builder/debt-strategy`, `/credit/factors`, `/credit/history`, `/credit-repair/{goodwill,inquiries,negotiate,payments}`, `/admin/{disputes,health}` and `/dashboard/analytics` all landed there. **So deep-link navigation is broadly unreliable in this app, not just under the shadowed prefixes**, and SF-30 as scoped describes a subset. Five more show an error or empty state whose title their own layout omits (`spending-insights`, `identity/dark-web` among them) — those are false positives of the check AND a real finding in their own right: a user who lands on an error state cannot see which screen they are on. Eight are small readings with no error text. One is a genuine crash: **`/student-loans` renders the ErrorBoundary ("Something went wrong") in 119 elements** — the same route an earlier interrupted sweep flagged and a later re-test cleared, so it is intermittent and now confirmed to occur. 24 routes had a first reading masked and were re-measured. The honest position is that **114 of 232 mobile routes are positively verified**, and the rest are unproven rather than failing — 71 because the harness has nothing to check against, 47 because what rendered was not demonstrably the route requested. |
| 2026-08-18 (rev 63) | **Half of the "34 routes land on Home" finding was my own harness being impatient, and measuring it separated the two causes cleanly.** `relaunch()` waited 12 seconds before firing the next deep link. A link fired before expo-router is ready is DROPPED SILENTLY: the app stays on its initial route, and the reading is the Home screen — a perfectly healthy-looking 67-element render. Measured directly on `/credit-builder/age` from a clean launch, same build, same command, only the wait differing: **12s → 67 elements, the Home screen; 18s → 4 elements, the screen's own "HTTP 500" state.** Settle raised to 20s. Re-testing five of the affected routes afterwards, all five navigated: `/loans/calculator` confirmed arrival at 42 elements, and `/credit-builder/age`, `/credit-repair/goodwill`, `/admin/health` and `/dashboard/analytics` each rendered their own 3-element error state instead of Home. **So of the 37 Home readings in rev 62: 18 were SF-30 path collisions, and 18 were this.** The two are cleanly separable, because collisions persist at ANY wait — `/credit/factors` renders the Credit TAB after 17 seconds exactly as it does after 12, tested twice. **This also corrects rev 62's claim that the failures were "not confined to the six colliding paths":** `/credit/factors`, `/credit/history` and `/credit/score-detail` ARE under a colliding prefix — `/credit` is one of the six and I overlooked it when listing counter-examples. The genuine counter-examples were the impatience set, which is now fixed. **The remaining blocker is different and smaller:** those four routes still fail the arrival check because their ERROR state renders no screen title, so there is nothing to assert against. That is a real user-facing gap in its own right — a person who lands on an error state cannot see which screen they are on — and fixing it converts the routes from unverifiable to verifiable. |
| 2026-08-18 (rev 64) | Built `src/components/ScreenError.tsx` and moved 17 screens onto it, fixing a user-facing gap and an entire class of unverifiable routes at once. Error branches were written per screen, and most dropped the screen's own header: an icon, a message and a Try Again button — three accessibility nodes, no title and no way back. **For the user that means landing on a failure with no way to tell which screen failed and no way to leave it** — the exact shape the back-nav work fixed on the SUCCESS path, kept on the error path. **For verification it means the route cannot be confirmed**, because the sweep looks for the screen's own title, so any screen whose API was down was unverifiable — which in a local environment is most of them. The component keeps `ScreenHeader`, so the title tells the user where they are, the back control works, and the sweep has something to assert against. 17 of the 33 screens sharing that shape were transformed mechanically; the other 16 differ in structure or expose no title and were SKIPPED and listed rather than force-fitted. **The arrival check also resolves the HTTP-error ambiguity recorded in rev 56:** Expo Go's bundle-load failure cannot render the app's own title, because it never loaded the app — so a small "HTTP 500" reading that CARRIES the expected title is the app's error state, and the marker now only reports ambiguity when arrival could not be confirmed. `/credit-builder/age` went from 3 elements and "wrong screen" to 6 elements and ARRIVAL CONFIRMED. **A regression I introduced and fixed in the same pass:** the component first rendered "Try again", and these 17 screens' tests assert "Try Again" — 18 suites went red. I had changed user-visible copy in a batch transform without checking what asserted it. Copy restored to the established form; 136 suites / 1,643 tests green. Verified: tsc 0, lint 0 errors, all seven mobile gates PASSED, and 3 of 4 spot-checked routes now confirm arrival on device — the fourth, `/financial/debt`, still fails correctly because it is under the SF-30 collision. |
| 2026-08-18 (rev 65) | Extended the arrival extractor to read the LAYOUT's title, recovering 40 of the 74 unverifiable routes without touching a single screen. Reading only the screen file assumed every screen draws its own header. It does not: 67 of the 74 draw no header at all and rely on the NATIVE stack header, whose title is declared in the nearest `_layout.tsx` as `<Stack.Screen name="x" options={{ title: "Y" }} />` — and that title IS rendered, so `idb` can read it. The extractor now falls back to walking up to the nearest layout. **Coverage goes from 164 of 238 screens to 204**, and the remaining 34 are mostly `(auth)`/`(tabs)` roots plus screens with a genuinely dynamic title (`investments/analyze/[symbol]`). Verified on three of the recovered routes: `/coach/budget` (12 el), `/credit` (27 el) and `/coach/goals` (5 el) all confirm arrival where they were previously unchecked. Worth noting a limit this exposes: `/credit` confirms against the title "Credit Score", which BOTH `app/credit/index.tsx` and the colliding `app/(tabs)/credit.tsx` render — so for the six SF-30 paths the check cannot distinguish the two files, and a confirmation there means "one of the two screens claiming this path rendered", not "the directory file did". That is a property of the collision, not of the check, and it disappears when SF-30 is decided. |
| 2026-08-18 (rev 66) | Fixed `src/app/dashboard/settings/transaction-rules/page.tsx`, where the fabrication was the smaller half of the problem. `useState(mockRules)` seeded the list with invented rules — "Coffee Shops → Food & Dining", matching "starbucks" — and `loadData` was `await new Promise(setTimeout 500)` followed by `setRules(mockRules)`: half a second of spinner over no request at all. **Every mutation was local too.** Save, Delete, Toggle and both reorder buttons changed React state and nothing else, so a user could build a categorisation rule, watch it appear in the list, and lose it on refresh — believing their transactions were being sorted automatically when nothing had been written. All five now go through the real endpoints (`GET`/`POST /api/financial/transaction-rules`, `PATCH`/`DELETE .../[id]` — the routes built earlier in this session), and the list is **re-read from the server after each write** rather than patched locally, so what is on screen is what was actually stored. Reordering PATCHes both moved rows, because `priority` decides which rule wins when two match the same transaction and therefore has to persist. A failed write says so and changes nothing. The page's local `TransactionRule` was checked against the service's before wiring: it is a strict subset with identical field names, so this is a benign shadow rather than the `financial/income` kind. **One self-inflicted break, caught by tsc:** removing the constant with a bracket counter matched the `[]` in the TYPE ANNOTATION (`TransactionRule[] = [`) rather than the array, cutting in the wrong place and orphaning 46 lines of object literals. Re-removed by locating the body between the interface's closing brace and its `];`. Verified: tsc 0, `audit:screen-data` PASSES, web baseline 85 → 84 (56 catalogue / 28 fabrication). |
| 2026-08-18 (rev 67) | Fixed `src/app/dashboard/subscriptions/page.tsx`. Two constants: `mockSubscriptions` (eight invented subscriptions) and `mockStats` (totals, counts, and **$563.15 of "potential annual savings"**), with `loadData` an 800 ms `setTimeout` over no request. The list now comes from `GET /api/financial/subscriptions`, and **the stats are computed from the same rows the list renders** — the failure mode that gave `financial/transactions` a spending chart contradicting its own ledger. Frequency is normalised to a monthly figure before summing (weekly × 52/12, quarterly ÷ 3, yearly ÷ 12), because a list mixing billing periods cannot be added directly. **The savings surface is removed rather than estimated:** both the "Potential Savings" tile and an "AI Savings Banner" reading **"We found $X in potential savings!"** — gated on `potentialAnnualSavings > 100`, a threshold over an invented number — asserted the result of an analysis that does not exist. Nothing in this app decides which subscriptions a user does not need, and its "Review Suggestions" button led to no suggestions. That is the same shape as the dark-web breach card removed in rev 52: a claim to have FOUND something, where no search ran. The page's local `Subscription` type was checked against the service's first — identical field names, a strict subset — and `nextBillingDate` was already read through `new Date(...)` at the sort, so the Date-to-string change at the JSON boundary does not bite. Verified: tsc 0, lint clean, `audit:screen-data` PASSES, web baseline 84 → 83 (56 catalogue / 27 fabrication). **Web constant-data fabrications are now 27, down from 34 when the triage finished.** |
| 2026-08-18 (rev 68) | Fixed `src/app/marketplace/analysis/page.tsx`, the second case today of an honest backend that its own screen ignored — and the more serious of the two. `handleUpload` set a two-second spinner and then `setResults(mockResults)`: an invented credit report naming real creditors ("Late payment on Chase card (30 days) - Jan 2023", "Collection account - Medical debt $450", "High utilization on Discover (78%)"), presented as the analysis of the file the user had just uploaded — **while the file itself went nowhere**, under a caption promising "Your data is encrypted and never stored". **`POST /api/credit-report/analyze` is already gated 501**, with a comment recording that it USED to fabricate (`creditScore: Math.floor(Math.random() * 150) + 600` plus invented disputeable items) and was deliberately stopped, because no report parser exists: "Credit report analysis is not available yet. Your file was not analyzed." Someone had already refused to fabricate on the server, and the screen fabricated anyway. The upload now actually posts the file, so the route's validation runs and the user learns at once if the file is unacceptable, and the route's own answer is displayed rather than replaced. When a parser lands, the 501 disappears and this page renders the real analysis with no further change. Removed alongside: the **"Paste Text"** button, which called the same handler with a click event where a `File` belonged and would have been rejected for having no file — there is no paste flow to reach. Verified: tsc 0, lint clean, `audit:screen-data` PASSES, web baseline 83 → 82 (56 catalogue / 26 fabrication). |
| 2026-08-18 (rev 69) | Fixed `src/app/admin/features/page.tsx`, where the invented data was less serious than the capability it implied. `initialFlags` listed six flags that do not exist — `ai_dispute_v2` at 100% rollout, `credit_score_simulator` and others — with toggles and rollout sliders that changed React state and nothing else. **There IS a real flag system, and this was not it.** `public.feature_flags` (migration `20260516000000`) is `key, enabled, description, updated_at`, read ONLY through the service-role client in `src/lib/flags/index.ts`, with the AUTH-04 kill-switch depending on it; the migration's own comment records that a non-service-role read returns empty so flags default false. **No HTTP route exposes it**, so this page could not simply be wired — and it should not be wired to the shape it had: the invented flags carry `rollout` and `environment`, and **the real table has neither column**. An operator dragging a slider to "50% rollout" or selecting "staging only" would be configuring something the flag system cannot express. That is worse than a missing screen, because a governance surface that appears to work is trusted: the same reasoning that removed the mobile admin destructive buttons in rev 28. The screen now states what is true and offers no controls. Wiring it properly needs a GET/PATCH route over `feature_flags`, an entry in the auth inventory, and a decision about whether rollout and environment targeting are wanted at all — recorded rather than assumed. Verified: tsc 0, `audit:screen-data` PASSES, web baseline 82 → 81 (56 catalogue / 25 fabrication). |
