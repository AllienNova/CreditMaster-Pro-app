...
- ⏳ **Notifications SCREEN wiring (remaining — needs a careful adapter, do NOT rush)**: `app/notifications/index.tsx` still uses `MOCK_NOTIFICATIONS`. **FINDING — cross-boundary type divergence**: the mobile `Notification` type (`services/api/types.ts:287`) declares `body` + enum `dispute_update|score_change|alert|recommendation|payment|system`, but the real web route returns `message` + a different enum (`payment_success|tip|document_uploaded|dispute_overdue|dispute_reminder|draft_reminder|score_reminder|subscription_expiring|welcome|system`). Wiring requires a **web→mobile adapter** (in `notificationApi.getAll` or `notificationStore`): `body ← message`, and map the web enum → the mobile enum (dispute_*→`dispute_update`, payment_success/subscription_expiring→`payment`, score_reminder→`score_change`, tip→`recommendation`, document_uploaded→`alert`, welcome/system→`system`). Then wire the screen to `useNotificationStore` (fetch on mount, render `notifications`, format `createdAt` for display instead of the mock `timestamp`, wire mark-read/mark-all/delete to store actions), remove `MOCK_NOTIFICATIONS`, add a screen test. This adapter is the crux — get the enum map + `body`/`message` right or notifications render blank/mis-iconed.
