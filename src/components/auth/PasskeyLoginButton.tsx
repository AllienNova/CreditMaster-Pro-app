...
export default function PasskeyLoginButton({
  ...
  useEffect(() => {
    const checkSupport = async () => {
      ...
      if (supported) {
        const conditionalAvailable = await webAuthnService.isConditionalMediationAvailable();
  ...
  const handlePasskeyLogin = async () => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        onError('Passkey verified, but automatic sign-in is not available. Please sign in manually.');
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <>
          <svg className="w-5 h-5 text-gray-700 dark:text-slate-200" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 2C9.24 2 7 4.24 7 7C7 8.85 8.04 10.44 9.54 11.24C6.32 12.04 4 15.04 4 18.5V20C4 20.55 4.45 21 5 21H19C19.55 21 20 20.55 20 20V18.5C20 15.04 17.68 12.04 14.46 11.24C15.96 10.44 17 8.85 17 7C17 4.24 14.76 2 12 2ZM12 4C13.66 4 15 5.34 15 7C15 8.66 13.66 10 12 10C10.34 10 9 8.66 9 7C9 5.34 10.34 4 12 4ZM12 12C14.21 12 16.17 13.28 17.35 15.21L6.65 15.21C7.83 13.28 9.79 12 12 12Z" fill="currentColor"/>
            <circle cx="12" cy="7" r="2" fill="currentColor" opacity="0.5"/>
          </svg>
          <span className="text-sm font-medium text-gray-700 dark:text-slate-200">Sign in with Passkey</span>
        </>
      )}
    </button>
  );
