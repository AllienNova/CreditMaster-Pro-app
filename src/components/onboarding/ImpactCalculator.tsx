...
const goalImpacts: Record<
  string,
  { icon: React.ReactNode; benefit: string; savings: string }
> = {
  buy_home: {
...
export function ImpactCalculator({
  ...
  const calculation = useMemo(() => {
    ...
    const months =
      timeframe === "3_months" ? 3 : timeframe === "6_months" ? 6 : 12;
    ...
    const recommendedMonths = isFeasible
      ? months
      : Math.ceil(pointsToGain / maxMonthlyGain);
    ...
    <div className="bg-gradient-to-br from-emerald-50 to-blue-50 rounded-xl p-6 border-2 border-emerald-200">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 mb-4">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span
            className={`font-semibold ${isFeasible ? "text-emerald-600" : "text-orange-600"}`}
          >
      ...
      {!isFeasible && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
          <p className="text-sm text-orange-800">
            <strong>Tip:</strong> Your goal is ambitious! We recommend{" "}
            {recommendedMonths} months for sustainable improvement.
          </p>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 mb-4">
        ...
        <p className="text-xs text-gray-600 dark:text-slate-300">
          in reduced interest over loan lifetime
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          {selectedGoals.slice(0, 3).map((goalId) => {
            ...
            return (
              <div
                key={goalId}
                className="bg-white dark:bg-slate-800 rounded-lg p-3 flex items-start gap-3"
              >
