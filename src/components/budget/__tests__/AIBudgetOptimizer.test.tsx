...
describe("AIBudgetOptimizer", () => {
  describe("Component Rendering", () => {
    ...
    it("should display savings opportunities", async () => {
      ...
      expect(
        screen.getByText(/Your dining expenses are 50% above average/i),
      ).toBeInTheDocument();
  ...
  describe("Error Handling", () => {
    it("should display error state when API fails", async () => {
      server.use(
        rest.get(
          "http://localhost/api/financial/budgets/recommendations",
          (req, res, ctx) => {
            return res(
              ctx.status(500),
              ctx.json({ error: "Failed to optimize budget" }),
            );
          },
        ),
      );
      ...
      expect(
        screen.queryByText(/AI Budget Optimizer/i),
