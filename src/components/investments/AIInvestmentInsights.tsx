        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
            ...
            <p className="text-blue-100 text-sm mt-2">
              {data.portfolioHealthScore >= 80 ? 'Excellent portfolio health' :
               data.portfolioHealthScore >= 60 ? 'Good portfolio health with room for improvement' :
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${getTypeColor(rec.type)}`}>
                          {rec.type.toUpperCase()}
                        </span>
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${getRiskColor(rec.riskLevel)}`}>
                          {rec.riskLevel} risk
                        </span>
                      </div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-blue-200">{rec.confidence}% confidence</div>
                    </div>
                  </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRiskColor(data.riskAnalysis.overallRisk)}`}>
                {data.riskAnalysis.overallRisk.toUpperCase()} ({data.riskAnalysis.riskScore}/100)
              </span>
            </div>
            <div className="space-y-2 mb-3">
              {data.riskAnalysis.factors.slice(0, 3).map((factor, idx) => (
                <div key={idx} className="flex items-start space-x-2 text-sm">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${getRiskColor(factor.level)} flex-shrink-0`}>
                    {factor.level}
                  </span>
                  <div>
                    <span className="font-medium">{factor.factor}:</span>
                    <span className="text-blue-100 ml-1">{factor.description}</span>
                  </div>
                </div>
              ))}
            </div>
            <div className="border-t border-white/20 pt-3">
              ...
              <ul className="space-y-1">
                {data.riskAnalysis.recommendations.slice(0, 2).map((rec, idx) => (
              ...
              {data.diversificationSuggestions.slice(0, 3).map((suggestion) => (
                <div key={suggestion.id} className="bg-white/10 rounded-lg p-3">
                  ...
                  <div className="flex items-center space-x-2 text-sm mb-2">
                    ...
                    <span className="font-medium">{suggestion.currentAllocation}%</span>
                    <span className="text-blue-200">→</span>
                    <span className="text-blue-200">Recommended:</span>
                    <span className="font-medium">{suggestion.recommendedAllocation}%</span>
                  </div>
                  <p className="text-sm text-blue-100">{suggestion.reasoning}</p>
                </div>
              ))}
            </div>
          </div>
                ...
                {data.marketPredictions.map((pred, idx) => (
                  <div key={idx} className="flex items-center justify-between text-sm">
                    <span className="text-blue-100">
                      {pred.timeframe === '1_month' ? '1 Month' :
                       pred.timeframe === '3_months' ? '3 Months' : '6 Months'}
                    </span>
                    <div className="text-right">
                      <div className={`font-medium ${pred.predictedReturn >= 0 ? 'text-green-300' : 'text-red-300'}`}>
                        {pred.predictedReturn > 0 ? '+' : ''}{pred.predictedReturn.toFixed(1)}%
                      </div>
                      <div className="text-xs text-blue-200">{pred.confidence}% confidence</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {data.performanceForecasts.slice(0, 3).map((forecast, idx) => (
                  <div key={idx} className="flex items-center justify-between text-sm">
                    ...
                    <div className="text-right">
                      <div className="font-medium">${(forecast.projectedValue / 1000).toFixed(1)}K</div>
                      <div className="text-xs text-blue-200">
                        {forecast.projectedReturn > 0 ? '+' : ''}{forecast.projectedReturn.toFixed(1)}%
                      </div>
                    </div>
                  </div>
                ))}
