...
export function PortfolioAnalyticsDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'allocation' | 'risk' | 'income'>('overview');

  // Calculate allocations
  const { assetAllocation, sectorAllocation, regionAllocation } = useMemo(() => {
    ...
    const toAllocation = (map: Map<string, number>, colorMap?: Record<string, string>): AllocationData[] => {
...
interface OverviewTabProps {
  metrics: PortfolioMetrics;
  topHoldings: (PortfolioHolding & { value: number; gain: number; gainPercent: number })[];
  benchmarkName: string;
  benchmarkReturn: number;
}

function OverviewTab({ metrics, topHoldings, benchmarkName, benchmarkReturn }: OverviewTabProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          valueColor={metrics.totalGain >= 0 ? 'text-green-400' : 'text-red-400'}
        />
        <MetricCard
          label="Day Change"
          ...
          valueColor={metrics.dayChange >= 0 ? 'text-green-400' : 'text-red-400'}
        />
        <MetricCard
          label="1Y Return"
          ...
          valueColor={metrics.oneYearReturn >= 0 ? 'text-green-400' : 'text-red-400'}
        />
      </div>

      {/* Performance Metrics */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <MetricCard label="Sharpe Ratio" value={metrics.sharpeRatio.toFixed(2)} small />
        <MetricCard label="Sortino Ratio" value={metrics.sortinoRatio.toFixed(2)} small />
        <MetricCard label="Beta" value={metrics.beta.toFixed(2)} small />
        <MetricCard label="Alpha" value={`${metrics.alpha >= 0 ? '+' : ''}${metrics.alpha.toFixed(2)}%`} small />
        <MetricCard label="Volatility" value={`${metrics.volatility.toFixed(1)}%`} small />
      </div>
              ...
              {topHoldings.map(h => (
                <tr key={h.symbol} className="hover:bg-gray-700/30">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-gray-500 dark:text-slate-400">{h.name}</div>
                  </td>
                  <td className="px-4 py-3 text-right text-white">
                    ${formatNumber(h.value)}
                  </td>
                  <td className="px-4 py-3 text-right text-gray-300">
                    {((h.value / topHoldings.reduce((s, x) => s + x.value, 0)) * 100).toFixed(1)}%
                  </td>
                  <td className={`px-4 py-3 text-right ${h.gain >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {h.gain >= 0 ? '+' : ''}${formatNumber(h.gain)}
                  </td>
                  <td className={`px-4 py-3 text-right ${h.gainPercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
...
function AllocationTab({ assetAllocation, sectorAllocation, regionAllocation }: AllocationTabProps) {
  return (
...
function AllocationChart({ title, data }: { title: string; data: AllocationData[] }) {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4">
      <h3 className="text-lg font-semibold text-white mb-4">{title} Allocation</h3>
...
function RiskTab({ metrics, holdings }: RiskTabProps) {
  ...
  const top10Weight = sortedByValue.slice(0, 10).reduce((s, h) => s + h.value, 0) / totalValue * 100;

  // Risk level assessment
  const getRiskLevel = (value: number, thresholds: [number, number]): 'low' | 'medium' | 'high' => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 dark:text-slate-500 mt-1">Based on volatility, concentration, and market exposure</p>
          </div>
        ...
        {riskIndicators.map((indicator, i) => (
          <div key={i} className="bg-gray-800 rounded-lg p-4">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-sm text-gray-400 dark:text-slate-500">{indicator.label}</div>
                <div className="text-xl font-semibold text-white mt-1">{indicator.value}</div>
              </div>
              <span className={`px-2 py-1 text-xs font-medium rounded ${levelColors[indicator.level]}`}>
                {indicator.level.toUpperCase()}
              </span>
            </div>
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-2">{indicator.description}</p>
          </div>
        ))}
      </div>
      ...
      {/* High Beta Holdings */}
      <div className="bg-gray-800 rounded-lg p-4">
        <h3 className="text-lg font-semibold text-white mb-4">High Beta Holdings</h3>
        <div className="space-y-2">
          {holdings
            ...
            .slice(0, 5)
            .map(h => (
              <div key={h.symbol} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-0">
          ...
          {holdings.filter(h => h.beta && h.beta > 1.3).length === 0 && (
            <p className="text-gray-500 dark:text-slate-400 text-sm">No high-beta holdings</p>
          )}
        </div>
      </div>
    </div>
...
function IncomeTab({ holdings, metrics }: IncomeTabProps) {
  ...
  const totalDividends = dividendHoldings.reduce((s, h) => s + h.annualDividend, 0);
...
function MetricCard({ label, value, subValue, valueColor = 'text-white', small = false }: MetricCardProps) {
  return (
    <div className={`bg-gray-800 rounded-lg ${small ? 'p-3' : 'p-4'}`}>
      <div className={`text-gray-400 dark:text-slate-500 ${small ? 'text-xs' : 'text-sm'}`}>{label}</div>
      <div className={`font-bold ${valueColor} ${small ? 'text-lg' : 'text-2xl'} mt-1`}>{value}</div>
      {subValue && (
        <div className={`text-gray-500 dark:text-slate-400 ${small ? 'text-xs' : 'text-sm'} mt-1`}>{subValue}</div>
      )}
    </div>
  );
}
...
function calculateRiskScore(metrics: PortfolioMetrics, concentration: number): number {
