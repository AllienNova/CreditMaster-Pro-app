...
export default function AssetAllocationPanel({
  ...
  const [analysis, setAnalysis] = useState<AssetAllocationAnalysis | null>(
    null,
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft" && activeSection > 0) {
        ...
        e.key === "ArrowRight" &&
        activeSection < swipeableSections.length - 1
      ) {
        e.preventDefault();
        handleSwipeLeft();
      }
  ...
  const [cachedAnalysisTimestamp, setCachedAnalysisTimestamp] =
  ...
  const analyzeAllocation = async () => {
    ...
    if (!isOnline) {
      try {
        ...
        setError(
          "You are offline. Analysis request queued for when you reconnect.",
        );
        setLoading(false);
        return;
      } catch (err) {
    ...
    try {
      ...
      if (!isOnline) {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to analyze allocation",
        );
      }
    } finally {
      setLoading(false);
    ...
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6 bg-gray-100 dark:bg-slate-900 rounded-lg transition-colors duration-200">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
          ...
          >
            <option value={RiskTolerance.VERY_CONSERVATIVE}>
              Very Conservative
            </option>
            ...
            <option value={RiskTolerance.VERY_AGGRESSIVE}>
              Very Aggressive
            </option>
          </select>
          <button
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              <div className="bg-gray-50 dark:bg-slate-750 p-4 rounded-lg transition-colors duration-200">
                ...
                <div className="text-xl sm:text-2xl font-semibold text-green-600 dark:text-green-400">
                  {(analysis.performanceMetrics.expectedReturn * 100).toFixed(
                    1,
                  )}
                  %
                </div>
          ...
          >
            ...
            <p className="mt-4 text-sm text-gray-600 dark:text-slate-400">
              ...
              Your current portfolio (red diamond) and recommended portfolio
              (green star) are plotted against the optimal frontier (blue
              curve).
            </p>
          </CollapsibleSection>
          ...
          {analysis.needsRebalancing &&
            analysis.rebalancingRecommendations.length > 0 && (
              <CollapsibleSection
              ...
              >
                <div className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700/50 rounded-lg transition-colors duration-200">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-yellow-700 dark:text-yellow-400 font-medium">
                      Portfolio deviation:{" "}
                      {analysis.deviationFromTarget.toFixed(1)}% from target
                      allocation
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {analysis.rebalancingRecommendations.map((rec, index) => (
                    ...
                    >
                      ...
                      <div className="text-sm sm:text-base text-gray-700 dark:text-slate-300 mb-3">
                        {rec.reason}
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs sm:text-sm">
                        ...
                        <div className="bg-gray-100 dark:bg-slate-750 p-2 rounded transition-colors duration-200">
                          ...
                          <span className="text-gray-900 dark:text-white font-semibold">
                            ${rec.valueToTrade.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                {onRebalance && (
                  <button
                    onClick={() =>
                      onRebalance(analysis.rebalancingRecommendations)
                    }
          ...
          {!analysis.needsRebalancing && (
            ...
            >
              ...
              <div className="text-sm sm:text-base text-green-300 mt-3">
                Your current allocation is within acceptable ranges for your{" "}
                {riskTolerance.toLowerCase().replace("_", " ")} risk tolerance.
              </div>
            </div>
          )}
        </div>
