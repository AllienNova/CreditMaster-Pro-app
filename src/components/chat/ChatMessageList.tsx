      ...
      {messages.length === 0 && !isLoading && (
        <div className="flex items-center justify-center h-full">
          <div className="text-center text-gray-500 dark:text-slate-400">
            ...
            <p className="mt-2 text-sm">No messages yet. Start a conversation!</p>
          </div>
        </div>
      )}

      {messages.map((message) => (
        <div key={message.id} className={`flex ${getMessageAlignment(message.role)}`}>
          <div
          ...
          >
            ...
            {message.metadata?.suggestedActions && message.metadata.suggestedActions.length > 0 && (
              <div className="mt-3 space-y-2" data-testid="suggested-actions">
                ...
                {message.metadata.suggestedActions.map((action: { label: string; action?: string }, index: number) => (
                  <button
                    key={index}
            ...
            {message.metadata?.educationalContent && message.metadata.educationalContent.length > 0 && (
              <div className="mt-3 space-y-2">
                <p className="text-xs font-semibold">Learn More:</p>
                {message.metadata.educationalContent.map((content: { title: string; summary?: string }, index: number) => (
                  <div key={index} className="text-xs bg-white dark:bg-slate-800 bg-opacity-20 rounded px-2 py-1">
