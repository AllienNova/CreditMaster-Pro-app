...
export default function DisputeList() {
  ...
  const filteredDisputes = disputes.filter((dispute) => {
    ...
    const matchesBureau =
      bureauFilter === "all" || dispute.bureau === bureauFilter;
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-3">
              ...
              <select
                value={bureauFilter}
                onChange={(e) =>
                  setBureauFilter(e.target.value as Bureau | "all")
                }
                className="px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
              ...
              <div className="p-6 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Previous
                  </button>
                  <button
...
function DisputeCard({ dispute }: { dispute: Dispute }) {
  ...
  return (
    <Link
      ...
      className="block p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors"
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[dispute.status]}`}
            >
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-slate-400">
            ...
            {dispute.estimatedResolutionDate && (
              <span>
                Est. Resolution: {formatDate(dispute.estimatedResolutionDate)}
              </span>
            )}
          </div>
        </div>
