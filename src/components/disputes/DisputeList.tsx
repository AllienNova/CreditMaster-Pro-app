...
function DisputeCard({ dispute }: { dispute: Dispute }) {
  ...
  return (
    <Link href={`/disputes/${dispute.id}`} className="block p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{dispute.itemType}</h3>
            <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[dispute.status]}`}>
              {dispute.status.replace('_', ' ').toUpperCase()}
            </span>
          </div>
          <p className="text-gray-600 dark:text-slate-300 mb-2">{dispute.itemDescription}</p>
