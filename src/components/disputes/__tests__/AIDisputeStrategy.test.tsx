...
describe("AIDisputeStrategy", () => {
  describe("Component Rendering", () => {
    ...
    it("should render dispute strategy after data loads", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/AI Dispute Intelligence/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Strategy recommendations and success predictions/i),
      ).toBeInTheDocument();
    });
    ...
    it("should display dispute information", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Top Dispute Opportunities/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
    ...
    it("should display dispute templates", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Recommended Strategy Templates/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Goodwill Adjustment Request/i),
      ).toBeInTheDocument();
  ...
  describe("User Interactions", () => {
    it("should toggle expand/collapse", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/Top Dispute Opportunities/i),
    ...
    it("should display dispute status", async () => {
      ...
      const opportunitiesElements = screen.getAllByText(
        /Dispute Opportunities/i,
      );
  ...
  describe("Error Handling", () => {
    it("should display error state when API fails", async () => {
      server.use(
        rest.get(
          "http://localhost/api/financial/disputes/ai-strategy",
          (req, res, ctx) => {
            return res(
              ctx.status(500),
      ...
      expect(
        screen.queryByText(/AI Dispute Intelligence/i),
      ).not.toBeInTheDocument();
