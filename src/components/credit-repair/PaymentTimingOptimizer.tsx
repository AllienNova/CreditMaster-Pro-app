...
export default function PaymentTimingOptimizer() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-teal-600 to-cyan-600 rounded-lg p-8 text-white mb-6">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="space-y-4">
          {cards.map((card) => {
            ...
            return (
              <div key={card.id} className={`border-2 rounded-lg p-4 ${isUrgent ? 'border-orange-400 bg-orange-50' : 'border-gray-200'}`}>
                ...
                <div className={`p-4 rounded-lg ${isUrgent ? 'bg-orange-100 border border-orange-300' : 'bg-teal-50 border border-teal-200'}`}>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <div className="text-xs text-gray-600 mb-1">Current Utilization</div>
                      <div className={`text-2xl font-bold ${utilization > 30 ? 'text-red-600' : utilization > 10 ? 'text-yellow-600' : 'text-green-600'}`}>
                        {utilization}%
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-600 mb-1">Optimal Payment Date</div>
                    ...
                    <div>
                      <div className="text-xs text-gray-600 mb-1">Days Until Statement</div>
                      <div className={`text-2xl font-bold ${isUrgent ? 'text-orange-600' : 'text-gray-700'}`}>
                        {daysUntil} days
                      </div>
                      {isUrgent && (
