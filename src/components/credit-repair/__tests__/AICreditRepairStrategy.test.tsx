...
describe("AICreditRepairStrategy", () => {
  describe("Component Rendering", () => {
    ...
    it("should render credit repair strategy after data loads", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Repair Strategy Effectiveness/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/AI Credit Repair Strategy/i),
      ).toBeInTheDocument();
    });
    ...
    it("should display quick wins", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Quick Wins \(Start Today\)/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Pay down Chase card from 78% to 20% utilization/i),
      ).toBeInTheDocument();
    });
    ...
    it("should display prioritized actions", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Priority Repair Actions/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Dispute inaccurate late payment/i),
      ).toBeInTheDocument();
    ...
    it("should display impact predictions", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Score Impact Predictions/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Pay down utilization to 10%/i),
      ).toBeInTheDocument();
    ...
    it("should display timeline estimates", async () => {
      ...
      expect(
        screen.getByText(/Immediate Actions \(0-30 days\)/i),
      ).toBeInTheDocument();
    ...
    it("should display strategy optimizations", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Recommended Strategies/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Aggressive Dispute Strategy/i),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText(/\+45-65 points in 60-90 days/i),
      ).toBeInTheDocument();
    });
    ...
    it("should display risk factors", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Risk Factors to Consider/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Collection agency may not agree to pay-for-delete/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Error Handling", () => {
    it("should display error state when API fails", async () => {
      server.use(
        rest.get(
          "http://localhost/api/financial/credit-repair/ai-strategy",
          (req, res, ctx) => {
            return res(
              ctx.status(500),
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Error loading AI repair strategy/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
