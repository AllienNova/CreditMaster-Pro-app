...
jest.mock('../admin/ModelMonitoring', () => ({
  ModelMonitoring: ({ className }: { className?: string }) => {
    ...
    const models = [
      { id: 'gpt-4o', provider: 'OpenAI', requests: 45000, cost: 892.50, latency: 320, errorRate: 0.015 },
      { id: 'claude-3-opus', provider: 'Anthropic', requests: 28000, cost: 445.20, latency: 280, errorRate: 0.018 },
    ];
    
    const costBreakdown = [
      { category: 'Chat', cost: 892.50, percentage: 48 },
      { category: 'Completion', cost: 445.20, percentage: 24 },
    ...
    return (
      <div className={className} data-testid="model-monitoring">
        ...
        <div data-testid="summary-cards">
          <div data-testid="total-requests">{metrics.totalRequests.toLocaleString()}</div>
          <div data-testid="total-cost">${metrics.totalCost.toFixed(2)}</div>
          <div data-testid="avg-latency">{metrics.avgLatency}ms</div>
          <div data-testid="error-rate">{(metrics.errorRate * 100).toFixed(2)}%</div>
        </div>
        ...
        <div data-testid="cost-breakdown">
          {costBreakdown.map(item => (
            <div key={item.category} data-testid={`cost-${item.category.toLowerCase()}`}>
