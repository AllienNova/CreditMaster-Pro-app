...
const DisputeWizard = ({ onComplete }: { onComplete: (data: unknown) => void }) => {
  ...
  return (
    <div data-testid="dispute-wizard">
      <div data-testid="step-indicator">Step {step} of {steps.length}</div>
...
describe('DisputeWizard Component', () => {
  describe('Step Navigation', () => {
    it('should render first step initially', () => {
      ...
      expect(screen.getByTestId('step-title')).toHaveTextContent('Select Bureau');
    });
...
describe('Dispute Letter Templates', () => {
  it('should have all template types', () => {
    const templates = [
      ...
      { id: 'incorrect_balance', name: 'Incorrect Balance', category: 'accuracy' },
    ];
