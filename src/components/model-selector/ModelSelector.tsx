...
export default function ModelSelector({
  ...
  const [selectedCategory, setSelectedCategory] = useState<AIModel['category'] | 'all'>(category || 'all');
  ...
  const getSpeedBadge = (speed: AIModel['speed']) => {
    const colors = { fast: 'bg-green-100 text-green-700', medium: 'bg-yellow-100 text-yellow-700', slow: 'bg-red-100 text-red-700' };
    return <span className={`text-xs px-2 py-0.5 rounded ${colors[speed]}`}>{speed}</span>;
  };
