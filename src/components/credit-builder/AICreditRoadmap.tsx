        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-green-100 text-sm">
              Personalized path to your credit goals
            </p>
          </div>
        </div>
        <button
          ...
          <div className="bg-white dark:bg-slate-800/10 backdrop-blur-sm rounded-lg p-4">
            ...
            <div className="w-full bg-white dark:bg-slate-800/20 rounded-full h-3 mb-3">
              <div
                ...
                style={{
                  width: `${data.progressMetrics.completionPercentage}%`,
                }}
              ></div>
            </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-start space-x-2">
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-xs font-medium ${getPriorityColor(milestone.priority)}`}
                          >
                            {milestone.priority}
                          </span>
                        </div>
                    ...
                    <div className="text-right flex-shrink-0 ml-3">
                      <div className="text-sm font-medium">
                        Score: {milestone.targetScore}
                      </div>
                      <div className="text-xs text-green-200">
                        {milestone.estimatedDays} days
                      </div>
                      <div className="text-xs text-green-200">
                        {milestone.successProbability}% success
                      </div>
                    </div>
                  </div>
                    ...
                    <ul className="space-y-1">
                      {milestone.actions
                        ...
                        .map((action, actionIdx) => (
                          <li
                            key={actionIdx}
                            className="text-xs text-green-100 flex items-start space-x-1"
                          >
                            <span>•</span>
                            <span>{action}</span>
                          </li>
          ...
          <div className="bg-white dark:bg-slate-800/10 backdrop-blur-sm rounded-lg p-4">
            ...
            <div className="space-y-2">
              {data.timelinePredictions.map((pred, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between text-sm bg-white dark:bg-slate-800/10 rounded-lg p-2"
                >
                  <div>
                    <div className="font-medium">{pred.milestone}</div>
                    <div className="text-xs text-green-200">
                      {pred.currentScore} → {pred.targetScore} (
                      {pred.requiredActions} actions)
                    </div>
                  </div>
              ...
              {data.strategyRecommendations.slice(0, 2).map((strategy) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${getDifficultyColor(strategy.difficulty)}`}
                      >
                        {strategy.difficulty}
                      </span>
          ...
          <div className="bg-white dark:bg-slate-800/10 backdrop-blur-sm rounded-lg p-4">
            ...
            <ul className="space-y-2">
              {data.nextSteps.map((step, idx) => (
                <li
                  key={idx}
                  className="text-sm text-green-100 flex items-start space-x-2"
                >
