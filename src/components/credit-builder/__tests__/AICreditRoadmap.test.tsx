...
describe("AICreditRoadmap", () => {
  describe("Component Rendering", () => {
    ...
    it("should render credit roadmap after data loads", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/AI Credit Building Roadmap/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
    ...
    it("should display milestones", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Credit Building Milestones/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Reduce credit utilization below 30%/i),
      ).toBeInTheDocument();
    ...
    it("should display timeline predictions", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Score Timeline Predictions/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
    ...
    it("should display progress metrics", async () => {
      ...
      expect(
        screen.getByText(/on track to reach your goal/i),
      ).toBeInTheDocument();
    });
  ...
  describe("User Interactions", () => {
    it("should toggle expand/collapse", async () => {
      ...
      if (toggleButton) {
        ...
        await waitFor(() => {
          expect(
            screen.queryByText(/Roadmap Progress/i),
  ...
  describe("Error Handling", () => {
    it("should display error state when API fails", async () => {
      server.use(
        rest.get(
          "http://localhost/api/financial/credit-builder/ai-roadmap",
          (req, res, ctx) => {
            return res(
              ctx.status(500),
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Error loading AI roadmap/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
      ...
      expect(
        screen.getByText(/Failed to fetch AI credit roadmap/i),
      ).toBeInTheDocument();
