...
interface DebtAccount {
  id: string;
  name: string;
  type: 'credit_card' | 'student_loan' | 'auto_loan' | 'mortgage' | 'personal_loan';
  balance: number;
  interestRate: number;
...
export default function DebtManagement() {
  ...
  const fetchDebts = useCallback(async () => {
    ...
    try {
      ...
      const debtAccounts: DebtAccount[] = data.data
        .filter((a: PlaidAccount) => a.accountType === 'credit' || a.accountType === 'loan')
        .map((a: PlaidAccount) => ({
          id: a.id,
          name: a.accountName,
  ...
  const calculatePayoffStrategy = (): PayoffStrategy | null => {
    ...
    while (remainingDebts.some(d => d.balance > 0) && month < 360) { // Max 30 years
      ...
      const totalBalance = remainingDebts.reduce((sum, d) => sum + Math.max(0, d.balance), 0);
      timeline.push({
        month,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Min Payment</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-gray-900 dark:text-white">{formatCurrency(totalMinimum)}</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">Per month</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Avg Interest</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-orange-600">{avgInterestRate.toFixed(1)}%</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">APR</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Payoff Date</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">
            {payoffStrategy ? payoffStrategy.payoffDate.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : 'N/A'}
          </div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">
            {payoffStrategy ? `${Math.ceil((payoffStrategy.payoffDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24 * 30))} months` : ''}
          </div>
        </div>
      </div>
      ...
      {/* Strategy Selector */}
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Payoff Strategy</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-2">Method</label>
            <select
              value={strategy}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-2">Extra Monthly Payment</label>
            <input
              type="number"
              value={extraPayment}
      ...
      {debts.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-12">
          <div className="text-center max-w-md mx-auto">
            ...
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">No Debts Found!</h3>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow">
          <div className="p-6 border-b border-gray-200 dark:border-slate-700">
            ...
            <p className="text-sm text-gray-600 dark:text-slate-300 mt-1">
              Sorted by {strategy === 'avalanche' ? 'highest interest rate' : 'smallest balance'}
            </p>
          </div>
          <div className="divide-y divide-gray-200 dark:divide-slate-700">
            {[...debts].sort((a, b) => {
              ...
              <div key={debt.id} className="p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{debt.name}</h3>
      ...
      {payoffStrategy && (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">12-Month Payoff Timeline</h3>
          <div className="space-y-3">
            {payoffStrategy.timeline.map((month) => {
              ...
              return (
                <div key={month.month}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold text-gray-900 dark:text-white">Month {month.month}</span>
                    <span className="font-bold text-red-600">{formatCurrency(month.balance)}</span>
                  </div>
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg">
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-700 dark:text-slate-200">Total Interest Paid:</span>
              <span className="font-bold text-gray-900 dark:text-white">{formatCurrency(payoffStrategy.totalInterest)}</span>
            </div>
          </div>
        </div>
      )}
