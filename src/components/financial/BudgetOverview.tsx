...
export default function BudgetOverview({ analysis, period }: BudgetOverviewProps) {
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-yellow-800 space-y-1">
                {analysis.categoriesOverBudget > 0 && (
                  <div>
                    <span className="font-medium">{analysis.categoriesOverBudget}</span> {analysis.categoriesOverBudget === 1 ? 'category is' : 'categories are'} over budget
                  </div>
                )}
                {analysis.categoriesNearLimit > 0 && (
                  <div>
                    <span className="font-medium">{analysis.categoriesNearLimit}</span> {analysis.categoriesNearLimit === 1 ? 'category is' : 'categories are'} near the limit (90%+)
                  </div>
                )}
              </div>
            </div>
