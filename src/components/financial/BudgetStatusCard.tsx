...
export default function BudgetStatusCard() {
  const { user } = useAuth();
  const [budgetStatus, setBudgetStatus] = useState<BudgetStatusData | null>(
    null,
  );
  ...
  const fetchBudgetStatus = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        "/api/financial/budgets/analyze?period=monthly",
      );
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-gray-500 dark:text-slate-400">
          {budgetStatus.daysRemaining} days left
        </span>
      </div>
            ...
            <div className="flex-1 text-sm">
              {budgetStatus.categoriesOverBudget > 0 && (
                <div className="text-red-700 font-medium">
                  ...
                  {budgetStatus.categoriesOverBudget === 1
                    ? "category"
                    : "categories"}{" "}
                  over budget
                </div>
              ...
              {budgetStatus.categoriesNearLimit > 0 && (
                <div className="text-yellow-700">
                  {budgetStatus.categoriesNearLimit}{" "}
                  {budgetStatus.categoriesNearLimit === 1
                    ? "category"
                    : "categories"}{" "}
                  near limit
                </div>
          ...
          <div key={category.category} className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs px-2 py-1 rounded-full ${getStatusColor(category.status)}`}
                >
