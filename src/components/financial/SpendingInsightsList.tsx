...
export default function SpendingInsightsList({ timeRange = '30d' }: SpendingInsightsListProps) {
  const { user } = useAuth();
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/financial/spending/insights?timeRange=${timeRange}`);
