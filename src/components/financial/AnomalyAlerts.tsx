...
export default function AnomalyAlerts({
  ...
  const fetchAnomalies = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/financial/spending/anomalies?timeRange=${timeRange}`,
      );
  ...
  const getSeverityColor = (severity: string): string => {
    switch (severity) {
      ...
      default:
        return "bg-gray-100 dark:bg-slate-800 border-gray-300 dark:border-slate-600 text-gray-800 dark:text-slate-100";
    }
  };
                  ...
                  {anomaly.suggestions && anomaly.suggestions.length > 0 && (
                    <div className="space-y-1">
                      ...
                      {anomaly.suggestions.map((suggestion, idx) => (
                        <div
                          key={idx}
                          className="text-xs text-gray-600 dark:text-slate-300 flex items-start gap-2"
                        >
                          <span>•</span>
