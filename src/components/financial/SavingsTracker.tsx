    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow p-6 text-white">
          ...
          <div className="text-3xl font-bold">{formatCurrency(data.totalSavings)}</div>
          <div className="text-sm opacity-90 mt-1">{data.accounts.length} accounts</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Monthly Growth</h3>
            <span className="text-2xl"></span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Interest Earned</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-blue-600">{formatCurrency(data.interestEarned)}</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">This month</div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          ...
          <div className={`text-3xl font-bold ${data.savingsRate >= 20 ? 'text-green-600' : 'text-orange-600'}`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow">
        <div className="p-6 border-b border-gray-200 dark:border-slate-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Savings Accounts</h2>
          <p className="text-sm text-gray-600 dark:text-slate-300 mt-1">{data.accounts.length} accounts</p>
        </div>
            ...
            <div key={account.id} className="p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold text-green-600">{formatCurrency(account.currentBalance)}</div>
                  {account.availableBalance !== undefined && (
                    <div className="text-sm text-gray-500 dark:text-slate-400">Available: {formatCurrency(account.availableBalance)}</div>
                  )}
                </div>
              </div>
            </div>
      ...
      {/* Growth History */}
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Savings Growth History</h3>
        <div className="space-y-3">
          {data.growthHistory.map((month) => {
            const maxAmount = Math.max(...data.growthHistory.map(m => Math.abs(m.amount)));
            ...
            return (
              <div key={month.month}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`font-bold ${month.amount >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {/* Projections */}
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">6-Month Savings Projection</h3>
        <div className="space-y-3">
          {data.projections.map((proj, index) => {
            const maxAmount = Math.max(...data.projections.map(p => p.projected));
            ...
            return (
              <div key={index}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold text-green-600">{formatCurrency(proj.projected)}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-3">
                  <div
