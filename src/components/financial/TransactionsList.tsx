...
useEffect(() => {
  ...
  const filteredTransactions = transactions
    .filter(txn => {
      // Search filter
      if (searchQuery && !txn.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
      ...
      // Category filter
      if (selectedCategory !== 'all' && !txn.category.includes(selectedCategory)) {
        return false;
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Total Transactions</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-gray-900 dark:text-white">{transactions.length}</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">Last {dateRange} days</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Total Income</h3>
            <span className="text-2xl"></span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Total Expenses</h3>
            <span className="text-2xl"></span>
          </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          {/* Account Filter */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-2">Account</label>
            <select
          ...
          {/* Category Filter */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-2">Category</label>
            <select
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow">
        ...
        <div className="divide-y divide-gray-200 dark:divide-slate-700">
          {filteredTransactions.map((txn) => (
            <div key={txn.id} className="p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{txn.name}</h3>
                    {txn.pending && (
                ...
                <div className={`text-2xl font-bold ${txn.amount > 0 ? 'text-red-600' : 'text-green-600'}`}>
