  ...
  const fetchActionPlans = useCallback(async () => {
    ...
    try {
      ...
      const plans: ActionPlan[] =
        data.recommendations?.map((rec: ApiRecommendation, idx: number) => ({
          id: rec.id || `plan-${idx}`,
          title: rec.title,
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-lg shadow">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">
            {filter === "completed"
              ...
              : "Get started by visiting your AI Coach"}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              className={`border-2 rounded-lg p-6 cursor-pointer hover:shadow-lg transition-all ${
                plan.completed
                  ...
                  : "bg-white dark:bg-slate-800 " +
                    getPriorityColor(plan.priority).split(" ")[2]
              }`}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center mb-2">
                    <div
                      className={`w-3 h-3 rounded-full mr-2 ${getPriorityBadgeColor(plan.priority)}`}
                    />
              ...
              <div className="space-y-3">
                ...
                <div className="text-xs text-gray-500 dark:text-slate-400">
                  {plan.steps.filter((s) => s.completed).length} of{" "}
        ...
        >
          <div className="space-y-6">
            <div className="flex items-center space-x-2">
              <span
                className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getPriorityColor(selectedPlan.priority)}`}
              >
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm text-gray-500 dark:text-slate-400">
                  {selectedPlan.steps.filter((s) => s.completed).length}/
                  {selectedPlan.steps.length} completed
                </span>
              </div>
                ...
                {selectedPlan.steps.map((step, idx) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        handleToggleStep(selectedPlan.id, step.id)
                      }
                    ...
                    <div className="flex-1">
                      <span
                        className={`text-sm ${step.completed ? "line-through text-gray-500" : "text-gray-900 dark:text-white"}`}
                      >
