...
export default function InvestmentPortfolio() {
  ...
  const fetchInvestmentData = useCallback(async () => {
    ...
    try {
      ...
      const allocation = [
        ...
        { type: 'Cash', value: totalValue * 0.10, percentage: 10 },
  ...
  const getAllocationColor = (index: number): string => {
    const colors = ['bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-blue-500'];
    ...
    return (
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-12">
        <div className="text-center max-w-md mx-auto">
          ...
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">No Investment Accounts</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow p-6 text-white">
          ...
          <div className="text-sm opacity-90 mt-1">{data.accounts.length} accounts</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Total Gain</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-green-600">{formatCurrency(data.totalGain)}</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">All time</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Return</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-green-600">+{data.gainPercentage.toFixed(1)}%</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">Percentage gain</div>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-slate-300">Accounts</h3>
            <span className="text-2xl"></span>
          </div>
          <div className="text-3xl font-bold text-gray-900 dark:text-white">{data.accounts.length}</div>
          <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">Investment accounts</div>
        </div>
      </div>

      {/* Asset Allocation */}
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Asset Allocation</h3>
        <div className="space-y-4">
          {data.allocation.map((asset, index) => (
            <div key={asset.type}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className={`w-4 h-4 ${getAllocationColor(index)} rounded`}></div>
                  <span className="font-semibold text-gray-900 dark:text-white">{asset.type}</span>
                </div>
                <div className="text-right">
                  <div className="font-bold text-gray-900 dark:text-white">{formatCurrency(asset.value)}</div>
                  <div className="text-sm text-gray-500 dark:text-slate-400">{asset.percentage}%</div>
                </div>
              </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow">
        <div className="p-6 border-b border-gray-200 dark:border-slate-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Investment Accounts</h2>
        </div>
        <div className="divide-y divide-gray-200 dark:divide-slate-700">
          {data.accounts.map((account) => (
            <div key={account.id} className="p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{account.accountName}</h3>
