...
export default function AIBudgetOptimizer() {
  ...
  const fetchOptimizations = useCallback(async () => {
    ...
    try {
      ...
      const recommendations: BudgetRecommendation[] = (result.data || []).map(
        (rec: ApiRecommendation) => ({
          ...
          potentialSavings:
            ...
            Math.abs(rec.currentAmount - rec.suggestedAmount),
          confidence:
            rec.impact === "high" ? 90 : rec.impact === "medium" ? 75 : 60,
          priority: rec.impact,
        }),
      );
      ...
      const optimizationScore = Math.min(
        100,
        Math.max(0, 100 - recommendations.length * 10),
      );
  ...
  const handleApplyRecommendation = async (
    ...
    try {
      const response = await fetch(
        "/api/financial/budgets/apply-recommendation",
        {
          method: "POST",
      ...
      toast.success(
        "Budget updated!",
        `${recommendation.category} budget optimized`,
      );
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium border ${getPriorityColor(recommendation.priority)}`}
                        >
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        <div>
                          ...
                          <span className="font-semibold">
                            {formatCurrency(recommendation.currentAmount)}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-500 dark:text-slate-400">
