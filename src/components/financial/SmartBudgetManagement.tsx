...
export default function SmartBudgetManagement() {
  ...
  const fetchBudget = useCallback(async () => {
    ...
    try {
      ...
      const budgetResponse = await fetch(
        `/api/financial/budgets?activeOnly=true`,
      );
      ...
      const analysisResponse = await fetch(
        `/api/financial/budgets/analyze?period=${selectedPeriod}`,
      );
  ...
  return (
    <div className="space-y-6" data-testid="smart-budget-page">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-slate-300 mt-1">
            AI-powered budget management and recommendations
          </p>
        </div>
      ...
      {/* Budget Overview */}
      {analysis && (
        <BudgetOverview analysis={analysis} period={selectedPeriod} />
      )}
