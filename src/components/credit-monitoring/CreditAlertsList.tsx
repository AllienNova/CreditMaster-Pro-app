...
export default function CreditAlertsList({
  alerts,
  onRefresh,
}: CreditAlertsListProps) {
  const { user } = useAuth();
  ...
  const formatDate = (date: Date): string => {
    ...
    return alertDate.toLocaleDateString("en-US", {
      ...
      day: "numeric",
      year:
        alertDate.getFullYear() !== now.getFullYear() ? "numeric" : undefined,
    });
  };
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h4
                        className={`font-semibold ${!alert.read ? "text-gray-900" : "text-gray-700 dark:text-slate-200"}`}
                      >
                        {alert.title}
                      </h4>
                      {!alert.read && (
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      {alert.bureau && (
                        <span className="px-2 py-1 bg-gray-100 dark:bg-slate-800 rounded text-gray-700 dark:text-slate-200">
                          {alert.bureau.charAt(0).toUpperCase() +
                            alert.bureau.slice(1)}
                        </span>
                      )}
                      <span
                        className={`px-2 py-1 rounded border ${getSeverityColor(alert.severity)}`}
                      >
                        {alert.severity}
                      </span>
                    </div>
