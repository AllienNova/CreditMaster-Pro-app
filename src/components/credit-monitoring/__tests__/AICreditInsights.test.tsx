...
describe('AICreditInsights', () => {
  describe('Component Rendering', () => {
    ...
    it('should render credit insights after data loads', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/AI Credit Intelligence/i)).toBeInTheDocument();
      }, { timeout: 3000 });

      expect(screen.getByText(/Score predictions and improvement strategies/i)).toBeInTheDocument();
    });
    ...
    it('should display factor impacts', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Factor Impact Analysis/i)).toBeInTheDocument();
      }, { timeout: 3000 });
    ...
    it('should display improvement opportunities', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Improvement Opportunities/i)).toBeInTheDocument();
      }, { timeout: 3000 });

      // Check for opportunity title
      expect(screen.getByText(/Reduce credit utilization/i)).toBeInTheDocument();
    ...
    it('should display credit alerts', async () => {
      ...
      expect(screen.getByText(/Credit utilization increased by 15%/i)).toBeInTheDocument();
  ...
  describe('User Interactions', () => {
    it('should toggle expand/collapse', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText(/Credit Health Score/i)).not.toBeInTheDocument();
      });
  ...
  describe('Error Handling', () => {
    it('should display error state when API fails', async () => {
      server.use(
        rest.get('http://localhost/api/financial/credit/ai-insights', (req, res, ctx) => {
          return res(
            ctx.status(500),
      ...
      expect(screen.queryByText(/AI Credit Intelligence/i)).not.toBeInTheDocument();
      expect(screen.queryByText(/Credit Health Score/i)).not.toBeInTheDocument();
    });
