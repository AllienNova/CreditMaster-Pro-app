        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm opacity-90">
              Score predictions and improvement strategies
            </p>
          </div>
        </div>
        <button
            ...
            {data.predictions[0] && (
              <div className="bg-white dark:bg-slate-800/10 backdrop-blur-sm rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-2xl font-bold">
                    {data.predictions[0].predictedScore}
                  </span>
                </div>
                ...
                <p className="text-xs opacity-75 mt-2">
                  {data.predictions[0].predictedScore > 700
                    ? "Trending up"
                    : "Stable"}
                </p>
              </div>
              ...
              {data.predictions.map((pred) => (
                ...
                >
                  <div className="text-xs opacity-75 mb-1">
                    {pred.timeframe === "30_days"
                      ? "30 Days"
                      : pred.timeframe === "90_days"
                        ? "90 Days"
                        : "6 Months"}
                  </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-xs ${getImpactColor(factor.currentImpact)}`}
                        >
                          {factor.currentImpact === "positive"
                            ? "↑"
                            : factor.currentImpact === "negative"
                              ? "↓"
                              : "→"}
                        </span>
                      </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full border ${getDifficultyColor(opp.difficulty)}`}
                        >
                          {opp.difficulty}
                        </span>
                      </div>
                ...
                {data.alerts.slice(0, 3).map((alert) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <span
                        className={`text-xs px-2 py-1 rounded border ${getSeverityColor(alert.severity)}`}
                      >
                        {alert.severity}
                      </span>
                      <div className="flex-1">
