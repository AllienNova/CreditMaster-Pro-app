...
export function Watchlist({
    ...
    result.sort((a, b) => {
      ...
      switch (currentSortBy) {
        ...
        case 'signal':
          ...
          comparison = (signalOrder[a.signal || 'undefined'] || 0) - (signalOrder[b.signal || 'undefined'] || 0);
          break;
      }
  ...
  return (
    <div className={`watchlist bg-gray-900 rounded-lg overflow-hidden ${className}`}>
      {/* Header */}
      ...
      <div className="divide-y divide-gray-800 max-h-[500px] overflow-y-auto">
        {filteredItems.length === 0 ? (
          <div className="px-4 py-8 text-center text-gray-500 dark:text-slate-400">
            {searchQuery ? 'No matching symbols found' : 'No items in watchlist'}
          </div>
        ) : (
  ...
  return (
    ...
    >
      ...
      <div className="col-span-2">
        ...
        <div className="text-xs text-gray-500 dark:text-slate-400 truncate">{item.name}</div>
      </div>
      ...
      <div className="col-span-2 flex justify-end gap-1">
        {showActions ? (
          <>
            <button
              onClick={(e) => { e.stopPropagation(); onTrade?.(item.symbol, 'buy'); }}
              className="px-2 py-1 text-xs bg-green-600 hover:bg-green-700 text-white rounded"
            >
              Buy
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); onTrade?.(item.symbol, 'sell'); }}
...
function SignalBadge({ signal, strength }: { signal: 'buy' | 'sell' | 'hold'; strength?: number }) {
  const colors = {
  ...
  return (
    <div className={`flex items-center gap-1 px-2 py-1 rounded border ${colors[signal]}`}>
