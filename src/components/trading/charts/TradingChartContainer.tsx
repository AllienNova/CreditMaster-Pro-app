...
export function TradingChartContainer({
  ...
  const displayOHLCV =
    crosshairInfo?.ohlcv || (data.length ? data[data.length - 1] : null);

  return (
    <div
      className={`trading-chart-container bg-gray-900 rounded-lg overflow-hidden ${className}`}
    >
      {/* Header */}
        ...
        <div className="flex items-center gap-4">
          ...
          {currentPriceInfo && (
            <div className="flex items-center gap-3">
              ...
              >
                {currentPriceInfo.change >= 0 ? "+" : ""}
                {currentPriceInfo.change.toFixed(2)} (
          ...
          <div className="hidden md:flex items-center gap-4 text-sm">
            ...
            <div className="flex gap-1">
              ...
              <span className="text-gray-300">
                {(displayOHLCV.volume / 1000000).toFixed(2)}M
              </span>
            </div>
          </div>
        )}
      ...
      {showIndicatorPanel && (
        <div className="px-4 py-2 bg-gray-800/30 border-b border-gray-700">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {INDICATOR_OPTIONS.filter((i) => i.category === "overlay").map(
              (ind) => (
                <button
                  key={ind.id}
                  onClick={() =>
                    toggleIndicator(ind.id as keyof IndicatorSettings)
                  }
            ...
            <span className="text-xs text-gray-500 dark:text-slate-400 ml-4 mr-2">
              Oscillators:
            </span>
          ...
          <button
            onClick={() =>
              onTradeExecute?.({
                ...
                side: "buy",
                quantity: 100,
                price: currentPriceInfo?.price || 0,
              })
            }
