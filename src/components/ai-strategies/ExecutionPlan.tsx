...
import type { StrategyExecutionPlanSummary, ExecutionPlanAction, ManualInterventionTask } from '@/types/ai-strategy';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-blue-50 rounded-lg p-6 border border-blue-200">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Execution Plan</h2>
            <p className="text-gray-700 dark:text-slate-200">
              ID: <span className="font-mono text-sm">{plan.execution_id}</span>
            </p>
          </div>
          <span className={`px-4 py-2 rounded-full text-sm font-medium border ${getStatusColor(plan.status)}`}>
            {plan.status.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          {plan.started_at && (
            <div>
              <p className="text-xs text-gray-600 dark:text-slate-300 mb-1">Started</p>
              <p className="text-sm font-medium text-gray-900 dark:text-white">{formatDate(plan.started_at)}</p>
            </div>
          )}
          {plan.completed_at && (
            <div>
              <p className="text-xs text-gray-600 dark:text-slate-300 mb-1">Completed</p>
              <p className="text-sm font-medium text-gray-900 dark:text-white">{formatDate(plan.completed_at)}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {plan.automated_actions && plan.automated_actions.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Automated Actions</h3>
          ...
          <div className="space-y-3">
            {plan.automated_actions.map((action: ExecutionPlanAction, index: number) => (
              <div
                key={index}
              ...
              >
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-gray-900 dark:text-white mb-1">
                    {action.action_name || action.primary_strategy || `Action ${index + 1}`}
                  </h4>
                  <p className="text-sm text-gray-600 dark:text-slate-300">
                    {action.description || action.strategy_type || 'Automated action'}
                  </p>
      ...
      {plan.manual_interventions && plan.manual_interventions.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Manual Interventions Required</h3>
          ...
          <div className="space-y-3">
            {plan.manual_interventions.map((intervention: ManualInterventionTask, index: number) => (
              <div key={index} className="flex items-start gap-3 p-4 bg-orange-50 rounded-lg border border-orange-200">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-gray-900 dark:text-white mb-1">
                    {intervention.task_name || intervention.step_name || `Task ${index + 1}`}
                  </h4>
                  <p className="text-sm text-gray-600 dark:text-slate-300 mb-2">
                    {intervention.description || intervention.action || 'Manual task required'}
                  </p>
      ...
      {plan.monitoring_schedule && (
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Monitoring Schedule</h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {plan.monitoring_schedule.estimated_timeline && (
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-lg">
                <p className="text-xs text-gray-600 dark:text-slate-300 mb-1">Estimated Timeline</p>
            ...
            {plan.monitoring_schedule.parallel_execution_groups && (
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-lg">
                <p className="text-xs text-gray-600 dark:text-slate-300 mb-1">Parallel Groups</p>
                <p className="text-lg font-bold text-gray-900 dark:text-white">
            ...
            {plan.monitoring_schedule.success_probability && (
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-lg">
                <p className="text-xs text-gray-600 dark:text-slate-300 mb-1">Success Probability</p>
                <p className="text-lg font-bold text-green-600">
                  {Math.round(plan.monitoring_schedule.success_probability * 100)}%
                </p>
              </div>
            )}
            ...
            {plan.monitoring_schedule.risk_level && (
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-lg">
                <p className="text-xs text-gray-600 dark:text-slate-300 mb-1">Risk Level</p>
