      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <span
              className={`px-3 py-1 rounded-full text-sm font-bold border-2 ${getRankBadgeColor(rank)}`}
            >
              #{rank}
            </span>
            <div>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                {strategy.strategy_orchestration?.primary_strategy
                  ?.strategy_name || "Strategy Recommendation"}
              </h3>
          ...
          <div className="text-right">
            <div
              className={`text-2xl font-bold ${getConfidenceColor(strategy.ai_confidence_score)}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
            ...
            <p className="text-xs text-gray-600 dark:text-slate-300 mt-1">
              {getAutomationLevelLabel(strategy.automation_level)}
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {strategy.strategy_orchestration?.strategy_type ||
                "Multi-faceted Approach"}
            </p>
            <p className="text-xs text-gray-600 dark:text-slate-300 mt-1">
              {strategy.strategy_orchestration?.parallel_strategies?.length ||
                0}{" "}
              parallel actions
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 dark:bg-slate-900 rounded-lg">
            <div>
              ...
              <p className="text-lg font-bold text-gray-900 dark:text-white">
                {Math.round(
                  strategy.predictive_analysis.predicted_success_rate * 100,
                )}
                %
              </p>
            </div>
