...
describe('AISpendingInsights', () => {
  describe('Component Rendering', () => {
    ...
    it('should display spending anomalies', async () => {
      ...
      expect(screen.getByText(/Unusual spending spike in Shopping category/i)).toBeInTheDocument();
    ...
    it('should display recommendations', async () => {
      ...
      expect(screen.getByText(/Smart Reduction Recommendations/i)).toBeInTheDocument();
  ...
  describe('Error Handling', () => {
    it('should display error state when API fails', async () => {
      server.use(
        rest.get('http://localhost/api/financial/spending/ai-insights', (req, res, ctx) => {
          return res(
            ctx.status(500),
      ...
      expect(screen.queryByText(/AI Spending Intelligence/i)).not.toBeInTheDocument();
    });
