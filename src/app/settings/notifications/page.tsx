...
export default function NotificationsSettingsPage() {
  ...
  return (
    <div>
      ...
      {/* Channel Headers */}
      <div className="hidden md:grid grid-cols-[1fr,80px,80px,80px] gap-4 pb-4 border-b border-gray-200 text-sm font-medium text-gray-500">
        ...
        {settings.map((setting) => (
          <div key={setting.id} className="py-4 grid md:grid-cols-[1fr,80px,80px,80px] gap-4 items-center">
            ...
            <div className="flex md:justify-center items-center gap-2">
              <span className="md:hidden text-sm text-gray-500">Email:</span>
              <button
              ...
              >
                <span className={`block w-4 h-4 bg-white rounded-full shadow transform transition ${setting.email ? "translate-x-5" : "translate-x-1"}`} />
              </button>
            </div>
            <div className="flex md:justify-center items-center gap-2">
              <span className="md:hidden text-sm text-gray-500">Push:</span>
              <button
                onClick={() => toggleSetting(setting.id, "push")}
                className={`w-10 h-6 rounded-full transition ${setting.push ? "bg-emerald-500" : "bg-gray-300"}`}
              >
                <span className={`block w-4 h-4 bg-white rounded-full shadow transform transition ${setting.push ? "translate-x-5" : "translate-x-1"}`} />
              </button>
            </div>
            <div className="flex md:justify-center items-center gap-2">
              <span className="md:hidden text-sm text-gray-500">SMS:</span>
              <button
                onClick={() => toggleSetting(setting.id, "sms")}
                className={`w-10 h-6 rounded-full transition ${setting.sms ? "bg-emerald-500" : "bg-gray-300"}`}
              >
                <span className={`block w-4 h-4 bg-white rounded-full shadow transform transition ${setting.sms ? "translate-x-5" : "translate-x-1"}`} />
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="mt-8 pt-8 border-t border-gray-200">
        ...
        <p className="text-sm text-gray-500 mb-4">Pause non-urgent notifications during specific hours</p>
