...
const defaultSettings: NotificationSetting[] = [
  {
    ...
    description: "Get notified when your credit score changes",
    email: true,
    push: true,
    sms: false,
  },
  {
    ...
    description: "Alert when new accounts appear on your report",
    email: true,
    push: true,
    sms: true,
  },
  {
    ...
    description: "Updates on your dispute status and results",
    email: true,
    push: true,
    sms: false,
  },
  ...
  },
  {
    ...
    description: "Credit tips, product updates, and offers",
    email: false,
    push: false,
    sms: false,
  },
...
export default function NotificationsSettingsPage() {
  ...
  const toggleSetting = (id: string, channel: "email" | "push" | "sms") => {
    setSettings(
      settings.map((s) => (s.id === id ? { ...s, [channel]: !s[channel] } : s)),
    );
  };
    ...
    <div>
      ...
      <div className="mb-8 p-6 bg-gradient-to-r from-blue-50 to-blue-50 rounded-xl border border-blue-100">
        ...
        <p className="text-sm text-gray-600 dark:text-slate-300 mb-4">
          Get instant alerts in your browser even when you&apos;re not on the
          site.
        </p>
      ...
      <div className="divide-y divide-gray-100 dark:divide-slate-700">
        {settings.map((setting) => (
          <div
            key={setting.id}
            className="py-4 grid md:grid-cols-[1fr,80px,80px,80px] gap-4 items-center"
          >
            <div>
            ...
            <div className="flex md:justify-center items-center gap-2">
              ...
              >
                <span
                  className={`block w-4 h-4 bg-white dark:bg-slate-800 rounded-full shadow transform transition ${setting.email ? "translate-x-5" : "translate-x-1"}`}
                />
              </button>
            </div>
            <div className="flex md:justify-center items-center gap-2">
              ...
              >
                <span
                  className={`block w-4 h-4 bg-white dark:bg-slate-800 rounded-full shadow transform transition ${setting.push ? "translate-x-5" : "translate-x-1"}`}
                />
              </button>
            </div>
            <div className="flex md:justify-center items-center gap-2">
              ...
              >
                <span
                  className={`block w-4 h-4 bg-white dark:bg-slate-800 rounded-full shadow transform transition ${setting.sms ? "translate-x-5" : "translate-x-1"}`}
                />
              </button>
            </div>
          </div>
