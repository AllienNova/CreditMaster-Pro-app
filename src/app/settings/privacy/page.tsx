      ...
      <div className="space-y-6">
        <div className="pb-6 border-b border-gray-200 dark:border-slate-700">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() =>
                  setSettings({
                    ...settings,
                    shareDataWithPartners: !settings.shareDataWithPartners,
                  })
                }
                className={`w-12 h-6 rounded-full transition ${settings.shareDataWithPartners ? "bg-emerald-500" : "bg-gray-300"}`}
              >
                <span
                  className={`block w-5 h-5 bg-white dark:bg-slate-800 rounded-full shadow transform transition ${settings.shareDataWithPartners ? "translate-x-6" : "translate-x-0.5"}`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <span
                  className={`block w-5 h-5 bg-white dark:bg-slate-800 rounded-full shadow transform transition ${settings.allowAnalytics ? "translate-x-6" : "translate-x-0.5"}`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <span
                  className={`block w-5 h-5 bg-white dark:bg-slate-800 rounded-full shadow transform transition ${settings.allowPersonalizedAds ? "translate-x-6" : "translate-x-0.5"}`}
                />
              </button>
            </div>
          </div>
        ...
        <div className="pb-6 border-b border-gray-200 dark:border-slate-700">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">
            Choose how long we keep your historical data
          </p>
          <select
            value={settings.dataRetentionPeriod}
            onChange={(e) =>
              setSettings({ ...settings, dataRetentionPeriod: e.target.value })
            }
