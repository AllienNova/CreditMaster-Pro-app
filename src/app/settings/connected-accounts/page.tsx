...
export default function ConnectedAccountsPage() {
  ...
  const handleReconnect = (id: string) => {
    setAccounts(accounts.map((a) => a.id === id ? { ...a, status: "pending" as const } : a));
    setTimeout(() => {
      setAccounts((prev) => prev.map((a) => a.id === id ? { ...a, status: "connected" as const, lastSync: "Just now" } : a));
    }, 2000);
  };
  ...
  const getStatusBadge = (status: ConnectedAccount["status"]) => {
    switch (status) {
      case "connected":
        return <span className="px-2 py-1 bg-emerald-100 text-emerald-700 text-xs rounded-full">Connected</span>;
      case "pending":
            ...
            <div key={account.id} className="flex items-center justify-between p-4 border border-gray-200 dark:border-slate-700 rounded-lg">
              ...
              <div className="flex items-center gap-3">
                ...
                {account.status === "error" ? (
                  <button onClick={() => handleReconnect(account.id)} className="text-sm text-emerald-500 hover:text-emerald-600 font-medium">
                    Reconnect
                  </button>
