          ...
          <div className="mt-4 text-center">
            <span className="text-3xl font-bold text-green-600">
              ...
              {(data?.creditHistory[data.creditHistory.length - 1]?.score ||
                0) - (data?.creditHistory[0]?.score || 0)}
            </span>
            ...
            <div className="space-y-3">
              {data?.scoreFactors.map((factor, i) => (
                <div key={i} className="flex items-center">
                  <span className="w-32 text-sm text-gray-600 dark:text-slate-300">
                    {factor.factor}
                  </span>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data?.recommendations.map((rec, i) => (
              <div
                key={i}
                className="flex items-start p-4 bg-gradient-to-r from-blue-50 to-blue-50 rounded-lg"
              >
