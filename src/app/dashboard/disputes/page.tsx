    ...
    const fetchDisputes = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Mock data
          setDisputes([
            { id: '1', bureau: 'experian', status: 'resolved', item_type: 'Late Payment', item_description: 'Capital One late payment March 2023', created_at: '2024-10-15T10:00:00Z', outcome: 'removed' },
            { id: '2', bureau: 'equifax', status: 'under_review', item_type: 'Collection', item_description: 'Medical collection ABC Collections', created_at: '2024-11-01T09:15:00Z', outcome: null },
            { id: '3', bureau: 'transunion', status: 'sent', item_type: 'Inquiry', item_description: 'Unauthorized hard inquiry XYZ Lender', created_at: '2024-11-10T16:45:00Z', outcome: null },
            { id: '4', bureau: 'experian', status: 'draft', item_type: 'Balance Error', item_description: 'Incorrect balance on Chase card', created_at: '2024-11-15T11:20:00Z', outcome: null },
          ]);
        }
      } catch {
        setDisputes([
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-cyan-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {['all', 'draft', 'sent', 'under_review', 'resolved'].map(status => (
            <button
              key={status}
            ...
            >
              ...
              <p className="text-sm capitalize">{status === 'all' ? 'Total' : status.replace('_', ' ')}</p>
            </button>
          ))}
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`font-semibold capitalize ${getBureauColor(dispute.bureau)}`}>
                      {dispute.bureau}
                    </span>
