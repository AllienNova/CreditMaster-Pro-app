...
const gradeColors: Record<VitalityGrade, { bg: string; text: string; border: string }> = {
  A: { bg: 'bg-emerald-100 dark:bg-emerald-900/30', text: 'text-emerald-600 dark:text-emerald-400', border: 'border-emerald-500' },
  B: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-600 dark:text-blue-400', border: 'border-blue-500' },
  C: { bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-600 dark:text-amber-400', border: 'border-amber-500' },
  D: { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-600 dark:text-orange-400', border: 'border-orange-500' },
  F: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-600 dark:text-red-400', border: 'border-red-500' },
};
...
function ScoreRing({ score, grade, size = 200 }: { score: number; grade: VitalityGrade; size?: number }) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className={`text-2xl font-bold mt-1 px-3 py-1 rounded-lg ${gradeStyle.bg} ${gradeStyle.text}`}>
          {grade}
        </span>
      </div>
    </div>
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          >
            <div className="w-5 h-5 rounded-full" style={{ backgroundColor: component.color }} />
          </div>
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white">{component.name}</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400">{(component.weight * 100).toFixed(0)}% weight</p>
          </div>
        </div>
        <div className="text-right">
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-gray-900 dark:text-white">{component.score}</span>
            <span className={`px-2 py-0.5 rounded ${gradeStyle.bg} ${gradeStyle.text} text-sm font-bold`}>
              {component.grade}
            </span>
          </div>
          <div className="flex items-center justify-end gap-1 mt-1">
            ...
            <span className="text-xs text-gray-500 dark:text-slate-400">{component.trend}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {component.details.map((detail, i) => (
          <div key={i}>
            <p className="text-xs text-gray-500 dark:text-slate-400">{detail.label}</p>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-medium text-gray-900 dark:text-white truncate">{quickWin.title}</h4>
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${impactColors[quickWin.impact]}`}>
              {quickWin.impact}
            </span>
          </div>
          <p className="text-sm text-gray-500 dark:text-slate-400 mb-2">{quickWin.description}</p>
          <div className="flex items-center gap-2">
...
export default function VitalityScorePage() {
  ...
  return (
    <PullToRefresh onRefresh={handleRefresh} className="min-h-screen bg-gray-50 dark:bg-slate-900 px-4 py-6 sm:p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">Financial Vitality Score</h1>
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-8">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
                {data.trend === 'improving' && (
                  <div className="flex items-center gap-1 px-3 py-1 bg-white dark:bg-slate-800/20 rounded-full">
                    ...
                    <span className="text-sm font-medium">+{data.trendPercentage}% this month</span>
                  </div>
                )}
                <div className="px-3 py-1 bg-white dark:bg-slate-800/20 rounded-full">
                  <span className="text-sm">Top {100 - data.percentile}% of users</span>
                </div>
              </div>
              ...
              <p className="text-white/80 mb-6">
                {pointsToNext > 0 ? (
                  <>Just {pointsToNext} points to reach &quot;{data.nextMilestone.description}&quot;</>
                ) : (
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500 dark:text-slate-400">
              {data.quickWins.reduce((sum, q) => sum + q.estimatedPoints, 0)} points available
            </span>
          </div>
        ...
        <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
          <p className="text-sm text-blue-700 dark:text-blue-300">
            Your Financial Vitality Score is updated daily based on your connected accounts and activities.
          </p>
        </div>
      </div>
    </PullToRefresh>
