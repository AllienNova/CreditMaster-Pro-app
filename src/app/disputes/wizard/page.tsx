...
export default function DisputeWizardPage() {
  ...
  const bureaus = [
    { id: 'experian', name: 'Experian', color: 'blue', icon: '🔵' },
    { id: 'equifax', name: 'Equifax', color: 'red', icon: '🔴' },
    { id: 'transunion', name: 'TransUnion', color: 'green', icon: '🟢' },
  ];
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              <Link href="/dashboard/disputes" className="text-gray-600 hover:text-gray-900">← Back</Link>
              <h1 className="text-xl font-bold text-gray-900">Dispute Wizard</h1>
            </div>
            <span className="text-sm text-gray-500">Step {['select-bureau', 'select-type', 'select-items', 'customize', 'review'].indexOf(step) + 1} of 5</span>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {step === 'select-bureau' && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Select Credit Bureau</h2>
            <p className="text-gray-600 mb-6">Choose which bureau to send your dispute to</p>
            <div className="grid grid-cols-3 gap-4">
              {bureaus.map(b => (
                <button key={b.id} onClick={() => { setBureau(b.id); nextStep(); }}
                  className={`p-6 rounded-xl border-2 transition-all hover:shadow-md ${bureau === b.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
        ...
        {step === 'select-type' && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Dispute Reason</h2>
            <p className="text-gray-600 mb-6">Why are you disputing this item?</p>
            <div className="grid grid-cols-2 gap-4">
              {disputeTypes.map(type => (
                <button key={type.id} onClick={() => { setDisputeType(type.id); nextStep(); }}
                  className={`p-4 rounded-xl border-2 text-left transition-all hover:shadow-md ${disputeType === type.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
                  <p className="font-semibold">{type.name}</p>
                  <p className="text-sm text-gray-500">{type.desc}</p>
                </button>
              ))}
            </div>
            <button onClick={prevStep} className="mt-6 text-gray-600 hover:text-gray-900">← Back</button>
          </div>
        )}

        {step === 'select-items' && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Select Items to Dispute</h2>
            <p className="text-gray-600 mb-6">Choose which items from your report to include</p>
            <div className="space-y-3">
              {items.map(item => (
                <button key={item.id} onClick={() => toggleItem(item.id)}
                  className={`w-full p-4 rounded-xl border-2 text-left transition-all ${item.selected ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium">{item.type}</p>
                      <p className="text-sm text-gray-600">{item.description}</p>
                    </div>
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${item.selected ? 'border-blue-500 bg-blue-500' : 'border-gray-300'}`}>
        ...
        {step === 'customize' && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Customize Your Letter</h2>
            <p className="text-gray-600 mb-6">Add any additional details (optional)</p>
            <textarea value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} placeholder="Add any additional context..."
              className="w-full h-40 p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
            <div className="flex justify-between mt-6">
        ...
        {step === 'review' && (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Review Your Dispute</h2>
            <div className="space-y-4 mb-6">
              <div className="p-4 bg-gray-50 rounded-lg"><span className="text-gray-500">Bureau:</span> <span className="font-medium capitalize">{bureau}</span></div>
              <div className="p-4 bg-gray-50 rounded-lg"><span className="text-gray-500">Reason:</span> <span className="font-medium">{disputeTypes.find(t => t.id === disputeType)?.name}</span></div>
              <div className="p-4 bg-gray-50 rounded-lg"><span className="text-gray-500">Items:</span> <span className="font-medium">{selectedCount} items selected</span></div>
            </div>
        ...
        {step === 'complete' && (
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">Your dispute has been generated and is ready to send. Track progress in your dashboard.</p>
