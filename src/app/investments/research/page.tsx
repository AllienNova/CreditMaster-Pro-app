  ...
  const fetchAnalysis = useCallback(async (symbol: string) => {
    ...
    try {
      const response = await fetch(
        `/api/investments/analyze/${normalizedSymbol}`,
      );
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm">
              ...
              <div className="p-6">
                ...
                {activeTab === "fundamental" && (
                  <FundamentalTab analysis={analysis} />
                )}
                {activeTab === "sentiment" && (
        ...
        {!loading && !analysis && !error && (
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400 mb-6 max-w-md mx-auto">
              Enter a stock symbol above to get comprehensive AI-powered
              analysis including technical indicators, fundamental metrics, and
              sentiment data.
            </p>
            <Link
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
        ...
        <QuickStat
          label="Target"
          value={formatCurrency(recommendation.targetPrice)}
        />
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            <RiskBar
              label="Fundamental"
              level={riskAssessment.fundamentalRisk}
            />
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <span
          className={`text-sm font-medium capitalize ${getRiskColor(level)}`}
        >
          {level.replace("_", " ")}
        </span>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 dark:bg-slate-700 rounded-lg p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <TrendBadge
            label="Short Term"
            direction={technical.trend.shortTerm}
          />
          <TrendBadge
            label="Medium Term"
            direction={technical.trend.mediumTerm}
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="bg-gray-50 dark:bg-slate-700 rounded-lg divide-y divide-gray-200 dark:divide-slate-600">
            <IndicatorRow
              label="SMA 20"
              value={technical.indicators.sma20.toFixed(2)}
            />
            <IndicatorRow
              label="SMA 50"
              value={technical.indicators.sma50.toFixed(2)}
            />
            <IndicatorRow
              label="SMA 200"
              value={technical.indicators.sma200.toFixed(2)}
            />
            <IndicatorRow
              label="EMA 12"
              value={technical.indicators.ema12.toFixed(2)}
            />
            <IndicatorRow
              label="EMA 26"
              value={technical.indicators.ema26.toFixed(2)}
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-gray-50 dark:bg-slate-700 rounded-lg divide-y divide-gray-200 dark:divide-slate-600">
            <IndicatorRow
              label="RSI (14)"
              value={technical.indicators.rsi.toFixed(2)}
            />
            <IndicatorRow
              label="MACD"
            ...
            <IndicatorRow
              label="ATR"
              value={technical.indicators.atr.toFixed(2)}
            />
            <IndicatorRow
              label="ADX"
              value={technical.indicators.adx.toFixed(2)}
            />
          </div>
        </div>
      </div>
...
function TrendBadge({
  ...
  return (
    <div className="text-center">
      ...
      <span
        className={`px-3 py-1.5 rounded-full text-sm font-semibold capitalize ${colors[direction]}`}
      >
        {direction}
      </span>
    </div>
...
function FundamentalTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <span
          className={`text-2xl font-bold capitalize ${getRatingColor(fundamental.overallRating)}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <MetricSection title="Valuation">
          <MetricRow
            label="P/E Ratio"
            value={fundamental.valuation.peRatio?.toFixed(2) ?? "N/A"}
          />
          <MetricRow
            label="Forward P/E"
            value={fundamental.valuation.forwardPE?.toFixed(2) ?? "N/A"}
          />
          <MetricRow
            label="PEG Ratio"
        ...
        <MetricSection title="Profitability">
          ...
          <MetricRow
            ...
            value={`${(fundamental.profitability.operatingMargin * 100).toFixed(1)}%`}
          />
          <MetricRow
            label="Net Margin"
          ...
          <MetricRow
            label="ROIC"
            value={`${(fundamental.profitability.roic * 100).toFixed(1)}%`}
          />
        </MetricSection>
        ...
        <MetricSection title="Growth">
          <MetricRow
            label="Revenue YoY"
            value={formatPercent(fundamental.growth.revenueGrowthYoY * 100)}
          />
          <MetricRow
            label="Revenue 3Y"
            value={formatPercent(fundamental.growth.revenueGrowth3Y * 100)}
          />
          <MetricRow
            label="EPS YoY"
            value={formatPercent(fundamental.growth.epsGrowthYoY * 100)}
          />
          <MetricRow
            label="EPS 3Y"
            value={formatPercent(fundamental.growth.epsGrowth3Y * 100)}
          />
          <MetricRow
            label="Earnings Est."
            value={formatPercent(
              fundamental.growth.earningsGrowthEstimate * 100,
            )}
          />
        </MetricSection>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <MetricSection title="Financial Health">
          <MetricRow
            label="Current Ratio"
            value={fundamental.financial.currentRatio.toFixed(2)}
          />
          <MetricRow
            label="Quick Ratio"
            value={fundamental.financial.quickRatio.toFixed(2)}
          />
          <MetricRow
            label="Debt/Equity"
            value={fundamental.financial.debtToEquity.toFixed(2)}
          />
          <MetricRow
            label="Interest Coverage"
            value={fundamental.financial.interestCoverage.toFixed(2)}
          />
          <MetricRow
            label="FCF Yield"
            value={`${(fundamental.financial.freeCashFlowYield * 100).toFixed(1)}%`}
          />
        </MetricSection>
        ...
        <MetricSection title="Dividend">
          <MetricRow
            label="Yield"
            value={`${(fundamental.dividend.dividendYield * 100).toFixed(2)}%`}
          />
          <MetricRow
            label="Payout Ratio"
            value={`${(fundamental.dividend.dividendPayoutRatio * 100).toFixed(1)}%`}
          />
          <MetricRow
            label="5Y Growth"
            value={formatPercent(fundamental.dividend.dividendGrowth5Y * 100)}
          />
          <MetricRow
            label="Consecutive Years"
            value={fundamental.dividend.yearsOfDividendGrowth.toString()}
          />
          <MetricRow
            label="Frequency"
            value={fundamental.dividend.dividendFrequency}
          />
        </MetricSection>
      </div>
        ...
        <div className="bg-gray-50 dark:bg-slate-700 rounded-lg p-6">
          ...
          {sentiment.newsSentiment.topHeadlines.length > 0 && (
            <div className="space-y-3 mt-4 pt-4 border-t border-gray-200 dark:border-slate-600">
              ...
              {sentiment.newsSentiment.topHeadlines
                .slice(0, 5)
                .map((headline, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    <span
          ...
          <div className="bg-gray-50 dark:bg-slate-700 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <SocialPlatformRow
                label="Twitter"
                bullish={
                  sentiment.socialSentiment.platforms.twitter.bullishPercent
                }
              />
              <SocialPlatformRow
                label="Reddit"
                bullish={
                  sentiment.socialSentiment.platforms.reddit.bullishPercent
                }
              />
              <SocialPlatformRow
                label="StockTwits"
                bullish={
                  sentiment.socialSentiment.platforms.stocktwits.bullishPercent
                }
              />
            </div>
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-4">
              ...
              | Trending Score:{" "}
              {sentiment.socialSentiment.trendingScore.toFixed(0)}
            </p>
          </div>
        ...
        <div className="bg-gray-50 dark:bg-slate-700 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="text-sm font-semibold text-gray-900 dark:text-white">
                {(
                  sentiment.institutionalActivity.institutionalOwnership * 100
                ).toFixed(1)}
                %
              </span>
            </div>
          ...
          {aiAnalysis.priceTargets.map((target) => {
            ...
            return (
              ...
              >
                ...
                <p
                  className={`text-2xl font-bold ${priceColors[target.scenario]}`}
                >
