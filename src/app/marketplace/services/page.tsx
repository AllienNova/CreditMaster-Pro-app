    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400">{service.yearsInBusiness} years in business</p>
        </div>
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className="flex items-center">
          ...
          <span className="ml-1 text-sm text-gray-500 dark:text-slate-400">({service.reviewCount.toLocaleString()})</span>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {service.services.map((s) => (
          <span key={s} className="px-2 py-1 text-xs bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded">{s}</span>
        ))}
      </div>

      <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-slate-700">
        <span className="text-sm text-gray-500 dark:text-slate-400">{service.guarantee}</span>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {services.map((s) => (
            <tr key={s.id} className="border-b border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900">
...
export default function ServicesPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Credit Repair Services</h1>
        <p className="text-gray-600 dark:text-slate-300">Vetted credit repair companies to help improve your score</p>
        {error && (
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-slate-700">
        ...
        <div className="flex gap-2">
          <button onClick={() => setView('cards')} className={`px-3 py-2 rounded-lg text-sm ${view === 'cards' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-slate-800'}`}>
            Cards
          </button>
          <button onClick={() => setView('table')} className={`px-3 py-2 rounded-lg text-sm ${view === 'table' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-slate-800'}`}>
            Compare
          </button>
        </div>
      </div>
