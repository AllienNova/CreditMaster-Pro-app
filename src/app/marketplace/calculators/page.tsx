    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-lg">
        <div className="flex justify-between mb-2">
          ...
          <span
            className={`font-bold ${utilization <= 10 ? "text-green-600" : utilization <= 30 ? "text-yellow-600" : "text-red-600"}`}
          >
        ...
        <div className="h-4 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden">
          <div
            className={`h-full ${utilization <= 10 ? "bg-green-500" : utilization <= 30 ? "bg-yellow-500" : "bg-red-500"}`}
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className="font-medium">${optimal.toFixed(0)}</span>
          </div>
          <div>
            <span className="text-gray-500 dark:text-slate-400">
              For 10% utilization:
            </span>{" "}
            <span className="font-medium">${excellent.toFixed(0)}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 dark:text-slate-300">
          Financial planning tools for better credit decisions
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {calculators.map((calc) => (
          ...
          >
            ...
            <p
              className={`font-medium mt-2 ${active === calc.id ? "text-white" : "text-gray-900 dark:text-white"}`}
            >
              {calc.name}
            </p>
          </button>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">
          {calculators.find((c) => c.id === active)?.name} Calculator
        </h2>
