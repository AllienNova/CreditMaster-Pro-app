...
function TradelineCard({ tradeline }: { tradeline: Tradeline }) {
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border ${tradeline.available ? "border-gray-200" : "border-gray-300 dark:border-slate-600 opacity-60"}`}
    >
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-white">
            {tradeline.provider?.name ||
              `Provider ${tradeline.providerId.slice(0, 8)}`}
          </h3>
          <p className="text-sm text-gray-500 dark:text-slate-400">
            {tradeline.bureausReporting.join(", ")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span
            className={`px-2 py-1 text-xs rounded-full ${tradeline.available ? "bg-green-100 text-green-800" : "bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300"}`}
          >
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <p className="font-semibold text-green-600">
            +{tradeline.estimatedScoreImpact} pts
          </p>
        </div>
      </div>
            ...
            .map((t) => (
              ...
              >
                ...
                <span className="text-sm text-gray-700 dark:text-slate-200">
                  ...
                  {Math.floor(t.ageMonths / 12)}y) - +{t.estimatedScoreImpact}{" "}
                  pts
                </span>
              </label>
            ))}
        ...
        <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-lg">
          <div>
            ...
            <p className="text-3xl font-bold text-blue-600">
              {Math.min(850, currentScore + totalImpact)}
            </p>
          </div>
...
export default function TradelinesPage() {
  ...
  const [filters, setFilters] = useState<TradelineFilters>({
    minCreditLimit: 0,
    maxPrice: 2000,
    minAge: 0,
  });
  const [sortBy, setSortBy] = useState<"price" | "impact" | "age" | "value">(
    "impact",
  );
  ...
  const fetchTradelines = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/marketplace/tradelines?${params.toString()}`,
      );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 dark:text-slate-300">
          Authorized user tradelines to boost your credit score
        </p>
      </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  value={filters.minCreditLimit || 0}
                  onChange={(e) =>
                    setFilters({
                      ...filters,
                      minCreditLimit: Number(e.target.value),
                    })
                  }
                  className="w-full mt-1 px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg"
                >
        ...
        <div className="lg:col-span-3">
          {loading ? (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl p-12 text-center border border-gray-200 dark:border-slate-700">
              ...
              <button
                onClick={() =>
                  setFilters({ minCreditLimit: 0, maxPrice: 2000, minAge: 0 })
                }
            ...
            <>
              <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">
                Showing {sortedTradelines.length} tradeline
                {sortedTradelines.length !== 1 ? "s" : ""}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {sortedTradelines.map((t) => (
                  <TradelineCard key={t.id} tradeline={t} />
                ))}
              </div>
            </>
          )}
