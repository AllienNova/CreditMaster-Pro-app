...
const mockCourses: Course[] = [
  ...
  { id: '2', title: 'Dispute Letter Mastery', description: 'Write effective dispute letters that get results', category: 'Disputes', duration: '3h 15m', lessons: 18, level: 'intermediate', progress: 30, thumbnail: '' },
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs text-gray-500 dark:text-slate-400">{course.category}</span>
        </div>
        <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{course.title}</h3>
        <p className="text-sm text-gray-600 dark:text-slate-300 mb-3 line-clamp-2">{course.description}</p>
        ...
        {course.progress > 0 && (
          <div className="mb-3">
            ...
            <div className="h-2 bg-gray-200 dark:bg-slate-700 rounded-full">
              <div className="h-2 bg-blue-600 rounded-full" style={{ width: `${course.progress}%` }} />
            </div>
          </div>
        )}
...
function ArticleCard({ article }: { article: Article }) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm border border-gray-200 dark:border-slate-700 hover:border-blue-300 transition-colors">
      ...
      <h4 className="font-medium text-gray-900 dark:text-white mt-1 mb-2">{article.title}</h4>
      <p className="text-sm text-gray-600 dark:text-slate-300 line-clamp-2 mb-3">{article.excerpt}</p>
...
export default function EducationPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Education Library</h1>
        <p className="text-gray-600 dark:text-slate-300">Courses and guides for financial literacy</p>
      </div>
