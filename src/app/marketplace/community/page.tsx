...
const mockPosts: Post[] = [
  {
    ...
    content: 'Welcome to the Fynvita community...',
    likes: 245,
    replies: 12,
    date: '2024-01-01',
    pinned: true,
  ...
  {
    id: '2',
    title: 'How I removed 5 collections in 30 days',
    author: 'CreditWarrior',
    category: 'Success Stories',
    content: 'I want to share my journey...',
    likes: 189,
    replies: 47,
    date: '2024-01-15',
  },
  {
    id: '3',
    title: 'Best dispute letter template for late payments?',
    author: 'NewToCredit',
    category: 'Questions',
    content: 'Looking for advice on disputing...',
    likes: 34,
    replies: 23,
    date: '2024-01-18',
  },
  {
    ...
    category: 'Success Stories',
    content: 'Just wanted to share that my goodwill letter worked...',
    likes: 156,
    replies: 38,
    date: '2024-01-17',
  },
  {
    ...
    author: 'CreditExpert',
    category: 'Education',
    content: 'Many people ask about how long bureaus have to respond...',
    likes: 89,
    replies: 15,
    date: '2024-01-16',
  },
...
function PostCard({ post }: { post: Post }) {
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm border ${post.pinned ? 'border-blue-300' : 'border-gray-200 dark:border-slate-700'}`}
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <span
            className={`px-2 py-0.5 text-xs rounded-full ${categoryColors[post.category] || 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200'}`}
          >
            {post.category}
          </span>
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-5 border border-green-200">
      <div className="flex items-center justify-between mb-3">
        <span className="font-medium text-gray-900 dark:text-white">{story.author}</span>
      ...
      <div className="flex items-center gap-4 mb-3">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400">Before</p>
        </div>
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400">After</p>
        </div>
        <div className="text-sm text-gray-500 dark:text-slate-400 ml-auto">
          ⏱️ {story.timeframe}
        </div>
      </div>
      <p className="text-sm text-gray-600 dark:text-slate-300">{story.summary}</p>
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 dark:text-slate-300">
            Connect with others on their credit journey
          </p>
        </div>
      ...
      <div className="flex gap-4 border-b border-gray-200 dark:border-slate-700">
        <button
          ...
          className={`pb-3 px-1 text-sm font-medium border-b-2 ${activeTab === 'discussions' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 dark:text-slate-400'}`}
        >
          Discussions
        </button>
        <button
          onClick={() => setActiveTab('stories')}
          className={`pb-3 px-1 text-sm font-medium border-b-2 ${activeTab === 'stories' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 dark:text-slate-400'}`}
        >
          Success Stories
        </button>
      </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm ${category === cat ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200'}`}
              >
