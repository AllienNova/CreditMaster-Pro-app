...
const mockCoaches: Coach[] = [
  {
    ...
    specialties: ["Credit Building", "Debt Management", "Budgeting"],
    rating: 4.9,
    reviews: 234,
    sessionPrice: 150,
    packagePrice: 500,
  ...
  },
  {
    ...
    certifications: ["FICO Pro", "NACCC"],
    specialties: ["Credit Repair", "Score Optimization", "Dispute Strategy"],
    rating: 4.8,
    reviews: 189,
    sessionPrice: 100,
    packagePrice: 350,
  ...
  },
  {
    ...
    specialties: ["Debt Snowball", "Financial Wellness", "Emergency Funds"],
    rating: 4.7,
    reviews: 156,
    sessionPrice: 125,
    packagePrice: 400,
  ...
  },
  {
    id: "4",
    name: "Robert Chen",
    title: "Wealth Building Advisor",
    certifications: ["CFP", "CFA"],
    specialties: ["Investment", "Retirement", "Tax Strategy"],
    rating: 4.6,
    reviews: 98,
    sessionPrice: 200,
    packagePrice: 700,
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-1">
          {coach.certifications.map((c) => (
            <span
              key={c}
              className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded font-medium"
            >
              {c}
            </span>
          ))}
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-1">
          {coach.specialties.map((s) => (
            <span
              key={s}
              className="px-2 py-1 text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 rounded"
            >
              {s}
            </span>
          ))}
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
        <div>
          <p className="text-gray-500 dark:text-slate-400">Single Session</p>
          <p className="font-semibold text-lg">${coach.sessionPrice}</p>
        </div>
        <div>
          <p className="text-gray-500 dark:text-slate-400">4-Session Package</p>
...
export default function CoachingPage() {
  ...
  const filteredCoaches = mockCoaches
    .filter(
      (c) =>
        specialty === "all" || c.specialties.some((s) => s.includes(specialty)),
    )
    .filter(
      (c) =>
        priceRange === "all" ||
        (priceRange === "under100" && c.sessionPrice < 100) ||
        (priceRange === "100-150" &&
          c.sessionPrice >= 100 &&
          c.sessionPrice <= 150) ||
        (priceRange === "over150" && c.sessionPrice > 150),
    );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 dark:text-slate-300">
          Work with certified coaches to improve your financial health
        </p>
      </div>
