...
const typeColors = {
  ...
  balance_transfer: "bg-blue-100 text-blue-800",
  home_equity: "bg-green-100 text-green-800",
};
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
      <div className="flex justify-between items-start mb-4">
        <div>
          <span
            className={`px-2 py-1 text-xs rounded-full ${typeColors[option.type]}`}
          >
            {typeLabels[option.type]}
          </span>
          <h3 className="font-semibold text-gray-900 dark:text-white text-lg mt-2">
            {option.lender}
          </h3>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            max={30}
            value={currentApr}
            onChange={(e) => setCurrentApr(Number(e.target.value))}
            className="w-full"
          />
        </div>
        <div>
...
export default function ConsolidationPage() {
  ...
  const filteredOptions =
    typeFilter === "all"
      ? mockOptions
      : mockOptions.filter((o) => o.type === typeFilter);
