...
const typeColors = { personal: 'bg-blue-100 text-blue-800', balance_transfer: 'bg-purple-100 text-purple-800', home_equity: 'bg-green-100 text-green-800' };
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <h3 className="font-semibold text-gray-900 text-lg mt-2">{option.lender}</h3>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
        <div>
          ...
          <p className="font-semibold text-indigo-600">{option.apr}</p>
        </div>
        <div>
...
function SavingsCalculator() {
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
      <h3 className="font-semibold text-gray-900 mb-4">💵 Savings Calculator</h3>
      <div className="space-y-4">
        <div>
          <label className="text-sm text-gray-600">Total Debt: ${totalDebt.toLocaleString()}</label>
        ...
        <div>
          <label className="text-sm text-gray-600">Current APR: {currentApr}%</label>
          <input type="range" min={5} max={30} value={currentApr} onChange={(e) => setCurrentApr(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-600">New APR: {newApr}%</label>
...
export default function ConsolidationPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Debt Consolidation</h1>
        <p className="text-gray-600">Compare options to consolidate and lower your debt payments</p>
      </div>
          ...
          <div className="flex gap-2">
            {['all', 'personal', 'balance_transfer'].map((type) => (
              <button key={type} onClick={() => setTypeFilter(type)}
                className={`px-4 py-2 rounded-lg text-sm ${typeFilter === type ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700'}`}>
