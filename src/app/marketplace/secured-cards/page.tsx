...
const mockCards: SecuredCard[] = [
  {
    ...
    maxDeposit: 2500,
    apr: "28.24%",
    rewards: "2% cash back on gas/restaurants",
    creditBureaus: ["Experian", "Equifax", "TransUnion"],
    features: ["No annual fee", "Cash back rewards", "Free FICO score"],
    rating: 4.8,
    recommended: true,
  },
  {
...
function CardComparisonRow({ card }: { card: SecuredCard }) {
  return (
    <div
      className={`bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border ${card.recommended ? "border-blue-300 ring-2 ring-blue-100" : "border-gray-200 dark:border-slate-700"}`}
    >
      {card.recommended && (
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 dark:text-slate-300">
          Compare the best secured cards for building credit
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-600 rounded-xl p-6 text-white">
        ...
        <p className="text-blue-100">
          ...
          <strong>Discover it® Secured</strong> for its cash back rewards and $0
          annual fee.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-slate-700">
        <select
          value={sortBy}
          onChange={(e) =>
            setSortBy(e.target.value as "rating" | "fee" | "deposit")
          }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredCards.map((card) => (
          <CardComparisonRow key={card.id} card={card} />
        ))}
      </div>
    </div>
  );
