...
[Date]`,
  ...
  const generateLetter = () => {
    ...
    const settlementAmount = Math.round((selectedDebt.currentBalance * settlementPercentage) / 100);

    let letter = template.template;
    letter = letter.replace(/\[ACCOUNT_NUMBER\]/g, 'XXXXXX' + Math.random().toString(36).substring(7));
    letter = letter.replace(/\[CURRENT_BALANCE\]/g, `$${selectedDebt.currentBalance.toLocaleString()}`);
    letter = letter.replace(/\[SETTLEMENT_AMOUNT\]/g, `$${settlementAmount.toLocaleString()}`);
    letter = letter.replace(/\[SETTLEMENT_PERCENTAGE\]/g, `${settlementPercentage}`);
    letter = letter.replace(/\[ORIGINAL_CREDITOR\]/g, selectedDebt.originalCreditor || selectedDebt.creditor);
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    onChange={(e) => setSettlementPercentage(parseInt(e.target.value))}
                    className="w-full"
                  />
        ...
        <div className="mt-8 bg-purple-50 border border-purple-200 rounded-xl p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm text-purple-800">
            <div>
              ...
              <p>
                A negotiated agreement where you pay a debt (often for less than owed) in exchange for the creditor deleting all references from your credit reports.
              </p>
            </div>
            <div>
              <p className="font-semibold mb-1">Why It Works</p>
              <p>
                Collections agencies buy debts for pennies on the dollar. Getting 30-50% in cash NOW is often better than hoping to collect 100% later.
              </p>
            </div>
            <div>
              <p className="font-semibold mb-1">Legal Status</p>
              <p>
                While not illegal, some creditors refuse pay-for-delete as policy. It's worth trying - worst case is they say no and you negotiate a standard settlement.
              </p>
            </div>
          </div>
        </div>
