...
export default function BudgetOptimizer() {
  ...
  const [budgetMethod, setBudgetMethod] = useState<'50-30-20' | '80-20' | '60-20-20'>('50-30-20');

  const expenseCategories = [
    'Housing', 'Transportation', 'Food & Dining', 'Utilities', 'Healthcare',
    'Insurance', 'Debt Payments', 'Entertainment', 'Shopping', 'Personal Care',
    'Education', 'Savings & Investments', 'Subscriptions', 'Childcare', 'Other'
  ];
  ...
  const fetchFinancialData = async () => {
    try {
      ...
      // Demo data
      setIncomes([
        { id: '1', source: 'Full-time Job', amount: 5000, frequency: 'monthly', type: 'salary' },
        { id: '2', source: 'Side Hustle', amount: 500, frequency: 'monthly', type: 'freelance' },
      ]);
      setExpenses([
        { id: '1', category: 'Housing', name: 'Rent', amount: 1500, frequency: 'monthly', type: 'fixed', essential: true },
        { id: '2', category: 'Transportation', name: 'Car Payment', amount: 350, frequency: 'monthly', type: 'fixed', essential: true },
        { id: '3', category: 'Food & Dining', name: 'Groceries', amount: 400, frequency: 'monthly', type: 'variable', essential: true },
        { id: '4', category: 'Utilities', name: 'Electric & Gas', amount: 150, frequency: 'monthly', type: 'variable', essential: true },
        { id: '5', category: 'Entertainment', name: 'Streaming Services', amount: 50, frequency: 'monthly', type: 'fixed', essential: false },
      ]);
    }
  };
  ...
  const getTotalMonthlyIncome = () => {
    return incomes.reduce((sum, income) => sum + normalizeToMonthly(income.amount, income.frequency), 0);
  };

  const getTotalMonthlyExpenses = () => {
    return expenses.reduce((sum, expense) => sum + normalizeToMonthly(expense.amount, expense.frequency), 0);
  };

  const getExpensesByType = () => {
    const essential = expenses.filter(e => e.essential).reduce((sum, e) => sum + normalizeToMonthly(e.amount, e.frequency), 0);
    const discretionary = expenses.filter(e => !e.essential).reduce((sum, e) => sum + normalizeToMonthly(e.amount, e.frequency), 0);
  ...
  const getExpensesByCategory = () => {
    ...
    expenses.forEach(expense => {
      ...
      byCategory[expense.category] = (byCategory[expense.category] || 0) + monthly;
    });
    return byCategory;
  };
        ...
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl p-6 mb-8 text-white shadow-xl">
          <div className="grid md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              ...
              <p className={`text-3xl font-bold ${surplus >= 0 ? '' : 'text-red-200'}`}>
                  ...
                  <input
                    ...
                    onKeyPress={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        addIncome({ source: target.value, amount, frequency: 'monthly', type: 'salary' });
                        target.value = '';
                      }
                    }}
                  />
                  ...
                  <input
                    ...
                    onKeyPress={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        addExpense({ name: target.value, amount, category, frequency: 'monthly', essential: false });
                        target.value = '';
                      }
                    }}
                  />
        ...
        <div className="mt-8 bg-green-50 border border-green-200 rounded-xl p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm text-green-800">
            <div>
              ...
              <p>
                Most popular framework. 50% for needs (housing, food), 30% for wants (entertainment), 20% for savings and debt payoff.
              </p>
            </div>
            <div>
              <p className="font-semibold mb-1">80/20 Rule</p>
              <p>
                Simplified approach. Pay yourself first by saving 20%, then spend the remaining 80% however you like. Great for beginners.
              </p>
            </div>
            <div>
              <p className="font-semibold mb-1">60/20/20 Rule</p>
              <p>
                Stricter version. 60% for needs, 20% for wants, 20% for savings. Best for aggressive debt payoff or savings goals.
              </p>
            </div>
          </div>
        </div>
