      ...
      <div className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Score Simulator</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 sticky top-8">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Score Projection</h2>
              ...
              {result && (
                <div className="space-y-4">
                  <div className={`text-center p-3 rounded-lg ${result.scoreChange >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                    ...
                    <p className="text-sm text-gray-600 dark:text-slate-300 mt-1">Projected change</p>
                  </div>

                  {result.factorChanges.length > 0 && (
                    <div className="border-t pt-4">
                      <h3 className="text-sm font-medium text-gray-700 dark:text-slate-200 mb-3">Factor Changes</h3>
                      {result.factorChanges.map(change => (
                        <div key={change.factor} className="flex justify-between text-sm mb-2">
                          <span className="text-gray-600 dark:text-slate-300 capitalize">{change.factor.replace(/([A-Z])/g, ' $1')}</span>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Your Current Profile</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={item.label} className="text-center p-3 bg-gray-50 dark:bg-slate-900 rounded-lg">
            ...
            {suggestions.length > 0 && (
              <div className="bg-gradient-to-r from-blue-50 to-blue-50 rounded-xl p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Suggested for You</h2>
                <div className="flex flex-wrap gap-2">
                  {suggestions.map(scenario => (
                    ...
                    >
                      <Icon name={scenario.icon} className="w-4 h-4 inline-block" /> {scenario.name}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {/* All Scenarios */}
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">All Scenarios</h2>
                ...
                {SIMULATION_SCENARIOS.map(scenario => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <Icon name={scenario.icon} className="w-6 h-6 inline-block" />
                      <div className="flex-1">
                        <h3 className="font-medium text-gray-900 dark:text-white">{scenario.name}</h3>
                        <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">{scenario.description}</p>
                        <div className="flex items-center gap-4 mt-2 text-xs">
                          <span className={`font-medium ${scenario.impact >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            {scenario.impact >= 0 ? '+' : ''}{scenario.impact} pts
                          </span>
                          <span className="text-gray-400 dark:text-slate-500 flex items-center gap-1"><Icon name="clock" className="w-3 h-3 inline-block" /> {scenario.timeframe}</span>
            ...
            {result && result.timeline.length > 1 && (
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Projected Timeline</h2>
                <div className="flex items-end gap-2 h-40">
                  {result.timeline.map((point, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center">
                      <span className="text-xs font-medium text-gray-700 dark:text-slate-200 mb-1">{point.score}</span>
                      <div
