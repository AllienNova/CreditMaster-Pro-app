...
interface IdentityAlert {
  id: string;
  type: 'hard_inquiry' | 'new_account' | 'address_change' | 'suspicious_activity';
  bureau: string;
  description: string;
  timestamp: Date;
...
export default function CreditFreezeManager() {
  ...
  const [selectedAction, setSelectedAction] = useState<'freeze' | 'unfreeze' | 'temporary'>('freeze');
  ...
  const updateBureauStatus = (
    ...
    setBureaus(prev =>
      prev.map(b =>
        b.id === bureauId
          ? { ...b, freezeStatus: status, pin, confirmationNumber: confirmation, lastUpdated: new Date() }
          : b
      )
    );
  ...
  const handleFreezeAction = (bureau: Bureau, action: 'freeze' | 'unfreeze' | 'temporary') => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Overall Status Card */}
        <div className={`bg-gradient-to-r from-${overallStatus.color}-600 to-${overallStatus.color}-700 rounded-xl p-6 mb-8 text-white shadow-xl`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm opacity-90 mb-1">Identity Protection Status</p>
              <p className="text-3xl font-bold flex items-center gap-3">
                <span>{overallStatus.icon}</span>
                <span>{overallStatus.status}</span>
              </p>
              <p className="text-sm opacity-90 mt-2">
                {bureaus.filter(b => b.freezeStatus === 'frozen').length} of 3 bureaus frozen
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Pro Tip:</strong> A credit freeze does NOT affect your credit score, existing accounts, or employment. It only prevents NEW account openings.
              </p>
            </div>
          </div>
        )}
              ...
              <div className="p-4">
                {bureau.pin && (
                  <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-xs font-semibold text-yellow-900 mb-1">Your PIN</p>
                    <p className="text-sm font-mono text-yellow-800">{bureau.pin}</p>
                  </div>
                )}

                {bureau.confirmationNumber && (
                  <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-xs font-semibold text-blue-900 mb-1">Confirmation #</p>
                    <p className="text-sm font-mono text-blue-800">{bureau.confirmationNumber}</p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  {bureau.freezeStatus === 'frozen' && (
                    <>
                      <button
                        onClick={() => window.open(bureau.unfreezeUrl, '_blank')}
