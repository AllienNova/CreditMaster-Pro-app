...
export default function GoalsPage() {
  const [activeGoals, setActiveGoals] = useState<CreditGoal[]>(MOCK_ACTIVE_GOALS);
  const [showTemplates, setShowTemplates] = useState(false);
  const currentScore = 635; // Mock current score

  const createGoal = (template: GoalTemplate) => {
    const newGoal = goalTrackerService.createGoalFromTemplate('user_1', template.id, currentScore);
