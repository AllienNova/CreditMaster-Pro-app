...
export default function DisputeAnalyticsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Dispute Analytics</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {disputeStats.map((stat) => (
          <div key={stat.label} className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
            ...
            <p className="text-sm mt-2 text-gray-400 dark:text-slate-500">{stat.change}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
          <div className="p-6 border-b border-gray-200 dark:border-slate-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Disputes by Type</h2>
          </div>
          <div className="p-6">
            {disputesByType.map((item) => (
              <div key={item.type} className="mb-4 last:mb-0">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-500 dark:text-slate-400">{item.total} total</span>
                </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
          <div className="p-6 border-b border-gray-200 dark:border-slate-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Disputes by Bureau</h2>
          </div>
              ...
              <div key={bureau.bureau} className="p-4">
                ...
                <div className="grid grid-cols-4 gap-2 text-center">
                  <div className="bg-gray-50 dark:bg-slate-900 rounded p-2">
                    <p className="text-lg font-bold text-gray-900 dark:text-white">{bureau.total}</p>
                  ...
                  <div className="bg-emerald-50 rounded p-2">
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-400">Won</p>
                  </div>
                  ...
                  <div className="bg-blue-50 rounded p-2">
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-400">Avg Days</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <table className="w-full">
          <thead className="bg-gray-50 dark:bg-slate-900">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Result</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
            {recentDisputes.map((dispute) => (
              <tr key={dispute.id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{dispute.bureau}</td>
                ...
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{dispute.result}</td>
              </tr>
            ))}
          </tbody>
        </table>
