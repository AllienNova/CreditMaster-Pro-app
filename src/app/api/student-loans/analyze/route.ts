...
export async function POST(request: NextRequest) {
  try {
    ...
    const weightedInterestRate = loans.reduce((sum, loan) =>
      sum + (loan.interest_rate * loan.current_balance), 0) / totalDebt;

    const analysis = {
      total_loans: loans.length,
      total_debt: totalDebt,
      weighted_interest_rate: weightedInterestRate,
      loans_by_status: loans.reduce((acc, loan) => {
        acc[loan.loan_status] = (acc[loan.loan_status] || 0) + 1;
        return acc;
      }, {} as Record<string, number>),
      loans_by_servicer: loans.reduce((acc, loan) => {
...
export async function GET() {
  return NextResponse.json({
    ...
    description: 'Analyzes complete student loan portfolio and provides insights',
  });
}
