...
describe('Student Loan Programs API', () => {
  const federalPrograms = [
    { id: 'idr', name: 'Income-Driven Repayment', types: ['SAVE', 'PAYE', 'IBR', 'ICR'] },
    { id: 'pslf', name: 'Public Service Loan Forgiveness', requirements: ['120 payments', 'qualifying employer'] },
    { id: 'fresh-start', name: 'Fresh Start Initiative', deadline: '2024-09-30' },
    { id: 'consolidation', name: 'Direct Consolidation', benefits: ['single payment', 'fixed rate'] },
  ...
  describe('IDR Calculator', () => {
    ...
    it('should calculate IBR payment', () => {
      const calculateIBRPayment = (agi: number, familySize: number, isNewBorrower: boolean) => {
        const povertyLine = 14580 + (familySize - 1) * 5140;
  ...
  describe('Consolidation', () => {
    it('should calculate weighted average interest rate', () => {
      ...
      const weightedRate = loans.reduce((sum, l) => sum + (l.balance * l.rate), 0) / totalBalance;
