/**
 * DEFAB-3 hardening tests for GET /api/marketplace/products/[id].
...
jest.mock("@/lib/marketplace", () => ({
  marketplaceService: { getProductById: jest.fn() },
}));
...
function req(): NextRequest {
  const url = "http://localhost:3000/api/marketplace/products/p1";
  return {
    url,
    method: "GET",
    headers: new Headers(),
...
describe("GET /api/marketplace/products/[id] — public hardening", () => {
  it("stamps rate-limit headers on a 200", async () => {
    jest.spyOn(publicCatalogLimiter, "check").mockResolvedValue(allow);
    (marketplaceService.getProductById as jest.Mock).mockResolvedValue(PRODUCT);
  ...
  it("returns 429 without hitting the service when exhausted", async () => {
    ...
    expect(marketplaceService.getProductById).not.toHaveBeenCalled();
  });
