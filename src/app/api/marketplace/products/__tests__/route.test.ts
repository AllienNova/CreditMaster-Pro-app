/**
 * Negative-auth tests for /api/marketplace/products (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/marketplace", () => ({ marketplaceService: { getProducts: jest.fn(), getFeaturedProducts: jest.fn() } }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/marketplace/products";
  return {
    url,
    method,
...
describe("negative-auth – /api/marketplace/products", () => {
  beforeEach(() => {
    jest.clearAllMocks();
