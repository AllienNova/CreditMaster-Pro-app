/**
 * DEFAB-3 hardening tests for GET /api/marketplace/providers/search.
...
jest.mock("@/lib/marketplace", () => ({
  providerService: { searchProviders: jest.fn() },
}));
...
const PROVIDER = { id: "pr1", name: "Acme Credit", rating: 4.6 };

function req(query = "?q=acme"): NextRequest {
  const url = `http://localhost:3000/api/marketplace/providers/search${query}`;
  return {
    url,
    method: "GET",
    headers: new Headers(),
...
describe("GET /api/marketplace/providers/search — public hardening", () => {
  it("stamps rate-limit headers on a 200", async () => {
    jest.spyOn(publicCatalogLimiter, "check").mockResolvedValue(allow);
    (providerService.searchProviders as jest.Mock).mockResolvedValue([PROVIDER]);
  ...
  it("returns 429 without hitting the service when exhausted", async () => {
    ...
    expect(providerService.searchProviders).not.toHaveBeenCalled();
  });
