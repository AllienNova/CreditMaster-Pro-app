...
describe("Providers API", () => {
  ...
  describe("Provider Sorting", () => {
    ...
    it("should sort by years in business descending", () => {
      const sorted = [...mockProviders].sort(
        (a, b) => b.yearsInBusiness - a.yearsInBusiness,
      );
  ...
  describe("Category Filtering", () => {
    ...
    it("should return only tradeline providers", () => {
      const tradelineProviders = mockProviders.filter(
        (p) => p.category === "tradelines",
      );
    ...
    it("should return only monitoring providers", () => {
      const monitoringProviders = mockProviders.filter(
  ...
  describe("Verification Status", () => {
    ...
    it("should return verification badge info", () => {
      ...
      expect(
        getVerificationBadge({ verified: true, yearsInBusiness: 12 }),
      ).toBe("platinum");
      expect(getVerificationBadge({ verified: true, yearsInBusiness: 7 })).toBe(
        "gold",
      );
      expect(getVerificationBadge({ verified: true, yearsInBusiness: 2 })).toBe(
        "silver",
      );
      expect(
        getVerificationBadge({ verified: false, yearsInBusiness: 15 }),
      ).toBeNull();
    });
  });
  ...
  describe("Top Providers", () => {
    it("should get top providers by rating", () => {
      ...
      expect(topProviders[0].rating).toBeGreaterThanOrEqual(
        topProviders[1].rating,
      );
    });
