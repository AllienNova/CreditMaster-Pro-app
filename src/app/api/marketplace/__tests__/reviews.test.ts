...
describe("Reviews API", () => {
  const mockReviews = [
    {
      ...
      content:
        "This product really helped improve my credit score significantly.",
  ...
  describe("POST /api/marketplace/reviews", () => {
    it("should validate required fields", () => {
      ...
      expect(
        validateReview({ productId: "p1", rating: 5, content: "Too short" }),
      ).toBe(false); // content too short (9 chars)
      expect(
        validateReview({
          productId: "p1",
          rating: 5,
      ...
      expect(
        validateReview({ rating: 5, content: "Valid content here." }),
      ).toBe(false); // no target
      expect(
        validateReview({
          productId: "p1",
          rating: 6,
  ...
  describe("Data Validation", () => {
    ...
    it("should have productId or providerId", () => {
      mockReviews.forEach((review) => {
        expect(review.productId !== null || review.providerId !== null).toBe(
          true,
        );
      });
    });
