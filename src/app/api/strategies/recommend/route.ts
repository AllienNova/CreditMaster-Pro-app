  ...
  async (request: NextRequest, _user: AuthedUser) => {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Invalid creditItem: missing required fields (type, description, date, bureau)",
        },
        { status: 400 },
      );
    }
    ...
    if (
      userProfile.paymentHistory < 0 ||
      userProfile.paymentHistory > 100 ||
      userProfile.utilization < 0 ||
      userProfile.utilization > 100
    ) {
      return NextResponse.json(
        {
          error:
            "Invalid percentages: paymentHistory and utilization must be between 0 and 100",
        },
        { status: 400 },
      );
    }
...
export const GET = withAuth(async (_request: NextRequest, _user: AuthedUser) => {
  return NextResponse.json({
    ...
    method: "POST",
    description:
      "Get ML-powered strategy recommendations for credit repair items",
    ...
    requestBody: {
      ...
      previousAttempts:
        "string[] (optional) - IDs of previously attempted strategies",
    },
    response: {
