/**
 * Negative-auth tests for /api/servicers/detect-errors (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/servicer-intelligence-engine", () => ({ servicerIntelligenceEngine: { detectErrorPatterns: jest.fn() } }));
jest.mock("@/lib/security/audit-logging", () => ({ logAIInteraction: jest.fn() }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/servicers/detect-errors";
  return {
    url,
    method,
...
describe("negative-auth – /api/servicers/detect-errors", () => {
  beforeEach(() => {
    jest.clearAllMocks();
