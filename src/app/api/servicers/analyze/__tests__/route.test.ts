/**
 * Negative-auth tests for /api/servicers/analyze (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/servicer-intelligence-engine", () => ({ servicerIntelligenceEngine: { analyzeServicer: jest.fn() } }));
jest.mock("@/lib/security/audit-logging", () => ({ logAIInteraction: jest.fn() }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/servicers/analyze";
  return {
    url,
    method,
...
describe("negative-auth – /api/servicers/analyze", () => {
  beforeEach(() => {
    jest.clearAllMocks();
