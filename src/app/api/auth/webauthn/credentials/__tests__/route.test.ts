/**
 * Negative-auth tests for /api/auth/webauthn/credentials (TASK-AUTH-03f)
 */
...
function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/auth/webauthn/credentials";
  return {
    url,
    method,
...
describe("negative-auth – /api/auth/webauthn/credentials", () => {
  beforeEach(() => {
    jest.clearAllMocks();
