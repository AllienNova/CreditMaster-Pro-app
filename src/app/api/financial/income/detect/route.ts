...
import { incomeTrackingService, Transaction } from '@/lib/financial/income-tracking-service';
...
export async function GET() {
  try {
    ...
    const { data: transactionsData, error: txError } = await supabase
      ...
      .gte('date', new Date(Date.now() - 180 * 24 * 60 * 60 * 1000).toISOString()) // Last 6 months
    ...
    if (!transactionsData || transactionsData.length === 0) {
      return NextResponse.json({
        patterns: [],
        count: 0,
        message: 'No transactions found. Connect a bank account to auto-detect income.',
      });
    }
    ...
    const transactions: Transaction[] = (transactionsData as TransactionRow[]).map(t => ({
      id: t.id,
      date: new Date(t.date),
      amount: t.amount,
