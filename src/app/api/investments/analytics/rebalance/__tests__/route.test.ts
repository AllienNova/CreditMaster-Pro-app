/**
 * Auth tests for /api/investments/analytics/rebalance (TASK-AUTH-03e / TASK-INV coverage)
 */
...
describe("negative-auth – /api/investments/analytics/rebalance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("GET returns 401 when the request is not authenticated (TASK-AUTH-03e)", async () => {
    const res = await GET(
      createMockRequest(
        "http://localhost:3000/api/investments/analytics/rebalance",
      ),
    );
...
describe("authenticated – /api/investments/analytics/rebalance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("GET instantiates PortfolioAnalytics with user.id and returns 200 on success", async () => {
    const fakeRecommendations = { trades: [], expectedImpact: {} };
    const mockSuggestRebalancing = jest
      .fn()
      .mockResolvedValue(fakeRecommendations);
    const { PortfolioAnalytics } = getPortfolioAnalyticsMock();
    PortfolioAnalytics.mockImplementation(() => ({
      suggestRebalancing: mockSuggestRebalancing,
    }));

    // Include targetRiskLevel so the optional enum param is defined (not null)
    const url = `http://localhost:3000/api/investments/analytics/rebalance?portfolioId=${VALID_UUID}&targetRiskLevel=moderate`;
    const res = await GET(createMockRequest(url));

    expect(res.status).toBe(200);
    const body = await res.json() as { success: boolean; data: unknown };
    expect(body.success).toBe(true);
    expect(body.data).toEqual(fakeRecommendations);
    expect(PortfolioAnalytics).toHaveBeenCalledWith(AUTH_USER.id);
  });

  it("GET returns 400 when portfolioId is missing", async () => {
    getPortfolioAnalyticsMock().PortfolioAnalytics.mockImplementation(() => ({}));
    const url = "http://localhost:3000/api/investments/analytics/rebalance";
    const res = await GET(createMockRequest(url));
