...
describe('/api/investments/portfolio-analysis', () => {
  ...
  const mockQuotes = {
    AAPL: { symbol: 'AAPL', price: 155.0, change: 5.0, changePercent: 3.33, volume: 50000000, timestamp: new Date() },
    MSFT: { symbol: 'MSFT', price: 260.0, change: 10.0, changePercent: 4.0, volume: 30000000, timestamp: new Date() },
  };
  ...
  describe('GET /api/investments/portfolio-analysis', () => {
    it('should return API documentation', async () => {
      const request = createMockRequest('http://localhost:3000/api/investments/portfolio-analysis');
  ...
  describe('POST /api/investments/portfolio-analysis', () => {
    beforeEach(() => {
      ...
      mockAnalysisEngine.analyzePortfolio.mockImplementation((portfolioId: string) =>
    ...
    it('should auto-fetch current prices if not provided', async () => {
      const holdingsWithoutPrices = mockHoldings.map(h => ({ ...h, currentPrice: undefined }));
      const validInput = {
    ...
    it('should handle market data fetch errors gracefully', async () => {
      ...
      mockMarketDataService.getHistoricalData.mockRejectedValue(new Error('Historical data unavailable'));
    ...
    it('should handle analysis engine errors', async () => {
      mockAnalysisEngine.analyzePortfolio.mockRejectedValue(new Error('Analysis failed'));
    ...
    it('should uppercase all symbols automatically', async () => {
      const lowercaseHoldings = [
        { symbol: 'aapl', shares: 10, averageCost: 150.0, assetClass: 'stock' as const },
        { symbol: 'msft', shares: 5, averageCost: 250.0, assetClass: 'stock' as const },
      ];
