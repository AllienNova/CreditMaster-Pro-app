/**
 * Negative-auth tests for /api/investments/recommendations (TASK-AUTH-03e)
 */
...
describe("negative-auth – /api/investments/recommendations", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("GET returns 401 when the request is not authenticated (TASK-AUTH-03e)", async () => {
    const res = await GET(
      createMockRequest(
        "http://localhost:3000/api/investments/recommendations",
      ),
    );
