...
describe('Asset Allocation Analysis API', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (getAssetAllocationService as jest.Mock).mockReturnValue(mockAllocationService);

    // Mock analyzeAllocation to return different results based on risk tolerance
    mockAllocationService.analyzeAllocation.mockImplementation(async (portfolio: any, riskTolerance: RiskTolerance) => {
      return {
        ...mockAnalysisResult,
        recommendedModel: {
          ...mockAnalysisResult.recommendedModel,
          riskTolerance,
          name: riskTolerance === RiskTolerance.VERY_AGGRESSIVE ? 'Very Aggressive' : 'Moderate',
        },
      };
    });

    // Mock getAllocationModel to return different models based on risk tolerance
    mockAllocationService.getAllocationModel.mockImplementation((riskTolerance: RiskTolerance) => {
      return {
        ...mockAllocationModel,
        riskTolerance,
        name: riskTolerance === RiskTolerance.VERY_AGGRESSIVE ? 'Very Aggressive' :
              riskTolerance === RiskTolerance.AGGRESSIVE ? 'Aggressive' :
              riskTolerance === RiskTolerance.CONSERVATIVE ? 'Conservative' :
              riskTolerance === RiskTolerance.VERY_CONSERVATIVE ? 'Very Conservative' :
              'Moderate',
      };
    });
  });
  ...
  describe('POST /api/investments/allocation-analysis', () => {
    it('should analyze portfolio allocation', async () => {
      ...
      const request = createMockRequest('http://localhost:3000/api/investments/allocation-analysis', {
        method: 'POST',
        body: validInput,
      });
  ...
  describe('GET /api/investments/allocation-analysis', () => {
    ...
    it('should return specific allocation model', async () => {
      ...
      expect(mockAllocationService.getAllocationModel).toHaveBeenCalledWith(RiskTolerance.MODERATE);
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle extreme risk tolerance values', async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(data.data.recommendedModel.riskTolerance).toBe(RiskTolerance.VERY_AGGRESSIVE);
      expect(mockAllocationService.analyzeAllocation).toHaveBeenCalledWith(
        expect.anything(),
