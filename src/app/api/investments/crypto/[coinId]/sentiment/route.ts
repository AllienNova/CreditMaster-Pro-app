    ...
    return NextResponse.json(
      {
        ...
        message: 'An error occurred while analyzing sentiment. Please try again later.',
      },
      { status: 500 }
    );
  }
...
function identifyStrengths(sentiment: any): string[] {
  ...
  if (sentiment.communityEngagement.developerActivity === 'high' || sentiment.communityEngagement.developerActivity === 'very_high') {
...
function identifyConcerns(sentiment: any): string[] {
  ...
  if (sentiment.fearGreedIndex.classification === 'extreme_greed') {
    concerns.push('Market showing signs of extreme greed - potential correction risk');
  }
  ...
  if (sentiment.communityEngagement.developerActivity === 'very_low' || sentiment.communityEngagement.developerActivity === 'low') {
...
function generateRecommendation(sentiment: any): string {
  if (sentiment.overallSentiment === 'very_bullish' && sentiment.fearGreedIndex.classification === 'extreme_greed') {
