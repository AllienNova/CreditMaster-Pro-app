  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: 'Unauthorized. Please log in to access trending cryptocurrencies.' },
        { status: 401 }
      );
    }
      ...
      trendingCoins.slice(0, limit).map(async (coin) => {
        try {
          ...
          const [sentiment, priceData] = await Promise.all([
            ...
            fetch(`https://api.coingecko.com/api/v3/coins/${coin.id}`).then(r => r.json()),
          ]);
    ...
    return NextResponse.json(
      {
        ...
        message: 'An error occurred while fetching trending cryptocurrencies. Please try again later.',
      },
      { status: 500 }
    );
  }
