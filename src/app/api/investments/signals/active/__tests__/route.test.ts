/**
 * Negative-auth tests for /api/investments/signals/active (TASK-AUTH-03e)
 */
...
describe("negative-auth – /api/investments/signals/active", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("GET returns 401 when the request is not authenticated (TASK-AUTH-03e)", async () => {
    const res = await GET(
      createMockRequest(
        "http://localhost:3000/api/investments/signals/active",
      ),
    );
