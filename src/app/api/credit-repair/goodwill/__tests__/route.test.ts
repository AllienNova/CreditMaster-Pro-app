...
describe('/api/credit-repair/goodwill', () => {
  ...
  describe('GET /api/credit-repair/goodwill', () => {
    ...
    it('should return all goodwill letters for user', async () => {
      ...
      const request = createMockRequest('http://localhost:3000/api/credit-repair/goodwill');
    ...
    it('should filter letters by status', async () => {
      ...
      const request = createMockRequest('http://localhost:3000/api/credit-repair/goodwill?status=draft');
      const response = await GET(request);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(db.goodwill.getGoodwillLettersByUser).toHaveBeenCalledWith(mockUser.id, {
        status: 'draft',
        limit: 50,
        offset: 0,
      });
  ...
  describe('POST /api/credit-repair/goodwill', () => {
    ...
    it('should create new goodwill letter with AI-generated content', async () => {
      ...
      (db.goodwill.createGoodwillLetter as jest.Mock).mockResolvedValue(mockLetter);
    ...
    it('should return 500 on service error', async () => {
      ...
      (negotiationService.generateGoodwillLetter as jest.Mock).mockRejectedValue(
