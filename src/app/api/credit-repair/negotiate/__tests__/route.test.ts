...
describe("/api/credit-repair/negotiate", () => {
  ...
  describe("GET /api/credit-repair/negotiate", () => {
    ...
    it("should filter negotiations by status", async () => {
      ...
      const request = createMockRequest(
        "http://localhost:3000/api/credit-repair/negotiate?status=draft",
      );
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(db.negotiations.getNegotiationsByUser).toHaveBeenCalledWith(
        mockUser.id,
        {
          status: "pending",
          limit: 50,
  ...
  describe("POST /api/credit-repair/negotiate", () => {
    ...
    it("should create negotiation without generating scripts", async () => {
      ...
      expect(data.success).toBe(true);
      expect(
        negotiationService.generateNegotiationScript,
...
describe("negative-auth – /api/credit-repair/negotiate (withAuth)", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
    ...
    it("GET returns 401 when the request is not authenticated (TASK-AUTH-03c)", async () => {
      ...
      const res = await GET(createMockRequest("http://localhost:3000/api/credit-repair/negotiate"));
    ...
    it("POST returns 401 when the request is not authenticated (TASK-AUTH-03c)", async () => {
      ...
      const res = await POST(createMockRequest("http://localhost:3000/api/credit-repair/negotiate", { method: "POST" }));
      expect(res.status).toBe(401);
    });
});
