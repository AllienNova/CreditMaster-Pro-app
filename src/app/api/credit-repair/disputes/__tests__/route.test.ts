...
describe("negative-auth – /api/credit-repair/disputes (withAuth)", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
    ...
    it("GET returns 401 when the request is not authenticated (TASK-AUTH-03c)", async () => {
      ...
      const res = await GET(createMockRequest("http://localhost:3000/api/credit-repair/disputes"));
    ...
    it("POST returns 401 when the request is not authenticated (TASK-AUTH-03c)", async () => {
      ...
      const res = await POST(createMockRequest("http://localhost:3000/api/credit-repair/disputes", { method: "POST" }));
      expect(res.status).toBe(401);
    });
});
