...
describe("/api/credit-repair/reports", () => {
  ...
  describe("GET /api/credit-repair/reports", () => {
    ...
    it("should return all credit reports for user", async () => {
      (db.creditReports.getCreditReportsByUser as jest.Mock).mockResolvedValue([
        mockReport,
      ]);
      (db.creditReports.getLatestCreditReport as jest.Mock).mockResolvedValue(
        mockReport,
      );
    ...
    it("should filter reports by bureau", async () => {
      (
        db.creditReports.getCreditReportsByBureau as jest.Mock
      ...
      const request = createMockRequest(
        "http://localhost:3000/api/credit-repair/reports?bureau=experian",
      );
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(db.creditReports.getCreditReportsByBureau).toHaveBeenCalledWith(
        mockUser.id,
        "experian",
        50,
      );
