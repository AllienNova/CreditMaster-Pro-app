...
describe('/api/credit-repair/score', () => {
  ...
  describe('GET /api/credit-repair/score', () => {
    it('should return existing score from database', async () => {
      ...
      (db.creditRepair.getCreditRepairScore as jest.Mock).mockResolvedValue(mockScore);

      const request = new NextRequest('http://localhost:3000/api/credit-repair/score');
      ...
      expect(db.creditRepair.getCreditRepairScore).toHaveBeenCalledWith(mockUser.id);
    ...
    it('should calculate and save new score if not in database', async () => {
      ...
      (db.creditRepair.getCreditRepairScore as jest.Mock).mockResolvedValue(null);
      ...
      (db.creditRepair.saveCreditRepairScore as jest.Mock).mockResolvedValue(mockScore);
      ...
      expect(data.data.score).toBe(mockScore.score);
      expect(data.data.factors).toEqual(mockScore.factors);
      expect(creditRepairService.getCreditRepairScore).toHaveBeenCalledWith(mockUser.id);
