...
describe('/api/credit-repair/cards', () => {
  ...
  describe('GET /api/credit-repair/cards', () => {
    it('should return all credit cards for user', async () => {
      ...
      // Mock database
      (db.creditCards.getCreditCardsByUser as jest.Mock).mockResolvedValue([mockCard]);
      (db.creditCards.calculateTotalUtilization as jest.Mock).mockResolvedValue(30);

      const request = createMockRequest('http://localhost:3000/api/credit-repair/cards');
      ...
      expect(db.creditCards.getCreditCardsByUser).toHaveBeenCalledWith(mockUser.id, {
    ...
    it('should filter cards by utilization range', async () => {
      ...
      const request = createMockRequest('http://localhost:3000/api/credit-repair/cards?minUtilization=20&maxUtilization=40');
      const response = await GET(request);
