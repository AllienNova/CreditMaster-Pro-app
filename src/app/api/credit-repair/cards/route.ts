...
export async function GET(request: NextRequest) {
  try {
    ...
    const cards = await db.creditCards.getCreditCardsByUser(user.id, {
      minUtilization: minUtilization ? Number.parseFloat(minUtilization) : undefined,
      maxUtilization: maxUtilization ? Number.parseFloat(maxUtilization) : undefined,
      limit,
      offset,
    });

    // 4. Calculate total utilization
    const totalUtilization = await db.creditCards.calculateTotalUtilization(user.id);
