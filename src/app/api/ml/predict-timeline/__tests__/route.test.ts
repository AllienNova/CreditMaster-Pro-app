/**
 * Negative-auth tests for /api/ml/predict-timeline (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/ml-prediction-models", () => ({ mlPredictionModels: { predictDisputeSuccess: jest.fn() } }));
jest.mock("@/lib/security/audit-logging", () => ({ logAIInteraction: jest.fn() }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/ml/predict-timeline";
  return {
    url,
    method,
...
describe("negative-auth – /api/ml/predict-timeline", () => {
  beforeEach(() => {
    jest.clearAllMocks();
