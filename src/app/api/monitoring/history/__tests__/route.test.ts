/**
 * Negative-auth tests for /api/monitoring/history (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/monitoring/real-time-monitoring", () => ({ RealtimeMonitoringService: { getEventHistory: jest.fn(() => []), clearEventHistory: jest.fn() } }));

import { GET, DELETE } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/monitoring/history";
  return {
    url,
    method,
...
describe("negative-auth – /api/monitoring/history", () => {
  beforeEach(() => {
    jest.clearAllMocks();
