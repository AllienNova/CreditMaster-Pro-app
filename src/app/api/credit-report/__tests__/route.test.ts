...
describe("Credit Report API Routes", () => {
  ...
  describe("Account Analysis", () => {
    ...
    const analyzeAccount = (account: Account) => {
      const utilization =
        account.limit > 0 ? (account.balance / account.limit) * 100 : 0;
      const latePayments = account.paymentHistory.filter(
        (p) => p !== "OK",
      ).length;
      const isNegative =
        account.status === "collections" || account.status === "charged_off";

      return {
        utilization: Math.round(utilization),
        latePayments,
        isNegative,
        scoreImpact: isNegative
          ? "severe"
          : latePayments > 0
            ? "moderate"
            : "positive",
  ...
  describe("Score Factors", () => {
    ...
    it("should identify high utilization as negative factor", () => {
      ...
      expect(factors).toContainEqual(
        expect.objectContaining({ factor: "High utilization" }),
      );
    });
    ...
    it("should identify late payments as negative factor", () => {
      ...
      expect(factors).toContainEqual(
        expect.objectContaining({ factor: "Late payments" }),
      );
    });
