/**
 * Negative-auth tests for /api/ai/spending-analysis (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/ai-personalization", () => ({
  getSpendingAnalyzer: jest.fn(),
}));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/ai/spending-analysis";
  return {
    url,
    method,
...
describe("negative-auth – /api/ai/spending-analysis", () => {
  beforeEach(() => {
    jest.clearAllMocks();
