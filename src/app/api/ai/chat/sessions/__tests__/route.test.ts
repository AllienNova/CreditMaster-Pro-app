/**
 * Negative-auth tests for /api/ai/chat/sessions (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/ai/chat-db-service", () => ({
  chatDbService: { listSessions: jest.fn(), createSession: jest.fn() },
}));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/ai/chat/sessions";
  return {
    url,
    method,
...
describe("negative-auth – /api/ai/chat/sessions", () => {
  beforeEach(() => {
    jest.clearAllMocks();
