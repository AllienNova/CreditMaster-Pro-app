/**
 * Negative-auth tests for /api/ai/financial-coach/budget (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/financial/budget-optimizer", () => ({
  budgetOptimizer: { optimizeBudget: jest.fn() },
}));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/ai/financial-coach/budget";
  return {
    url,
    method,
...
describe("negative-auth – /api/ai/financial-coach/budget", () => {
  beforeEach(() => {
    jest.clearAllMocks();
