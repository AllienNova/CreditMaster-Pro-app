/**
 * Negative-auth tests for /api/ai/orchestrate (TASK-AUTH-03f)
 * Route gated by withPermission("ai:orchestrate_strategies").
 */
...
jest.mock("@/lib/student-loan-ai-engine", () => ({
  studentLoanAIEngine: { orchestrateStrategies: jest.fn() },
}));
...
describe("negative-auth – /api/ai/orchestrate", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("POST returns 403 when the role lacks ai:orchestrate_strategies", async () => {
    mockHasPermission.mockReturnValue(false);
    const res = await POST(createMockRequest("POST"));
    expect(res.status).toBe(403);
  });

  it("GET returns 403 when the role lacks ai:orchestrate_strategies", async () => {
    mockHasPermission.mockReturnValue(false);
    const res = await GET(createMockRequest("GET"));
    expect(res.status).toBe(403);
  });
