...
describe('Analytics API Routes', () => {
  ...
  describe('Cohort Analysis', () => {
    ...
    it('should calculate average retention by period', () => {
      ...
      const avgRetention = period1Retention.reduce((a, b) => a + b, 0) / period1Retention.length;
  ...
  describe('Revenue Analytics', () => {
    ...
    it('should calculate MRR growth', () => {
      const growth = revenueData.map((d, i) => {
        if (i === 0) return 0;
        return Math.round(((d.mrr - revenueData[i - 1].mrr) / revenueData[i - 1].mrr) * 100);
      });
    ...
    it('should calculate churn rate', () => {
      const churnRates = revenueData.map(d => Math.round((d.churnedMrr / d.mrr) * 100));
  ...
  describe('Funnel Analysis', () => {
    ...
    it('should calculate overall conversion', () => {
      const overallConversion = (funnel[funnel.length - 1].count / funnel[0].count) * 100;
    ...
    it('should identify biggest drop-off', () => {
      ...
      const biggest = dropOffs.reduce((max, d) => d.dropOff > max.dropOff ? d : max);
