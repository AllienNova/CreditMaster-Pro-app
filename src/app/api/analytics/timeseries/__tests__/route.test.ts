/**
 * Negative-auth tests for /api/analytics/timeseries (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/analytics", () => ({ AnalyticsEngine: { getTimeSeriesData: jest.fn() } }));

import { GET } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/analytics/timeseries";
  return {
    url,
    method,
...
describe("negative-auth – /api/analytics/timeseries", () => {
  beforeEach(() => {
    jest.clearAllMocks();
