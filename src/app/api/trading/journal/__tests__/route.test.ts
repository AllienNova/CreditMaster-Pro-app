/**
 * Negative-auth tests for /api/trading/journal (TASK-AUTH-03e)
 */
...
describe("negative-auth – /api/trading/journal", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("GET returns 401 when the request is not authenticated (TASK-AUTH-03e)", async () => {
    const res = await GET(
      createMockRequest("http://localhost:3000/api/trading/journal"),
    );
  ...
  it("POST returns 401 when the request is not authenticated (TASK-AUTH-03e)", async () => {
    const res = await POST(
      createMockRequest("http://localhost:3000/api/trading/journal", "POST"),
    );
    expect(res.status).toBe(401);
  });
});
