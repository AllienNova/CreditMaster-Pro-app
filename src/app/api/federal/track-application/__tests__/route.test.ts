/**
 * Negative-auth tests for /api/federal/track-application (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/federal-integration-service", () => ({ federalIntegrationService: { trackApplicationStatus: jest.fn() } }));
jest.mock("@/lib/security/audit-logging", () => ({ logAIInteraction: jest.fn() }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/federal/track-application";
  return {
    url,
    method,
...
describe("negative-auth – /api/federal/track-application", () => {
  beforeEach(() => {
    jest.clearAllMocks();
