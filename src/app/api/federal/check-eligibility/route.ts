...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check permissions
    if (!rbac.hasPermission(validation.user, 'federal_programs:check_eligibility')) {
    ...
    switch (program_type) {
      case 'fresh_start':
        eligibility = await federalIntegrationService.checkFreshStartEligibility(ssn);
        break;
      case 'rehabilitation':
        eligibility = await federalIntegrationService.checkRehabilitationEligibility(ssn);
        break;
      case 'discharge':
        eligibility = await federalIntegrationService.checkDischargeEligibility(ssn);
        break;
      default:
        return NextResponse.json(
          { error: 'Invalid program_type. Must be: fresh_start, rehabilitation, or discharge' },
          { status: 400 }
        );
    }
