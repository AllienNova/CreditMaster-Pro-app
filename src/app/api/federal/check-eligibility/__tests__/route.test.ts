/**
 * Negative-auth tests for /api/federal/check-eligibility (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/federal-integration-service", () => ({ federalIntegrationService: { checkFreshStartEligibility: jest.fn() } }));
jest.mock("@/lib/security/audit-logging", () => ({ logAIInteraction: jest.fn() }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/federal/check-eligibility";
  return {
    url,
    method,
...
describe("negative-auth – /api/federal/check-eligibility", () => {
  beforeEach(() => {
    jest.clearAllMocks();
