...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check permissions
    if (!rbac.hasPermission(validation.user, 'federal_programs:submit_application')) {
    ...
    switch (program_type) {
      case 'fresh_start':
        result = await federalIntegrationService.submitFreshStartApplication(application_data);
        break;
      case 'rehabilitation':
        result = await federalIntegrationService.submitRehabilitationApplication(application_data);
        break;
      case 'consolidation':
        result = await federalIntegrationService.submitConsolidationApplication(application_data);
        break;
      default:
        return NextResponse.json(
          { error: 'Invalid program_type. Must be: fresh_start, rehabilitation, or consolidation' },
          { status: 400 }
        );
    }
