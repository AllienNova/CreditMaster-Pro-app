/**
 * Negative-auth tests for /api/federal/submit-application (TASK-AUTH-03f)
 */
...
jest.mock("@/lib/federal-integration-service", () => ({ federalIntegrationService: { submitFreshStartApplication: jest.fn() } }));
jest.mock("@/lib/security/audit-logging", () => ({ logAIInteraction: jest.fn() }));
jest.mock("@/lib/security/input-validation", () => ({ validateInput: jest.fn(() => ({ isValid: true, errors: [] })) }));

import { GET, POST } from "../route";

function createMockRequest(method = "GET"): NextRequest {
  const url = "http://localhost:3000/api/federal/submit-application";
  return {
    url,
    method,
...
describe("negative-auth – /api/federal/submit-application", () => {
  beforeEach(() => {
    jest.clearAllMocks();
