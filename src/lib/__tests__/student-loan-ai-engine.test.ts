...
describe("StudentLoanAIEngine runtime safeguards", () => {
  ...
  it("aligns automation workflows with orchestration metadata when automation is preferred", async () => {
    ...
    const orchestration =
      recommendation.strategy_orchestration as StrategyOrchestrationShape;
    expect(workflow.workflow_name).toBe(
      orchestration.primary_strategy.strategy_name,
    );
  ...
  it("emits JSON-serializable orchestration artifacts with normalized checkpoint dates", async () => {
    ...
    expect(() =>
      JSON.stringify(recommendation.strategy_orchestration),
    ).not.toThrow();
  });
});
