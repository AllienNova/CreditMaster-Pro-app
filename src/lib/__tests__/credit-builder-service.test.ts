...
class CreditBuilderService {
  ...
  async getUtilizationRecommendations(profile: { utilization: number; score: number }) {
  ...
  async getPaymentRecommendations(profile: { onTimePaymentRate: number; score: number }) {
    const recommendations = [];
...
describe('CreditBuilderService', () => {
  ...
  describe('Recommendations', () => {
    it('should generate utilization recommendations', async () => {
      ...
      const recommendations = await service.getUtilizationRecommendations(profile);
    ...
    it('should prioritize high-impact actions', () => {
      const actions = [
        ...
        { impact: 'medium', effort: 'medium', action: 'Request credit increase' },
  ...
  describe('Score Factors', () => {
    ...
    it('should identify negative factors', () => {
      ...
      if (creditProfile.utilization > 30) negativeFactors.push('High utilization');
      if (creditProfile.missedPayments > 0) negativeFactors.push('Missed payments');
      if (creditProfile.averageAccountAge < 3) negativeFactors.push('Low credit age');
      if (creditProfile.recentInquiries > 2) negativeFactors.push('Too many inquiries');
  ...
  describe('Goal Setting', () => {
    it('should calculate time to goal', () => {
      ...
      const monthsToGoal = Math.ceil((targetScore - currentScore) / monthlyIncrease);
      expect(monthsToGoal).toBe(20);
    });

    it('should validate realistic goals', () => {
      const isRealisticGoal = (current: number, target: number, months: number) => {
