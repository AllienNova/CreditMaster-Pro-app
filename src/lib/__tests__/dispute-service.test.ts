describe('DisputeService', () => {
  ...
  describe('Letter Generation', () => {
    it('should generate dispute letter with required fields', () => {
      const letterData = {
        userInfo: { name: 'John Doe', address: '123 Main St', city: 'New York', state: 'NY', zip: '10001' },
        bureau: 'experian',
    ...
    it('should include proper legal references', () => {
      const letter = generateDisputeLetter({
        userInfo: { name: 'Test User', address: '123 Test St', city: 'Test', state: 'TS', zip: '00000' },
        bureau: 'experian',
        disputeType: 'collection',
  ...
  describe('Success Prediction', () => {
    it('should calculate success probability based on factors', () => {
      ...
      const probability = Object.values(factors).reduce((sum, f) => sum + f, 0) / Object.keys(factors).length;
    ...
    it('should identify high-success dispute types', () => {
      const successRates: Record<string, number> = {
        identity_theft: 0.85,
        duplicate_account: 0.80,
        incorrect_balance: 0.70,
        late_payment: 0.55,
        collection: 0.50,
      };

      const highSuccess = Object.entries(successRates).filter(([, rate]) => rate >= 0.7);
  ...
  describe('Timeline Management', () => {
    it('should track dispute timeline', () => {
      const timeline = [
        ...
        { date: '2024-02-10', event: 'Investigation complete', status: 'resolved' },
      ];
...
function generateDisputeLetter(data: {
  userInfo: { name: string; address: string; city: string; state: string; zip: string };
  bureau: string;
  disputeType: string;
  creditor: string;
