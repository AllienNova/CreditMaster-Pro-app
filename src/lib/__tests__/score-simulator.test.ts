...
describe('Score Simulator', () => {
  describe('Action Impact Calculation', () => {
    ...
    const calculateImpact = (action: ScoreAction) => {
      const impacts: Record<string, (value: number, score: number) => number> = {
        'pay_down_balance': (value, score) => {
        ...
        'increase_credit_limit': (value) => Math.min(15, Math.round(value / 500)),
        'open_new_account': () => -5, // Initial drop due to hard inquiry
      };

      const impactFn = impacts[action.type];
      if (!impactFn) return 0;

      const impact = impactFn(action.value, action.currentScore);
      const newScore = Math.max(300, Math.min(850, action.currentScore + impact));

      return {
        impact,
        currentScore: action.currentScore,
        projectedScore: newScore,
        timeline: action.type === 'remove_collection' ? '30-45 days' : '1-2 billing cycles',
      };
    };

    it('should calculate impact of paying down balance', () => {
      const result = calculateImpact({ type: 'pay_down_balance', value: 40, currentScore: 650 });
      expect(result.impact).toBe(20);
      expect(result.projectedScore).toBe(670);
    });

    it('should calculate impact of removing collection', () => {
      const result = calculateImpact({ type: 'remove_collection', value: 0, currentScore: 580 });
    ...
    it('should cap score at 850', () => {
      const result = calculateImpact({ type: 'remove_collection', value: 0, currentScore: 840 });
      expect(result.projectedScore).toBe(850);
    });

    it('should not go below 300', () => {
      const result = calculateImpact({ type: 'open_new_account', value: 0, currentScore: 302 });
  ...
  describe('Multi-Action Simulation', () => {
    ...
    it('should track progression through steps', () => {
      const result = simulateActions(600, ['pay_off_collection', 'dispute_inquiry']);
  ...
  describe('Score Range Classification', () => {
    const classifyScore = (score: number) => {
      if (score >= 800) return { rating: 'Exceptional', color: 'emerald', tier: 1 };
  ...
  describe('Factor Weighting', () => {
    ...
    it('should have weights summing to 100', () => {
      const total = Object.values(scoreFactorWeights).reduce((a, b) => a + b, 0);
      expect(total).toBe(100);
    });

    it('should prioritize payment history', () => {
      expect(scoreFactorWeights.paymentHistory).toBeGreaterThan(scoreFactorWeights.creditUtilization);
    });

    it('should have utilization as second most important', () => {
      const sorted = Object.entries(scoreFactorWeights).sort((a, b) => b[1] - a[1]);
