...
export class ScoreSimulatorService {
  ...
  private calculateMixScore(profile: UserCreditProfile): number {
    ...
    if (profile.installmentLoans > 0 && profile.revolvingAccounts >= 2)
      score += 10;
  ...
  simulateScenarios(
    ...
    for (const scenario of scenarios) {
      const impact = this.applyScenarioToFactors(
        projectedFactors,
        scenario,
        profile,
      );
    ...
    for (const key of Object.keys(currentFactors) as (keyof ScoreFactors)[]) {
      if (currentFactors[key] !== projectedFactors[key]) {
        factorChanges.push({
          ...
          impact: Math.round(
            (projectedFactors[key] - currentFactors[key]) * FACTOR_WEIGHTS[key],
          ),
        });
      }
    }
  ...
  private applyScenarioToFactors(
    ...
    switch (scenario.category) {
      case "utilization":
        const targetUtil =
          scenario.id === "pay_down_10"
            ? 10
            : scenario.id === "pay_down_30"
              ? 30
              : 50;
  ...
  private generateRecommendations(
    ...
    if (
      profile.utilization > 30 &&
      !selectedIds.some((id) => id.includes("pay_down"))
    ) {
      recommendations.push(
        "Consider paying down balances to reduce utilization below 30%",
      );
    }
    if (profile.onTimePayments < 100 && !selectedIds.includes("on_time_6mo")) {
      recommendations.push(
        "Focus on making all payments on time to build payment history",
      );
    }
    if (profile.negativeItems > 0 && !selectedIds.includes("remove_negative")) {
      recommendations.push(
        "Dispute inaccurate negative items to potentially improve your score",
      );
    }
    if (
      profile.installmentLoans === 0 &&
      !selectedIds.includes("credit_builder_loan")
    ) {
      recommendations.push(
        "A credit builder loan could improve your credit mix",
      );
    }
    if (selectedScenarios.length === 0) {
      recommendations.push(
        "Select scenarios above to see how they would impact your score",
      );
    }
  ...
  getSuggestedScenarios(profile: UserCreditProfile): SimulationScenario[] {
    return SIMULATION_SCENARIOS.filter((scenario) => {
      if (scenario.category === "utilization" && profile.utilization <= 10)
        return false;
      if (scenario.category === "negative" && profile.negativeItems === 0)
        return false;
      if (scenario.category === "mix" && profile.installmentLoans > 0)
        return false;
      return true;
    }).slice(0, 5);
  }
