...
describe('TaxOptimizationEngine — Quarterly Tax Methods', () => {
  ...
  describe('calculateQuarterlyEstimatedTax', () => {
    it('should return 4 quarterly estimates', () => {
      const estimates = engine.calculateQuarterlyEstimatedTax(createMockProfile());
    ...
    it('should calculate higher payments for self-employed', () => {
      const w2Estimates = engine.calculateQuarterlyEstimatedTax(createMockProfile());
    ...
    it('should use prior year tax when provided', () => {
      const withPrior = engine.calculateQuarterlyEstimatedTax(createMockProfile(), 30000);
      const withoutPrior = engine.calculateQuarterlyEstimatedTax(createMockProfile(), 0);
    ...
    it('should handle zero income', () => {
      const estimates = engine.calculateQuarterlyEstimatedTax(
        createMockProfile({ grossIncome: 0, w2Income: 0, federalWithheld: 0, stateWithheld: 0 }),
      );
      for (const est of estimates) {
  ...
  describe('calculateSafeHarborAmount', () => {
    it('should return a SafeHarborResult object', () => {
      const result = engine.calculateSafeHarborAmount(createMockProfile(), 20000);
    ...
    it('should choose the lesser safe harbor method', () => {
      ...
      expect(['current_year_90', 'prior_year_100', 'prior_year_110']).toContain(result.recommendedMethod);
    });
  ...
  describe('estimateUnderpaymentPenalty', () => {
    it('should return an UnderpaymentPenalty object', () => {
      const result = engine.estimateUnderpaymentPenalty(createMockProfile(), 20000, 10000);
    ...
    it('should use custom penalty rate when provided', () => {
      ...
      if (result8.hasUnderpayment && result5.hasUnderpayment) {
        expect(result8.estimatedPenalty).toBeGreaterThanOrEqual(result5.estimatedPenalty);
      }
    });

    it('should handle zero prior year tax', () => {
      const result = engine.estimateUnderpaymentPenalty(createMockProfile(), 0, 0);
      expect(result).toHaveProperty('estimatedPenalty');
      expect(result).toHaveProperty('hasUnderpayment');
    });
  });
  ...
  describe('getQuarterlyPaymentSchedule', () => {
    ...
    it('should use prior year tax when provided', () => {
      const withPrior = engine.getQuarterlyPaymentSchedule(createMockProfile(), 25000);
      expect(withPrior).toHaveLength(4);
    });
  });
  ...
  describe('getQuarterlyDueDates', () => {
    ...
    const getDueDates = (eng: TaxOptimizationEngine, year: number): Date[] =>
      (eng as unknown as { getQuarterlyDueDates(taxYear: number): Date[] }).getQuarterlyDueDates(year);
