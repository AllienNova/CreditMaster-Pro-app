...
export class ReportGenerator {
  ...
  static async generateReport(
    ...
    console.log(`📄 Generating ${options.format} report: ${report.report_id}`);
   ...
   */
  private static async generateJSON(report: AnalyticsReport): Promise<GeneratedReport> {
   ...
   * Generate CSV report
   */
  private static async generateCSV(report: AnalyticsReport): Promise<GeneratedReport> {
    let csv = '';
    ...
    } else if (report.report_type === 'system' && this.isSystemAnalyticsOverview(report.data)) {
...
<body>
  ...
  <div class="summary">
    <strong>Note:</strong> This report was automatically generated by CreditMaster Pro Analytics Engine.
  </div>
</body>
</html>
    `.trim();
  ...
  private static async generatePDF(
    ...
    console.log('📄 PDF generation requires puppeteer or pdfkit');
  ...
  static async generateMultipleFormats(
    ...
    console.log(`📄 Generating reports in ${formats.length} formats`);
    
    const reports = await Promise.all(
      formats.map(format =>
        this.generateReport(report, { format })
      )
    );
   ...
   * Export data to CSV
   */
  static exportToCSV(data: Array<Record<string, unknown>>, headers: string[]): string {
  ...
  private static isUserAnalytics(data: AnalyticsReport['data']): data is UserAnalytics {
    return typeof (data as UserAnalytics)?.total_disputes === 'number';
  }

  private static isSystemAnalyticsOverview(data: AnalyticsReport['data']): data is SystemAnalyticsOverview {
    const candidate = data as Partial<SystemAnalyticsOverview>;
    return Boolean(candidate?.disputes && candidate?.workflows && candidate?.ai_usage);
  }
}
