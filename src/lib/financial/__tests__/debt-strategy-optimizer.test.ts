      ...
      message: {
        content: JSON.stringify({
          ...
          alternativeApproaches: [
            "Pure avalanche for max savings",
            "Pure snowball for motivation",
          ],
        }),
      },
    },
...
describe("DebtStrategyOptimizer", () => {
  ...
  describe("calculateAvalanche", () => {
    ...
    it("should calculate payoff timeline accurately", () => {
      ...
      expect(result.schedule.length).toBeLessThanOrEqual(
        result.totalMonths + 1,
      );
    });
    ...
    it("should handle edge case with zero interest rate", () => {
      const zeroInterestDebt: Debt[] = [{ ...mockDebts[0], interestRate: 0 }];
  ...
  describe("Integration Tests", () => {
    ...
    it("should warn about high-interest debt", async () => {
      const highInterestDebts = [{ ...mockDebts[0], interestRate: 25.99 }];
