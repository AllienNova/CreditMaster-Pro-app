...
describe("SavingsOptimizer", () => {
  ...
  describe("findRecurringCharges()", () => {
    ...
    it("should detect recurring charges with >90% accuracy", async () => {
      ...
      const netflix = recurringCharges.find(
        (charge) => charge.merchantName === "netflix",
      );
    ...
    it("should identify subscriptions with >85% accuracy", async () => {
      ...
      const accuracyRate =
        (subscriptions.length / recurringCharges.length) * 100;
      expect(accuracyRate).toBeGreaterThanOrEqual(85);
    });

    it("should filter by minimum amount", async () => {
      const recurringCharges = await optimizer.findRecurringCharges(
        mockUserId,
        10,
      );
  ...
  describe("suggestCancelableSubscriptions()", () => {
    ...
    it("should suggest cancelable subscriptions", async () => {
      const recommendations =
        await optimizer.suggestCancelableSubscriptions(mockUserId);
    ...
    it("should calculate potential savings correctly", async () => {
      ...
      recommendations.forEach((rec) => {
        ...
        expect(rec.potentialAnnualSavings).toBe(
          rec.potentialMonthlySavings * 12,
        );
      });
    });
  ...
  describe("analyzeSpendingForSavings()", () => {
    ...
    it("should analyze spending and return comprehensive analysis", async () => {
      const analysis = await optimizer.analyzeSpendingForSavings(
        mockUserId,
        "monthly",
      );
    ...
    it("should identify savings opportunities", async () => {
      ...
      analysis.opportunities.forEach((opp) => {
        expect(opp.type).toMatch(
          /subscription|spending_reduction|category_optimization|recurring_charge|one_time/,
        );
    ...
    it("should analyze spending trends", async () => {
      ...
      expect(analysis.trends).toBeDefined();
      expect(analysis.trends.spendingTrend).toMatch(
        /increasing|decreasing|stable/,
      );
      expect(analysis.trends.savingsTrend).toMatch(
  ...
  describe("generateSavingsGoalRecommendations()", () => {
    ...
    it("should provide realistic goal amounts", async () => {
      const recommendations =
        await optimizer.generateSavingsGoalRecommendations(mockUserId, 10000);
      ...
      recommendations.forEach((rec) => {
        ...
        if (rec.recommendedMonthlyContribution > 0) {
          expect(rec.recommendedMonthlyContribution).toBeLessThanOrEqual(
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle irregular spending patterns", async () => {
      ...
      const randomStore = recurringCharges.find(
        (charge) => charge.merchantName === "Random Store",
      );
      if (randomStore) {
