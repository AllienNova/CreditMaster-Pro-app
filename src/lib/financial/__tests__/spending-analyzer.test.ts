...
describe('SpendingAnalyzer', () => {
  ...
  describe('analyzeSpendingPatterns()', () => {
    it('should analyze spending patterns for monthly period', async () => {
      const analysis = await analyzer.analyzeSpendingPatterns(mockUserId, 'monthly');
    ...
    it('should calculate spending velocity correctly', async () => {
      ...
      expect(analysis.velocity.trend).toMatch(/accelerating|decelerating|stable/);
  ...
  describe('detectAnomalies()', () => {
    ...
    it('should adjust sensitivity correctly', async () => {
      const lowSensitivity = await analyzer.detectAnomalies(mockUserId, 'low', 30);
      const highSensitivity = await analyzer.detectAnomalies(mockUserId, 'high', 30);
    ...
    it('should identify anomalies requiring action', async () => {
      ...
      expect(result.summary.requiresImmediateAction).toBe(actionRequired.length);
    });
  });
  ...
  describe('getSpendingTrends()', () => {
    ...
    it('should generate forecast with confidence level', async () => {
      ...
      expect(trends.forecast.range.low).toBeLessThan(trends.forecast.range.expected);
      expect(trends.forecast.range.expected).toBeLessThan(trends.forecast.range.high);
    });

    it('should filter by specific categories', async () => {
      const categories = ['Groceries', 'Transportation'];
      const trends = await analyzer.getSpendingTrends(mockUserId, '3m', categories);

      expect(trends.categoryTrends.length).toBeLessThanOrEqual(categories.length);
    });
  });
  ...
  describe('generateInsights()', () => {
    ...
    it('should filter insights by type', async () => {
      const patternsOnly = await analyzer.generateInsights(mockUserId, 'patterns');
      const trendsOnly = await analyzer.generateInsights(mockUserId, 'trends');
      const anomaliesOnly = await analyzer.generateInsights(mockUserId, 'anomalies');
    ...
    it('should include actionable insights (>80% relevance)', async () => {
      ...
      const actionableInsights = result.insights.filter(i => i.actionItems.length > 0);
  ...
  describe('compareToLastPeriod()', () => {
    it('should compare current period to previous period', async () => {
      const comparison = await analyzer.compareToLastPeriod(mockUserId, '1m', 'total');
