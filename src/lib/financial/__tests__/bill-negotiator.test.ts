...
const createMockQueryChain = (userId?: string): Record<string, any> => {
  ...
  chain.then = (resolve: any) => {
    ...
    const data =
      storedUserId && storedUserId !== mockUserId ? [] : mockTransactions;
...
const mockChat = jest.fn().mockResolvedValue({
  content: JSON.stringify({
    ...
    mainPoints: [
      {
        point: "Market rate is lower",
        supportingData: "Competitors offer $49.99",
        priority: "high",
      },
      {
        point: "Loyal customer",
        supportingData: "24 months tenure",
        priority: "high",
      },
    ],
    counterarguments: [
      {
        objection: "Standard rate",
        response: "What retention offers are available?",
      },
    ],
    fallbackOptions: [
      {
        option: "Promotional rate",
...
describe("BillNegotiator", () => {
  ...
  describe("identifyNegotiableBills()", () => {
    it("should identify negotiable telecom bills", async () => {
      ...
      const telecomBills = bills.filter(
        (b) => b.billType === "internet" || b.billType === "mobile",
      );
  ...
  describe("analyzeMarketRates()", () => {
    it("should fetch current market rates for providers", async () => {
      const analysis = await negotiator.analyzeMarketRates(
        "internet",
        "Comcast",
      );
    ...
    it("should handle location-based rate variations", async () => {
      const analysis1 = await negotiator.analyzeMarketRates(
        "internet",
        "Comcast",
        "CA",
      );
      const analysis2 = await negotiator.analyzeMarketRates(
        "internet",
        "Comcast",
        "NY",
      );
  ...
  describe("generateNegotiationScript()", () => {
    ...
    it("should adapt script based on user profile (tenure, payment history)", async () => {
      ...
      const script = await negotiator.generateNegotiationScript(
        mockBill,
        userProfile,
      );
  ...
  describe("trackNegotiationOutcome()", () => {
    it("should record successful negotiations with savings", async () => {
      ...
      await expect(
        negotiator.trackNegotiationOutcome("bill-comcast-test", outcome),
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle new users with minimal data", async () => {
      const savingsEstimate =
        await negotiator.calculatePotentialSavings("new-user");
