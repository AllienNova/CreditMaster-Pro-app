...
const CATEGORY_BENCHMARKS: Record<string, { min: number; max: number; ideal: number }> = {
  housing: { min: 0.25, max: 0.35, ideal: 0.28 },
  transportation: { min: 0.10, max: 0.15, ideal: 0.12 },
  food: { min: 0.10, max: 0.15, ideal: 0.12 },
  utilities: { min: 0.05, max: 0.10, ideal: 0.07 },
  healthcare: { min: 0.05, max: 0.10, ideal: 0.07 },
  insurance: { min: 0.10, max: 0.15, ideal: 0.12 },
  savings: { min: 0.10, max: 0.20, ideal: 0.15 },
  debt_payments: { min: 0.05, max: 0.20, ideal: 0.10 },
  entertainment: { min: 0.05, max: 0.10, ideal: 0.05 },
  personal: { min: 0.05, max: 0.10, ideal: 0.05 },
  education: { min: 0.00, max: 0.10, ideal: 0.05 },
  other: { min: 0.00, max: 0.10, ideal: 0.02 },
};
...
class BudgetOptimizer {
   ...
   */
  async optimizeBudget(request: OptimizeBudgetRequest): Promise<BudgetOptimizationResult> {
    const { userId, includeTemplates = true, includeScenarios = true, targetSavingsRate } = request;
    ...
    const totalBudgeted = categorySummaries.reduce((sum, c) => sum + c.budgeted, 0);
    const totalSpent = categorySummaries.reduce((sum, c) => sum + c.spent, 0);

    // Generate optimizations
    const optimizations = this.generateOptimizations(categorySummaries, context);
    const potentialMonthlySavings = optimizations.reduce((sum, o) => sum + o.potentialSavings, 0);
    ...
    const aiAnalysis = await this.generateAIAnalysis(context, categorySummaries, optimizations);
   ...
   */
  private analyzeBudgetCategories(context: FinancialContext): BudgetCategorySummary[] {
  ...
  private generateOptimizations(
    ...
    for (const category of categories) {
      const benchmark = CATEGORY_BENCHMARKS[category.category] || CATEGORY_BENCHMARKS.other;
      const idealAmount = income * benchmark.ideal;
      ...
      if (category.spent > maxAmount) {
        ...
        optimizations.push({
          ...
          difficulty: savings > income * 0.1 ? 'hard' : savings > income * 0.05 ? 'moderate' : 'easy',
  ...
  private getSuggestedTemplate(annualIncome: number): BudgetTemplate {
    const template = BUDGET_TEMPLATES.find(
      (t) => annualIncome >= t.forIncomeRange.min && annualIncome < t.forIncomeRange.max
    ) || BUDGET_TEMPLATES[1];
    const monthlyIncome = annualIncome / 12;
    template.categories = Object.entries(CATEGORY_BENCHMARKS).map(([category, benchmark]) => ({
      category,
      ...
      isRequired: ['housing', 'food', 'utilities', 'transportation'].includes(category),
      tips: this.getCategoryTips(category),
    }));
    return template;
  }

  private generateScenarios(context: FinancialContext, optimizations: BudgetOptimization[], targetSavingsRate?: number): BudgetScenario[] {
    const scenarios: BudgetScenario[] = [];
    const easyOpts = optimizations.filter((o) => o.difficulty === 'easy');
    if (easyOpts.length > 0) {
      const easySavings = easyOpts.reduce((sum, o) => sum + o.potentialSavings, 0);
      scenarios.push(this.createScenario('easy_wins', 'Easy Wins', 'Simple changes',
        easyOpts.map((o) => ({ category: o.category, currentAmount: o.currentAmount, newAmount: o.suggestedAmount, difference: o.currentAmount - o.suggestedAmount })),
        easySavings, context));
    }
    return scenarios;
  }

  private createScenario(id: string, name: string, description: string, changes: BudgetChange[], projectedSavings: number, context: FinancialContext): BudgetScenario {
    const impact: BudgetImpact = {
      ...
      debtPayoffAcceleration: context.debts.totalDebt > 0 ? Math.round(context.debts.totalDebt / (context.debts.monthlyPayments + projectedSavings) - context.debts.totalDebt / context.debts.monthlyPayments) : 0,
      lifestyleImpact: projectedSavings > context.transactions.totalIncome * 0.1 ? 'significant' : 'minimal',
    };
    return { id, name, description, changes, projectedSavings, impact };
  }

  private async generateAIAnalysis(context: FinancialContext, categories: BudgetCategorySummary[], optimizations: BudgetOptimization[]): Promise<string> {
    const aiService = this.getAIService();
    if (!aiService) return this.getDefaultAnalysis(optimizations);
    try {
      const response = await aiService.chat(AI_MODEL, [
        { role: 'system', content: 'You are a financial advisor. Be concise.' },
        { role: 'user', content: `Analyze: Income $${context.transactions.totalIncome}/mo, Savings ${((context.transactions.netCashFlow / context.transactions.totalIncome) * 100).toFixed(1)}%` },
      ], { temperature: 0.3, max_tokens: 150 });
      return response.choices[0]?.message?.content || this.getDefaultAnalysis(optimizations);
    } catch { return this.getDefaultAnalysis(optimizations); }
  }
  ...
  private determineTrend(spent: number, budgeted: number): 'increasing' | 'decreasing' | 'stable' {
  ...
  private extractKeyInsights(categories: BudgetCategorySummary[], optimizations: BudgetOptimization[]): string[] {
    const insights: string[] = [];
    const overBudget = categories.filter((c) => c.percentUsed > 100);
    if (overBudget.length > 0) insights.push(`${overBudget.length} categories over budget`);
    const totalSavings = optimizations.reduce((sum, o) => sum + o.potentialSavings, 0);
    if (totalSavings > 0) insights.push(`Potential savings: $${totalSavings.toFixed(0)}/month`);
    return insights;
  }

  private getDefaultAnalysis(optimizations: BudgetOptimization[]): string {
    const savings = optimizations.reduce((sum, o) => sum + o.potentialSavings, 0);
    return savings > 0 ? `Potential savings: $${savings.toFixed(0)}/month` : 'Your budget is well-managed.';
  }
}
