...
const SUBSCRIPTION_PATTERNS: Record<string, { type: string; category: string }> = {
...
export class SavingsOptimizer {
  ...
  async analyzeSpendingForSavings(
    ...
    const transactions = await this.fetchTransactionsForPeriod(userId, periodStart, periodEnd);
    ...
    const subscriptionRecommendations = await this.suggestCancelableSubscriptions(userId);
    ...
    const categoryBreakdown = this.calculateCategoryBreakdown(transactions, opportunities);
  ...
  async findRecurringCharges(
    ...
    for (const [merchantName, merchantTransactions] of Object.entries(merchantGroups)) {
      ...
      const pattern = this.detectRecurringPattern(merchantName, merchantTransactions);
      if (pattern && pattern.confidence >= RECURRING_CHARGE_CONFIDENCE_THRESHOLD) {
    ...
    const recurringCharges: RecurringCharge[] = recurringPatterns
      ...
      .map((pattern) => {
        const isSubscription = this.isSubscriptionMerchant(pattern.merchantName);
        ...
        return {
          ...
          firstDetectedAt: pattern.transactions[pattern.transactions.length - 1].date,
          lastChargeAt: pattern.transactions[0].date,
          nextExpectedAt: this.calculateNextExpectedDate(pattern),
          variance: pattern.variance,
          isSubscription,
          subscriptionType: subscriptionInfo?.type as RecurringCharge['subscriptionType'],
   ...
   */
  async suggestCancelableSubscriptions(userId: string): Promise<SubscriptionRecommendation[]> {
    const recurringCharges = await this.findRecurringCharges(userId);
    ...
    for (const subscription of subscriptions) {
      const recommendation = await this.analyzeSubscription(userId, subscription);
      if (recommendation) {
    ...
    return recommendations.sort((a, b) => b.potentialAnnualSavings - a.potentialAnnualSavings);
  }

  /**
   * Calculate total potential savings across all recommendations
   */
  async calculatePotentialSavings(userId: string): Promise<PotentialSavingsSummary> {
    ...
    const totalMonthlySavings =
      subscriptionSavings + categoryOptimizationSavings + recurringChargeSavings + oneTimeSavings;
    ...
    return {
      ...
      topOpportunities,
      implementationDifficulty: this.calculateImplementationDifficulty(topOpportunities),
      estimatedTimeToImplement: this.estimateImplementationTime(topOpportunities),
    };
  }
  ...
  async generateSavingsGoalRecommendations(
    ...
    const emergencyFundGoal = existingGoals.find((g) => g.category === 'emergency_fund');
    const recommendedEmergencyFund = monthlyIncome * 6; // 6 months of expenses

    if (!emergencyFundGoal || emergencyFundGoal.targetAmount < recommendedEmergencyFund) {
      ...
      recommendations.push({
        ...
        recommendedMonthlyContribution: Math.min(availableMonthlySavings * 0.5, remainingAmount / 12),
        targetDate: this.calculateTargetDate(remainingAmount, availableMonthlySavings * 0.5),
        priority: 1,
        reasoning: 'Emergency fund is the foundation of financial security. Aim for 6 months of expenses.',
        confidence: 95,
        basedOn: [
          { factor: 'Monthly Income', value: this.formatCurrency(monthlyIncome) },
          { factor: 'Recommended Amount', value: this.formatCurrency(recommendedEmergencyFund) },
        ],
        aiGenerated: false,
      });
    }
    ...
    if (hasHighInterestDebt) {
      recommendations.push({
        ...
        recommendedMonthlyContribution: Math.min(availableMonthlySavings * 0.3, 500),
        priority: 2,
        reasoning: 'Paying off high-interest debt saves money on interest and improves credit score.',
        confidence: 90,
        basedOn: [
          { factor: 'Available Monthly Savings', value: this.formatCurrency(availableMonthlySavings) },
        ],
        aiGenerated: false,
      });
    }
    ...
    const retirementGoal = existingGoals.find((g) => g.category === 'retirement');
    if (!retirementGoal) {
      recommendations.push({
        id: crypto.randomUUID(),
        type: 'retirement',
        title: 'Start Retirement Savings',
        description: 'Begin saving for retirement with consistent monthly contributions',
        recommendedAmount: monthlyIncome * 12 * 10, // 10 years of income
        recommendedMonthlyContribution: Math.min(availableMonthlySavings * 0.15, monthlyIncome * 0.15),
        priority: 3,
        reasoning: 'Starting early with retirement savings leverages compound interest for long-term growth.',
        confidence: 85,
        basedOn: [
          { factor: 'Monthly Income', value: this.formatCurrency(monthlyIncome) },
    ...
    if (this.aiService) {
      try {
        const aiRecommendations = await this.generateAISavingsGoalRecommendations(
          userId,
          monthlyIncome,
  ...
  private calculateSpendingSummary(
    ...
    const essentialCategories = ['housing', 'utilities', 'groceries', 'transportation', 'healthcare', 'insurance'];
  ...
  private isSubscriptionMerchant(merchantName: string): boolean {
    ...
    return Object.keys(SUBSCRIPTION_PATTERNS).some((pattern) => merchantLower.includes(pattern));
  }

  /**
   * Get subscription info for a merchant
   */
  private getSubscriptionInfo(merchantName: string): { type: string; category: string } | null {
    const merchantLower = merchantName.toLowerCase();
  ...
  private async classifyRecurringChargeImportance(
    ...
    if (cached && Date.now() - cached.timestamp < AI_CACHE_TTL) {
      const cachedData = cached.data as Record<string, RecurringCharge['importance']>;
...
Return JSON array with format: [{"merchant": "name", "importance": "essential|useful|optional|unnecessary", "reason": "brief explanation"}]`;
    ...
    try {
      ...
      for (const classification of classifications) {
        const charge = recurringCharges.find((c) => c.merchantName === classification.merchant);
        if (charge) {
  ...
  private async analyzeSubscription(
    ...
    if (subscription.importance === 'unnecessary') {
      ...
      reason = 'This subscription appears to be unnecessary and can be cancelled to save money.';
    } else if (subscription.importance === 'optional') {
      type = 'cancel';
      potentialSavings = monthlyAmount;
      reason = 'This subscription is optional and could be cancelled if you want to reduce expenses.';
    } else if (subscription.importance === 'useful') {
      type = 'downgrade';
      potentialSavings = monthlyAmount * 0.3; // Assume 30% savings from downgrade
      reason = 'Consider downgrading to a lower-tier plan to save money while keeping the service.';
    } else {
...
Return JSON array with format: [{"merchant": "name", "action": "cancel|downgrade|consolidate|negotiate", "alternatives": [{"name": "alt", "monthlyCost": 0, "savings": 0}], "reason": "explanation"}]`;
    ...
    try {
      ...
      for (const enhancement of enhancements) {
        const recommendation = recommendations.find((r) => r.merchantName === enhancement.merchant);
        if (recommendation) {
  ...
  private async identifySavingsOpportunities(
    ...
    for (const charge of recurringCharges.filter((c) => c.isSubscription)) {
      if (charge.importance === 'unnecessary' || charge.importance === 'optional') {
        opportunities.push({
          id: crypto.randomUUID(),
          type: 'subscription',
          ...
          actionable: true,
          actions: [{ label: 'Cancel Subscription', type: 'cancel', data: { chargeId: charge.id } }],
          aiGenerated: false,
        });
      }
    }
    ...
    for (const [category, amount] of Object.entries(categorySpending)) {
      ...
      if (amount > benchmark * 1.2) {
        ...
        opportunities.push({
          ...
          actionable: true,
          actions: [{ label: 'Set Budget Limit', type: 'reduce', data: { category, limit: benchmark } }],
          aiGenerated: false,
        });
      }
    }
   ...
   */
  private getCategoryBenchmark(category: string, monthlyIncome: number): number {
  ...
  private calculateCategoryBreakdown(
    ...
    for (const [category, currentSpending] of Object.entries(categorySpending)) {
  ...
  private async analyzeSpendingTrends(
    ...
    const currentCategorySpending = this.calculateCategorySpending(transactions);
    ...
    const topSpendingCategories = Object.entries(currentCategorySpending)
      .map(([category, amount]) => {
        ...
        const percentChange = previousAmount > 0 ? ((amount - previousAmount) / previousAmount) * 100 : 0;
    ...
    return {
      spendingTrend,
      savingsTrend: spendingTrend === 'decreasing' ? 'improving' : spendingTrend === 'increasing' ? 'declining' : 'stable',
...
Top Opportunities:
${opportunities.slice(0, 5).map((opp, i) => `${i + 1}. ${opp.title} - Save $${opp.potentialMonthlySavings.toFixed(2)}/month`).join('\n')}
...
Provide brief, actionable insights (1-2 sentences each).`;
  ...
  private generateFallbackInsights(
    ...
    if (summary.savingsRate < 10) {
      insights.push('Your savings rate is below the recommended 20%. Consider implementing the top savings opportunities to improve your financial health.');
    }

    if (opportunities.length > 0) {
      const topOpp = opportunities[0];
      insights.push(`Your biggest savings opportunity is ${topOpp.title}, which could save you $${topOpp.potentialAnnualSavings.toFixed(0)} per year.`);
    }

    if (summary.subscriptionSpending > summary.totalSpending * 0.1) {
      insights.push('Subscriptions account for more than 10% of your spending. Review and cancel unused subscriptions to free up cash.');
    }
  ...
  private async getExistingGoals(userId: string): Promise<SavingsGoal[]> {
    ...
    return goals.map((g) => ({
      ...
      projectedCompletionDate: g.projected_completion_date ? new Date(g.projected_completion_date) : undefined,
      onTrack: g.on_track,
      contributions: [],
      icon: g.icon,
      color: g.color,
...
Return JSON array with format: [{"type": "vacation|major_purchase|education|custom", "title": "Goal Name", "description": "Brief description", "recommendedAmount": 0, "recommendedMonthlyContribution": 0, "priority": 1-5, "reasoning": "Why this goal"}]`;
    ...
    try {
      ...
      return aiGoals.map((goal: any) => ({
        ...
        targetDate: this.calculateTargetDate(goal.recommendedAmount, goal.recommendedMonthlyContribution),
        priority: goal.priority,
        reasoning: goal.reasoning,
        confidence: 80,
        basedOn: [
          { factor: 'Monthly Income', value: this.formatCurrency(monthlyIncome) },
          { factor: 'Available Savings', value: this.formatCurrency(availableMonthlySavings) },
        ],
        aiGenerated: true,
      }));
    } catch (error) {
   ...
   */
  private calculateTargetDate(targetAmount: number, monthlyContribution: number): Date | undefined {
  ...
  private calculateImplementationDifficulty(
    ...
    const avgDifficulty = opportunities.reduce((sum, opp) => {
      const difficultyScore = opp.difficulty === 'easy' ? 1 : opp.difficulty === 'medium' ? 2 : 3;
   ...
   */
  private estimateImplementationTime(opportunities: SavingsOpportunity[]): string {
    if (opportunities.length === 0) return '0 hours';
    const totalMinutes = opportunities.reduce((sum, opp) => {
      const minutes = opp.difficulty === 'easy' ? 15 : opp.difficulty === 'medium' ? 30 : 60;
      return sum + minutes;
    }, 0);
