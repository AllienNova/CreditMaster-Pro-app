...
const REFINANCING_RATES = {
  ...
  home_equity: { excellent: 0.06, good: 0.08, fair: 0.10 },
};
...
class DebtStrategyEngine {
   ...
   */
  async analyzeDebtStrategy(request: AnalyzeDebtStrategyRequest): Promise<DebtStrategyAnalysis> {
    const { userId, extraMonthlyPayment = 0, includeRefinancing = true, targetPayoffDate } = request;
    ...
    const { recommended, reason } = this.determineRecommendedStrategy(strategies, context);

    // Find refinancing opportunities
    const refinancingOpportunities = includeRefinancing
      ? this.findRefinancingOpportunities(debts, context.creditProfile.currentScore)
      : [];

    // Generate AI insights
    const aiInsights = await this.generateAIInsights(context, strategies, refinancingOpportunities);
   ...
   */
  private calculateAvalancheStrategy(debts: DebtSummary, extraPayment: number): DebtStrategyPlan {
    const sortedDebts = [...debts.debts].sort((a, b) => b.interestRate - a.interestRate);
    return this.calculatePayoffPlan('avalanche', 'Avalanche Method', 
      'Pay off highest interest debt first to minimize total interest paid',
      sortedDebts, debts.monthlyPayments + extraPayment, extraPayment);
  }

  /**
   * Calculate Snowball strategy (smallest balance first)
   */
  private calculateSnowballStrategy(debts: DebtSummary, extraPayment: number): DebtStrategyPlan {
    const sortedDebts = [...debts.debts].sort((a, b) => a.balance - b.balance);
    return this.calculatePayoffPlan('snowball', 'Snowball Method',
   ...
   */
  private calculateHybridStrategy(debts: DebtSummary, extraPayment: number): DebtStrategyPlan {
    // Hybrid: Start with smallest debt for quick win, then switch to avalanche
    const sortedDebts = [...debts.debts];
    const smallest = sortedDebts.reduce((min, d) => d.balance < min.balance ? d : min, sortedDebts[0]);
    const others = sortedDebts.filter(d => d.id !== smallest.id).sort((a, b) => b.interestRate - a.interestRate);
    const hybridOrder = [smallest, ...others];
    
    return this.calculatePayoffPlan('hybrid', 'Hybrid Method',
      'Quick win first, then focus on high interest',
      hybridOrder, debts.monthlyPayments + extraPayment, extraPayment);
  }
  ...
  private calculatePayoffPlan(
    ...
    name: string,
    description: string,
    orderedDebts: Array<{ id: string; name: string; balance: number; interestRate: number; minimumPayment: number }>,
    ...
    while (remainingDebts.length > 0 && month < 360) { // Max 30 years
      ...
      for (const debt of paidOff) {
        ...
        milestones.push({
          ...
          debtName: debt.name,
          totalPaidOff,
          remainingDebt: remainingDebts.reduce((sum, d) => sum + Math.max(0, d.currentBalance), 0) - debt.balance,
    ...
    return {
      ...
      quickWins,
      motivationScore: strategy === 'snowball' ? 85 : strategy === 'hybrid' ? 75 : 65,
    };
  }

  private determineRecommendedStrategy(strategies: DebtStrategyPlan[], context: FinancialContext): { recommended: string; reason: string } {
    ...
    if (interestDiff > 500) {
      return { recommended: 'avalanche', reason: `Saves $${interestDiff.toFixed(0)} in interest over snowball method` };
    }
    // If user has high debt-to-income, recommend snowball for motivation
    if (context.debts.debtToIncomeRatio > 40) {
      return { recommended: 'snowball', reason: 'Quick wins help maintain motivation with high debt load' };
    }
    // Default to hybrid
    return { recommended: 'hybrid', reason: 'Balances quick wins with interest savings' };
  }

  private findRefinancingOpportunities(debts: DebtSummary, creditScore: number): RefinancingOpportunity[] {
    const opportunities: RefinancingOpportunity[] = [];
    const creditTier = creditScore >= 740 ? 'excellent' : creditScore >= 670 ? 'good' : 'fair';

    for (const debt of debts.highInterestDebts) {
      const potentialRate = REFINANCING_RATES.personal_loan[creditTier];
      if (debt.interestRate > potentialRate + 2) { // At least 2% savings
        const monthlySavings = (debt.balance * (debt.interestRate - potentialRate) / 100) / 12;
        opportunities.push({
          debtId: debt.id,
          debtName: debt.name,
          ...
          requirements: ['Good credit score', 'Stable income', 'Low debt-to-income ratio'],
          considerations: ['May have origination fee', 'Fixed vs variable rate'],
        });
      }
    }
  ...
  private async generateAIInsights(context: FinancialContext, strategies: DebtStrategyPlan[], refinancing: RefinancingOpportunity[]): Promise<string[]> {
    ...
    try {
      ...
      const response = await aiService.chat(AI_MODEL, [
        { role: 'system', content: 'You are a debt counselor. Give 3 brief, actionable tips as a JSON array of strings.' },
  ...
  private getStrategyAdvantages(strategy: string): string[] {
    const advantages: Record<string, string[]> = {
      avalanche: ['Minimizes total interest paid', 'Fastest debt-free date', 'Mathematically optimal'],
      snowball: ['Quick wins boost motivation', 'Simplifies finances faster', 'Psychological benefits'],
      hybrid: ['Balance of savings and motivation', 'Flexible approach', 'Best of both methods'],
    };
  ...
  private getStrategyDisadvantages(strategy: string): string[] {
    const disadvantages: Record<string, string[]> = {
      avalanche: ['May take longer to see progress', 'Can be demotivating', 'Requires discipline'],
      snowball: ['Pays more interest overall', 'Slower debt-free date', 'Not mathematically optimal'],
  ...
  private getMotivationalTips(debts: DebtSummary): string[] {
    return [
      ...
      `You can be debt-free in ${Math.ceil(debts.totalDebt / debts.monthlyPayments)} months or less!`,
    ];
  }

  private getWarningFlags(debts: DebtSummary, context: FinancialContext): string[] {
    const flags: string[] = [];
    if (debts.debtToIncomeRatio > 50) flags.push('High debt-to-income ratio - consider debt counseling');
    if (debts.highInterestDebts.length > 0) flags.push('High interest debt is costing you significantly');
    if (context.accounts.totalSavings < 1000) flags.push('Build small emergency fund to avoid new debt');
    return flags;
  }

  private getDefaultInsights(debts: DebtSummary): string[] {
    return [
  ...
  private createDebtFreeAnalysis(userId: string): DebtStrategyAnalysis {
    return {
      ...
      refinancingOpportunities: [],
      aiInsights: ['Stay debt-free by building emergency fund', 'Invest your former debt payments'],
