...
export function calculateMACD(
  ...
  if (macdLine.length >= signalPeriod) {
    ...
    let signal =
      macdLine.slice(0, signalPeriod).reduce((a, b) => a + b.value, 0) /
      signalPeriod;
    ...
    for (let i = signalPeriod - 1; i < macdLine.length; i++) {
      ...
      const prevHistogram =
        i > signalPeriod - 1 ? macdLine[i - 1].value - signal : histogram;
...
export function calculateADX(data: OHLCV[], period: number = 14): ADXResult[] {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (plusDMs.length >= period) {
      ...
      const smoothedMinusDM = minusDMs
        .slice(-period)
        .reduce((a, b) => a + b, 0);
...
export function calculateVWAP(
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const variance =
      tpvList.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
      tpvList.length;
