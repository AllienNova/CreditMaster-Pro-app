...
function makeLLMAnalysis(overrides: Partial<LLMAnalysis> = {}): LLMAnalysis {
  return {
    ...
    signalQuality: { technical: 0.8, fundamental: 0.7, sentiment: 0.6, overall: 0.7 },
    conflicts: [],
...
function makeTechnicalSignal(overrides: Partial<TechnicalSignal> = {}): TechnicalSignal {
  return {
    symbol: 'AAPL',
    indicator: 'RSI',
    signal: 'bullish',
...
describe('SignalFusionService', () => {
  ...
  describe('fuseSignals', () => {
    ...
    it('should generate buy action when all sources are bullish', async () => {
      const input = makeInput({
        ...
        llmAnalysis: makeLLMAnalysis({ overallSignal: 'strong_buy', confidence: 0.9 }),
        technicalSignals: [makeTechnicalSignal({ signal: 'bullish', strength: 90 })],
      });
    ...
    it('should generate sell action when all sources are bearish', async () => {
      const input = makeInput({
        ...
        llmAnalysis: makeLLMAnalysis({ overallSignal: 'strong_sell', confidence: 0.9 }),
        technicalSignals: [makeTechnicalSignal({ signal: 'bearish', strength: 90 })],
      });
    ...
    it('should return hold/no_action for conflicting signals', async () => {
      const input = makeInput({
        ruleBasedSignals: [makeRuleSignal({ side: 'buy', confidence: 0.9 })],
        mlPrediction: makeMLPrediction({ signal: 'sell', confidence: 0.9 }),
        llmAnalysis: makeLLMAnalysis({ overallSignal: 'sell', confidence: 0.9 }),
        technicalSignals: [makeTechnicalSignal({ signal: 'bullish', strength: 90 })],
      });
    ...
    it('should include tradeParams for actionable signals', async () => {
      const input = makeInput({
        ...
        llmAnalysis: makeLLMAnalysis({ overallSignal: 'strong_buy', confidence: 0.95 }),
        technicalSignals: [makeTechnicalSignal({ signal: 'bullish', strength: 95 })],
      });
      ...
      if (result.action !== 'hold' && result.action !== 'no_action') {
        ...
        expect(result.tradeParams!.positionSizePercent).toBeGreaterThanOrEqual(1);
  ...
  describe('signal normalization', () => {
    ...
    it('should normalize LLM no_action to 0', async () => {
      const input = makeInput({
        llmAnalysis: makeLLMAnalysis({ overallSignal: 'no_action', confidence: 0.9 }),
      });
    ...
    it('should normalize technical neutral to 0', async () => {
      const input = makeInput({
        technicalSignals: [makeTechnicalSignal({ signal: 'neutral', strength: 80 })],
      });
  ...
  describe('score to action thresholds', () => {
    it('should return no_action for very low confidence', async () => {
      ...
      const input = makeInput({
        ...
        technicalSignals: [makeTechnicalSignal({ signal: 'neutral', strength: 1 })],
      });
  ...
  describe('getWeights / setWeights', () => {
    it('should return default weights', () => {
      ...
      expect(w.ml).toBeCloseTo(0.30, 2);
      expect(w.llm).toBeCloseTo(0.25, 2);
      expect(w.technical).toBeCloseTo(0.20, 2);
    });
