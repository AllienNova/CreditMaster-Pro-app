...
import { PositionManager, createPositionManager, getPositionManager } from '../positions/position-manager';
...
function makeMockBrokerClient(overrides: Partial<BrokerClient> = {}): BrokerClient {
  return {
...
describe('OrderManager Integration Tests', () => {
  ...
  describe('Market Order Lifecycle', () => {
    it('should create a market order with valid fields and pending status', async () => {
      ...
      const { order, validation } = await orderManager.createOrder(
        request, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should submit a pending order to the broker', async () => {
      const { order } = await orderManager.createOrder(
        makeBuyMarketRequest(), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should open a position after fill and calculate zero initial P&L', async () => {
      ...
      const position = await positionManager.openPosition(
        fill, order!, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should calculate realized P&L when position is closed', async () => {
      ...
      const position = await positionManager.openPosition(
        buyFill, buyOrder, TEST_USER_ID, TEST_ACCOUNT_ID,
      );

      // Sell at 160
      const { position: closedPos, realizedPL } = await positionManager.closePosition({
  ...
  describe('Limit Order with Partial Fills', () => {
    it('should validate limit price requirement', async () => {
      ...
      const { order, validation } = await orderManager.createOrder(
        { symbol: 'AAPL', side: 'buy', quantity: 100, type: 'limit', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );

      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'MISSING_LIMIT_PRICE')).toBe(true);
    ...
    it('should build position from partial fills (averaging in)', async () => {
      ...
      const position = await positionManager.openPosition(
        fill1, order!, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
      expect(position.quantity).toBe(100);
      ...
      const updatedPosition = await positionManager.openPosition(
        fill2, order!, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
  ...
  describe('Stop Order Lifecycle', () => {
    it('should validate stop price requirement', async () => {
      const { order, validation } = await orderManager.createOrder(
        { symbol: 'AAPL', side: 'buy', quantity: 50, type: 'stop', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );

      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'MISSING_STOP_PRICE')).toBe(true);
      expect(order).toBeNull();
    });
    ...
    it('should validate stop_limit orders require both prices', async () => {
      ...
      const { validation: v1 } = await orderManager.createOrder(
        { symbol: 'AAPL', side: 'buy', quantity: 50, type: 'stop_limit', stopPrice: 155, timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );
      expect(v1.isValid).toBe(false);
      expect(v1.errors.some((e) => e.code === 'MISSING_LIMIT_PRICE')).toBe(true);

      // stop_limit with limit price but missing stop price
      const { validation: v2 } = await orderManager.createOrder(
        { symbol: 'AAPL', side: 'buy', quantity: 50, type: 'stop_limit', limitPrice: 156, timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );
      ...
      const { order, validation: v3 } = await orderManager.createOrder(
        { symbol: 'AAPL', side: 'buy', quantity: 50, type: 'stop_limit', stopPrice: 155, limitPrice: 156, timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should handle sell stop for short protection', async () => {
      ...
      const shortOrder = (await orderManager.createOrder(
        { symbol: 'TSLA', side: 'sell', quantity: 50, type: 'market', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
      const shortFill = makeFill(shortOrder, 50, 200.0);
      const position = await positionManager.openPosition(
        shortFill, shortOrder, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
  ...
  describe('Order Cancellation', () => {
    it('should cancel a pending order', async () => {
      const { order } = await orderManager.createOrder(
        makeBuyLimitRequest(), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should cancel all open orders', async () => {
      await orderManager.createOrder(
        makeBuyLimitRequest({ symbol: 'AAPL' }), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
      await orderManager.createOrder(
        makeBuyLimitRequest({ symbol: 'GOOGL' }), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
      await orderManager.createOrder(
        makeBuyLimitRequest({ symbol: 'MSFT' }), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
  ...
  describe('Position Reconciliation After Multiple Fills', () => {
    it('should correctly average entry price across multiple fills', async () => {
      const order1 = (await orderManager.createOrder(
        makeBuyLimitRequest({ symbol: 'MSFT', quantity: 100, limitPrice: 300 }),
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
      ...
      await positionManager.openPosition(
        fill1, order1, TEST_USER_ID, TEST_ACCOUNT_ID,
      );

      // Fill 2: add 50 more @ 302
      const order2 = (await orderManager.createOrder(
        makeBuyLimitRequest({ symbol: 'MSFT', quantity: 50, limitPrice: 302 }),
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
      const fill2 = makeFill(order2, 50, 302.0);
      await positionManager.openPosition(
        fill2, order2, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should handle partial close and track remaining position', async () => {
      ...
      const sellOrder = (await orderManager.createOrder(
        makeSellLimitRequest({ symbol: 'NVDA', quantity: 80, limitPrice: 520 }),
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
      const sellFill = makeFill(sellOrder, 80, 520.0);
      const { position: reduced, realizedPL } = await positionManager.reducePosition(
        position.id, sellFill, sellOrder,
      );
    ...
    it('should fully close position when reducing to zero', async () => {
      ...
      const sellOrder = (await orderManager.createOrder(
        makeSellLimitRequest({ symbol: 'META', quantity: 50, limitPrice: 420 }),
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
      const sellFill = makeFill(sellOrder, 50, 420.0);
      const { position: closed, realizedPL } = await positionManager.reducePosition(
        position.id, sellFill, sellOrder,
      );
  ...
  describe('Error Scenarios', () => {
    it('should reject order with empty symbol', async () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'REQUIRED_FIELD')).toBe(true);
      expect(order).toBeNull();
    });
    ...
    it('should reject order with zero quantity', async () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'INVALID_QUANTITY')).toBe(true);
      expect(order).toBeNull();
    });
    ...
    it('should reject order exceeding max order value', async () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'MAX_ORDER_VALUE')).toBe(true);
      expect(order).toBeNull();
    });

    it('should reject order when max open orders reached', async () => {
      const om = createOrderManager({ maxOpenOrders: 2, maxOrderValue: 1_000_000 });
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'MAX_OPEN_ORDERS')).toBe(true);
      expect(order).toBeNull();
    });

    it('should reject order when max daily orders reached', async () => {
      const om = createOrderManager({ maxDailyOrders: 1, maxOrderValue: 1_000_000 });
      ...
      const { order, validation } = await om.createOrder(
        makeBuyMarketRequest({ symbol: 'GOOGL' }), TEST_USER_ID, TEST_ACCOUNT_ID,
      );

      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'MAX_DAILY_ORDERS')).toBe(true);
      expect(order).toBeNull();
    });
    ...
    it('should throw when reducing a non-existent position', async () => {
      const fakeOrder = (await orderManager.createOrder(
        makeSellLimitRequest(), TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
  ...
  describe('Edge Cases', () => {
    it('should handle sell order (short) position side correctly', async () => {
      const { order } = await orderManager.createOrder(
        { symbol: 'SPY', side: 'sell', quantity: 100, type: 'market', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should handle extended hours adjustment', async () => {
      ...
      expect(validation.warnings.some((w) => w.field === 'extendedHours')).toBe(true);
      // Order should be adjusted to not use extended hours
      expect(order).not.toBeNull();
    });

    it('should warn about missing stop loss when required', async () => {
      const om = createOrderManager({ requireStopLoss: true, maxOrderValue: 1_000_000 });
      ...
      expect(validation.isValid).toBe(true);
      expect(validation.warnings.some((w) => w.field === 'stopLossPrice')).toBe(true);
    });
    ...
    it('should handle multiple symbols in open orders simultaneously', async () => {
      ...
      // Cancel one
      const aaplOrder = orderManager.getOpenOrders().find((o) => o.symbol === 'AAPL');
    ...
    it('should return all order events when no orderId filter is passed', async () => {
      await orderManager.createOrder(
        makeBuyMarketRequest({ symbol: 'A' }), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
      await orderManager.createOrder(
        makeBuyMarketRequest({ symbol: 'B' }), TEST_USER_ID, TEST_ACCOUNT_ID,
      );
  ...
  describe('Broker Reconciliation', () => {
    ...
    it('should detect orders missing on broker side', async () => {
      ...
      expect(result.corrections.some((c) => c.type === 'missing_broker')).toBe(true);
    });
  ...
  describe('Position P&L Calculations', () => {
    ...
    it('should track total P&L as sum of realized and unrealized', async () => {
      ...
      const sellOrder = (await orderManager.createOrder(
        makeSellLimitRequest({ symbol: 'XYZ', quantity: 40, limitPrice: 110 }),
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
    ...
    it('should handle partial close via closePosition with remaining qty', async () => {
      ...
      const { position: partial, realizedPL } = await positionManager.closePosition({
        positionId: position.id,
        closePrice: 220.0,
        closeQuantity: 40,
        timestamp: new Date(),
  ...
  describe('Validation Edge Cases', () => {
    it('should reject order with invalid side value', async () => {
      const { order, validation } = await orderManager.createOrder(
        { symbol: 'AAPL', side: 'invalid' as 'buy', quantity: 100, type: 'market', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );

      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.code === 'INVALID_SIDE')).toBe(true);
      expect(order).toBeNull();
    });

    it('should reject order with missing side', async () => {
      const { order, validation } = await orderManager.createOrder(
        { symbol: 'AAPL', side: '' as 'buy', quantity: 100, type: 'market', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );
    ...
    it('should accumulate multiple validation errors', async () => {
      const { order, validation } = await orderManager.createOrder(
        { symbol: '', side: 'bad' as 'buy', quantity: -5, type: 'limit', timeInForce: 'day' },
        TEST_USER_ID, TEST_ACCOUNT_ID,
      );
  ...
  describe('Position Filtering and Querying', () => {
    ...
    beforeEach(async () => {
      ...
      for (let i = 0; i < symbols.length; i++) {
        ...
        const { order } = await orderManager.createOrder(
          { symbol: symbols[i], side, quantity: 100, type: 'market', timeInForce: 'day', strategyId: i === 0 ? 'strat-A' : undefined },
          TEST_USER_ID, TEST_ACCOUNT_ID,
        );
    ...
    it('should filter positions by status', async () => {
      const openPositions = await positionManager.getPositions({ status: ['open'] });
      ...
      const stillOpen = await positionManager.getPositions({ status: ['open'] });
    ...
    it('should filter positions by strategyId', async () => {
      const result = await positionManager.getPositions({ strategyId: 'strat-A' });
      expect(result).toHaveLength(1);
  ...
  describe('Trade History Querying', () => {
    ...
    it('should limit trade results', async () => {
      ...
      const limited = positionManager.getTrades({ symbol: 'LIMIT_TEST', limit: 1 });
  ...
  describe('Portfolio Summary with Short Positions', () => {
    it('should calculate gross and net exposure with both long and short positions', async () => {
      // Long AAPL
      const longOrder = (await orderManager.createOrder(
        makeBuyMarketRequest({ symbol: 'AAPL', quantity: 100 }),
        TEST_USER_ID, TEST_ACCOUNT_ID,
      )).order!;
      const longFill = makeFill(longOrder, 100, 150.0);
      await positionManager.openPosition(
        longFill, longOrder, TEST_USER_ID, TEST_ACCOUNT_ID,
      );
  ...
  describe('Position Reconciliation with Broker', () => {
    it('should match positions with broker when quantities agree', async () => {
      ...
      const result = await positionManager.reconcileWithBroker([
        { symbol: 'RECON_MATCH', side: 'long', qty: 100, avg_entry_price: 150, market_value: 15000, unrealized_pl: 0 },
      ]);
    ...
    it('should detect quantity mismatch and adjust local position', async () => {
      ...
      const result = await positionManager.reconcileWithBroker([
        { symbol: 'RECON_MISMATCH', side: 'long', qty: 80, avg_entry_price: 148, market_value: 11840, unrealized_pl: 0 },
      ]);
    ...
    it('should detect position missing locally', async () => {
      const result = await positionManager.reconcileWithBroker([
        { symbol: 'EXTERNAL_POS', side: 'long', qty: 200, avg_entry_price: 50, market_value: 10000, unrealized_pl: 0 },
      ]);

      expect(result.missingLocal).toBe(1);
      expect(result.corrections.some((c) => c.type === 'missing_local')).toBe(true);
    });
  });
  ...
  describe('Reconcile with autoCloseOnReconcile', () => {
    it('should auto-close local positions missing on broker when enabled', async () => {
      ...
      await autoClosePM.openPosition(fill, order!, TEST_USER_ID, TEST_ACCOUNT_ID);
