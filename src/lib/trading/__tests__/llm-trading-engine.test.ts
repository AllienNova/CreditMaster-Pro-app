...
function makeTradeIdeaParams(
  overrides: Partial<TradeIdeaParams> = {},
): TradeIdeaParams {
  return {
    symbol: "AAPL",
    context: makeContext(),
...
describe("LLMTradingEngine", () => {
  ...
  describe("fallback behavior", () => {
    it("should return defaults when prompt matches no mock category", async () => {
      ...
      const analysis = await engine.analyzeMarketConditions(
        ["XYZ"],
        [makeContext({ symbol: "XYZ" })],
      );
