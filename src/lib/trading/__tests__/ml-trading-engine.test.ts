...
function makeModelConfig(
  overrides: Partial<MLModelConfig> = {},
): MLModelConfig {
  return {
    id: "test-model",
    name: "Test Model",
...
describe("MLTradingEngine", () => {
  ...
  describe("extractFeatures", () => {
    it("should extract features from sufficient historical data (>=50 bars)", async () => {
      ...
      const features = await engine.extractFeatures(
        "AAPL",
        data,
        config.features,
      );
  ...
  describe("predict", () => {
    ...
    it("should return hold/0.5 for gradient_boosting without trained weights", async () => {
      engine.registerModel(
        makeModelConfig({ architecture: "gradient_boosting" }),
      );
      const prediction = await engine.predict(
        "test-model",
        makeFeatureVector(),
    ...
    it("should fall back to hold/0.5 for unknown architecture", async () => {
      engine.registerModel(
        makeModelConfig({
          architecture: "transformer" as MLModelConfig["architecture"],
        }),
      );
      const prediction = await engine.predict(
        "test-model",
  ...
  describe("train", () => {
    ...
    it("should train successfully with valid data and return evaluation", async () => {
      ...
      const trainingData: FeatureVector[] = Array.from(
        { length: 50 },
        (_, i) => ({
          symbol: "AAPL",
  ...
  describe("predictBatch", () => {
    it("should skip symbols with fewer than 50 data points", async () => {
      ...
      const historicalDataMap = new Map<
        string,
        ReturnType<typeof makeHistoricalData>
      >();
  ...
  describe("ensemble architecture", () => {
    it("should handle ensemble with sub-models", async () => {
      engine.registerModel(
        makeModelConfig({ id: "sub-rf", architecture: "random_forest" }),
      );
      engine.registerModel(
        makeModelConfig({ id: "sub-nn", architecture: "neural_network" }),
      );
      engine.registerModel(
        makeModelConfig({
          id: "ensemble-model",
