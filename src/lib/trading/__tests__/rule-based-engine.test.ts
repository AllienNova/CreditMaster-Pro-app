...
function makeRule(overrides: Partial<TradingRule> = {}): TradingRule {
  return {
    ...
    entryConditions: makeConditionGroup('and', [
      makeCondition({ type: 'indicator', indicator: 'rsi', indicatorPeriod: 14, operator: 'lt', value: 30 }),
    ]),
    exitConditions: makeConditionGroup('and', [
      makeCondition({ type: 'indicator', indicator: 'rsi', indicatorPeriod: 14, operator: 'gt', value: 70 }),
    ]),
    positionSizing: { method: 'percent_portfolio', value: 5 },
    stopLoss: { type: 'fixed_percent', value: 2 },
    filters: {},
    execution: { orderType: 'market', timeInForce: 'day', extendedHours: false },
    createdAt: new Date(),
    updatedAt: new Date(),
    ...overrides,
  };
...
describe('RuleBasedEngine', () => {
      ...
      engine.addRule(
        makeRule({
          entryConditions: makeConditionGroup('or', [
            makeCondition({ indicator: 'rsi', indicatorPeriod: 14, operator: 'lt', value: 30 }),
            makeCondition({ indicator: 'sma', indicatorPeriod: 20, operator: 'gt', value: 200 }),
          ]),
        })
      );
  ...
  describe('condition operators', () => {
    it('should evaluate gt, gte, lt, lte operators', async () => {
      ...
      engine.addRule(makeRule({
        id: 'gt-rule',
        entryConditions: makeConditionGroup('and', [
          makeCondition({ indicator: 'rsi', indicatorPeriod: 14, operator: 'gt', value: 70 }),
        ]),
      }));
      ...
      expect(signals.filter(s => s.type === 'entry').length).toBeGreaterThanOrEqual(1);
    });
    ...
    it('should evaluate eq operator with epsilon', async () => {
      engine.addRule(makeRule({
        id: 'eq-rule',
        entryConditions: makeConditionGroup('and', [
          makeCondition({ indicator: 'rsi', indicatorPeriod: 14, operator: 'eq', value: 50 }),
        ]),
      }));
    ...
    it('should evaluate neq operator', async () => {
      engine.addRule(makeRule({
        id: 'neq-rule',
        entryConditions: makeConditionGroup('and', [
          makeCondition({ indicator: 'rsi', indicatorPeriod: 14, operator: 'neq', value: 50 }),
        ]),
      }));
  ...
  describe('position sizing methods', () => {
    it('should calculate fixed_dollar position size', async () => {
      engine.addRule(makeRule({ positionSizing: { method: 'fixed_dollar', value: 10_000 } }));
      const signals = await engine.evaluateRules([makeMarketData()], 100_000);
    ...
    it('should calculate percent_portfolio position size', async () => {
      engine.addRule(makeRule({ positionSizing: { method: 'percent_portfolio', value: 5 } }));
      const signals = await engine.evaluateRules([makeMarketData()], 100_000);
      const entry = signals.find(s => s.type === 'entry');
      if (entry?.quantity) {
    ...
    it('should calculate fixed_shares position size', async () => {
      engine.addRule(makeRule({ positionSizing: { method: 'fixed_shares', value: 100 } }));
      const signals = await engine.evaluateRules([makeMarketData()], 100_000);
      const entry = signals.find(s => s.type === 'entry');
      if (entry?.quantity) {
