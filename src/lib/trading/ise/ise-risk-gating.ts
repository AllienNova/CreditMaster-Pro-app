...
import type { RotationDecision, ActiveInstrumentSet, UserTier } from "./types";
...
export class ISERiskGating {
  ...
  canOpenNewPosition(symbol: string): TradeGateResult {
    ...
    if (this.rotationService.isInCooldown(symbol)) {
      ...
      this.logDecision(
        ...
        "new_entry",
        false,
        `In cooldown (${mins}min remaining)`,
        false,
      );
      return {
        allowed: false,
  ...
  canScaleIn(symbol: string): TradeGateResult {
    ...
    if (
      this.rotationService.isActive(symbol) ||
      this.manualOverrides.has(symbol)
    ) {
    ...
    if (this.config.allowExistingPositionManagement) {
      this.logDecision(
        ...
        "scale_in",
        true,
        "Existing position management allowed",
        false,
      );
      return {
        allowed: true,
        reason:
          "Scale-in allowed for existing positions (not recommended - consider exiting instead)",
      };
    }
    ...
    this.logDecision(
      ...
      "scale_in",
      false,
      "Scale-in blocked for inactive symbol",
      false,
    );
    return {
      allowed: false,
  ...
  canAdjustStop(symbol: string): TradeGateResult {
    ...
    this.logDecision(
      ...
      "stop_adjust",
      true,
      "Stop adjustments always allowed",
      false,
    );
  ...
  private getSuggestions(blockedSymbol: string): string[] {
    ...
    if (activeSymbols.length > 0) {
      suggestions.push(
        `Active alternatives: ${activeSymbols.slice(0, 3).join(", ")}`,
      );
    }

    // Get top ranked that aren't active yet
    const topRanked = this.rankingService.getTopN(5);
    const notActive = topRanked.filter(
      (r) => !activeSymbols.includes(r.symbol),
    );
    if (notActive.length > 0) {
      suggestions.push(
        `Top opportunities: ${notActive
          .slice(0, 2)
          .map((r) => r.symbol)
          .join(", ")}`,
      );
    ...
    if (this.config.allowManualOverride) {
      suggestions.push(
        "Use manual override if you want to trade this symbol anyway",
      );
    }
