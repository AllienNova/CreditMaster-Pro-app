...
export function scoreLiquidity(
  ...
  const volumeRatio =
    Math.log(features.adv20 + 1) / Math.log(config.minVolumeForMaxScore + 1);
  ...
  const spreadScore =
    1 - clamp(spreadPercent / (config.maxSpreadForMaxScore * 10), 0, 1);
  ...
  // Combine scores
  const rawScore =
    (volumeScore * 0.4 + spreadScore * 0.35 + slippageScore * 0.25) *
    assetMultiplier;
...
export function scorePCTTFitness(
  ...
  const chopPenalty =
    features.chopIndex > 50 ? (features.chopIndex - 50) / 50 : 0;
  ...
  const rawScore = sigmoid(
    2 * qScore + 1.5 * geometryScore + 1.0 * regimeScore - 0.5 + eventBonus,
  );
...
export function scoreOpportunity(
  ...
  const momZ = clamp(
    features.momentumZ,
    -config.momentumClipZ,
    config.momentumClipZ,
  );
  const momentumScore =
    (momZ + config.momentumClipZ) / (2 * config.momentumClipZ);
  ...
  const volumeConfirmation =
    features.volumeZ > 0 ? clamp(features.volumeZ / 2, 0, 0.2) : 0;
...
export function scoreRealizedEdge(
  ...
  const recentExpNorm =
    performance.recentExpectancyR / config.targetExpectancyR;
  const recencyScore = sigmoid(recentExpNorm * 2 - 1);

  // Confidence based on sample size
  const confidenceScore = sigmoid(
    (performance.totalTrades - config.minTradesForConfidence) / 20,
  );
...
export function scoreUserFit(
  ...
  const budgetFit = sigmoid(
    (constraints.totalBudget - minNotional) / Math.max(1, minNotional),
  );
  score *= budgetFit;
  ...
  if (
    ...
    instrument.maxLeverage > constraints.maxLeverage
  ) {
    score *= 0.8;
  }
...
export function explainScore(score: ScoreBreakdown, tier: UserTier): string {
  ...
  const contributions = [
    {
      name: "Liquidity",
      value: score.liquidity * weights.liquidity,
      raw: score.liquidity,
    },
    {
      name: "PCTT Fitness",
      value: score.pcttFitness * weights.pcttFitness,
      raw: score.pcttFitness,
    },
    {
      name: "Opportunity",
      value: score.opportunity * weights.opportunity,
      raw: score.opportunity,
    },
    {
      name: "Performance",
      value: score.realizedEdge * weights.realizedEdge,
      raw: score.realizedEdge,
    },
  ...
  if (score.pcttDetails.qScore >= 0.7) {
    parts.push(
      `Strong Q-score (${(score.pcttDetails.qScore * 100).toFixed(0)}%).`,
    );
  }
