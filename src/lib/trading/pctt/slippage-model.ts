...
export class SlippageModel {
  ...
  estimateSlippage(
    ...
    const timeOfDayComponent = (baseComponent + volatilityComponent) * (timeMultiplier - 1);
    ...
    const rawSlippage = baseComponent + volatilityComponent + sizeImpactComponent + 
                        timeOfDayComponent + spreadComponent;
  ...
  adjustPositionSize(
    ...
    const effectiveRiskPerShare = originalRiskPerShare + slippageEstimate.worstCaseSlippage;
    
    // Calculate adjusted quantity
    const adjustedQuantity = Math.floor(targetDollarRisk / effectiveRiskPerShare);
