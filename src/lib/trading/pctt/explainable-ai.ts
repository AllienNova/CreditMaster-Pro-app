...
import type { StructureObject, PCTTSignal, BoundaryLine, PCTTEvent } from './pctt-core';
...
const REGIME_DESCRIPTIONS: Record<string, string> = {
  trend_up: 'The market is in an uptrend with strong directional momentum. Price is making higher highs and higher lows.',
  trend_down: 'The market is in a downtrend with bearish momentum. Price is making lower highs and lower lows.',
  range: 'The market is range-bound, oscillating between support and resistance without clear direction.',
  transition: 'The market is transitioning between regimes. Direction is unclear and caution is warranted.',
};
...
const EVENT_DESCRIPTIONS: Record<PCTTEvent, string> = {
  ...
  freeze_up: 'Breakout confirmed. Structure frozen. Waiting for retest opportunity.',
  freeze_down: 'Breakdown confirmed. Structure frozen. Waiting for retest opportunity.',
  retest_up: 'Price is retesting the broken resistance (now support). Watching for rejection.',
  retest_down: 'Price is retesting the broken support (now resistance). Watching for rejection.',
  entry_long: 'LONG SIGNAL: Price rejected from support with bullish confirmation.',
  entry_short: 'SHORT SIGNAL: Price rejected from resistance with bearish confirmation.',
  failure: 'Setup has failed. Price invalidated the pattern by breaching the safety line.',
};

const Q_SCORE_INTERPRETATIONS = [
  { min: 0.8, label: 'Excellent', description: 'Very high quality structure with multiple confirmed touches and minimal violations.' },
  { min: 0.7, label: 'Good', description: 'Solid structure quality. Reliable for trading with proper risk management.' },
  { min: 0.6, label: 'Acceptable', description: 'Moderate structure quality. Consider waiting for better setup.' },
  { min: 0.5, label: 'Marginal', description: 'Borderline quality. High risk, use smaller position size.' },
  { min: 0, label: 'Poor', description: 'Low quality structure. Not recommended for trading.' },
];
...
export class PCTTExplainableAI {
   ...
   */
  explainSignal(signal: PCTTSignal, structure: StructureObject): TradeNarrative {
    ...
    return {
      ...
      execution: `Enter ${signal.type.toUpperCase()} at $${signal.entryPrice.toFixed(2)} (current price). ` +
        `Use a limit order within the entry zone for better fill. ` +
        `Position should not exceed 2-3% of portfolio risk.`,
      management: `Initial stop at $${signal.stopPrice.toFixed(2)} (${(riskAmount / signal.entryPrice * 100).toFixed(1)}% risk). ` +
        `First target at $${signal.targetPrices[0].toFixed(2)} (${rr.toFixed(1)}R). ` +
        `Consider scaling out: 50% at 1R, 25% at 2R, trail remainder.`,
      invalidation: `Trade is INVALID if price closes ${signal.type === 'long' ? 'below' : 'above'} ` +
  ...
  generateLiveCommentary(structure: StructureObject): LiveCommentary {
    ...
    this.recentEvents = this.recentEvents.filter(e => e.time.getTime() > cutoff);
  ...
  private analyzeFactors(structure: StructureObject): DecisionFactor[] {
    ...
    if (structure.support) {
      factors.push({
        ...
        impact: structure.support.qScore >= 0.65 ? 'positive' : 
                structure.support.qScore >= 0.5 ? 'neutral' : 'negative',
        weight: 0.25,
    ...
    if (structure.resistance) {
      factors.push({
        ...
        impact: structure.resistance.qScore >= 0.65 ? 'positive' :
                structure.resistance.qScore >= 0.5 ? 'neutral' : 'negative',
        weight: 0.25,
        explanation: this.interpretQScore(structure.resistance.qScore, 'resistance'),
      });
    }
    ...
    factors.push({
      ...
      impact: structure.regime === 'range' ? 'neutral' :
              structure.regime === 'transition' ? 'negative' : 'positive',
      weight: 0.2,
    ...
    factors.push({
      ...
      impact: structure.efficiencyRatio >= 0.5 ? 'positive' :
              structure.efficiencyRatio >= 0.3 ? 'neutral' : 'negative',
      weight: 0.15,
      explanation: structure.efficiencyRatio >= 0.5 
    ...
    factors.push({
      ...
      impact: structure.distanceToSupport < 1 ? 'positive' :
              structure.distanceToSupport < 2 ? 'neutral' : 'negative',
      weight: 0.075,
      explanation: structure.distanceToSupport < 1
    ...
    factors.push({
      ...
      impact: structure.distanceToResistance < 1 ? 'negative' :
              structure.distanceToResistance < 2 ? 'neutral' : 'positive',
      weight: 0.075,
      explanation: structure.distanceToResistance < 1
  ...
  private calculateConfidenceBreakdown(structure: StructureObject): ConfidenceBreakdown {
    ...
    const distanceRatio = structure.distanceToResistance / 
                          (structure.distanceToSupport + 0.1);
  ...
  private identifyRisks(structure: StructureObject): RiskExplanation[] {
    ...
    if (!structure.resistance || structure.resistance.qScore < 0.5) {
      risks.push({
        ...
        mitigation: 'Use conservative targets and be prepared for false breakouts.',
      });
    }
  ...
  private generateVisualCues(structure: StructureObject): VisualCue[] {
    ...
    if (structure.resistance) {
      cues.push({
        ...
        tooltip: this.interpretQScore(structure.resistance.qScore, 'resistance'),
      });
    }
    ...
    cues.push({
      type: 'highlight',
      position: { x: 0, y: 0 },
      color: structure.regime === 'trend_up' ? 'rgba(38, 166, 154, 0.1)' :
             structure.regime === 'trend_down' ? 'rgba(239, 83, 80, 0.1)' :
  ...
  private generateSummary(structure: StructureObject, confidence: number): string {
    const regime = structure.regime.replace('_', ' ');
    const event = structure.event === 'idle' 
      ? 'waiting for setup'
      : structure.event.replace('_', ' ');
    
    return `${regime.charAt(0).toUpperCase() + regime.slice(1)} market, ${event}. ` +
  ...
  private generateNarrative(
    ...
    if (positiveFactors.length > 0) {
      parts.push(`Positive factors: ${positiveFactors.map(f => f.name.toLowerCase()).join(', ')}.`);
    }
    if (negativeFactors.length > 0) {
      parts.push(`Concerns: ${negativeFactors.map(f => f.name.toLowerCase()).join(', ')}.`);
    }
  ...
  private generateSetupExplanation(signal: PCTTSignal, structure: StructureObject): string {
    const direction = signal.type === 'long' ? 'bullish' : 'bearish';
    const line = signal.type === 'long' ? 'support' : 'resistance';
    
    return `A ${direction} retest setup has formed. Price broke through ${line}, ` +
  ...
  private generateTriggerExplanation(signal: PCTTSignal, structure: StructureObject): string {
    if (signal.type === 'long') {
      return 'Price has shown rejection from support with a bullish candlestick pattern. ' +
             'The candle closed above the level after wicking below, indicating buyer strength.';
    } else {
      return 'Price has shown rejection from resistance with a bearish candlestick pattern. ' +
  ...
  private describeCurrentState(structure: StructureObject): string {
    return `${EVENT_DESCRIPTIONS[structure.event]} ` +
  ...
  private interpretQScore(qScore: number, type: 'support' | 'resistance'): string {
