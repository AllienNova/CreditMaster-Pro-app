...
export class TrailingStopService {
  ...
  private priceCache: Map<
    string,
    { price: number; high: number; low: number; timestamp: Date }
  > = new Map();
  ...
  async getActiveStops(userId: string): Promise<TrailingStop[]> {
    return Array.from(this.activeStops.values()).filter(
      (s) => s.userId === userId,
    );
  }
  ...
  private async updateStopForPrice(
    ...
    if (!stop.activated && stop.config.activationPrice) {
      if (stop.side === "long" && price >= stop.config.activationPrice) {
        ...
        stop.side === "short" &&
        price <= stop.config.activationPrice
      ) {
        stop.activated = true;
    ...
    if (stop.config.onlyTrailUp) {
      ...
      if (stop.side === "short" && newStopPrice >= stop.currentStopPrice)
        return;
    }
    ...
    if (stop.config.lockInProfitPercent) {
      ...
      if (profitPercent >= stop.config.lockInProfitPercent) {
        const breakeven =
          stop.entryPrice * (stop.side === "long" ? 1.001 : 0.999);
  ...
  private async calculateInitialStop(
    ...
    switch (config.type) {
      ...
      case "chandelier":
        const chandelierAtr = await this.getATR(
          params.symbol,
      ...
      case "parabolic_sar":
        return this.calculateParabolicSAR(
          params.symbol,
          side,
          currentPrice,
          config,
  ...
  private async calculateStopPrice(
    stop: TrailingStop,
    currentPrice: number,
  ): Promise<number> {
    ...
    switch (config.type) {
      ...
      case "chandelier":
        ...
        const highestHigh = await this.getHighestHigh(
          stop.symbol,
          config.chandelierPeriod || 22,
        );
        const lowestLow = await this.getLowestLow(
          stop.symbol,
          config.chandelierPeriod || 22,
        );
        return side === "long"
  ...
  private applyTimeTightening(
    ...
    if (minutesActive < timeBasedTightening.startAfterMinutes)
      return stop.currentStopPrice;

    const hoursActive =
      (minutesActive - timeBasedTightening.startAfterMinutes) / 60;
    const tighteningFactor =
      1 - (hoursActive * timeBasedTightening.tighteningRate) / 100;
    ...
    if (stop.side === "long") {
      ...
      return Math.max(
        ...
        stop.highestPrice - tightenedDistance,
      );
    } else {
      ...
      return Math.min(
        stop.currentStopPrice,
        stop.lowestPrice + tightenedDistance,
      );
    }
  }
  ...
  private sarState: Map<string, { af: number; ep: number; sar: number }> =
    new Map();
