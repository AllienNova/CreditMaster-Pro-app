...
export const HISTORICAL_SCENARIOS: HistoricalScenario[] = [
  {
    ...
    description:
      "Lehman Brothers collapse, credit freeze, equity market crash of ~55%",
  ...
  },
  {
    ...
    description:
      "Fed tightening cycle, growth-to-value rotation, crypto collapse",
  ...
  },
  {
    ...
    description:
      "Hypothetical: persistent high inflation + economic contraction",
...
export class StressTestEngine {
  ...
  runHistoricalScenario(
    ...
    const sorted = [...positionLosses].sort(
      (a, b) => a.lossPercent - b.lossPercent,
    );
    ...
    const durationDays = Math.max(
      1,
      (scenario.endDate.getTime() - scenario.startDate.getTime()) /
        (1000 * 60 * 60 * 24),
    );
    ...
    const recoveryEstimateDays = Math.round(
      durationDays * 2 * (lossPercent / 0.3),
    );
  ...
  monteCarloSimulation(
    ...
    const expectedReturn =
      terminalReturns.reduce((a, b) => a + b, 0) / config.numSimulations;
    const variance =
      terminalReturns.reduce((sum, r) => sum + (r - expectedReturn) ** 2, 0) /
    ...
    return {
      ...
      percentiles: {
        ...
        p50: terminalReturns[Math.floor(n * 0.5)] * 100,
  ...
  private computeRiskScore(
    ...
    const diversificationPenalty =
      positions.length < 5 ? 15 - positions.length * 3 : 0;
  ...
  private generateRecommendations(
    ...
    // GFC vulnerability
    const gfcResult = historicalResults.find((r) =>
      r.scenarioName.includes("2008"),
    );
    ...
    if (recs.length === 0) {
      recs.push(
        "Portfolio stress test results are within acceptable parameters.",
      );
    }
  ...
  private emptyMonteCarloResult(config: MonteCarloConfig): MonteCarloResult {
    return {
      ...
      volatility: 0,
      varEstimates: config.confidenceLevels.map((cl) => ({
        confidence: cl,
        var: 0,
        varPercent: 0,
      })),
      cvarEstimates: config.confidenceLevels.map((cl) => ({
        confidence: cl,
        cvar: 0,
        cvarPercent: 0,
      })),
