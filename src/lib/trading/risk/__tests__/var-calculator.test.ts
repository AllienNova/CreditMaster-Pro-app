...
function generateReturns(count: number, mean: number, stdDev: number, seed: number = 42): number[] {
...
function makeSamplePositions(): VaRPosition[] {
  return [
    { symbol: 'AAPL', value: 50000, weight: 0.50 },
    { symbol: 'MSFT', value: 30000, weight: 0.30 },
    { symbol: 'GOOG', value: 20000, weight: 0.20 },
  ];
}
...
describe('VaRCalculator', () => {
  ...
  beforeEach(() => {
    ...
    __mockMonitor.getCorrelationMatrix.mockImplementation((symbols: string[]) => {
      const n = symbols.length;
      const matrix = Array.from({ length: n }, (_, i) =>
        Array.from({ length: n }, (__, j) => (i === j ? 1 : 0.5))
      );
  ...
  describe('setReturns', () => {
    it('should store return data for a symbol', () => {
      ...
      const positions: VaRPosition[] = [{ symbol: 'AAPL', value: 100000, weight: 1.0 }];
      const result = calc.historicalVaR(positions, { ...DEFAULT_VAR_CONFIG, windowDays: 100 });
  ...
  describe('parametricVaR', () => {
    ...
    it('should increase VaR with higher confidence level', () => {
      const positions = makeSamplePositions();
      const var95 = calc.parametricVaR(positions, { ...DEFAULT_VAR_CONFIG, confidenceLevel: 0.95 });
      const var99 = calc.parametricVaR(positions, { ...DEFAULT_VAR_CONFIG, confidenceLevel: 0.99 });
    ...
    it('should increase VaR with longer horizon (square root of time)', () => {
      const positions = makeSamplePositions();
      const var1d = calc.parametricVaR(positions, { ...DEFAULT_VAR_CONFIG, horizonDays: 1 });
      const var10d = calc.parametricVaR(positions, { ...DEFAULT_VAR_CONFIG, horizonDays: 10 });
    ...
    it('should return zero VaR for zero-value portfolio', () => {
      const positions: VaRPosition[] = [{ symbol: 'AAPL', value: 0, weight: 0 }];
      const result = calc.parametricVaR(positions);
    ...
    it('should scale VaR proportionally with portfolio value', () => {
      const small: VaRPosition[] = [{ symbol: 'AAPL', value: 10000, weight: 1.0 }];
      const large: VaRPosition[] = [{ symbol: 'AAPL', value: 100000, weight: 1.0 }];
  ...
  describe('historicalVaR', () => {
    ...
    it('should use actual return distribution when data is available', () => {
      ...
      const positions: VaRPosition[] = [{ symbol: 'AAPL', value: 100000, weight: 1.0 }];
      const result = calc.historicalVaR(positions, { ...DEFAULT_VAR_CONFIG, windowDays: 252 });
    ...
    it('should handle multi-day horizon by scaling returns', () => {
      ...
      const positions: VaRPosition[] = [{ symbol: 'AAPL', value: 100000, weight: 1.0 }];
      const var1d = calc.historicalVaR(positions, { ...DEFAULT_VAR_CONFIG, horizonDays: 1 });
      const var5d = calc.historicalVaR(positions, { ...DEFAULT_VAR_CONFIG, horizonDays: 5 });
    ...
    it('should compute VaR from multiple-asset portfolio returns', () => {
      ...
      const result = calc.historicalVaR(positions, { ...DEFAULT_VAR_CONFIG, windowDays: 200 });
  ...
  describe('conditionalVaR', () => {
    ...
    it('should compute expected shortfall from actual returns when available', () => {
      ...
      const positions: VaRPosition[] = [{ symbol: 'AAPL', value: 100000, weight: 1.0 }];
      const result = calc.conditionalVaR(positions, { ...DEFAULT_VAR_CONFIG, windowDays: 252 });
    ...
    it('should produce higher ES at 99% confidence than 95%', () => {
      ...
      const positions: VaRPosition[] = [{ symbol: 'SPY', value: 100000, weight: 1.0 }];
      ...
      expect(es99.expectedShortfall).toBeGreaterThanOrEqual(es95.expectedShortfall);
    });
  });
  ...
  describe('edge cases and branches', () => {
    it('should use actual return volatility when >= 10 data points are available (getVolatilities branch)', () => {
      ...
      const positions: VaRPosition[] = [{ symbol: 'AAPL', value: 100000, weight: 1.0 }];
      const result = calc.parametricVaR(positions, { ...DEFAULT_VAR_CONFIG, windowDays: 50 });
    ...
    it('should handle NaN values in the correlation matrix (isNaN fallback at line 125)', () => {
      const { __mockMonitor } = jest.requireMock('@/lib/trading/correlation-monitor');
