...
function makeSamplePositions(): Position[] {
  return [
    { symbol: 'AAPL', value: 50000, weight: 0.50, sector: 'Technology' },
    { symbol: 'JPM', value: 30000, weight: 0.30, sector: 'Financials' },
    { symbol: 'GLD', value: 20000, weight: 0.20, sector: 'Commodities' },
  ];
}
...
describe('StressTestEngine', () => {
  ...
  beforeEach(() => {
    ...
    const { __mockMonitor, CorrelationMonitor: MockCtor } = jest.requireMock('@/lib/trading/correlation-monitor');
  ...
  describe('runHistoricalScenario', () => {
    ...
    it('should return empty result for zero-value portfolio', () => {
      ...
      const result = engine.runHistoricalScenario(positions, HISTORICAL_SCENARIOS[0]);
  ...
  describe('monteCarloSimulation', () => {
    ...
    it('should produce deterministic results with a seed', () => {
      ...
      expect(result1.expectedReturnPercent).toBeCloseTo(result2.expectedReturnPercent, 5);
    ...
    it('should compute VaR at specified confidence levels', () => {
      ...
      expect(result.varEstimates[1].var).toBeGreaterThanOrEqual(result.varEstimates[0].var);
    });
    ...
    it('should compute CVaR (Expected Shortfall) at specified confidence levels', () => {
      ...
      expect(result.cvarEstimates[0].cvar).toBeGreaterThanOrEqual(result.varEstimates[0].var);
    });
  ...
  describe('runCustomScenario', () => {
    it('should apply custom shocks to positions', () => {
      const positions = makeSamplePositions();
      const shocks: Record<string, number> = { AAPL: -0.20, JPM: -0.30, GLD: 0.05 };

      const result = engine.runCustomScenario(positions, 'Tech Sell-off', shocks);

      expect(result.scenarioName).toBe('Tech Sell-off');
      expect(result.positionLosses).toHaveLength(3);
      expect(result.positionLosses.find(p => p.symbol === 'AAPL')?.lossPercent).toBe(-20);
      expect(result.positionLosses.find(p => p.symbol === 'GLD')?.lossPercent).toBe(5);
    });
    ...
    it('should use provided defaultShock for unmapped symbols', () => {
      ...
      const result = engine.runCustomScenario(positions, 'Custom', {}, -0.10);
  ...
  describe('generateReport', () => {
    it('should produce a complete stress report', () => {
      ...
      expect(report.historicalResults).toHaveLength(HISTORICAL_SCENARIOS.length);
    ...
    it('should recommend trimming concentrated positions', () => {
      const positions: Position[] = [
        { symbol: 'AAPL', value: 90000, weight: 0.90 },
        { symbol: 'SPY', value: 10000, weight: 0.10 },
      ];
      const config: MonteCarloConfig = {
        numSimulations: 100,
        horizonDays: 10,
  ...
  describe('generateReport — high risk score recommendations', () => {
    it('should recommend risk reduction when riskScore >= 70', () => {
      ...
      const hasDiversifyWarning = report.recommendations.some(
        r => r.includes('Diversify')
      );

      // At least one of these should be true for such a concentrated portfolio
      expect(hasHighRiskWarning || hasConcentrationWarning || hasDiversifyWarning).toBe(true);
    });
    ...
    it('should recommend sector diversification when sector weight > 40%', () => {
      const positions: Position[] = [
        ...
        { symbol: 'JNJ', value: 10000, weight: 0.10, sector: 'Healthcare' },
      ];
      const config: MonteCarloConfig = {
        numSimulations: 100,
        horizonDays: 10,
    ...
    it('should report within acceptable parameters for well-diversified portfolio', () => {
      ...
      const positions: Position[] = [
        { symbol: 'AAPL', value: 10000, weight: 0.10, sector: 'Technology' },
        { symbol: 'MSFT', value: 10000, weight: 0.10, sector: 'Technology' },
        { symbol: 'JPM', value: 10000, weight: 0.10, sector: 'Financials' },
        { symbol: 'GLD', value: 10000, weight: 0.10, sector: 'Commodities' },
        { symbol: 'XOM', value: 10000, weight: 0.10, sector: 'Energy' },
        { symbol: 'JNJ', value: 10000, weight: 0.10, sector: 'Healthcare' },
        { symbol: 'PG', value: 10000, weight: 0.10, sector: 'Consumer' },
        { symbol: 'NEE', value: 10000, weight: 0.10, sector: 'Utilities' },
        { symbol: 'AMT', value: 10000, weight: 0.10, sector: 'Real Estate' },
        { symbol: 'UNP', value: 10000, weight: 0.10, sector: 'Industrials' },
      ];
      const config: MonteCarloConfig = {
        numSimulations: 100,
        horizonDays: 5,
  ...
  describe('monteCarloSimulation — Cholesky & CVaR edge cases', () => {
    it('should fall back to identity matrix for non-positive-definite correlation', () => {
      ...
      const positions: Position[] = [
        { symbol: 'A', value: 40000, weight: 0.40 },
        { symbol: 'B', value: 30000, weight: 0.30 },
        { symbol: 'C', value: 30000, weight: 0.30 },
      ];
      const config: MonteCarloConfig = {
        numSimulations: 100,
        horizonDays: 5,
  ...
  describe('HISTORICAL_SCENARIOS', () => {
    ...
    it('each scenario should have valid start and end dates', () => {
      HISTORICAL_SCENARIOS.forEach(s => {
        ...
        expect(s.endDate.getTime()).toBeGreaterThanOrEqual(s.startDate.getTime());
      });
    });
  });
});
