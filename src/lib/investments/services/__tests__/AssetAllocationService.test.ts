...
describe("AssetAllocationService", () => {
  ...
  describe("risk metrics", () => {
    ...
    it("should calculate Value at Risk (VaR)", async () => {
      ...
      expect(analysis.riskMetrics.conditionalVaR).toBeGreaterThan(
  ...
  describe("rebalancing recommendations", () => {
    it("should prioritize high deviation positions", async () => {
      ...
      if (highPriority.length > 0 && lowPriority.length > 0) {
        expect(
          Math.abs(
            highPriority[0].currentPercentage -
              highPriority[0].targetPercentage,
          ),
        ).toBeGreaterThan(
          Math.abs(
            lowPriority[0].currentPercentage - lowPriority[0].targetPercentage,
          ),
        );
      }
    });
