...
describe('SentimentAnalysisService', () => {
  ...
  describe('getNewsSentiment', () => {
    ...
    it('should calculate counts correctly', async () => {
      ...
      const totalCount = sentiment.positiveCount + sentiment.negativeCount + sentiment.neutralCount;
  ...
  describe('getAnalystConsensus', () => {
    ...
    it('should have valid consensus rating', async () => {
      ...
      expect(['strong_buy', 'buy', 'hold', 'sell', 'strong_sell']).toContain(consensus.consensusRating);
    });
    ...
    it('should have price targets', async () => {
      ...
      expect(consensus.highPriceTarget).toBeGreaterThanOrEqual(consensus.averagePriceTarget);
      expect(consensus.lowPriceTarget).toBeLessThanOrEqual(consensus.averagePriceTarget);
    });
  ...
  describe('getInsiderActivity', () => {
    ...
    it('should have valid insider sentiment', async () => {
      ...
      expect(['bullish', 'neutral', 'bearish']).toContain(activity.insiderSentiment);
    });
  ...
  describe('getInstitutionalOwnership', () => {
    ...
    it('should have valid ownership trend', async () => {
      ...
      expect(['increasing', 'stable', 'decreasing']).toContain(ownership.ownershipTrend);
    });
  });
  ...
  describe('getMarketSentiment', () => {
    ...
    it('should have Fear & Greed Index', async () => {
      ...
      expect(['extreme_fear', 'fear', 'neutral', 'greed', 'extreme_greed']).toContain(market.fearGreed.label);
    });
    ...
    it('should have VIX data', async () => {
      ...
      expect(['low', 'normal', 'elevated', 'high', 'extreme']).toContain(market.vix.level);
    });
    ...
    it('should have market breadth indicators', async () => {
      ...
      expect(['bullish', 'neutral', 'bearish']).toContain(market.breadth.breadthSignal);
    });
  });
  ...
  describe('analyzeSentiment', () => {
    ...
    it('should calculate composite sentiment', async () => {
      ...
      expect(['very_positive', 'positive', 'neutral', 'negative', 'very_negative']).toContain(
        analysis.compositeSentiment.label
      );
    });
    ...
    it('should have weighted components', async () => {
      ...
      const totalWeight = analysis.compositeSentiment.components.reduce((sum, c) => sum + c.weight, 0);
    ...
    it('should determine overall signal', async () => {
      ...
      expect(['strong_buy', 'buy', 'hold', 'sell', 'strong_sell']).toContain(analysis.overallSignal);
    });
    ...
    it('should identify risks', async () => {
      ...
      expect(typeof analysis.risks[0] === 'string' || analysis.risks.length === 0).toBe(true);
    });
    ...
    it('should identify opportunities', async () => {
      ...
      expect(typeof analysis.opportunities[0] === 'string' || analysis.opportunities.length === 0).toBe(true);
    });
    ...
    it('should support optional components', async () => {
      ...
      expect(analysisAll.insiderActivity.transactions90Days).toBeInstanceOf(Array);
      expect(analysisAll.institutionalOwnership.topHolders).toBeInstanceOf(Array);
