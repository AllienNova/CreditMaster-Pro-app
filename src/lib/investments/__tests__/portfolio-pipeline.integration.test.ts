...
describe("PortfolioAnalytics Pipeline", () => {
  ...
  describe("calculateRiskMetrics", () => {
    it("should calculate comprehensive risk metrics for a portfolio", async () => {
      const metrics = await analytics.calculateRiskMetrics(
        "portfolio-001",
        "1Y",
      );
      ...
      expect(metrics.valueAtRisk.var99).toBeGreaterThanOrEqual(
    ...
    it("should return cached risk metrics when available", async () => {
      ...
      const result = await analytics.calculateRiskMetrics(
        "portfolio-001",
        "1Y",
      );
    ...
    it("should throw when portfolio is not found", async () => {
      ...
      await expect(
        analytics.calculateRiskMetrics("nonexistent", "1Y"),
    ...
    it("should handle different time horizons correctly", async () => {
      const metrics3M = await analytics.calculateRiskMetrics(
        "portfolio-001",
        "3M",
      );
  ...
  describe("getDiversificationScore", () => {
    it("should calculate diversification score for a multi-holding portfolio", async () => {
      ...
      expect(
        score.sectorDiversification.herfindahlIndex,
      ).toBeGreaterThanOrEqual(0);
      expect(score.sectorDiversification.herfindahlIndex).toBeLessThanOrEqual(
        1,
      );
    ...
    it("should throw when portfolio has no holdings", async () => {
      ...
      await expect(
        analytics.getDiversificationScore("portfolio-001"),
      ).rejects.toThrow("Portfolio portfolio-001 has no holdings");
    });
  ...
  describe("getCorrelationMatrix", () => {
    it("should compute correlation matrix for holdings with >= 2 symbols", async () => {
      const matrix = await analytics.getCorrelationMatrix(
        "portfolio-001",
        "1Y",
      );
      ...
      expect(matrix.metadata.missingDataHandling).toBe(
    ...
    it("should throw when portfolio has fewer than 2 holdings", async () => {
      ...
      await expect(
        analytics.getCorrelationMatrix("portfolio-001", "1Y"),
      ).rejects.toThrow(
        "Portfolio portfolio-001 needs at least 2 holdings for correlation analysis",
      );
    });
    ...
    it("should return cached correlation matrix when available", async () => {
      ...
      const result = await analytics.getCorrelationMatrix(
        "portfolio-001",
        "1Y",
      );
  ...
  describe("getVolatilityAnalysis", () => {
    it("should compute volatility analysis with regime detection", async () => {
      ...
      expect(vol.historicalVolatility.weekly).toBeGreaterThanOrEqual(
        vol.historicalVolatility.daily,
      );
      expect(vol.historicalVolatility.monthly).toBeGreaterThanOrEqual(
        vol.historicalVolatility.weekly,
      );
      expect(vol.historicalVolatility.annualized).toBeGreaterThanOrEqual(
        vol.historicalVolatility.monthly,
      );
      ...
      expect(vol.regimeThresholds.high).toBeGreaterThanOrEqual(
        vol.regimeThresholds.normal,
      );
      expect(vol.regimeThresholds.normal).toBeGreaterThanOrEqual(
        vol.regimeThresholds.low,
      );
    ...
    it("should return cached volatility analysis when available", async () => {
      ...
      const result = await analytics.getVolatilityAnalysis(
        "portfolio-001",
        "1Y",
      );
  ...
  describe("getSectorExposure", () => {
    ...
    it("should group multiple holdings in the same sector", async () => {
      ...
      const exposures = await analytics.getSectorExposure("portfolio-001");
      const techExposure = exposures.find(
        (e) => e.sector === SectorType.TECHNOLOGY,
      );
  ...
  describe("suggestRebalancing", () => {
    it("should generate rebalancing recommendations", async () => {
      const rec = await analytics.suggestRebalancing(
        "portfolio-001",
        RiskLevel.MODERATE,
      );
      ...
      const targetSum = rec.targetAllocation.reduce(
        (s, a) => s + a.allocation,
        0,
      );
    ...
    it("should throw when portfolio not found", async () => {
      ...
      await expect(analytics.suggestRebalancing("nonexistent")).rejects.toThrow(
        "Portfolio nonexistent not found",
      );
    });
  ...
  describe("getPortfolioPerformance", () => {
    it("should compute portfolio performance metrics", async () => {
      const perf = await analytics.getPortfolioPerformance(
        "portfolio-001",
        undefined,
        "1Y",
      );
    ...
    it("should return cached performance when available", async () => {
      ...
      const result = await analytics.getPortfolioPerformance(
        "portfolio-001",
        "SPY",
        "1Y",
      );
  ...
  describe("Edge Cases", () => {
    it("should handle holdings with no historical data returning empty returns", async () => {
      ...
      (marketDataService.getHistory as jest.Mock).mockRejectedValue(
        new Error("No data"),
      );
    ...
    it("should correctly map time horizons to days", async () => {
      ...
      const metrics1M = await analytics.calculateRiskMetrics(
        "portfolio-001",
        "1M",
      );
      ...
      const metrics6M = await analytics.calculateRiskMetrics(
        "portfolio-001",
        "6M",
      );
  ...
  describe("Rebalancing Trade Actions & Priorities", () => {
    ...
    it('should assign "high" priority when drift > 10%', async () => {
      ...
      const holdings = [
        {
          ...createMockHoldings(3)[0],
          shares: 950,
          currentPrice: 100,
          currentValue: 95000,
        },
        {
          ...createMockHoldings(3)[1],
          shares: 25,
          currentPrice: 100,
          currentValue: 2500,
        },
        {
          ...createMockHoldings(3)[2],
          shares: 25,
          currentPrice: 100,
          currentValue: 2500,
        },
      ...
      const highPriorityTrades = rec.trades.filter(
        (t) => t.priority === "high",
      );
    ...
    it('should assign "low" priority for buy when drift <= 5', async () => {
      ...
      const lowPriorityBuys = rec.trades.filter(
        (t) => t.action === "buy" && t.priority === "low",
      );
  ...
  describe("Performance — Holdings with Missing Data", () => {
    ...
    it("should return contribution 0 for holdings with only 1 data point (line 628)", async () => {
      ...
      const googlContrib = [
        ...perf.attribution.topContributors,
        ...perf.attribution.topDetractors,
      ].find(
        (c: { symbol: string; contribution: number }) => c.symbol === "GOOGL",
      );
      if (googlContrib) {
  ...
  describe("Private Helper Edge Cases (via public API)", () => {
    ...
    it("should handle zero std dev in correlation calculation (line 900)", async () => {
      ...
      (marketDataService.getHistory as jest.Mock).mockImplementation(
        (symbol: string) => Promise.resolve(flatData(symbol)),
      );
    ...
    it("should handle calculateStandardDeviation with empty returns (line 789)", async () => {
      ...
      (marketDataService.getHistory as jest.Mock).mockImplementation(
        (symbol: string) => Promise.resolve(singlePointData(symbol)),
      );
    ...
    it("should handle downside deviation with all positive returns (line 801)", async () => {
      ...
      (marketDataService.getHistory as jest.Mock).mockImplementation(
        (symbol: string) => Promise.resolve(upData(symbol)),
      );
    ...
    it("should handle empty drawdowns array giving currentDrawdown = 0 (line 941)", async () => {
      ...
      (marketDataService.getHistory as jest.Mock).mockImplementation(
        (symbol: string) => Promise.resolve(steadyUp(symbol)),
      );
