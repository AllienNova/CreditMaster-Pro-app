...
export function createRetryableFetch(options?: RetryOptions) {
  return async function retryableFetch(
    ...
    return withRetry(async () => {
      ...
      if (
        !response.ok &&
        DEFAULT_OPTIONS.retryableStatusCodes.includes(response.status)
      ) {
...
const noopRetryCallback = () => {
  /* retry in progress */
};

export const experianRetry = <T>(fn: () => Promise<T>) =>
  withRetry(fn, {
    maxRetries: 3,
    initialDelay: 2000,
...
export const plaidRetry = <T>(fn: () => Promise<T>) =>
  withRetry(fn, {
    maxRetries: 3,
    initialDelay: 1000,
    onRetry: noopRetryCallback,
...
export const stripeRetry = <T>(fn: () => Promise<T>) =>
  withRetry(fn, {
    maxRetries: 2,
    initialDelay: 500,
    onRetry: noopRetryCallback,
...
export const aimlRetry = <T>(fn: () => Promise<T>) =>
  withRetry(fn, {
    maxRetries: 2,
    initialDelay: 1000,
