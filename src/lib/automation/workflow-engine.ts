...
export class WorkflowEngine {
  ...
  private static async executeAction(
    ...
    const { action_type: rawActionType, ...params } = step.config as Record<
      string,
      unknown
    > & {
      action_type?: string;
    ...
    const actionType =
      typeof rawActionType === "string" ? rawActionType : "custom_action";
  ...
  private static async evaluateCondition(
    ...
    const { condition_type: rawConditionType, ...params } =
      step.config as Record<string, unknown> & {
        condition_type?: string;
      };
    const conditionType =
      typeof rawConditionType === "string"
        ? rawConditionType
        : "custom_condition";
  ...
  private static async executeDelay(step: WorkflowStep): Promise<void> {
    ...
    const delayMs =
      typeof config.delay_ms === "number" ? config.delay_ms : 1000;
  ...
  private static async sendNotification(
    ...
    const message =
      typeof config.message === "string"
        ? config.message
        : "Workflow notification triggered";
   ...
   */
  private static async getWorkflowTemplate(
    templateId: string,
   ...
   */
  private static getBuiltInTemplate(
    templateId: string,
