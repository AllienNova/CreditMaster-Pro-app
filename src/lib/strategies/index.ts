...
export const ADVANCED_STRATEGIES: Strategy[] = [
  ...
  },
  {
    ...
    key_tactics: [
      "Court verification letters",
      "Prove courts don't report",
  ...
  // Tier 2: Core Professional Strategies (65-75% Success Rate)
  {
    ...
    key_tactics: [
      "Force documentation proof",
      "Third-party collector targeting",
      "Validation tracking",
    ],
  ...
  },
  {
    ...
    key_tactics: [
      "Positive payment history emphasis",
  ...
  },
  {
    ...
    key_tactics: [
      "Unauthorized inquiry claims",
      "Permissible purpose challenges",
  ...
  // Tier 4: Advanced/Aggressive Strategies (40-50% Success Rate)
  {
    ...
    target_items: ["account", "collection"],
    key_tactics: [
      "Format violation identification",
      "Technical compliance demands",
      "Reporting accuracy",
    ],
    prerequisites: [],
    is_active: true,
...
export function getStrategiesByType(type: string): Strategy[] {
  return ADVANCED_STRATEGIES.filter(
    (s) => s.strategy_type === type && s.is_active,
  );
}
...
export function getStrategiesBySuccessRate(minRate: number): Strategy[] {
  return ADVANCED_STRATEGIES.filter(
    (s) => s.success_rate >= minRate && s.is_active,
  );
}
