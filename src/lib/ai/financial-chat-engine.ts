...
import { chatCache, ChatMessage as CacheChatMessage, ChatSession as CacheChatSession } from '@/lib/cache/chat-cache';
import {
  CHAT_SYSTEM_PROMPT,
...
export class FinancialChatEngine {
  ...
  async sendMessage(
    ...
    // Save user message
    const userMessage = await this.saveMessage(sessionId, MessageRole.USER, message);
   ...
   */
  async detectIntent(message: string, context: ChatContext): Promise<ChatIntent> {
    const prompt = INTENT_DETECTION_PROMPT
      .replace('{{MESSAGE}}', message)
      .replace('{{CONTEXT}}', JSON.stringify(context, null, 2));

    try {
      const response = await this.aiml.chat('claude-4.5-sonnet', [
   ...
   */
  async generateResponse(intent: ChatIntent, context: ChatContext): Promise<string> {
  ...
  async getUserSessions(userId: string, limit: number = 20): Promise<ChatSession[]> {
    ...
    // Cache the result (convert to cache format with string timestamps)
    const cacheSessions: CacheChatSession[] = sessions.map((session: ChatSession) => ({
      id: session.id,
  ...
  private async saveMessage(
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (this.supabase as any).rpc('increment_message_count', { session_id: sessionId });
    ...
    switch (intent.type) {
      ...
      case IntentType.BUDGET_PLANNING:
        suggestions.push({
          ...
          description: 'Review your spending habits and find savings opportunities',
          parameters: {},
          priority: 'medium',
        });
        break;
    ...
    switch (intent.type) {
      case IntentType.INVESTMENT_ADVICE:
        content.push({
          ...
          summary: 'Learn about different types of investment risks and how to manage them',
          category: 'investing',
          difficulty: 'beginner',
          readTime: 5,
          keyTakeaways: [
  ...
  private async getPortfolioData(userId: string): Promise<any> {
    try {
      ...
      const totalValue = data.reduce((sum: number, holding: any) => sum + (holding.current_value || 0), 0);

      // Calculate allocation percentages from holdings
      const allocationTotals = data.reduce(
        (acc: { stocks: number; bonds: number; crypto: number; cash: number }, h: any) => {
          ...
          if (assetType.includes('bond') || assetType.includes('fixed')) {
            acc.bonds += value;
          } else if (assetType.includes('crypto') || assetType.includes('bitcoin') || assetType.includes('eth')) {
            acc.crypto += value;
      ...
      const allocation = {
        stocks: totalValue > 0 ? Math.round((allocationTotals.stocks / totalValue) * 100) : 0,
        bonds: totalValue > 0 ? Math.round((allocationTotals.bonds / totalValue) * 100) : 0,
        crypto: totalValue > 0 ? Math.round((allocationTotals.crypto / totalValue) * 100) : 0,
        cash: totalValue > 0 ? Math.round((allocationTotals.cash / totalValue) * 100) : 0,
      };

      // Calculate performance from gain/loss data
      const totalGainLoss = data.reduce((sum: number, h: any) => sum + (h.gain_loss || 0), 0);
      const totalCost = data.reduce((sum: number, h: any) => sum + (h.cost_basis || h.current_value || 0), 0);
   ...
   */
  private async analyzeInvestment(symbol: string, context: ChatContext): Promise<any> {
    try {
      ...
      const { data: holding } = await (this.supabase as any)
        ...
        .single() as { data: { current_value?: number; cost_basis?: number; quantity?: number } | null };
      ...
      const recommendation = returnPct > 20 ? 'HOLD' : returnPct > 0 ? 'BUY' : returnPct > -10 ? 'HOLD' : 'REVIEW';
   ...
   */
  private async getTradingSignal(symbol: string, context: ChatContext): Promise<any> {
    try {
      ...
      if (holding) {
        const gainLossPct = (holding.cost_basis || 0) > 0
          ? (((holding.current_value || 0) - (holding.cost_basis || 0)) / (holding.cost_basis || 1)) * 100
          : 0;
        ...
        return {
          ...
          timeframe: '1d',
          reason: gainLossPct >= 0 ? `Position up ${gainLossPct.toFixed(1)}%` : `Position down ${Math.abs(gainLossPct).toFixed(1)}%`,
        };
      }
   ...
   */
  private async createBudget(parameters: any, context: ChatContext): Promise<any> {
    const monthlyIncome = parameters.monthlyIncome || 0;

    // Standard budget categories using 50/30/20 rule
    const categories = monthlyIncome > 0
      ? [
          { name: 'Housing', amount: Math.round(monthlyIncome * 0.28), percentage: 28, type: 'needs' },
          { name: 'Utilities', amount: Math.round(monthlyIncome * 0.05), percentage: 5, type: 'needs' },
          { name: 'Food & Groceries', amount: Math.round(monthlyIncome * 0.12), percentage: 12, type: 'needs' },
          { name: 'Transportation', amount: Math.round(monthlyIncome * 0.05), percentage: 5, type: 'needs' },
          { name: 'Healthcare', amount: Math.round(monthlyIncome * 0.05), percentage: 5, type: 'needs' },
          { name: 'Entertainment', amount: Math.round(monthlyIncome * 0.10), percentage: 10, type: 'wants' },
          { name: 'Shopping', amount: Math.round(monthlyIncome * 0.10), percentage: 10, type: 'wants' },
          { name: 'Dining Out', amount: Math.round(monthlyIncome * 0.05), percentage: 5, type: 'wants' },
          { name: 'Savings', amount: Math.round(monthlyIncome * 0.10), percentage: 10, type: 'savings' },
          { name: 'Debt Payoff', amount: Math.round(monthlyIncome * 0.05), percentage: 5, type: 'savings' },
          { name: 'Investments', amount: Math.round(monthlyIncome * 0.05), percentage: 5, type: 'savings' },
        ]
      : [];
    ...
    return {
      monthlyIncome,
      categories,
      totalNeeds: categories.filter(c => c.type === 'needs').reduce((sum, c) => sum + c.amount, 0),
      totalWants: categories.filter(c => c.type === 'wants').reduce((sum, c) => sum + c.amount, 0),
      totalSavings: categories.filter(c => c.type === 'savings').reduce((sum, c) => sum + c.amount, 0),
      created: true,
  ...
  private async optimizeDebt(userId: string): Promise<any> {
    try {
      ...
      const avgApr = accounts.reduce((sum: number, acc: any) => sum + (acc.apr || 0), 0) / accounts.length;
      ...
      const totalInterestSaved = Math.round((totalDebt * (avgApr / 100) * (estimatedPayoffTime / 12)) * 0.3);
      ...
      return {
        ...
        totalInterestSaved: 0,
        message: 'Unable to analyze debt accounts. Using default avalanche strategy recommendation.',
      };
    }
  }
  ...
  private async assessRisk(context: ChatContext): Promise<any> {
    try {
      ...
      const totalPortfolio = holdings?.reduce((sum: number, h: any) => sum + (h.current_value || 0), 0) || 0;
      const totalDebt = debts?.reduce((sum: number, d: any) => sum + (d.balance || 0), 0) || 0;
      const highAprdDebt = debts?.filter((d: any) => (d.apr || 0) > 15).reduce((sum: number, d: any) => sum + (d.balance || 0), 0) || 0;

      // Calculate risk factors
      const debtToAssetRatio = totalPortfolio > 0 ? (totalDebt / totalPortfolio) * 100 : totalDebt > 0 ? 100 : 0;
      const concentrationRisk = holdings && holdings.length < 5 ? 20 : 0;
      const highInterestRisk = highAprdDebt > 5000 ? 15 : highAprdDebt > 0 ? 8 : 0;

      const riskScore = Math.min(100, Math.max(0, 50 + (debtToAssetRatio > 50 ? 20 : 0) + concentrationRisk + highInterestRisk));

      const category = riskScore < 40 ? 'conservative' : riskScore < 70 ? 'moderate' : 'aggressive';

      const recommendations: string[] = [];
      if (concentrationRisk > 0) recommendations.push('Diversify portfolio across more holdings to reduce concentration risk');
      if (highInterestRisk > 0) recommendations.push('Prioritize paying down high-interest debt (>15% APR)');
      if (debtToAssetRatio > 50) recommendations.push('Work on reducing debt-to-asset ratio below 50%');
      if (recommendations.length === 0) recommendations.push('Maintain current balanced approach to risk management');
      ...
      return {
        riskScore: 65,
        category: 'moderate',
        recommendations: ['Complete your financial profile to get personalized risk assessment'],
      };
    }
  }
   ...
   */
  private async generateReport(type: string, context: ChatContext): Promise<any> {
    ...
    try {
      switch (type.toLowerCase()) {
        ...
        case 'expense':
          ...
          const totalSpending = transactions?.reduce((sum: number, t: any) => sum + Math.abs(t.amount || 0), 0) || 0;
          reportData.data = { transactions: transactions?.slice(0, 50), totalSpending, period: '30 days' };
        ...
        case 'net-worth':
          ...
          const totalAssets = (assets?.totalValue || 0) + (accounts?.filter((a: any) => a.balance > 0).reduce((sum: number, a: any) => sum + a.balance, 0) || 0);
          const totalLiabilities = accounts?.filter((a: any) => a.balance < 0 || ['credit_card', 'loan'].includes(a.account_type)).reduce((sum: number, a: any) => sum + Math.abs(a.balance || 0), 0) || 0;

          reportData.data = { totalAssets, totalLiabilities, netWorth: totalAssets - totalLiabilities };
    ...
    try {
      ...
      if (!transactions || transactions.length === 0) {
        return {
          totalSpending: 0,
          categories: [],
          insights: ['No spending data available for the selected period. Link your accounts to track spending.'],
          period: `${days} days`,
        };
      }
      ...
      if (topCategory) {
        insights.push(`Your highest spending category is ${topCategory.name} at ${topCategory.percentage}% of total spending.`);
      }
      if (categories.length > 1 && categories[0].percentage > 40) {
        insights.push(`Consider reducing ${categories[0].name} spending - it represents over 40% of your budget.`);
      }
      const avgDaily = totalSpending / days;
      insights.push(`You're spending an average of $${avgDaily.toFixed(2)} per day.`);
  ...
  private async recommendStrategy(context: ChatContext): Promise<any> {
    try {
      ...
      const hasRetirementGoal = goals?.some((g: any) => g.type === 'retirement');
      const hasShortTermGoals = goals?.some((g: any) => {
        const targetDate = new Date(g.target_date);
        return targetDate.getTime() - Date.now() < 2 * 365 * 24 * 60 * 60 * 1000; // < 2 years
      });

      let strategy = 'balanced';
      let reasoning = 'Based on your moderate risk profile and diversified goals';
      const actions: string[] = [];

      if (riskTolerance === 'conservative' || hasShortTermGoals) {
        strategy = 'conservative';
        reasoning = 'Given your conservative risk tolerance or near-term goals, preserving capital is prioritized';
        ...
        strategy = 'growth';
        reasoning = 'Your higher risk tolerance and long time horizon support a growth-focused approach';
      ...
      if (hasRetirementGoal) {
        actions.push('Maximize tax-advantaged retirement account contributions');
      }
      ...
      return {
        strategy: 'balanced',
        reasoning: 'Default recommendation - complete your profile for personalized strategy',
        actions: ['Set up your risk profile', 'Define your financial goals', 'Connect your investment accounts'],
      };
    }
  }
}
