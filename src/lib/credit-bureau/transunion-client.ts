...
export class TransUnionClient {
  ...
  async getCreditReport(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `TransUnion API error: ${response.status} ${errorText}`,
        );
      }
  ...
  async submitDispute(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `TransUnion dispute submission failed: ${response.status}`,
        );
      }
  ...
  private transformResponse(
    data: TransUnionCreditReportResponse,
    userId: string,
  ): CreditReport {
  ...
  private parsePublicRecords(
    records: TransUnionPublicRecordPayload[],
  ): PublicRecord[] {
