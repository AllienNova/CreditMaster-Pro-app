...
jest.mock('@/lib/supabase/client', () => ({
  getSupabase: () => new Proxy({} as Record<string, unknown>, {
...
function makeOkJsonResponse(body: Record<string, unknown>, status = 200): Partial<Response> {
  return {
...
describe('ExperianClient — error handling', () => {
  ...
  it('should handle partial report response with some data missing', async () => {
    ...
    (global.fetch as jest.Mock).mockResolvedValueOnce(
      makeOkJsonResponse({
        riskModel: { score: 720 },
        tradelines: [
          { creditorName: 'Bank A', balance: 5000 },
        ],
  ...
  it('should handle dispute submission failure with 500', async () => {
    ...
    const result = await client.submitDispute(DISPUTE_SUBMISSION, TEST_USER_PII);
    expect(result.success).toBe(false);
...
describe('EquifaxClient — error handling', () => {
  ...
  it('should handle 429 Too Many Requests', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce(
      makeErrorResponse(429, '{"error":"rate_limit_exceeded","retry_after":30}')
    );
...
describe('CreditBureauService — credential validation error handling', () => {
  ...
  it('should return mixed results when some bureaus succeed and others fail', async () => {
    ...
    (global.fetch as jest.Mock).mockResolvedValueOnce(makeOkJsonResponse({ access_token: 'tok' }));
    // Equifax: fail (500)
    (global.fetch as jest.Mock).mockResolvedValueOnce(makeErrorResponse(500));
...
describe('Cross-bureau error response structure', () => {
  const clients = [
    { name: 'ExperianClient', factory: () => new ExperianClient('cid', 'csecret', true), bureau: 'experian' as Bureau },
    { name: 'EquifaxClient', factory: () => new EquifaxClient('key', 'cid', 'sandbox'), bureau: 'equifax' as Bureau },
    { name: 'TransUnionClient', factory: () => new TransUnionClient('sid', 'key', 'test'), bureau: 'transunion' as Bureau },
  ];
  ...
  for (const { name, factory, bureau } of clients) {
    it(`${name}: error response has correct bureau, timestamp, and success=false`, async () => {
      (global.fetch as jest.Mock).mockRejectedValueOnce(new Error('total failure'));
    ...
    it(`${name}: dispute error response has correct structure`, async () => {
      (global.fetch as jest.Mock).mockRejectedValueOnce(new Error('dispute network failure'));

      // Experian needs a token first — mock it
      if (bureau === 'experian') {
        global.fetch = jest.fn();
        (global.fetch as jest.Mock).mockRejectedValueOnce(new Error('token failure'));
      }
