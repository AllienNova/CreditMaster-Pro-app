...
describe('CreditBureauService', () => {
  ...
  describe('getCreditReport', () => {
    ...
    it('should return a successful credit report for Experian', async () => {
      ...
      const clientInstance = (ExperianClient as jest.Mock).mock.results.slice(-1)[0]?.value;
      if (clientInstance) {
      ...
      const result = await CreditBureauService.getCreditReport('user-1', 'experian');
    ...
    it('should return error when user profile is not found', async () => {
      ...
      const result = await CreditBureauService.getCreditReport('nonexistent-user', 'experian');
      expect(result.success).toBe(false);
  ...
  describe('getAllCreditReports', () => {
    ...
    it('should return results for all three bureaus in parallel', async () => {
      ...
      for (const bureau of BUREAUS) {
        const MockClass =
          bureau === 'experian' ? ExperianClient :
          bureau === 'equifax' ? EquifaxClient :
          TransUnionClient;
        const instance = (MockClass as jest.Mock).mock.results.slice(-1)[0]?.value;
        if (instance) {
  ...
  describe('submitDispute', () => {
    ...
    it('should submit dispute successfully to Equifax', async () => {
      const clientInstance = (EquifaxClient as jest.Mock).mock.results.slice(-1)[0]?.value;
      if (clientInstance) {
  ...
  describe('analyzeCreditReport', () => {
    it('should identify high utilization as a negative item', async () => {
      ...
      const highUtil = analysis.negative_items.find((i) => i.type === 'high_utilization');
    ...
    it('should identify late payments as a negative item', async () => {
      ...
      const analysis = await CreditBureauService.analyzeCreditReport(report);
      const latePayments = analysis.negative_items.find((i) => i.type === 'late_payments');
    ...
    it('should identify public records as negative items', async () => {
      ...
      const analysis = await CreditBureauService.analyzeCreditReport(report);
      const publicRecords = analysis.negative_items.find((i) => i.type === 'public_records');
    ...
    it('should identify too many hard inquiries in last 6 months', async () => {
      ...
      const report: CreditReport = {
        ...
        accounts: [],
        inquiries: [
          { id: 'i1', inquiry_date: recentDate.toISOString(), creditor_name: 'A', inquiry_type: 'hard' },
          { id: 'i2', inquiry_date: recentDate.toISOString(), creditor_name: 'B', inquiry_type: 'hard' },
          { id: 'i3', inquiry_date: recentDate.toISOString(), creditor_name: 'C', inquiry_type: 'hard' },
          { id: 'i4', inquiry_date: recentDate.toISOString(), creditor_name: 'D', inquiry_type: 'hard' },
        ],
        public_records: [],
        raw_data: {},
        created_at: '2026-02-17',
      ...
      const analysis = await CreditBureauService.analyzeCreditReport(report);
      const tooMany = analysis.negative_items.find((i) => i.type === 'too_many_inquiries');
    ...
    it('should return correct score factors based on credit score ranges', async () => {
      ...
      const report800: CreditReport = {
        id: 'rpt-6', user_id: 'u1', bureau: 'experian', credit_score: 810,
        report_date: '2026-02-17', raw_data: {}, created_at: '2026-02-17',
      };
      const analysis800 = await CreditBureauService.analyzeCreditReport(report800);
      expect(analysis800.score_factors).toContain('Exceptional credit history');

      // Poor: <580
      const report500: CreditReport = {
        id: 'rpt-7', user_id: 'u1', bureau: 'experian', credit_score: 500,
        report_date: '2026-02-17', raw_data: {}, created_at: '2026-02-17',
      };
      const analysis500 = await CreditBureauService.analyzeCreditReport(report500);
  ...
  describe('credential cache management', () => {
    it('flushCredentialCache should force re-read from env', () => {
      ...
      // Still cached
      expect(CreditBureauService.getBureauCredentials('experian').apiKey).toBe('key-v1');

      CreditBureauService.flushCredentialCache();
      expect(CreditBureauService.getBureauCredentials('experian').apiKey).toBe('key-v2');
    });
