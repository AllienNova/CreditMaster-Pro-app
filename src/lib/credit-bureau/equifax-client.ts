...
export class EquifaxClient {
   ...
   */
  private transformResponse(data: EquifaxCreditReportResponse, userId: string): CreditReport {
  ...
  private parseAccounts(tradelines: EquifaxTradeline[]): CreditAccount[] {
    return tradelines.map((tradeline) => ({
      ...
      creditor_name: tradeline.creditorName || tradeline.businessName || 'Unknown Creditor',
  ...
  private parseInquiries(inquiries: EquifaxInquiryPayload[]): CreditInquiry[] {
    return inquiries.map((inquiry) => ({
      ...
      inquiry_type: ['h', 'hard'].includes((inquiry.inquiryType || '').toLowerCase()) ? 'hard' : 'soft'
    }));
  }

  private parsePublicRecords(records: EquifaxPublicRecordPayload[]): PublicRecord[] {
