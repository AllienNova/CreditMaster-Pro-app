...
function fullValidEnv(): Record<string, string> {
	return {
		...
		OPENAI_DEFAULT_MODEL: "custom-openai-model",
		ANTHROPIC_API_KEY: "test-anthropic-key",
		ANTHROPIC_BASE_URL: "https://custom-anthropic.example.com/v1",
		GOOGLE_API_KEY: "test-google-key",
		GOOGLE_DEFAULT_MODEL: "custom-google-model",
		DEEPSEEK_API_KEY: "test-deepseek-key",
		DEEPSEEK_BASE_URL: "https://custom-deepseek.example.com/v1",
		DEEPSEEK_DEFAULT_MODEL: "custom-deepseek-model",
		MISTRAL_API_KEY: "test-mistral-key",
		MISTRAL_BASE_URL: "https://custom-mistral.example.com/v1",
		MISTRAL_DEFAULT_MODEL: "custom-mistral-model",
	};
}
...
describe("validateEnv - happy path", () => {
	...
	it("should return all fields when every env var is set", () => {
		...
		expect(config).toEqual<EnvConfig>({
			...
			openaiDefaultModel: "custom-openai-model",
			anthropicApiKey: "test-anthropic-key",
			anthropicBaseUrl: "https://custom-anthropic.example.com/v1",
			googleApiKey: "test-google-key",
			googleDefaultModel: "custom-google-model",
			deepseekApiKey: "test-deepseek-key",
			deepseekBaseUrl: "https://custom-deepseek.example.com/v1",
			deepseekDefaultModel: "custom-deepseek-model",
			mistralApiKey: "test-mistral-key",
			mistralBaseUrl: "https://custom-mistral.example.com/v1",
			mistralDefaultModel: "custom-mistral-model",
		});
	});
});
...
describe("_resetAll", () => {
	...
	it("should reset secret hashes so detectSecretRotation throws", () => {
		...
		expect(() => detectSecretRotation()).toThrow("Environment not initialized");
	});
