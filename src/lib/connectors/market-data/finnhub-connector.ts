...
export class FinnhubConnector extends BaseConnector<FinnhubConfig> {
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const response = await this.apiRequest(
        "/stock/symbol?exchange=US&limit=1",
      );
  ...
  async getCompanyProfile(symbol: string): Promise<FinnhubCompanyProfile> {
    return this.apiRequest<FinnhubCompanyProfile>(
  ...
  async getPatternRecognition(
    ...
    return (data.points || []).map((pattern) => {
      ...
      return {
        ...
        type: pattern.patterntype.toLowerCase() as
          | "bullish"
          | "bearish"
          | "neutral",
