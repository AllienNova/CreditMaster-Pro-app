...
const mockInsert = jest.fn().mockResolvedValue({ data: null, error: null } as never);
...
describe('OnboardingABTest', () => {
  ...
  describe('variant weight distribution', () => {
    it('distributes traffic roughly according to weights (50/50)', () => {
      ...
      for (let i = 0; i < numUsers; i++) {
        const variant = abTest.selectVariant(`weight-user-${i}`, 'exp-onboarding-flow');
        counts[variant]++;
      }
    ...
    it('distributes traffic roughly according to weights (80/20)', () => {
      ...
      for (let i = 0; i < numUsers; i++) {
        const variant = abTest.selectVariant(`w-user-${i}`, 'exp-onboarding-flow');
        counts[variant]++;
      }
    ...
    it('distributes traffic across three variants', () => {
      ...
      for (let i = 0; i < numUsers; i++) {
        const variant = abTest.selectVariant(`tri-user-${i}`, 'exp-onboarding-flow');
        counts[variant]++;
      }
  ...
  describe('trackConversion', () => {
    ...
    it('does not throw when Supabase insert fails', async () => {
      mockInsert.mockRejectedValueOnce(new Error('DB connection error') as never);
      abTest.registerExperiment(createExperiment());
      abTest.selectVariant('user-1', 'exp-onboarding-flow');
    ...
    it('tracks multiple conversions for same user', async () => {
      ...
      const e1 = await abTest.trackConversion('user-1', 'exp-onboarding-flow', 'step_1');
      const e2 = await abTest.trackConversion('user-1', 'exp-onboarding-flow', 'step_2');
    ...
    it('tracks conversions for multiple users', async () => {
      ...
      const e1 = await abTest.trackConversion('user-1', 'exp-onboarding-flow', 'signup');
      const e2 = await abTest.trackConversion('user-2', 'exp-onboarding-flow', 'signup');
  ...
  describe('getResults', () => {
    ...
    it('correctly computes conversion rate', async () => {
      ...
      // 3 conversions
      await abTest.trackConversion('cr-user-0', 'exp-onboarding-flow', 'signup');
      await abTest.trackConversion('cr-user-1', 'exp-onboarding-flow', 'signup');
      await abTest.trackConversion('cr-user-2', 'exp-onboarding-flow', 'signup');
  ...
  describe('statistical significance', () => {
    ...
    it('returns non-significant for similar conversion rates', async () => {
      ...
      for (let i = 0; i < 200; i++) {
        if (i < 2) {
          await abTest.trackConversion(`sig-user-${i}`, 'exp-onboarding-flow', 'signup');
        }
      }
  ...
  describe('reset', () => {
    it('clears all internal state', async () => {
      ...
      expect(() => abTest.selectVariant('user-1', 'exp-onboarding-flow')).toThrow(
        'not registered',
      );
    });
  });
