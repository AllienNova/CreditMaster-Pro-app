...
export function addRateLimitHeaders(
  ...
  response.headers.set(RATE_LIMIT_HEADERS.RESET, String(Math.floor(result.resetAt.getTime() / 1000)));
...
export function getRateLimitConfig(endpoint: string): { windowMs: number; maxRequests: number } {
  ...
  if (endpoint.includes('/ai') || endpoint.includes('/credit-repair')) return API_RATE_LIMITS.ai;
  if (endpoint.includes('/bureau') || endpoint.includes('/experian')) return API_RATE_LIMITS.bureau;
  if (endpoint.includes('/payment') || endpoint.includes('/stripe')) return API_RATE_LIMITS.payment;
...
export async function withRateLimit(
  identifier: string,
  endpoint: string,
  checkFn: (id: string, config: { windowMs: number; maxRequests: number }) => Promise<RateLimitResult>
