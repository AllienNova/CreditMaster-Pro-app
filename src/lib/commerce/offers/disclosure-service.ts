...
const DEFAULT_DISCLOSURES: Record<DisclosureType, { title: string; shortText: string; fullText: string }> = {
...
class DisclosureService {
  ...
  async getRequiredDisclosures(context: {
    ...
    if (
      context.category &&
      ['credit_card', 'personal_loan', 'mortgage', 'auto_loan', 'student_loan'].includes(
        context.category
      )
    ) {
      const rateDisclosure = await this.getDisclosureByType('rate_disclosure', region);
    ...
    if (
      ...
      const fcraDisclosure = await this.getDisclosureByType('fcra_disclosure', region);
  ...
  generateDisclosureBanner(disclosures: Disclosure[]): string {
    ...
    return content.hasDisclosures ? '[Disclosure content available - use generateDisclosureBannerContent]' : '';
  }
   ...
   */
  async unlinkDisclosureFromOffer(offerId: string, disclosureId: string): Promise<void> {
  ...
  async checkOfferCompliance(offerId: string): Promise<{
    ...
    if (!offer) {
      return { compliant: false, missingDisclosures: [], warnings: ['Offer not found'] };
    }
    ...
    const linkedTypes = new Set(
      (linkedDisclosures || []).map((d: Record<string, unknown>) => (d.disclosures as Record<string, unknown>)?.type)
    );

    // Check required disclosures
    const requiredTypes: DisclosureType[] = ['advertiser_disclosure', 'rate_disclosure'];
