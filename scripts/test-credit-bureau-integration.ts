...
async function testInsertCreditAccounts(reportId: string) {
  ...
  data.forEach((account, index) => {
    console.log(
      `   💳 Account ${index + 1}: ${account.creditor_name} - $${account.balance}`,
    );
  });
}
...
async function testInsertCreditInquiries(reportId: string) {
  ...
  data.forEach((inquiry, index) => {
    console.log(
      `   🔍 Inquiry ${index + 1}: ${inquiry.creditor_name} (${inquiry.inquiry_type})`,
    );
  });
}
...
async function testLatestCreditScoresView() {
  ...
  if (error) {
    throw new Error(
      `Failed to query latest_credit_scores view: ${error.message}`,
    );
  }
  ...
  data.forEach((score) => {
    console.log(
      `   📊 ${score.bureau}: ${score.credit_score} (as of ${score.report_date})`,
    );
  });
}
...
async function testAccountSummaryView() {
  ...
  if (data) {
    ...
    console.log(
      `   📈 Utilization rate: ${data.utilization_rate?.toFixed(2)}%`,
    );
  } else {
    console.log(
      `   ℹ️  No account summary data yet (this is normal for new data)`,
    );
  }
}
...
async function testRLSPolicies() {
  ...
  if (error) {
    console.log(
      `   ⚠️  RLS policies may be blocking access (expected with anon key)`,
    );
    console.log(
      `   ℹ️  Use SUPABASE_SERVICE_ROLE_KEY to bypass RLS for testing`,
    );
  } else {
    console.log(
      `   ✅ RLS policies are configured (${data.length} reports accessible)`,
    );
  }
}
