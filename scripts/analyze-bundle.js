...
function main() {
  ...
  const { totalSize: staticSize, files: staticFiles } =
  ...
  const { totalSize: chunksSize, files: chunkFiles } =
  ...
  if (largeChunks.length > 0) {
    ...
    log(
      "   Consider code splitting or lazy loading for these modules.",
      colors.yellow,
    );
  }
  ...
  if (totalBundleSize > 5000000) {
    ...
    log(
      "\n✅ Bundle size is acceptable but could be optimized further.",
      colors.green,
    );
  } else {
  ...
  log(
    `   Largest Chunk: ${chunkFiles[0] ? formatBytes(chunkFiles[0].size) : "N/A"}`,
    colors.cyan,
  );
