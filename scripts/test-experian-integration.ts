...
import type {
  UserPII,
  CreditReportRequest,
} from "../src/lib/credit-bureau/types";
...
async function testExperianIntegration(): Promise<void> {
  ...
  if (!clientId || !clientSecret) {
    ...
    console.error(
      "Please set EXPERIAN_CLIENT_ID and EXPERIAN_CLIENT_SECRET in .env.local",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      console.log(
        `   Public Records: ${response.data.public_records?.length || 0}`,
      );
    } else {
