-- CRITICAL: GDPR Art. 17 erasure was aborting for every user.
--
-- Found by finally running the function end-to-end against a seeded user —
-- something every prior erasure migration in this wave listed under
-- "Not-tested". It failed immediately:
--
--   ERROR: operator does not exist: text = uuid
--   QUERY: DELETE FROM user_quotas WHERE user_id = $1
--   CONTEXT: PL/pgSQL function delete_user_data_cascade(uuid,text) line 295
--
-- The delete loop assumed every registered table types user_id as uuid. Two do
-- not: user_quotas and webauthn_challenges use TEXT. Passing a uuid into
-- `user_id = $1` against a text column raises, and the loop has NO exception
-- handler — so the whole erasure aborts, every table after the failing one is
-- skipped, and the caller gets a hard error instead of a completed request.
--
-- Both offending tables were registered by round 2 (20260731000050). The
-- array-integrity test could not catch this: it checks commas, duplicates and
-- membership, none of which say anything about column TYPES. Only executing
-- the function does.
--
-- FIX: cast both sides to text in the loop, which works against a uuid or a
-- text column. The audit_logs UPDATE further down already did exactly this,
-- and documented why — the loop simply never got the same treatment.
--
-- ── actor_email, old_values, new_values ─────────────────────────────────
-- Also closes a real pseudonymisation gap. Contrary to what I previously
-- reported, ip_address and user_agent were ALREADY being nulled. What was NOT:
--   actor_email  a direct identifier. The actor is already identified by
--                user_id, nulled on the same row, so redacting the address
--                costs the audit trail nothing.
--   old_values / new_values  before-and-after snapshots of the user's own
--                records — the highest-PII columns on the table. The event,
--                its action and its timestamp survive; the payload does not.
--
-- DELIBERATELY LEFT: `details` and `metadata` are free-form jsonb and MAY
-- contain personal data, but `details` also carries the erasure marker
-- ({"deleted_user_id": ...}) that records the request happened. Blanket-nulling
-- them would destroy that record. Flagged as residual risk rather than silently
-- either kept or wiped — it needs a per-writer review of what those payloads
-- actually hold.
...
BEGIN
  ...
  LOOP
    IF to_regclass('public.' || v_table) IS NOT NULL THEN
      -- Cast BOTH sides to text. Not every registered table types user_id as
      -- uuid: user_quotas and webauthn_challenges use TEXT. Passing a uuid
      -- into `user_id = $1` against a text column raises
      --   operator does not exist: text = uuid
      -- and because this loop has no exception handler, that aborts the ENTIRE
      -- erasure — every table after the failing one is skipped and the caller
      -- sees a hard error instead of a completed Art. 17 request.
      -- The audit_logs UPDATE below already casts for exactly this reason;
      -- the loop did not.
      EXECUTE format('DELETE FROM %I WHERE user_id::text = $1::text', v_table)
        USING p_user_id;
    END IF;
  END LOOP;
  ...
  SET user_id     = NULL,
      ip_address  = NULL,
      user_agent  = NULL,
      -- actor_email is a direct identifier and was NOT being cleared. The
      -- actor is already identified by user_id, which is nulled on the same
      -- row, so redacting the address costs the audit trail nothing.
      actor_email = NULL,
      -- old_values / new_values are before-and-after snapshots of the user's
      -- own records — the highest-PII columns on this table. The event, its
      -- action, and its timestamp survive; the payload does not.
      old_values  = NULL,
      new_values  = NULL
