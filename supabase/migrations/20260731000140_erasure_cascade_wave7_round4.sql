-- GDPR Art. 17 erasure cascade — round 4.
--
-- Registers 10 more user-data tables, built by the savings/property/crypto and
-- shared-goals/gig-income clusters. delete_user_data_cascade is redefined
-- WHOLESALE from a hardcoded array, so this remains the single serialisation
-- point; clusters report, they do not register.
--
-- ROUND-BY-ROUND, SELF-REPORTING HAS UNDERCOUNTED EVERY TIME:
--   round 2   3 reported, 17 found
--   round 3   3 reported, 14 found
--   round 4   9 reported, 10 found   <- the gap finally closed
-- The trend is the point. Round 4 is close because the clusters had learned to
-- report, but it was still the introspection diff that produced the list. Keep
-- running it after every wave; the query is in 20260731000050's header.
--
-- Two of these come from services that were nearly deleted as DEAD.
-- shared_goal_members and financial_journeys back SharedGoalsService and
-- FinancialJourneyService, both of which have zero HTTP reachability -- and
-- both of which git log --follow showed had been hardened by labelled IDOR
-- sweeps (88276d2 "TASK-ANC-2a", 1d24dd9 "TASK-ANC-2b") with their own
-- .idor.test.ts suites. Unwired is not abandoned. Four deletions have now been
-- reversed today on that distinction.
--
-- EXCLUSIONS, unchanged and deliberate. They reappear in the diff every round
-- by design:
--   payments         revenue ledger; user_id is ON DELETE SET NULL so the
--                    financial record outlives the person
--   audit_logs       security trail (ip_address, user_agent, actor_email)
--   tax_audit_log    statutory retention
--   analytics_events NEW this round. The credit cluster built it with ON DELETE
--                    SET NULL, explicitly following the payments precedent, to
--                    retain anonymised aggregate history past account deletion.
--                    Registering it for cascade would defeat that choice.
-- The open owner decision on audit_logs / tax_audit_log stands: after an
-- Art. 17 erasure a user's IP, user agent and email REMAIN in those two.
-- Recommended resolution is pseudonymisation, not deletion or silent retention.
--
-- COMMA DISCIPLINE. Round 2 shipped a missing comma at exactly this join.
-- Adjacent SQL string literals CONCATENATE -- it is not a syntax error -- so
-- two table names silently fused, to_regclass returned NULL, and BOTH dropped
-- out of erasure while psql exited 0. Generated here with an explicit comma-fix
-- pass and verified by EVALUATING the array, not by reading it.
