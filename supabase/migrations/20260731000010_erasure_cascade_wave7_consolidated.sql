-- ============================================================================
-- Consolidated erasure-cascade registration for the Wave 7 build phase.
--
-- WHY ONE MIGRATION AND NOT FOUR: `delete_user_data_cascade` is redefined
-- wholesale via CREATE OR REPLACE with a HARDCODED v_tables array — it is NOT
-- additive. Four builders added tables concurrently; had each registered its own,
-- whichever file sorted LAST would have silently dropped the others from GDPR
-- Art. 17 erasure. That was not hypothetical: it was observed (a per-feature
-- erasure migration landed covering its own tables but not the Plaid tables
-- still in flight) and reverted in 9ba9c5d. The step was then serialised here.
--
-- REGISTERED (all carry user_id and hold personal data):
--   bureau_disputes    — FCRA dispute records incl. the bureau reference_id
--   credit_alerts      — fraud / identity-theft alert payloads
--   plaid_items        — Plaid access tokens (live bank credentials)
--   financial_accounts — linked bank accounts and balances
--
-- DELIBERATELY NOT REGISTERED (verified: no user_id column — operator/partner
-- configuration, not personal data; erasing them would destroy platform config):
--   affiliate_partners, commission_rules
--
-- Function body is otherwise reproduced verbatim from the live definition
-- (pg_proc.prosrc), preserving the to_regclass-guarded delete loop, the
-- both-sides-::text audit_logs comparison, and the REVOKE/GRANT.
-- ============================================================================
