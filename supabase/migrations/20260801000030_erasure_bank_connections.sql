-- TASK-BNK-01 (erasure) — follow plaid_items -> bank_connections in the
-- Art. 17 cascade.
--
-- 20260801000020 generalised plaid_items into bank_connections. The cascade
-- array still named the old table, which under the to_regclass guard means it
-- would have been SKIPPED silently — no error, no failed erasure, just a live
-- bank credential surviving a user who asked to be forgotten.
--
-- This is exactly the class the coverage guard added in e93ceb6 exists to
-- catch: bank_connections carries user_id, so the guard fails until it is
-- either registered here or deliberately excluded. It is registered, not
-- excluded, and the distinction matters — the payments ledgers are
-- pseudonymised because a financial record must outlive the person, but a
-- credential that can still read someone's bank account must not.
--
-- The function body below is reproduced verbatim from 20260731000220 with that
-- one array entry substituted, generated rather than retyped so no other entry
-- can drift. The array-integrity test asserts the element count and guards
-- against the silent SQL string-concatenation failure that a missing comma
-- would introduce.
