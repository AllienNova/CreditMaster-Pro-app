-- Register tax_deductions in the erasure cascade.
--
-- 20260815000000 added the table; this adds it to delete_user_data_cascade.
--
-- CAUGHT BY A GATE, TWICE. erasure-cascade-array-integrity.test.ts failed the
-- moment the table existed ("every user-scoped table is either registered or
-- deliberately excluded"): a user-scoped table absent from this array means
-- "delete my account" leaves the user's itemised deductions — amounts, dates,
-- categories, notes — behind, a GDPR Art. 17 failure no route test would show.
--
-- Then it failed AGAIN, on user_achievements, because the first attempt at this
-- migration copied its table list from 20260801000030 — two cascade revisions
-- stale. Redefining the function from an outdated list SILENTLY DROPS every
-- table registered since, so fixing erasure for one table would have broken it
-- for another. That is the hazard of a whole-function replace: the diff looks
-- like an addition while the effect is a removal.
--
-- This file is therefore built from 20260809000010, the current definition, and
-- adds exactly one entry.

-- Register user_achievements for Art. 17 erasure.
--
-- 20260809000000 built the achievements tables so the restored
-- /api/gamification/achievements route could work against a real database.
-- user_achievements carries user_id, so it must be erased with the user;
-- achievement_definitions is a public catalogue with no personal data and is
-- correctly absent.
--
-- This was not noticed by reading. The erasure-coverage guard added in
-- e93ceb6 FAILED the moment the table existed — it derives the user-scoped set
-- from the migrations and demands every member be registered or deliberately
-- excluded. That is the guard doing exactly the job it was written for.
--
-- Body reproduced from 20260801000030 with one entry inserted, generated
-- rather than retyped so no other entry can drift.
...
DECLARE
  v_tables TEXT[] := ARRAY[
    ...
    -- -----------------------------------------------------------------------
    -- Delta: 20260815000000_tax_deductions_table.sql
    -- -----------------------------------------------------------------------
    'tax_deductions',
    -- -----------------------------------------------------------------------
    ...
    'bank_connections',     -- bank aggregator credentials (plaid | truelayer)
    'user_achievements',    -- per-user achievement progress (20260809000000)
