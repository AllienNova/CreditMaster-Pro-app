-- ============================================================================
-- P0 (second defect): GDPR Art. 17 erasure still aborted after the
-- resource_type default, this time with:
--
--     ERROR: operator does not exist: uuid = text
--
-- The audit-log anonymization step compared `user_id = p_user_id::text`. Its own
-- comment explains the mistake: "user_id in the authoritative schema
-- (20260217000000) is TEXT". That is the WRONG TWIN. `audit_logs` is declared by
-- BOTH 002_production_enhancements.sql and 20260217000000_infrastructure_
-- persistence.sql; 002 sorts first, so ITS shape wins and the live column is
-- `uuid` (verified: information_schema reports uuid). Comparing uuid = text
-- aborts the function, and because the whole body is one implicit transaction,
-- NOTHING is erased.
--
-- This is the twin-schema hazard biting a second time: an author read the
-- losing definition and coded against a column type that does not exist.
--
-- FIX: cast BOTH sides to text, which is correct whether the column is uuid or
-- text — so reconciling the twin later cannot silently re-break erasure.
-- Function is otherwise reproduced verbatim from the live definition
-- (pg_proc.prosrc), including the to_regclass-guarded delete loop and the
-- orders/positions entries added in 20260731000001.
-- ============================================================================
